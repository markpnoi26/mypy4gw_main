...
from Core import ColorPalette, get_texture_for_model
    ...
    def DrawLootConfig(self):
        ...
        if PyImGui.begin("Embedded Loot config", True, flags):
            ...
            if game_toggle_button(
                "##BasicLootFilterWhiteButton", "Loot White Items", state, width=20, height=20, color=color
            ):
            ...
            if game_toggle_button(
                "##BasicLootFilterBlueButton", "Loot Blue Items", state, width=20, height=20, color=color
            ):
            ...
            if game_toggle_button(
                "##BasicLootFilterPurpleButton", "Loot Purple Items", state, width=20, height=20, color=color
            ):
            ...
            if game_toggle_button(
                "##BasicLootFilterGoldButton", "Loot Gold Items", state, width=20, height=20, color=color
            ):
            ...
            if game_toggle_button(
                "##BasicLootFilterGreenButton", "Loot Green Items", state, width=20, height=20, color=color
            ):
                self.loot_singleton.loot_greens = not self.loot_singleton.loot_greens
            PyImGui.same_line(0, 3)
            self.loot_singleton.loot_gold_coins = ImGui.toggle_button(
                IconsFontAwesome5.ICON_COINS + "##BasicLootFilterGoldCoinsButton",
                self.loot_singleton.loot_gold_coins,
                width=20,
                height=20,
            )
                        ...
                        for subgroup, items in group_items.items():
                            if PyImGui.tree_node(subgroup):
                                if PyImGui.begin_table(
                                    f"##table_{group_name}_{subgroup}",
                                    3,
                                    ...
                                    for item_model_id in items:
                                        ...
                                        new_state = ImGui.image_toggle_button(
                                            f"##loot_toggle_{item_model_id}", item_texture, state, width=48, height=48
                                        )
