...
from Core import ModelID, ItemType
from Core import UIManager
from Core import GLOBAL_CACHE
from Core import AutoInventoryHandler
...
class AutoHandlderModule:
    ...
    def show_item_type_dialog_popup(self):
        ...
        if PyImGui.begin_popup_modal("Item Type Lookup", True, PyImGui.WindowFlags.AlwaysAutoResize):
            ...
            self.auto_handler.item_type_search_mode = PyImGui.radio_button(
                "Contains", self.auto_handler.item_type_search_mode, 0
            )
            PyImGui.same_line(0, -1)
            self.auto_handler.item_type_search_mode = PyImGui.radio_button(
                "Starts With", self.auto_handler.item_type_search_mode, 1
            )
    ...
    def show_model_id_dialog_popup(self):
        ...
        if PyImGui.begin_popup_modal("ModelID Lookup", True, PyImGui.WindowFlags.AlwaysAutoResize):
            ...
            self.auto_handler.model_id_search_mode = PyImGui.radio_button(
                "Contains", self.auto_handler.model_id_search_mode, 0
            )
            PyImGui.same_line(0, -1)
            self.auto_handler.model_id_search_mode = PyImGui.radio_button(
                "Starts With", self.auto_handler.model_id_search_mode, 1
            )
    ...
    def DrawAutoHandler(self):
        ...
        if PyImGui.begin("Embedded AutoHandler", True, flags):
            ...
            self.auto_handler.module_active = ImGui.toggle_button(
                active_button + "##AutoHandlerActive", self.auto_handler.module_active
            )
            ...
            if PyImGui.button(IconsFontAwesome5.ICON_SYNC + "##autosalvreload"):
                ...
                ConsoleLog(
                    self.MODULE_NAME, "Settings reloaded from Auto Inv.ini", PySystem.Console.MessageType.Success
                )
            ...
            if PyImGui.begin_tab_bar("AutoID&SalvageTabs"):
                if PyImGui.begin_tab_item("Identification"):
                    ...
                    if game_toggle_button(
                        "##autoIDWhite", "Identify White Items", state, width=20, height=20, color=color
                    ):
                    ...
                    if game_toggle_button(
                        "##autoIDBlue", "Identify Blue Items", state, width=20, height=20, color=color
                    ):
                    ...
                    if game_toggle_button(
                        "##autoIDPurple", "Identify Purple Items", state, width=20, height=20, color=color
                    ):
                    ...
                    if game_toggle_button(
                        "##autoIDGold", "Identify Gold Items", state, width=20, height=20, color=color
                    ):
                ...
                if PyImGui.begin_tab_item("Salvage"):
                    ...
                    color = ColorPalette.GetColor("GW_White")
                    if game_toggle_button(
                        "##autoSalvageWhite", "Salvage White Items", state, width=20, height=20, color=color
                    ):
                    ...
                    color = ColorPalette.GetColor("GW_Blue")
                    if game_toggle_button(
                        "##autoSalvageBlue", "Salvage Blue Items", state, width=20, height=20, color=color
                    ):
                    ...
                    color = ColorPalette.GetColor("GW_Purple")
                    if game_toggle_button(
                        "##autoSalvagePurple", "Salvage Purple Items", state, width=20, height=20, color=color
                    ):
                    ...
                    color = ColorPalette.GetColor("GW_Gold")
                    if game_toggle_button(
                        "##autoSalvageGold", "Salvage Gold Items", state, width=20, height=20, color=color
                    ):
                ...
                if PyImGui.begin_tab_item("Deposit"):
                    self.auto_handler.deposit_materials = ImGui.toggle_button(
                        IconsFontAwesome5.ICON_HAMMER + "##depositmaterials", self.auto_handler.deposit_materials
                    )
                    ImGui.show_tooltip("Deposit Materials")
                    PyImGui.same_line(0, 3)
                    self.auto_handler.deposit_trophies = ImGui.toggle_button(
                        IconsFontAwesome5.ICON_TROPHY + "##deposittrophies", self.auto_handler.deposit_trophies
                    )
                    ImGui.show_tooltip("Deposit Trophies")
                    PyImGui.same_line(0, 3)
                    self.auto_handler.deposit_event_items = ImGui.toggle_button(
                        IconsFontAwesome5.ICON_HAT_WIZARD + "##depositeventitems", self.auto_handler.deposit_event_items
                    )
                    ImGui.show_tooltip("Deposit Event Items")
                    PyImGui.same_line(0, 3)
                    self.auto_handler.deposit_dyes = ImGui.toggle_button(
                        IconsFontAwesome5.ICON_FLASK + "##depositdyes", self.auto_handler.deposit_dyes
                    )
                    ...
                    if game_toggle_button(
                        "##depositBlue", "Deposit Blue Items", state, width=20, height=20, color=color
                    ):
                    ...
                    color = ColorPalette.GetColor("GW_Purple")
                    if game_toggle_button(
                        "##depositPurple", "Deposit Purple Items", state, width=20, height=20, color=color
                    ):
                    ...
                    color = ColorPalette.GetColor("GW_Gold")
                    if game_toggle_button(
                        "##depositGold", "Deposit Gold Items", state, width=20, height=20, color=color
                    ):
                    ...
                    if game_toggle_button(
                        "##depositGreen", "Deposit Green Items", state, width=20, height=20, color=color
                    ):
                    ...
                    PyImGui.same_line(0, -1)
                    self.auto_handler.keep_gold = PyImGui.input_int(
                        "##keep_gold", self.auto_handler.keep_gold, 1, 1000, PyImGui.InputTextFlags.NoFlag
                    )
    ...
    def update(self):
        ...
        if not auto_handler.initialized:
            ...
            ConsoleLog(
                self.MODULE_NAME, "Auto Handler Widget Options initialized", PySystem.Console.MessageType.Success
            )
