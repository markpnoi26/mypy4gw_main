...
RUNNER_PATH = REPO_ROOT / "Core" / "modular" / "runner.py"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _install_stubs(root)
        runner_mod = importlib.import_module("Core.modular.runner")
...
def _assert_runner_source_contract() -> None:
    ...
    imported_names = {
        ...
        if isinstance(node, ast.ImportFrom) and node.module == "Core.BottingTree"
...
def _install_stubs(modular_root: Path) -> None:
    ...
    paths_mod = types.ModuleType("Core.modular.paths")
    paths_mod.modular_data_root = lambda: str(modular_root)
    sys.modules["Core.modular.paths"] = paths_mod

    botting_tree_mod = types.ModuleType("Core.BottingTree")
    botting_tree_mod.BottingTree = _BottingTree
    sys.modules["Core.BottingTree"] = botting_tree_mod
