...
def _load_target_registry(root: Path) -> object | None:
    registry_path = root.resolve().parents[1] / "Core" / "modular" / "domain" / "target_registry.py"
    if not registry_path.exists():
        registry_path = Path(__file__).resolve().parents[3] / "Core" / "modular" / "domain" / "target_registry.py"
    if not registry_path.exists():
        return None
