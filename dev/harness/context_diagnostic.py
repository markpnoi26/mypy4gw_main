...
def dump_ssm():
    ...
    ok, mod = _safe(__import__,
                    "Core.native_src.ShMem.SysShaMem",
...
CONTEXTS = [
    ...
    ("CharContext",        "Core.native_src.context.CharContext",        "CharContext",              ["player_name_str", "token1", "map_id", "current_map_id", "observe_map_id", "token2", "language"]),
    ("WorldContext",       "Core.native_src.context.WorldContext",       "WorldContext",             ["experience", "level", "morale", "foes_killed"]),
    ("AccAgentContext",    "Core.native_src.context.AccAgentContext",    "AccAgentContext",          ["instance_timer", "rand1", "rand2"]),
    ("MapContext",         "Core.native_src.context.MapContext",         "MapContext",               ["map_type", "map_id", "h0080"]),
    ("GuildContext",       "Core.native_src.context.GuildContext",       "GuildContext",             []),
    ("PreGameContext",     "Core.native_src.context.PreGameContext",     "PreGameContext",           ["frame_id", "chosen_character_index"]),
    ("CinematicContext",   "Core.native_src.context.CinematicContext",   "Cinematic",                ["h0000", "h0004"]),
    ("GameplayContext",    "Core.native_src.context.GameplayContext",    "GameplayContext",          ["mission_map_zoom"]),
    ("WorldMapContext",    "Core.native_src.context.WorldMapContext",    "WorldMapContext",          ["frame_id", "zoom"]),
    ("MissionMapContext",  "Core.native_src.context.MissionMapContext",  "MissionMapContext",        ["frame_id"]),
    ("PartyContext",       "Core.native_src.context.PartyContext",       "PartyContext",             []),
    ("TextParser",         "Core.native_src.context.TextContext",        "TextParser",               ["language_id", "entries_per_file"]),
    ("ServerRegion",       "Core.native_src.context.ServerRegionContext","ServerRegion",             ["region_id"]),
    ("AvailableCharacters","Core.native_src.context.AvailableCharacterContext","AvailableCharacterArray", ["available_characters_list"]),
    # ── special: InstanceInfo with deeper AreaInfo sub-sampling ──
    ("InstanceInfo",       "Core.native_src.context.InstanceInfoContext","InstanceInfo",             None),
]
...
def dump_contexts():
    ...
    ok, mod = _safe(__import__,
                    "Core.native_src.context.InstanceInfoContext",
...
def dump_agentarray_api():
    ...
    ok, mod = _safe(__import__, "Core.AgentArray", fromlist=["AgentArray"])
    if not ok:
...
def dump_layouts():
    ...
    pairs = [
        ("Pointers_SHMemStruct", "Core.native_src.ShMem.structs.PointersSSM",
         "Pointers_SHMemStruct"),
        ("AgentArraySHMemStruct", "Core.native_src.ShMem.structs.AgentArraySSM",
         "AgentArraySHMemStruct"),
        ("SharedMemoryHeader", "Core.native_src.ShMem.SysShaMem",
         "SharedMemoryHeader"),
        ("CharContextStruct", "Core.native_src.context.CharContext",
         "CharContextStruct"),
        ("WorldContextStruct", "Core.native_src.context.WorldContext",
         "WorldContextStruct"),
        ("InstanceInfoStruct", "Core.native_src.context.InstanceInfoContext",
         "InstanceInfoStruct"),
        ("GameContextStruct", "Core.native_src.context.GameContext",
         "GameContextStruct"),
        ("PreGameContextStruct", "Core.native_src.context.PreGameContext",
...
def dump_env():
    ...
    instanceinfo_path = os.path.join(
        DUMP_DIR, "Core", "native_src", "context", "InstanceInfoContext.py"
    )
...
def dump_demo_map_parity():
    ...
    ok, Map = _safe(__import__, "Core.Map", fromlist=["Map"])
    if not ok:
    ...
    # ── Deep gate trace ──
    gates = {}
    ok2, ctx = _safe(__import__, "Core.Context", fromlist=["GWContext"])
    if ok2:
    ...
    char_ctx = None
    ok2, cc = _safe(__import__, "Core.native_src.context.CharContext", fromlist=["CharContext"])
    if ok2:
    ...
    inst = None
    ok2, ic = _safe(__import__, "Core.native_src.context.InstanceInfoContext", fromlist=["InstanceInfo"])
    if ok2:
    ...
    # Player number
    ok2, pl = _safe(__import__, "Core.Player", fromlist=["Player"])
    if ok2:
...
def dump_email_chain():
    ...
    ok, mod_map = _safe(__import__, "Core.Map", fromlist=["Map"])
    if ok:
        Map = mod_map.Map
    ...
    ok, mod_ctx = _safe(__import__, "Core.Context", fromlist=["GWContext"])
    if ok:
    ...
    ok, mod_pl = _safe(__import__, "Core.Player", fromlist=["Player"])
    if ok:
    ...
    ok, mod_cc = _safe(__import__,
                       "Core.native_src.context.CharContext",
...
def dump_party_demo():
    ...
    ok, gc = _safe(__import__, "Core.GlobalCache", fromlist=["GLOBAL_CACHE"])
    if not ok:
        result["error"] = str(gc)
        return result
...
def dump_demo_full():
    ...
    # ── Agent ──
    if agent_id:
        ok2, ag = _safe(__import__, "Core.Agent", fromlist=["Agent"])
        if ok2:
            A = ag.Agent
            for label, fn in [
    ...
    # ── Effects ──
    if agent_id:
        ok2, ef = _safe(__import__, "Core.Effects", fromlist=["Effects"])
        if ok2:
            E = ef.Effects
            for label, fn in [
    ...
    # ── Skillbar ──
    if agent_id:
        ok2, sb = _safe(__import__, "Core.Skillbar", fromlist=["SkillBar"])
        if ok2:
            S = sb.SkillBar
            for label, fn in [
    ...
    # ── Skills ──
    ok2, sk = _safe(__import__, "Core.Skill", fromlist=["Skill"])
    if ok2:
        Sk = sk.Skill
    ...
    # ── Quests ──
    ok2, q = _safe(__import__, "Core.Quest", fromlist=["Quest"])
    if ok2:
        Q = q.Quest
        for label, fn in [
