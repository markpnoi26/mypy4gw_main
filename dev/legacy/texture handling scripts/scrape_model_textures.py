...
def log_missing_image(model_id, readable_name):
    with open(MISSING_LOG_PATH, "a", encoding="utf-8") as f:
        f.write(f"{model_id},{readable_name}\n")


def sanitize_filename(name: str) -> str:
    return name.replace("_", " ")

def get_image_url_from_wiki(item_name: str):
    ...
    try:
        ...
        if not img_tag or not img_tag.get("src"):
            return None, None
        src = img_tag["src"]
        full_url = "https://wiki.guildwars.com" + src
...
# ----[ MAIN FUNCTION ]----
def scrape_all_model_ids():
    print(f"Saving to: {OUTPUT_FOLDER}\n")
    for name, model_id in ModelID.__members__.items():
        readable_name = sanitize_filename(name)
        file_prefix = f"[{model_id}] - {readable_name}"
        print(f"🔎 {file_prefix}...")

        existing = [f for f in os.listdir(OUTPUT_FOLDER) if f.startswith(f"[{model_id}]")]
        if existing:
            print(f"Already exists, skipping.")
            continue

        image_url, ext = get_image_url_from_wiki(name)
        if not image_url:
            log_missing_image(model_id, readable_name)
            continue
