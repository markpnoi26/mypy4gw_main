...
def tessellate_geometry(geometry, trapezoids, tolerance=1e-5):
    """
    Tessellate quads based on their neighbors using adjacency information.
    """
    merged_geometry = []
    visited = set()

    def are_quads_mergeable(quad1, quad2):
        """
        Check if two quads share an edge within a tolerance, allowing merging.
        """
        shared_edges = 0
        for i in range(4):
            for j in range(4):
                if (
                    abs(quad1[i][0] - quad2[j][0]) < tolerance and
                    abs(quad1[i][1] - quad2[j][1]) < tolerance
                ):
                    shared_edges += 1
        return shared_edges >= 2

    def merge_quads(quad1, quad2):
        """
        Merge two quads into one by combining and deduplicating their vertices.
        """
        points = quad1 + quad2
        unique_points = list(set(points))  # Deduplicate vertices
        if len(unique_points) <= 4:
            return unique_points  # Return as-is if already a quad
        return unique_points[:4]  # Handle more than 4 points (simplistic convex hull needed)

    def process_quad(index, neighbors):
        """
        Process a quad by merging it with its neighbors if possible.
        """
        if index in visited:
            return
        visited.add(index)
        current_quad = geometry[index]
        for neighbor_id in trapezoids[index].neighbor_ids:
            if 0 <= neighbor_id < len(trapezoids):  # Ensure valid neighbor ID
                neighbor_quad = geometry[neighbor_id]
                if neighbor_id not in visited and are_quads_mergeable(current_quad, neighbor_quad):
                    current_quad = merge_quads(current_quad, neighbor_quad)
                    visited.add(neighbor_id)
            else:
                PySystem.Console.Log(
                    module_name,
                    f"Invalid neighbor ID {neighbor_id} for trapezoid {index}. Skipping.",
                    PySystem.Console.MessageType.Warning,
                )
        merged_geometry.append(current_quad)
...
def precompute_layer_geometry1(pathing_map, width, height, tolerance=1e-5):
    ...
    """
    global precomputed_geometry, map_boundaries, quad_count
    ...
    for layer in pathing_map:
        ...
        tessellated_geometry = tessellate_geometry(geometry, trapezoids, tolerance)
        precomputed_geometry[layer.zplane] = tessellated_geometry
        PySystem.Console.Log(module_name, f"Layer {layer.zplane}: Tessellated into {len(tessellated_geometry)} quads.", PySystem.Console.MessageType.Info)
