...
Core imports depend on PyScanner/PyPathing/etc.
...
def test_trapezoid_x_bounds_at_y_flat_edges():
    from Core.Pathing import _trapezoid_x_bounds_at_y
...
def test_is_within_margin_fully_inside():
    from Core.Pathing import _is_within_margin
    trap = _make_rect_trap(0.0, 1000.0, 0.0, 800.0)
...
def test_clamp_to_safe_zone_y_clamped_first():
    from Core.Pathing import _clamp_to_safe_zone
    trap = _MockTrap(0.0, 1000.0, 0.0, 200.0, 800.0, 800.0)
...
def test_correct_margin_empty_path():
    assert IMPORT_OK
    from Core.Pathing import _correct_margin
...
if __name__ == "__main__":
    ...
    if not IMPORT_OK:
        print(f"SKIP: Cannot import Core (game not running): {_import_error}")
