...
class BottingTreeAccountConfig:
    ...
    def __post_init__(self) -> None:
        ...
        self.controlled_account_emails = tuple(
            str(email or '').strip() for email in self.controlled_account_emails if str(email or '').strip()
        )
    ...
    def coerce(
        ...
        *,
        ...
        resolved_account_mode = (
            ...
            else (BottingTreeAccountMode.MULTI_ACCOUNT if multi_account else BottingTreeAccountMode.SINGLE_ACCOUNT)
        )
        if isinstance(value, cls):
            config = cls(
                mode=value.mode,
