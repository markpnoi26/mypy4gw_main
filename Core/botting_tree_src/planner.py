...
class BottingTreePlannerMixin:
    ...
    if TYPE_CHECKING:
        ...
        def _tick_service_tree(
            self, node: BehaviorTree.Node, service_tree: BehaviorTree, service_name: str
    ...
    def SetMainRoutine(
        ...
        if routine is None:
            ...
            if routine_items and all(
                isinstance(item, tuple) and len(item) == 2 and isinstance(item[0], str) for item in routine_items
            ):
    ...
    def SetNamedPlannerSteps(
        ...
        self._set_planner_tree(
            self._build_named_planner_tree(self._planner_steps, start_from=start_from, name=name, repeat=repeat)
        )
