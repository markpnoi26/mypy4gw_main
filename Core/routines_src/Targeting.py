...
class Targeting:
    ...
    def FilterAllyArray(array, distance, other_ally=False, filter_skill_id=0):
        ...
        if filter_skill_id != 0:
            array = AgentArray.Filter.ByCondition(
                array, lambda agent_id: not Checks.Agents.HasEffect(agent_id, filter_skill_id)
            )
    ...
    def TargetLowestAllyEnergy(other_ally=False, filter_skill_id=0):
        ...
        ally_array = AgentArray.Filter.ByCondition(
            ally_array, lambda agent_id: not Checks.Agents.HasEffect(agent_id, BLOOD_IS_POWER)
        )
        ally_array = AgentArray.Filter.ByCondition(
            ally_array, lambda agent_id: not Checks.Agents.HasEffect(agent_id, BLOOD_RITUAL)
        )
    ...
    def GetEnemyCastingSpellOrChant(max_distance=4500.0, aggressive_only=False):
        ...
        def _filter_spells_or_chants(enemy_array):
            ...
            for enemy_id in enemy_array:
                casting_skill_id = Agent.GetCastingSkillID(enemy_id)
                if GLOBAL_CACHE.Skill.Flags.IsSpell(casting_skill_id) or GLOBAL_CACHE.Skill.Flags.IsChant(
                    casting_skill_id
                ):
                    result_array.append(enemy_id)
            return result_array
    ...
    def GetEnemyInjured(max_distance=4500.0, aggressive_only=False):
        ...
        enemy_array = AgentArray.Sort.ByCondition(
            enemy_array, lambda agent_id: (Agent.GetHealth(agent_id), Utils.Distance(player_pos, Agent.GetXY(agent_id)))
        )
        return Utils.GetFirstFromArray(enemy_array)
    ...
    def GetEnemyHealthy(max_distance=4500.0, aggressive_only=False):
        ...
        enemy_array = AgentArray.Sort.ByCondition(
            enemy_array,
            lambda agent_id: (-Agent.GetHealth(agent_id), Utils.Distance(player_pos, Agent.GetXY(agent_id))),
        )
        return Utils.GetFirstFromArray(enemy_array)
    ...
    def GetEnemyWithEffect(effect_skill_id, max_distance=4500.0, aggressive_only=False):
        ...
        enemy_array = AgentArray.Filter.ByCondition(
            enemy_array, lambda agent_id: Checks.Effects.HasEffect(agent_id, effect_skill_id)
        )
        enemy_array = AgentArray.Sort.ByDistance(enemy_array, player_pos)
        return Utils.GetFirstFromArray(enemy_array)
