...
class Agents:
    ...
    def GetNearestAliveAgentByModelID(model_id: int, max_distance: float = 4500.0) -> int:
        ...
        agent_array = AgentArray.Filter.ByCondition(
            agent_array, lambda agent_id: Agent.GetModelID(agent_id) == model_id
        )
    ...
    def GetFilteredEnemyArray(x, y, max_distance=4500.0, aggressive_only=False):
        ...
        enemy_array = AgentArray.Filter.ByCondition(
            enemy_array, lambda agent_id: Utils.Distance((x, y), Agent.GetXY(agent_id)) <= max_distance
        )
    ...
    @staticmethod
    def GetResurrectionTarget(
        max_distance=4500.0, reserve: bool = False, skill_id: int = 0, aftercast_delay: int = 250
    ):
        from ..Py4GWcorelib import Utils
    ...
    def GetCorpses(max_distance=4500.0):
        ...
        def _AllowedAlliegance(agent_id):
            ...
            if alliegance == "Ally" or alliegance == "Neutral" or alliegance == "Enemy" or alliegance == "NPC/Minipet":
                return True
            return False
    ...
    @staticmethod
    def GetNearestExploitableCorpse(
        max_distance=4500.0, reserve: bool = False, skill_id: int = 0, aftercast_delay: int = 250
    ):
        from ..AgentArray import AgentArray
        from ..Agent import Agent
    ...
    def GetNearestGadgetByID(gadget_id: int, max_distance=4500.0):
        ...
        gadget_array = AgentArray.GetGadgetArray()
        gadget_array = AgentArray.Filter.ByCondition(
            gadget_array, lambda agent_id: Agent.GetGadgetID(agent_id) == gadget_id
        )
        gadget_array = AgentArray.Filter.ByDistance(gadget_array, Player.GetXY(), max_distance)
        gadget_array = AgentArray.Sort.ByDistance(gadget_array, Player.GetXY())
        return Utils.GetFirstFromArray(gadget_array)
