...
class Movement:
    ...
    def FollowPath(
        ...
        while idx < len(path_points):
            ...
            ConsoleLog(
                "FollowPath",
                f"Starting point {idx+1}/{total_points} - ({target_x}, {target_y}) distance {Utils.Distance(Player.GetXY(), (target_x, target_y))}",
            ...
            if stop_on_party_wipe and (Checks.Party.IsPartyWiped() or GLOBAL_CACHE.Party.IsPartyDefeated()):
                ConsoleLog(
                    "FollowPath", "Party wiped detected, stopping all movement.", Console.MessageType.Warning, log=True
                )
                ActionQueueManager().ResetAllQueues()
                return False
            ...
            ConsoleLog(
                "FollowPath",
                f"Issued move command to ({target_x}, {target_y}).",
            ...
            while True:
                ...
                if custom_exit_condition():
                    ConsoleLog(
                        "FollowPath", "Custom exit condition met, stopping movement.", Console.MessageType.Info, log=log
                    )
                    return False
                ...
                if stop_on_party_wipe and (Checks.Party.IsPartyWiped() or GLOBAL_CACHE.Party.IsPartyDefeated()):
                    ConsoleLog(
                        "FollowPath",
                        "Party wiped detected, stopping all movement.",
                ...
                if Agent.IsValid(Player.GetAgentID()) and Agent.IsCasting(Player.GetAgentID()):
                    ConsoleLog(
                        "FollowPath",
                        "Player casting detected, waiting 750ms...",
                        Console.MessageType.Debug,
                        log=detailed_log,
                    )
                ...
                if custom_pause_fn:
                    ...
                    while custom_pause_fn():
                        ...
                        if custom_exit_condition():
                            ConsoleLog(
                                "FollowPath",
                                "Custom exit condition met while movement was paused, stopping movement.",
                                Console.MessageType.Info,
                                log=log,
                            )
                            return False
                        if stop_on_party_wipe and (
                            Checks.Map.MapValid()
                            and (Checks.Party.IsPartyWiped() or GLOBAL_CACHE.Party.IsPartyDefeated())
                        ):
                            ConsoleLog(
                                "FollowPath",
                                "Party wiped detected during pause, stopping all movement.",
                                Console.MessageType.Warning,
                                log=True,
                            )
                            ActionQueueManager().ResetAllQueues()
                        ...
                        ConsoleLog(
                            "FollowPath",
                            "Custom pause condition active, pausing movement...",
                            Console.MessageType.Debug,
                            log=log,
                        )
                ...
                if delta > timeout and timeout > 0:
                    ConsoleLog(
                        "FollowPath",
                        f"Timeout reached, stopping movement. distance to failes point {Utils.Distance(Player.GetXY(), (target_x, target_y))}",
                        Console.MessageType.Warning,
                        log=log,
                    )
                    return False
                ...
                if not (current_distance < previous_distance):
                    ...
                    ConsoleLog(
                        "FollowPath",
                        f"move to {target_x + offset_x}, {target_y + offset_y}",
                        Console.MessageType.Info,
                        log=log,
                    )
                    if not _map_still_valid():
                    ...
                    if retries >= max_retries:
                        ...
                        if stuck_count >= max_stuck_commands:
                            ConsoleLog(
                                "FollowPath",
                                "Too many stucks, performing strafe recovery.",
                                Console.MessageType.Warning,
                                log=log,
                            )
                    ...
                    ConsoleLog(
                        "FollowPath",
                        "Progress detected, reset retry counters.",
                        Console.MessageType.Debug,
                        log=detailed_log,
                    )
                ...
                if current_distance <= tolerance:
                    ConsoleLog(
                        "FollowPath",
                        f"Reached target point {idx+1}/{total_points}.",
                    ...
                    ConsoleLog(
                        "FollowPath",
                        f"Current distance to target: {current_distance}, waiting...",
                        Console.MessageType.Debug,
                        log=detailed_log,
                    )
            ...
            if progress_callback:
                ...
                ConsoleLog(
                    "FollowPath",
                    f"Progress callback: {((idx + 1) / total_points) * 100:.1f}% done.",
                    Console.MessageType.Debug,
                    log=detailed_log,
                )
