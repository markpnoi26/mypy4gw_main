...
class Keybinds:
    ...
    def ActivateWeaponSet(index: int, log=False):
        if index < 1 or index > 4:
            return
        yield from Keybinds.PressKeybind(
            ControlAction.ControlAction_ActivateWeaponSet1.value + (index - 1), 75, log=log
        )
    ...
    def TargetPartyMember(index: int, log=False):
        if index < 1 or index > 12:
            return
        yield from Keybinds.PressKeybind(
            ControlAction.ControlAction_TargetPartyMember1.value + (index - 1), 75, log=log
        )
    ...
    def OpenHeroPetCommander(hero_index: int, log=False):
        if hero_index < 1 or hero_index > 7:
            return
        yield from Keybinds.PressKeybind(
            ControlAction.ControlAction_OpenHero1PetCommander.value + (hero_index - 1), 75, log=log
        )
    ...
    def OpenHeroCommander(hero_index: int, log=False):
        if hero_index < 1 or hero_index > 7:
            return
        yield from Keybinds.PressKeybind(
            ControlAction.ControlAction_OpenHeroCommander1.value + (hero_index - 1), 75, log=log
        )
    ...
    def HeroSkill(hero_index: int, skill_slot: int, log=False):
        ...
        if hero_index < 1 or hero_index > party_size:
            return
        yield from Keybinds.PressKeybind(
            ControlAction.ControlAction_Hero1Skill1.value + (hero_index - 1) * 8 + (skill_slot - 1), 75, log=log
        )
