...
class Merchant:
    ...
    def RestockKitsToTarget(
        ...
        *,
        ...
        initial_id_kits = Merchant._count_model_in_inventory(
            ModelID.Identification_Kit.value
        ) + Merchant._count_model_in_inventory(ModelID.Superior_Identification_Kit.value)
    ...
    def _get_trader_batch_size(trader_items: list[int] | None = None) -> int:
        offered_items = (
            trader_items if trader_items is not None else list(GLOBAL_CACHE.Trading.Trader.GetOfferedItems())
        )
    ...
    @staticmethod
    def _scan_material_item_ids(
        selected_models: set[int] | None = None, exact_quantity: int | None = None
    ) -> list[int]:
    ...
    def SellMaterial(model_id: int):
        ...
        def _is_material_trader():
            merchant_models = [
                GLOBAL_CACHE.Item.GetModelID(item_id) for item_id in GLOBAL_CACHE.Trading.Trader.GetOfferedItems()
            ]
        ...
        while True:
            ...
            ConsoleLog(
                MODULE_NAME, f"Attempted to request sell quote for item {quoted_id}.", Console.MessageType.Warning
            )
            ConsoleLog(
                MODULE_NAME, f"Received sell quote for item {item_id} at cost: {cost}", Console.MessageType.Warning
            )
            if cost >= 0:
                break
