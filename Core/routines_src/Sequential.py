    ...
    class Skills:
        ...
        def CastSkillID(skill_id: int, extra_condition=True, log=False):
            ...
            if log:
                ConsoleLog(
                    "CastSkillID",
                    f"Cast {GLOBAL_CACHE.Skill.GetName(skill_id)}, slot: {GLOBAL_CACHE.SkillBar.GetSlotBySkillID(skill_id)}",
        ...
        def DepositGold(gold_amount_to_leave_on_character: int, log=False):
            ...
            if available_space <= 0 or gold_to_deposit <= 0:
                if log:
                    ConsoleLog(
                        "DepositGold",
                        "No gold deposited (either storage full or not enough excess gold).",
            ...
            if log:
                ConsoleLog(
                    "DepositGold",
                    f"Deposited {gold_to_deposit} gold. Remaining on character: {gold_amount_to_leave_on_character}.",
