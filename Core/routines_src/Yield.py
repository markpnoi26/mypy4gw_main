...
from Core.py4gwcorelib_src.Timer import Timer, ThrottledTimer
from Core.enums_src.IO_enums import Key
from Core.py4gwcorelib_src.Keystroke import Keystroke
from Core.Agent import Agent
from Core.FrameTree import Frame, FrameId
from Core.Player import Player
from Core.routines_src import Checks
from ..Map import Map
...
class Yield:
    ...
    class Upkeepers:
        ...
        CITY_SPEED_EFFECTS = [
            1860,  # Sugar_Rush_short, // 1 minute
        ...
        def Upkeep_EssenceOfCelerity():
            yield from Yield.Upkeepers._upkeep_consumable(
        ...
        def Upkeep_Morale(target_morale=110):
            ...
            morale_models = [(m.value if hasattr(m, "value") else int(m)) for m in Yield.Upkeepers.MORALE_ITEMS]
            party_morale_models = [model_id for model_id in morale_models if model_id in PARTY_MORALE_MODELS]
            self_morale_models = [model_id for model_id in morale_models if model_id not in PARTY_MORALE_MODELS]
        ...
        def Upkeep_Alcohol(target_alc_level=2, disable_drunk_effects=False):
            ...
            alcohol_models = [(m.value if hasattr(m, "value") else int(m)) for m in Yield.Upkeepers.ALCOHOL_ITEMS]
    ...
    class RerollCharacter:
        @staticmethod
        def DeleteCharacter(
            character_name_to_delete: str, timeout_ms: int = 10000, log: bool = False
            ...
            if _timeout_reached():
                ConsoleLog(
                    "Reroll", "Timeout while waiting to reach character select screen.", Console.MessageType.Error, log
                )
                yield from Yield.wait(100)
                return False
            ...
            character_index = (
                pregame.chars_list.index(character_name_to_delete)
                if character_name_to_delete in pregame.chars_list
                else -1
            )
        ...
        @staticmethod
        def CreateCharacter(
            character_name: str, campaign_name: str, profession_name: str, timeout_ms: int = 15000, log: bool = False
        ...
        @staticmethod
        def DeleteAndCreateCharacter(
            character_name_to_delete: str,
            ...
            result = yield from Yield.RerollCharacter.DeleteCharacter(
                character_name_to_delete, timeout_ms=timeout_ms // 2, log=log
            )
            if not result:
                return
            yield from Yield.wait(1000)  # brief wait before creating new character
            yield from Yield.RerollCharacter.CreateCharacter(
                new_character_name, campaign_name, profession_name, timeout_ms=timeout_ms // 2, log=log
            )
        ...
        def Reroll(target_character_name: str, timeout_ms: int = 10000, log: bool = False) -> Generator[Any, Any, None]:
            ...
            if target_character_name not in character_names:
                ConsoleLog(
                    "Reroll",
                    f"Character '{target_character_name}' not found among login characters.",
            ...
            if Player.GetName() == target_character_name and not Map.Pregame.InCharacterSelectScreen():
                ConsoleLog(
                    "Reroll",
                    f"Already logged in as '{target_character_name}'. No reroll needed.",
            ...
            if pregame is None:
                ...
                return
            character_index = (
                pregame.chars_list.index(target_character_name) if target_character_name in pregame.chars_list else -1
            )
            last_known_index = pregame.chosen_character_index
            ...
            if character_index == -1:
                ConsoleLog(
                    "Reroll",
                    f"Character '{target_character_name}' not found in character list.",
                    Console.MessageType.Error,
                )
                yield from Yield.wait(100)
                return
            ...
            while last_known_index != character_index and not _failed():
                ...
                if distance != 0:
                    ...
                    ConsoleLog(
                        "Reroll",
                        f"Navigating {'Right' if distance > 0 else 'Left'} (Current: {last_known_index}, Target: {character_index})",
