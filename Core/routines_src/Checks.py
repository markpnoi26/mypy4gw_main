...
class Checks:
    ...
    class Inventory:
        ...
        def IsModelInInventoryOrBank(model_id: int):
            ...
            return (
                GLOBAL_CACHE.Inventory.GetModelCount(model_id) + GLOBAL_CACHE.Inventory.GetModelCountInStorage(model_id)
            ) > 0
        ...
        def IsModelInInventoryOrEquipped(model_id: int):
            ...
            return (
                GLOBAL_CACHE.Inventory.GetModelCount(model_id)
                + GLOBAL_CACHE.Inventory.GetModelCountInEquipped(model_id)
            ) > 0
    ...
    class Agents:
        ...
        def HasIllusionaryWeaponry(agent_id: int) -> bool:
            ...
            for skill_id in iw_skill_ids:
                if not skill_id:
                    continue
                if Checks.Agents.HasEffect(agent_id, skill_id) or Checks.Agents._shared_agent_has_skill_equipped(
                    agent_id, skill_id
                ):
                    return True
            return False
    ...
    class Skills:
        ...
        def CanCast():
            ...
            if (
                Checks.Player.IsCasting()
                or Checks.Player.IsDead()
                or Checks.Player.IsKnockedDown()
        ...
        def apply_expertise_reduction(base_cost: int, expertise_level: int, skill_id) -> int:
            ...
            _, skill_profession = GLOBAL_CACHE.Skill.GetProfession(skill_id)
            if (
                skill_type == 14  # attack skills
                or GLOBAL_CACHE.Skill.Flags.IsRitual(skill_id)
                or GLOBAL_CACHE.Skill.Flags.IsTouchRange(skill_id)
                or skill_profession == "Ranger"
            ):
            ...
            for effect in player_effects:
                ...
                match effect_id:
                    ...
                    case 1223:  # Anguished Was Lingwah
                        if (
                            GLOBAL_CACHE.Skill.Flags.IsHex(skill_id)
                            and GLOBAL_CACHE.Skill.GetProfession(skill_id)[0] == 8
                        ):
                            match attr:
                                case a if 0 < a <= 1:
                                    cost -= 1
                    ...
                    case 806:  # Cultist's Fervor
                        if (
                            GLOBAL_CACHE.Skill.Flags.IsSpell(skill_id)
                            and GLOBAL_CACHE.Skill.GetProfession(skill_id)[0] == 4
                        ):
                            match attr:
                                case a if 0 < a <= 1:
                                    cost -= 1
                    ...
                    case 310:  # Divine Spirit
                        if (
                            GLOBAL_CACHE.Skill.Flags.IsSpell(skill_id)
                            and GLOBAL_CACHE.Skill.GetProfession(skill_id)[0] == 3
                        ):
                            cost -= 5
                        continue
                    ...
                    case 2145:  # Expert Focus
                        if (
                            GLOBAL_CACHE.Skill.Flags.IsAttack(skill_id)
                            and GLOBAL_CACHE.Skill.Data.GetWeaponReq(skill_id) == 2
                        ):
                            match attr:
                                case a if 0 < a <= 7:
                                    cost -= 1
                    ...
                    case 1394:  # Healer's Covenant
                        if (
                            GLOBAL_CACHE.Skill.Flags.IsSpell(skill_id)
                            and GLOBAL_CACHE.Skill.Attribute.GetAttribute(skill_id).attribute_id == 15
                        ):
                            match attr:
                                case a if 0 < a <= 3:
                                    cost -= 1
                    ...
                    case 1739:  # Renewing Memories
                        if GLOBAL_CACHE.Skill.Flags.IsItemSpell(skill_id) or GLOBAL_CACHE.Skill.Flags.IsWeaponSpell(
                            skill_id
                        ):
                    ...
                    case 987:  # Way of the Empty Palm
                        if (
                            GLOBAL_CACHE.Skill.Data.GetCombo(skill_id) == 2
                            or GLOBAL_CACHE.Skill.Data.GetCombo(skill_id) == 3
