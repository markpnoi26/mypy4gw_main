...
class BTAgents:
    ...
    @staticmethod
    def WaitUntilOutOfCombat(
        range: float = Range.Earshot.value, timeout_ms: int = 60000, log: bool = False
    ) -> BehaviorTree:
        """
    ...
    def GetAgentIDByName(agent_name: str) -> BehaviorTree:
        ...
        def _search_name(node):
            ...
            return BehaviorTree.NodeState.SUCCESS if found != 0 else BehaviorTree.NodeState.FAILURE

        tree = BehaviorTree.SequenceNode(
            name="GetAgentIDByNameRoot",
            children=[BehaviorTree.ConditionNode(name="SearchName", condition_fn=_search_name)],
        )
    ...
    def GetAgentIDByModelID(modelID_or_encStr: int | str, log: bool = False) -> BehaviorTree:
        ...
        tree = BehaviorTree.ActionNode(name="GetAgentIDByModelID", action_fn=_search_model_id)
        return BehaviorTree(tree)
    ...
    def TargetAgentByName(agent_name: str, log: bool = False):
        ...
        """
        tree = BehaviorTree.SequenceNode(
            name="TargetAgentByName",
            children=[
                BehaviorTree.SubtreeNode(
                    name="GetAgentIDByNameSubtree", subtree_fn=lambda node: BTAgents.GetAgentIDByName(agent_name)
                ),
                BehaviorTree.SubtreeNode(
                    name="ChangeTargetSubtree",
    ...
    def TargetNearestNPC(distance: float = 4500.0, log: bool = False):
        ...
        def _find_nearest_npc(node):
            ...
            if nearest_npc != 0:
                _log(
                    "TargetNearestNPC", f"Found nearest NPC with ID {nearest_npc} within distance {distance}.", log=log
                )
                return BehaviorTree.NodeState.SUCCESS
    ...
    def TargetNearestNPCXY(x, y, distance, log: bool = False):
        ...
        def _find_nearest_npc_xy(node):
            ...
            if nearest_npc != 0:
                _log(
                    "TargetNearestNPCXY",
                    f"Found nearest NPC with ID {nearest_npc} near ({x}, {y}) within distance {distance}.",
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
    ...
    def TargetNearestGadgetXY(x, y, distance, log: bool = False):
        ...
        def _find_nearest_gadget_xy(node):
            ...
            if nearest_gadget != 0:
                _log(
                    "TargetNearestGadgetXY",
                    f"Found nearest gadget with ID {nearest_gadget} near ({x}, {y}) within distance {distance}.",
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
        ...
        tree = BehaviorTree.SequenceNode(
            name="TargetNearestGadgetXYRoot",
            children=[
    ...
    def TargetNearestItemXY(x, y, distance, log: bool = False):
        ...
        def _find_nearest_item_xy(node):
            ...
            if nearest_item != 0:
                _log(
                    "TargetNearestItemXY",
                    f"Found nearest item with ID {nearest_item} near ({x}, {y}) within distance {distance}.",
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
    ...
    def TargetNearestEnemy(distance, log: bool = False):
        ...
        def _find_nearest_enemy(node):
            ...
            if nearest_enemy != 0:
                _log(
                    "TargetNearestEnemy",
                    f"Found nearest enemy with ID {nearest_enemy} within distance {distance}.",
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
    ...
    def TargetNearestItem(distance, log: bool = False):
        ...
        def _find_nearest_item(node):
            ...
            if nearest_item != 0:
                _log(
                    "TargetNearestItem",
                    f"Found nearest item with ID {nearest_item} within distance {distance}.",
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
    ...
    def TargetNearestChest(distance, log: bool = False):
        ...
        def _find_nearest_chest(node):
            ...
            if nearest_chest != 0:
                _log(
                    "TargetNearestChest",
                    f"Found nearest chest with ID {nearest_chest} within distance {distance}.",
                    log=log,
                )
                return BehaviorTree.NodeState.SUCCESS
