...
def get_nonsalvageable_gold_item_ids() -> list[int]:
    ...
    for item_id in item_array:
        ...
        if (
            ...
            and not GLOBAL_CACHE.Item.Usage.IsSalvageable(item_id)
        ):
...
def wait_for_outbound_messages(
    ...
    *,
    ...
    from Core import GLOBAL_CACHE, Player, Routines
...
def add_toggle_widgets_state(
    ...
    *,
    ...
    from Core import GLOBAL_CACHE, Player, Routines, SharedCommandType
...
def add_restock_consumables_state(bot, *, name: str = "Restock Consumables", log: LogFn | None = None) -> None:
    ...
    bot.States.AddCustomState(
        lambda: Inventory.OpenXunlaiWindow() if not Inventory.IsStorageOpen() else None, f"{name} Open Xunlai"
    )
    ...
    if scheduled == 0:
        bot.States.AddCustomState(
            lambda: log_fn("restock_cons found no restock methods to execute."), f"{name} Warn: No Restock Methods"
        )
...
def add_deposit_materials_state(
    ...
    *,
    ...
    from Core import GLOBAL_CACHE, Inventory, Player, Routines, SharedCommandType
    ...
    def _deposit_local():
        ...
        if multibox:
            ...
            extra_data = (
                "deposit",
                encode_material_model_filter(selected_models),
                str(max_deposit_items or 0),
                str(exact_quantity or 0),
            )
            sent_messages: list[tuple[str, int]] = []
            for account_email in iter_other_account_emails():
                message_index = GLOBAL_CACHE.ShMem.SendMessage(
