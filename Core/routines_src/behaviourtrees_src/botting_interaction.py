...
from Core.routines_src.behaviourtrees_src.botting_movement import cutscene_active, request_skip_cinematic
...
def add_dialog_multibox_state(
    ...
    *,
    ...
    def _wait_for_send_dialog_to_target(sender_email: str, refs: list[tuple[str, int]]):
        from Core import GLOBAL_CACHE, Routines, SharedCommandType
    ...
    def _dialogs_multibox_core():
        from Core import GLOBAL_CACHE, Player, SharedCommandType
        ...
        account_emails = [
            str(getattr(account, "AccountEmail", "") or "") for account in GLOBAL_CACHE.ShMem.GetAllAccountData()
        ]
...
def add_loot_chest_state(
    ...
    *,
    ...
    def _loot():
        from Core import AgentArray, GLOBAL_CACHE, Player, Routines, SharedCommandType
...
def add_interact_item_state(
    ...
    *,
    ...
    def _interact():
        from Core import Agent, AgentArray, Item, Player
...
def add_key_press_state(bot, *, key_name: str, name: str | None = None, log: LogFn | None = None) -> bool:
    from Core import Key, Keystroke
...
def add_debug_nearby_enemies_state(
    ...
    *,
    ...
    def _debug_nearby_enemies() -> None:
        from Core import Agent, AgentArray, Player, Utils
