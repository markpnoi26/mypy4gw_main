...
class BTItems:
        ...
        def _equip_inventory_bag(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if state["stage"] == "fallback_wait_before_double_click":
                ...
                if not slot.is_usable:
                    _fail_log(
                        "EquipInventoryBag", "Frame does not exist for backpack slot 0.", Console.MessageType.Error
                    )
                    _reset_state()
                    return BehaviorTree.NodeState.FAILURE
    ...
    def DestroyBonusItems(
        ...
        bonus_models_to_destroy = [
            model_id for model_id in BTItems.BONUS_ITEM_MODELS if model_id not in excluded_models
        ]
    ...
    def WaitForAnyModelInInventory(
        ...
        def _wait_for_any_model() -> BehaviorTree.NodeState:
            ...
            for model_id in model_ids:
                ...
                if item_id != 0:
                    _log(
                        "WaitForAnyModelInInventory",
                        f"Detected inventory model {model_id} as item_id={item_id}.",
                        message_type=Console.MessageType.Info,
                        log=log,
                    )
                    return BehaviorTree.NodeState.SUCCESS
                ...
                if deposit_all and gold_on_char > target_gold:
                    ...
                    if to_deposit > 0:
                        ...
                        if log:
                            _log(
                                "WithdrawGold",
                                f"Deposited {to_deposit} gold (excess).",
                                message_type=Console.MessageType.Info,
                                log=log,
                            )
        ...
        def _collect_items(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if log:
                excluded_models_text = (
                    ", ".join(str(model_id) for model_id in sorted(set(exclude_models or []))) or "none"
                )
                _log(
                    "SellInventoryItems",
    ...
    def IdentifyInventoryItems(
        ...
        def tick(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if state["gen"] is None:
                ...
                PySystem.Console.Log(
                    ...
                    f"Auto-collected {len(item_ids)} unidentified items (rarities={sorted(active_rarities)}).",
        ...
        return BehaviorTree(BehaviorTree.ActionNode(name="IdentifyInventoryItems", action_fn=tick, aftercast_ms=0))
    ...
    def SalvageInventoryItems(
        ...
        def tick(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if state["gen"] is None:
                ...
                PySystem.Console.Log(
                    ...
                    f"Auto-collected {len(item_ids)} salvageable items (rarities={sorted(active_rarities)}).",
                    PySystem.Console.MessageType.Info,
                )
                if not item_ids:
                    return BehaviorTree.NodeState.SUCCESS
        ...
        return BehaviorTree(BehaviorTree.ActionNode(name="SalvageInventoryItems", action_fn=tick, aftercast_ms=0))
