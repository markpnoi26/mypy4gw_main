...
def build_toggle_summary(toggle: str, enabled: bool, results: list[dict[str, str | int]]) -> dict:
    ...
    zero_targets = any(
        str(entry.get("backend", "")) == ENGINE_HERO_AI and int(entry.get("targeted", 0) or 0) == 0 for entry in results
    )
    return {
        "toggle": str(toggle),
...
def initialize_desired_auto_state_defaults(
    bot, *, combat: bool = True, looting: bool = True, following: bool = True
) -> dict[str, bool]:
