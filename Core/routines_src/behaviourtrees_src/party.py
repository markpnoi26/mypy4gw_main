...
class BTParty:
    ...
    def ForceHeroState(behavior: int, log: bool = False, aftercast_ms: int = 125) -> BehaviorTree:
        ...
        def _force_hero_state() -> BehaviorTree.NodeState:
            ...
            if behavior_value not in (0, 1, 2):
                _fail_log(
                    "BTParty.ForceHeroState",
                    f"Failed to update hero behavior: invalid behavior value {behavior_value}.",
                )
                return BehaviorTree.NodeState.FAILURE
            used = 0
    ...
    @staticmethod
    def WaitForActiveQuest(
        quest_id: int, timeout_ms: int = 10000, throttle_interval_ms: int = 250, log: bool = False
    ) -> BehaviorTree:
        """
