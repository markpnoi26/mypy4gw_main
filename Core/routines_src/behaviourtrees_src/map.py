...
class BTMap:
    ...
    def TravelToOutpost(
        ...
        def arrived_early(target_outpost_id: int) -> bool:
            ...
            if target_outpost_id <= 0:
                _fail_log(
                    "TravelToOutpost", f"Failed to travel: invalid outpost target '{outpost_name or outpost_id}'."
                )
                return False
        ...
        def map_arrival(target_outpost_id: int) -> BehaviorTree.NodeState:
            ...
            if target_outpost_id <= 0:
                return BehaviorTree.NodeState.FAILURE
            if Map.IsMapReady() and GLOBAL_CACHE.Party.IsPartyLoaded() and Map.IsMapIDMatch(0, target_outpost_id):
                _log("TravelToOutpost", f"Arrived at {Map.GetMapName(target_outpost_id)}", log=log)
                return BehaviorTree.NodeState.SUCCESS
            return BehaviorTree.NodeState.RUNNING

        tree = BehaviorTree.SelectorNode(
            children=[
                BehaviorTree.ConditionNode(
                    name="ArrivedEarly", condition_fn=lambda: arrived_early(resolved_outpost_id)
                ),
                BehaviorTree.SequenceNode(
                    name="TravelSequence",
                    children=[
                        BehaviorTree.ActionNode(
                            name="TravelAction", action_fn=lambda: travel_action(resolved_outpost_id), aftercast_ms=3000
                        ),
                        BehaviorTree.WaitNode(
                            name="MapArrival", check_fn=lambda: map_arrival(resolved_outpost_id), timeout_ms=timeout
                        ),
    ...
    @staticmethod
    def TravelToRegion(
        outpost_id, region: int, district: int = 1, language: int = 0, log: bool = False, timeout: int = 10000
    ):
        """
        ...
        def arrived_early() -> bool:
            ...
            if (
                ...
                and int(Map.GetRegion()[0]) == target_region
                and int(Map.GetDistrict()) == target_district
                and int(Map.GetLanguage()[0]) == target_language
            ):
        ...
        def map_arrival() -> BehaviorTree.NodeState:
            ...
            if (
                ...
                and Map.IsMapIDMatch(0, outpost_id)
                and int(Map.GetRegion()[0]) == target_region
                and int(Map.GetDistrict()) == target_district
                and int(Map.GetLanguage()[0]) == target_language
            ):
        ...
        tree = BehaviorTree.SelectorNode(
            children=[
                ...
                BehaviorTree.SequenceNode(
                    ...
                    children=[
                        BehaviorTree.ActionNode(
                            name="TravelToRegionAction", action_fn=lambda: travel_action(), aftercast_ms=2000
                        ),
                        BehaviorTree.WaitNode(
                            name="WaitForMapArrival", check_fn=lambda: map_arrival(), timeout_ms=timeout
                        ),
                        BehaviorTree.WaitForTimeNode(name="PostArrivalWait", duration_ms=1000),
                    ],
                ),
                ...
                children=[
                    ...
                    BehaviorTree.SubtreeNode(
                        ...
                        subtree_fn=lambda _node: BTMap.WaitforMapLoad(
                            map_id=int(state["resolved_map_id"] or target_map_id)
                        ),
                    ),
                ],
            )
    ...
    def WaitforMapLoad(
        ...
        timeout: int = 10000,
        map_name: str = "",
        player_instance_uptime_ms: int = 1500,
        map_instance_uptime_ms: int = 1500,
        throttle_interval_ms: int = 500,
        post_arrival_wait_ms: int = 1000,
    ) -> BehaviorTree:
        """
        ...
        tree = BehaviorTree.SequenceNode(
            name="WaitforMapLoadRoot",
            children=[
                BehaviorTree.WaitUntilNode(
                    name="WaitForMapLoadUntil",
