...
class BTMovement:
    ...
    def Move(
        ...
        def _set_blackboard(node: BehaviorTree.Node, move_state: str, reason: str = "") -> None:
            ...
            path_points: list[Point2D] = [
                (float(path_x), float(path_y)) for path_x, path_y in (state["path_points"] or [])
            ]
        ...
        def _issue_move(target_x: float, target_y: float) -> None:
            ...
            if log:
                if move_x != target_x or move_y != target_y:
                    ...
                    _log(
                        "Move",
                        f"Moving to waypoint ({target_x}, {target_y}).",
                        message_type=Console.MessageType.Info,
                        log=log,
                    )
        ...
        def _move(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if state["completed"] and state["result_state"] == "finished":
                if log:
                    _log(
                        "Move",
                        f"Movement already finished ({state['result_reason']}).",
                        message_type=Console.MessageType.Info,
                        log=log,
                    )
            ...
            if state["completed"] and state["result_state"] == "failed":
                if log:
                    _log(
                        "Move",
                        f"Movement already failed ({state['result_reason']}).",
            ...
            if state["path_gen"] is None and state["path_points"] is None:
                ...
                if path_points_override is not None:
                    state["path_gen"] = None
                    state["path_points"] = [(float(path_x), float(path_y)) for path_x, path_y in path_points_override]
                    state["path_index"] = 0
                    state["move_issued"] = False
                    state["last_distance"] = None
                    ...
                    if _debug_enabled(node):
                        _log(
                            "Move", f"Starting autopath to ({x}, {y}).", message_type=Console.MessageType.Info, log=True
                        )
                    _set_blackboard(node, "running")
                ...
                try:
                    ...
                    if Utils.Distance(current_pos, (x, y)) <= effective_tolerance:
                        if _debug_enabled(node):
                            _log(
                                "Move",
                                "Already within tolerance of destination.",
                    ...
                    if len(state["path_points"]) == 0:
                        if _debug_enabled(node):
                            _fail_log(
                                "Move", "Autopath returned no path points; failing because there is no path to follow."
                            )
            ...
            if Checks.Player.IsDead():
                ...
                if log:
                    _log(
                        "Move",
                        "Player is dead; movement remains active and waiting.",
                        message_type=Console.MessageType.Warning,
                        log=log,
                    )
            ...
            if pause_reason:
                ...
                if not state["pause_logged"] and log:
                    _log(
                        "Move",
                        f"Movement paused due to {pause_reason}.",
                        message_type=Console.MessageType.Info,
                        log=log,
                    )
            ...
            if state["path_points"] is None or state["path_index"] >= len(state["path_points"]):
                if log:
                    _log(
                        "Move",
                        "Movement finished with no remaining path points.",
                        message_type=Console.MessageType.Success,
                        log=log,
                    )
                _finalize_move(node, "finished")
            ...
            if current_distance <= effective_tolerance:
                ...
                if log:
                    _log(
                        "Move",
                        f"Reached waypoint, advancing to index {state['path_index']}.",
                        message_type=Console.MessageType.Info,
                        log=log,
                    )
        ...
        def _timeout(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if state["completed"] and state["result_state"] == "finished":
                if log:
                    _log(
                        "Move",
                        f"Timeout watcher finished because movement succeeded ({state['result_reason']}).",
                        message_type=Console.MessageType.Info,
                        log=log,
                    )
                _reset_timeout()
            ...
            if state["completed"] and state["result_state"] == "failed":
                if log:
                    _log(
                        "Move",
                        f"Timeout watcher finished because movement failed: {state['result_reason']}.",
                        message_type=Console.MessageType.Info,
                        log=log,
                    )
                _reset_timeout()
        ...
        def _map_transition(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            map_changed: bool = initial_map_id != 0 and current_map_id != 0 and current_map_id != initial_map_id
    ...
    def MoveDirect(
        ...
        return BTMovement.Move(
            ...
            log=log,
            path_points_override=[(float(path_point.x), float(path_point.y)) for path_point in path_points],
        )
