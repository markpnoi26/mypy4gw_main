...
from Core.routines_src.behaviourtrees_src.botting_movement import cutscene_active
...
def add_travel_gh_state(
    ...
    *,
    ...
    def _run_travel_gh():
        from Core import GLOBAL_CACHE, Map, Player, Routines, SharedCommandType
...
def add_flag_all_accounts_state(
    bot, *, x: float, y: float, flagger: Callable[[float, float], int], name: str, log: LogFn | None = None
) -> None:
    log_fn = log or _noop_log
...
def add_broadcast_summoning_stone_state(
    ...
    *,
    ...
    def _broadcast_summoning_stone():
        from Core import GLOBAL_CACHE, Map, Player, SharedCommandType
