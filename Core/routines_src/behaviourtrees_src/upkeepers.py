...
class BTUpkeepers:
    ...
    def _get_party_morale_debug_snapshot() -> dict[str, object]:
        ...
        shared_player_morale = next(
            (
                int(morale)
                for agent_id, morale in raw_entries
                if int(agent_id or 0) == player_agent_id and int(morale or 0) > 0
            ),
            None,
        )
        return {
    ...
    def _format_party_morale_snapshot(
        ...
        *,
        ...
        raw_entries_summary = (
            ", ".join(f"agent={int(agent_id)} morale={int(morale)}" for agent_id, morale in shared_entries)
    ...
    def _log_party_morale_debug(
        ...
        *,
        ...
        raw_entries = (
            ", ".join(f"agent={int(agent_id)} morale={int(morale)}" for agent_id, morale in snapshot['shared_entries'])
            or "<no shared entries>"
        )
        ConsoleLog(
            source,
    ...
    def _get_shared_effect_ids_for_agent(agent_id: int, headless_cached_data=None) -> set[int]:
        ...
        for account in accounts:
            ...
            return {
                int(getattr(buff, "SkillId", 0) or 0) for buff in buffs if int(getattr(buff, "SkillId", 0) or 0) > 0
            }
        return set()

    @staticmethod
    def _has_any_shared_effect_for_agent(agent_id: int, effect_ids: list[int], headless_cached_data=None) -> bool:
        shared_effect_ids = BTUpkeepers._get_shared_effect_ids_for_agent(
            agent_id, headless_cached_data=headless_cached_data
        )
    ...
    def OutpostImpService(
        ...
        def _tick_outpost_imp_service(node: BehaviorTree.Node):
            ...
            """
            if Map.IsMapLoading() or not Checks.Map.MapValid() or not Map.IsMapReady() or Map.IsExplorable():
                _reset_cache_data()
                return BehaviorTree.NodeState.RUNNING
            ...
            if current_visit_signature is None or current_visit_signature[0] != current_map_id:
            ...
            for account in targets:
                ...
                if filter_effect_ids and agent_id > 0:
                    try:
                        ...
                        account_effect_ids = {
                            int(value)
                            for value in GetEffectAndBuffIds(agent_id, cached_data=headless_cached_data)
                            if int(value) > 0
                        }
                    except Exception:
                        buffs = getattr(getattr(getattr(account, "AgentData", None), "Buffs", None), "Buffs", [])
                ...
                target_debug_rows.append(f"send:{account_email}:agent={agent_id}:effects={sorted(account_effect_ids)}")
                sent_any = True
