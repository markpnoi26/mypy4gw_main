...
def _account_agent_id(account) -> int:
    return int(getattr(account, "PlayerID", 0) or getattr(getattr(account, "AgentData", None), "AgentID", 0) or 0)
...
def _account_character_name(account) -> str:
    return str(
        getattr(account, "CharacterName", "") or getattr(getattr(account, "AgentData", None), "CharacterName", "") or ""
    )
                ...
                children=[
                    ...
                    BehaviorTree(
                        BehaviorTree.SubtreeNode(
                            ...
                            subtree_fn=lambda node: BTShared.SendAndWait(
                                command=node.blackboard.get(
                                    f"{refs_blackboard_key}_prepared_command", SharedCommandType.TravelToMap
                                ),
                                params=tuple(
                                    node.blackboard.get(f"{refs_blackboard_key}_prepared_params", (0.0, 0.0, 0.0, 0.0))
                                ),
                                recipients=list(node.blackboard.get(f"{refs_blackboard_key}_recipient_emails", [])),
                                refs_blackboard_key=refs_blackboard_key,
                                timeout_ms=timeout_ms,
    ...
    def SummonAccountByEmail(
        ...
        def _prepare_summon(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if target_data is None:
                node.blackboard[f"{refs_blackboard_key}_recipient_emails"] = []
                _fail_log(
                    "BTShared.SummonAccountByEmail", f"Failed to summon account: target '{target_email}' was not found."
                )
                return BehaviorTree.NodeState.FAILURE
    ...
    def InviteAllAccounts(
        ...
        def _invite_all_accounts_sequential(node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if not character_name:
                _fail_log(
                    "BTShared.InviteAllAccounts",
                    f"Failed to invite account: target '{receiver_email}' has no character name.",
                )
                return BehaviorTree.NodeState.FAILURE
    ...
    def KickAllAccounts(
        ...
        def _kick_all_accounts_sequential(_node: BehaviorTree.Node) -> BehaviorTree.NodeState:
            ...
            if not character_name:
                _fail_log(
                    "BTShared.KickAllAccounts",
                    f"Failed to kick account: target '{receiver_email}' has no character name.",
                )
                return BehaviorTree.NodeState.FAILURE
            ...
            if not bool(state["sent"]):
                ...
                if not character_name:
                    _fail_log(
                        ...
                        f"Failed to invite account: target '{target_email}' has no character name.",
                    )
                    return BehaviorTree.NodeState.FAILURE
                GLOBAL_CACHE.Party.Players.InvitePlayer(character_name)
                GLOBAL_CACHE.ShMem.SendMessage(
