...
class ImGui:
    ...
    def push_style_color(idx: int, col: tuple[float, float, float, float]):
        return PyImGui.push_style_color(idx, col)

    @staticmethod
    def pop_style_color(count: int = 1):
        return PyImGui.pop_style_color(count)

    @staticmethod
    def get_x_position_aligned(
        alignment: Alignment,
        parent_pos: tuple[float, float],
        parent_size: tuple[float, float],
        child_size: tuple[float, float],
    ...
    @staticmethod
    def is_mouse_in_rect(
        rect: tuple[float, float, float, float], mouse_pos: Optional[tuple[float, float]] = None
    ) -> bool:
    ...
    def set_window_within_displayport(
        ...
        *,
        ...
        pos = PyImGui.get_window_pos()
        size = PyImGui.get_window_size()
        clamped_x, clamped_y = ImGui.get_clamped_to_displayport(
            pos=pos, size=size, min_visible_x=min_visible_x, min_visible_y=min_visible_y, relative=relative
        )
    ...
    @staticmethod
    def _is_textured_theme() -> bool:
        return ImGui.get_style().Theme in ImGui.Textured_Themes
    ...
    @staticmethod
    def BeginWithClose(
        ini_key: str, name: str, p_open=None, flags: int = PyImGui.WindowFlags.NoFlag
    ) -> tuple[bool, bool]:
    ...
    @staticmethod
    def begin_with_close(
        name: str, p_open: Optional[bool] = None, flags: int = PyImGui.WindowFlags.NoFlag
    ) -> tuple[bool, bool]:
    ...
    def calc_text_size(text: str):
        return PyImGui.calc_text_size(text)

    @staticmethod
    def invisible_button(label: str, width: float, height: float):
        return PyImGui.invisible_button(label, (int(width), int(height)))
    ...
    def selectable(
        label: str,
        selected: bool,
        flags: PyImGui.SelectableFlags = PyImGui.SelectableFlags.NoFlag,
    ...
    def begin_selectable(
        ...
        height = (
            size[1]
            if size[1] > 0
            else (PyImGui.get_text_line_height_with_spacing() + (style.FramePadding.value2 or 0) * 2)
        )
        ...
        ImGui.__selectable_stack.insert(
            0,
            {
    ...
    @staticmethod
    def color_edit3(label: str, color: Tuple[float, float, float]):
        return PyImGui.color_edit3(label, color)

    @staticmethod
    def color_edit4(label: str, color: Tuple[float, float, float, float]):
        return PyImGui.color_edit4(label, color)

    @staticmethod
    def dummy(width: float, height: float):
        return PyImGui.dummy((int(width), int(height)))
    ...
    @staticmethod
    def text(
        text: str, font_size: int | None = None, font_style: str | None = None, render_markdown: bool = False
    ) -> None:
    ...
    @staticmethod
    def text_centered(
        text: str, width: float = 0, height: float = 0, font_size: int | None = None, font_style: str | None = None
    ) -> None:
        width = PyImGui.get_content_region_avail()[0] if width == 0 else width
    ...
    @staticmethod
    def text_colored(
        text: str, color: tuple[float, float, float, float], font_size: int | None = None, font_style: str | None = None
    ):
    ...
    def text_decorated(
        text: str,
        decorator: TextDecorator = TextDecorator.None_,
        font_size: int | None = None,
        font_style: str | None = None,
        color: tuple[float, float, float, float] | None = None,
    ) -> None:
        decorator_color = (
            Color.from_tuple(color)
            if color
            else (ImGui.get_style().TextSelectedBg if decorator == TextDecorator.Highlight else ImGui.get_style().Text)
        )
    ...
    def button(
        ...
        height=0.0,
        disabled: bool = False,
        appearance: ControlAppearance = ControlAppearance.Default,
    ) -> bool:
    ...
    @staticmethod
    def small_button(
        label: str, disabled: bool = False, appearance: ControlAppearance = ControlAppearance.Default
    ) -> bool:
        # MATCHING IMGUI SIGNATURES AND USAGE
        enabled = not disabled
        if disabled:
    ...
    def icon_button(
        ...
        if current_style_var.img_style_enum:
            PyImGui.push_style_var_vec2(
                current_style_var.img_style_enum, (current_style_var.value1, current_style_var.value2 or 0)
            )
    ...
    @staticmethod
    def toggle_icon_button(
        label: str, v: bool, width: float = 0.0, height: float = 0.0, disabled: bool = False
    ) -> bool:
        def group_text_with_icons(text: str):
            """
            Splits the string into groups of (is_icon, run_string).
    ...
    @staticmethod
    def image_toggle_button(
        label: str, texture_path: str, v: bool, width=32.0, height=32.0, disabled: bool = False
    ) -> bool:
        # MATCHING IMGUI SIGNATURES AND USAGE
        enabled = not disabled
        clicked = False
        ...
        ImGui.DrawTextureInDrawList(texture_pos, texture_size, texture_path, tint=texture_tint)
    ...
    @staticmethod
    def search_field(
        label: str, text: str, placeholder: str = "Search...", flags: int = PyImGui.InputTextFlags.NoFlag
    ) -> tuple[bool, str]:
        def _functions_tail():
            if not PyImGui.is_item_active() and not PyImGui.is_item_focused() and not text:
                ...
                placeholder_rect = (
                    ...
                    item_rect[1],
                    inputfield_size[0] - (current_frame_padding.value1 * 2),
                    inputfield_size[1],
                )
        ...
        label_rect = (
            item_rect_max[0] - (label_size[0] if label_size[0] > 0 else 0),
            item_rect_min[1] + ((height - label_size[1]) / 2) + 2,
            label_size[0],
            label_size[1],
        )
    ...
    def separator():
        ...
        ThemeTextures.Separator.value.get_texture().draw_in_drawlist(item_rect[:2], item_rect[2:])
    ...
    def bullet_text(text: str):
        ...
        PyImGui.push_clip_rect(
            cursor[0] + frame_padding.value1 + height,
            cursor[1],
            cursor[0] + frame_padding.value1 + text_size[0],
            text_size[1],
            True,
        )
        PyImGui.bullet_text(text)
    ...
    def collapsing_header(label: str, flags: int = 0) -> bool:
        ...
        arrow_rect = (
            item_rect_min[0] + 2 + frame_padding.value1,
            item_rect_min[1] + (frame_padding.value2 or 0),
            line_height,
            line_height,
        )
        ...
        tint = (
            (
                (
                    style.ComboTextureBackgroundActive.get_current().rgb_tuple
                    if PyImGui.is_item_active()
                    else style.ComboTextureBackgroundHovered.get_current().rgb_tuple
                )
                if ImGui.is_mouse_in_rect(item_rect)
                else style.ComboTextureBackground.get_current().rgb_tuple
            )
            if True
            else (64, 64, 64, 255)
        )
        ...
        frame_tint = (255, 255, 255, 255) if ImGui.is_mouse_in_rect(item_rect) and True else (200, 200, 200, 255)

        ThemeTextures.CollapsingHeader_Background.value.get_texture().draw_in_drawlist(
            item_rect[:2], (item_rect[2], item_rect[3]), tint=tint
        )

        ThemeTextures.CollapsingHeader_Frame.value.get_texture().draw_in_drawlist(
            item_rect[:2], (item_rect[2], item_rect[3]), tint=frame_tint
        )

        (
            ThemeTextures.ArrowExpanded if new_open else ThemeTextures.ArrowCollapsed
        ).value.get_texture().draw_in_drawlist(arrow_rect[:2], arrow_rect[2:], tint=style.Text.get_current().rgb_tuple)

        display_label = label.split("##")[0]
        text_x = (arrow_rect[0] + arrow_rect[2]) + (frame_padding.value1 * 2)
        text_y = item_rect_min[1] + padding

        PyImGui.push_clip_rect(item_rect[0], item_rect[1], width - 5, height, True)
    ...
    def tree_node(label: str) -> bool:
        ...
        ImGui.push_style_color(PyImGui.ImGuiCol.HeaderActive, (0, 0, 0, 0))
        PyImGui.push_clip_rect(
            PyImGui.get_cursor_screen_pos()[0] + 20, PyImGui.get_cursor_screen_pos()[1], 1000, 1000, True
        )
        new_open = PyImGui.tree_node(label)
        PyImGui.pop_clip_rect()
    ...
    @staticmethod
    def begin_child(
        id: str, size: tuple[float, float] = (0, 0), border: bool = False, flags: int = PyImGui.WindowFlags.NoFlag
    ) -> bool:
    ...
    def begin_tab_item(label: str, popen: bool | None = None, flags: int = 0) -> bool:
        ...
        PyImGui.push_clip_rect(*tab_texture_rect, True)
    ...
    @staticmethod
    def draw_vertical_scroll_bar(
        scroll_bar_size: float,
        force_scroll_bar: bool = False,
        window_rect: Optional[tuple[float, float, float, float]] = None,
        border_padding: bool = False,
    ):
        import math
        ...
        if force_scroll_bar or scroll_max_y > 0:
            ...
            scroll_bar_rect = (
                item_rect_max[0] - scroll_bar_size - window_padding[0],
                item_rect_min[1] + window_padding[1],
                item_rect_max[0] - window_padding[0],
            ...
            scroll_grab_rect = (
                scroll_bar_rect[0],
                scroll_bar_rect[1] + thumb_offset,
                scroll_bar_rect[2],
                scroll_bar_rect[1] + thumb_offset + thumb_height,
            )
    ...
    @staticmethod
    def draw_horizontal_scroll_bar(
        scroll_bar_size: float,
        force_scroll_bar: bool = False,
        window_rect: Optional[tuple[float, float, float, float]] = None,
        border_padding: bool = False,
        ...
        if force_scroll_bar or scroll_max_x > 0:
            ...
            scroll_bar_rect = (
                ...
                item_rect_min[0]
                + visible_size_x
                - (scroll_bar_size + 2 if scroll_max_y > 0 else 0)
                - window_padding[0],
                item_rect_min[1] + visible_size_y - window_padding[1],
            )
    ...
    @staticmethod
    def begin_table(
        id: str, columns: int, flags: int = PyImGui.TableFlags.NoFlag, width: float = 0, height: float = 0
    ) -> bool:
    ...
    def progress_bar(fraction: float, size_arg_x: float, size_arg_y: float, overlay: str = ""):
        ...
        item_rect_min = PyImGui.get_item_rect_min()
        item_rect_max = PyImGui.get_item_rect_max()
        center = item_rect_min[0] + ((item_rect_max[0] - item_rect_min[0]) / 2), item_rect_min[1] + (
            (item_rect_max[1] - item_rect_min[1]) / 2
        )
    ...
    def ImageButtonExtended(
        ...
        return ImGui.overlay_instance.ImageButtonExtended(
            caption, texture_path, size, uv0, uv1, bg_color, tint_color, frame_padding
        )
    ...
    @staticmethod
    def floating_button(
        caption, x, y, width=18, height=18, color: Color = Color(255, 255, 255, 255), name="", font_size: int = -1
    ):
        if not name:
            name = caption
    ...
    def floating_checkbox(caption, state, x, y, width=18, height=18, color: Color = Color(255, 255, 255, 255)):
        ...
        if PyImGui.begin(f"##invisible_window{caption}", flags):
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.CheckMark, color.shift(white, 0.5).to_tuple_normalized()
            )  # Checkmark color
    ...
    def DrawTextWithTitle(title, text_content, lines_visible=10):
        ...
        if PyImGui.begin_child(
            f"ScrollableTextArea_{title}",
            size=(window_width, content_height),
            border=True,
    ...
    @staticmethod
    def render_tokenized_markup(
        tokenized_lines: list[list[dict]], max_width: float, COLOR_MAP: dict[str, tuple[float, float, float, float]]
    ):
        """
    ...
    class FloatingIcon:
        ...
        # /Core/docs/floating_icon_class.md
        def __init__(
            self,
            icon_path: str,
