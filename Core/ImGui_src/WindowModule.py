...
class WindowModule:
    ...
    def __init__(
        ...
        collapse=False,
        can_close=False,
        forced_theme: Optional[StyleTheme] = None,
        resize_on_collapse: bool = False,
    ):
        ...
        self.can_resize = (int(self.window_flags) & int(PyImGui.WindowFlags.NoResize)) == 0 and (
            int(self.window_flags) & int(PyImGui.WindowFlags.AlwaysAutoResize)
        ) == 0
        ...
        self.__close_button_rect = (
            self.__decorators_right - 29,
            self.__decorators_top + 9,
            11,
            11,
        )  # Internal use only
        self.__title_bar_rect = (
            self.__decorators_left + 5,
            self.__decorators_top + 2,
            self.__decorators_width - 10,
            26,
        )  # Internal use only
    ...
    def begin(self, p_open: Optional[bool] = None, flags: int = PyImGui.WindowFlags.NoFlag) -> bool:
        ...
        match (self.__current_theme):
            case StyleTheme.Guild_Wars:
                ...
                internal_flags = int(PyImGui.WindowFlags.NoTitleBar | PyImGui.WindowFlags.NoBackground) | int(
                    self.window_flags
                )
                ...
                if self.open and is_expanded:
                    if not is_first_run:
                        if (
                            self.__resize
                            or self.window_size[0] < self.__decorated_window_min_size[0]
                            or self.window_size[1] < self.__decorated_window_min_size[1]
                        ):
                            if not has_always_auto_resize:
                                self.window_size = (
                                    max(self.__decorated_window_min_size[0], self.window_size[0]),
                                    max(self.__decorated_window_min_size[1], self.window_size[1]),
                                )
                                PyImGui.set_next_window_size(
                                    (self.window_size[0], self.window_size[1]), PyImGui.ImGuiCond.Always
                                )
            ...
            case _:
                if self.can_close:
                    expanded, self.open = PyImGui.begin(
                        name=self.window_name, p_open=self.open, flags=self.window_flags
                    )
    ...
    def __get_geometry(self):
        ...
        self.__window_decorator_rect = (window_pos[0] - 10, window_pos[1], window_size[0] + 20, window_size[1] + 10)
        ...
        self.__window_clip_rect = (self.window_pos[0], self.window_pos[1], self.window_size[0], self.window_size[1])
    ...
    def __draw_decorations(self):
        ...
        if ImGui.is_mouse_in_rect(self.__close_button_rect) and (
            (int(self.window_flags) & int(PyImGui.WindowFlags.NoMouseInputs)) == 0
        ):
        ...
        if not self.collapse and self.open:
            ...
            if has_title_bar:
                ThemeTextures.Title_Bar.value.get_texture().draw_in_drawlist(
                    pos=self.__title_bar_rect[:2], size=self.__title_bar_rect[2:]
                )

                if self.can_close:
                    ThemeTextures.Close_Button.value.get_texture().draw_in_drawlist(
                        pos=self.__close_button_rect[:2], size=self.__close_button_rect[2:], state=close_button_state
                    )

                self.__draw_title_bar_fake(self.__faketitle_bar_rect)

                # Draw the title text
                PyImGui.push_clip_rect(*self.__title_text_rect, False)

                PyImGui.draw_list_add_text(*self.__title_text_rect[:2], style.Text.color_int, self.window_display_name)
                PyImGui.pop_clip_rect()
    ...
    def __draw_title_bar_fake(self, title_bar_rect):
        ...
        PyImGui.set_cursor_screen_pos(
            (
                self.__close_button_rect[0] + self.__close_button_rect[2],
                self.__close_button_rect[1] + self.__close_button_rect[3],
            )
        )
        ...
        if self.__dragging:
            ...
            if can_drag:
                if self.__drag_started:
                    ...
                    new_window_pos = (
                        title_bar_rect[0] + 10 + delta[0],
                        title_bar_rect[1] + title_bar_rect[3] - 3 + delta[1],
                    )
