...
class Style:
    ...
    class StyleVar:
        def __init__(
            ...
            img_style_enum: "ImGuiStyleVar|None" = None,
            display_name: str | None = None,
        ):
            self.style = style
            self.img_style_enum: ImGuiStyleVar | None = img_style_enum
            self.display_name: str | None = (
                display_name if display_name else Utils.split_uppercase(img_style_enum.name) if img_style_enum else None
            )
        ...
        def push_style_var(self, value1: float | None = None, value2: float | None = None):
            var = (
                Style.StyleVar(style=self.style, value1=value1, value2=value2, img_style_enum=self.img_style_enum)
                if value1 is not None
                else self.get_current()
            )
        ...
        def to_json(self):
            return (
                {"value1": self.value1, "value2": self.value2} if self.value2 is not None else {"value1": self.value1}
            )
        ...
        def __eq__(self, value):
            return isinstance(value, Style.StyleVar) and (self.img_style_enum, self.value1, self.value2) == (
                value.img_style_enum,
                value.value1,
                value.value2,
            )
    ...
    class StyleColor(Color):
        def __init__(
            ...
            b: int,
            a: int = 255,
            img_color_enum: PyImGui.ImGuiCol | None = None,
            color_type: StyleColorType = StyleColorType.Default,
            display_name: str | None = None,
        ):
            ...
            self.display_name = (
                display_name if display_name else Utils.split_uppercase(img_color_enum.name) if img_color_enum else None
            )
            self.pushed_stack: list[Style.StyleColor] = []

        def __hash__(self):
            return hash((self.img_color_enum, self))

        def __eq__(self, other):
            return isinstance(other, Style.StyleColor) and self == other
        ...
        def load_from_json(self, data):
            ...
            self.img_color_enum = (
                getattr(PyImGui.ImGuiCol, img_color_enum) if img_color_enum in PyImGui.ImGuiCol.__members__ else None
            )
    ...
    def __init__(self, theme: StyleTheme = StyleTheme.Py4GW):
        ...
        self.SeparatorTextBorderSize: Style.StyleVar = Style.StyleVar(
            self, 3, None, ImGuiStyleVar.SeparatorTextBorderSize
        )
        ...
        self.PrimaryButton = Style.StyleColor(
            self, 26, 38, 51, 255, PyImGui.ImGuiCol.Button, StyleColorType.Custom, "Primary Button"
        )
        self.PrimaryButtonHovered = Style.StyleColor(
            self, 51, 76, 102, 255, PyImGui.ImGuiCol.ButtonHovered, StyleColorType.Custom, "Primary Button Hovered"
        )
        self.PrimaryButtonActive = Style.StyleColor(
            self, 102, 127, 153, 255, PyImGui.ImGuiCol.ButtonActive, StyleColorType.Custom, "Primary Button Active"
        )

        self.DangerButton = Style.StyleColor(
            self, 26, 38, 51, 255, PyImGui.ImGuiCol.Button, StyleColorType.Custom, "Danger Button"
        )
        self.DangerButtonHovered = Style.StyleColor(
            self, 51, 76, 102, 255, PyImGui.ImGuiCol.ButtonHovered, StyleColorType.Custom, "Danger Button Hovered"
        )
        self.DangerButtonActive = Style.StyleColor(
            self, 102, 127, 153, 255, PyImGui.ImGuiCol.ButtonActive, StyleColorType.Custom, "Danger Button Active"
        )

        self.ToggleButtonEnabled = Style.StyleColor(
            self, 26, 38, 51, 255, PyImGui.ImGuiCol.Button, StyleColorType.Custom, "Toggle Button Enabled"
        )
        self.ToggleButtonEnabledHovered = Style.StyleColor(
            self,
            51,
            76,
            102,
            ...
            StyleColorType.Custom,
            "Toggle Button Enabled Hovered",
        )
        self.ToggleButtonEnabledActive = Style.StyleColor(
            self,
            102,
            127,
            153,
        ...
        self.ToggleButtonDisabled = Style.StyleColor(
            self, 26, 38, 51, 255, PyImGui.ImGuiCol.Button, StyleColorType.Custom, "Toggle Button Disabled"
        )
        self.ToggleButtonDisabledHovered = Style.StyleColor(
            self,
            51,
            76,
            102,
            ...
            PyImGui.ImGuiCol.ButtonHovered,
            StyleColorType.Custom,
            "Toggle Button Disabled Hovered",
        )
        self.ToggleButtonDisabledActive = Style.StyleColor(
            self,
            102,
            127,
            153,
        ...
        self.TextTreeNode = Style.StyleColor(
            self, 204, 204, 204, 255, PyImGui.ImGuiCol.Text, StyleColorType.Custom, "Text Tree Node"
        )
        self.TextObjectiveCompleted = Style.StyleColor(
            self, 204, 204, 204, 255, PyImGui.ImGuiCol.Text, StyleColorType.Custom, "Text Objective Completed"
        )
        self.Hyperlink = Style.StyleColor(
            self, 102, 187, 238, 255, PyImGui.ImGuiCol.Text, StyleColorType.Custom, "Text Hyperlink"
        )

        self.ComboTextureBackground = Style.StyleColor(
            self, 26, 23, 30, 255, None, StyleColorType.Texture, "Combo Background (Textured)"
        )
        self.ComboTextureBackgroundHovered = Style.StyleColor(
            self, 61, 59, 74, 255, None, StyleColorType.Texture, "Combo Background Hovered (Textured)"
        )
        self.ComboTextureBackgroundActive = Style.StyleColor(
            self, 143, 143, 148, 255, None, StyleColorType.Texture, "Combo Background Active (Textured)"
        )

        self.ButtonTextureBackground = Style.StyleColor(
            self, 26, 23, 30, 255, None, StyleColorType.Texture, "Button Background (Textured)"
        )
        self.ButtonTextureBackgroundHovered = Style.StyleColor(
            self, 61, 59, 74, 255, None, StyleColorType.Texture, "Button Background Hovered (Textured)"
        )
        self.ButtonTextureBackgroundActive = Style.StyleColor(
            self, 143, 143, 148, 255, None, StyleColorType.Texture, "Button Background Active (Textured)"
        )
        self.ButtonTextureBackgroundDisabled = Style.StyleColor(
            self, 143, 143, 148, 255, None, StyleColorType.Texture, "Button Background Disabled (Textured)"
        )
        # endregion

        # Collect attributes
        attributes = {name: getattr(self, name) for name in dir(self)}
        self.Colors: dict[str, Style.StyleColor] = {
            name: attributes[name]
            for name in attributes
            if isinstance(attributes[name], Style.StyleColor) and attributes[name].color_type == StyleColorType.Default
        }
        self.TextureColors: dict[str, Style.StyleColor] = {
            name: attributes[name]
            for name in attributes
            if isinstance(attributes[name], Style.StyleColor) and attributes[name].color_type == StyleColorType.Texture
        }
        self.CustomColors: dict[str, Style.StyleColor] = {
            name: attributes[name]
            for name in attributes
            if isinstance(attributes[name], Style.StyleColor) and attributes[name].color_type == StyleColorType.Custom
        }
        self.StyleVars: dict[str, Style.StyleVar] = {
            name: attributes[name] for name in attributes if isinstance(attributes[name], Style.StyleVar)
        }
