...
class BotConfig:
    def __init__(
        ...
        # Properties
        self.config_properties = ConfigProperties(
            self,
    ...
    def _reset_pause_on_danger_fn(self, aggro_area=None) -> None:
        ...
        self._set_pause_on_danger_fn(
            lambda a=aggro_area: Checks.Agents.InDanger(aggro_area=a)
    ...
    def _set_on_follow_path_failed(self, on_follow_path_failed: Callable[[], bool]) -> None:
        ...
        if self.config_properties.log_actions.is_active():
            ConsoleLog(
                "OnFollowPathFailed",
                f"Set OnFollowPathFailed to {on_follow_path_failed}",
