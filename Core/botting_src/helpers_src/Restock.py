...
class _Restock:
    ...
    def restock_honeycomb(self):
        if self._config.upkeep.honeycomb.is_active() or self._config.upkeep.morale.is_active():
    ...
    @_yield_step(
        label="RestockConsetPconsSummoningStonesCitySpeed", counter_key="RESTOCK_CONSET_PCONS_SUMMON_STONES_CITY_SPEED"
    )
