...
from Core import ConsoleLog, Console, Agent, Player
    ...
    def upkeep_hero_ai(self):
        ...
        while True:
            pause_requested = bool(
                getattr(self._config.upkeep, "hero_ai_paused", None) and self._config.upkeep.hero_ai_paused.is_active()
            )
            ...
            if (
                ...
                and not self.cancel_movement_triggered
            ):
                yield from Routines.Yield.Movement.StopMovement()
                self.cancel_movement_triggered = True
