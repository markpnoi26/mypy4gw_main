...
from Core.enums import SharedCommandType

from Core import ConsoleLog, Console
from Core.Player import Player
from Core.Map import Map
from Core.GlobalCache.SharedMemory import (
    AccountStruct,
)
...
class _Multibox:
    ...
    class _AccountData:
        def __init__(self, account_data: AccountStruct):
            ...
            self.PlayerPrimaryProfession = (
                int(account_data.AgentData.Profession[0]) if account_data.AgentData.Profession else 0
            )
    ...
    class _HeroAI:
        ...
        def _set_hero_ai_options_by_email(self, email: str, option: str, value: Any, skill_index: int = 0):
            ...
            current_options.Skills[skill_index] = bool(
                value
                if ((option == "Skills") and (skill_index >= 1) and (skill_index < 8))
                else current_options.Skills[skill_index]
            )
    ...
    def _set_account_isolation(self, isolated: bool, account_email: str = ""):
        ...
        if not target_email:
            ConsoleLog(
                "Messaging",
                "SetAccountIsolation failed: no account email available.",
    ...
    def _invite_all_accounts(self):
        ...
        for account in all_accounts:
            if (
                ...
                GLOBAL_CACHE.ShMem.SendMessage(
                    player_data.AccountEmail, account.AccountEmail, SharedCommandType.InviteToParty, (0, 0, 0, 0)
                )
                yield from Routines.Yield.wait(500)
        yield
    ...
    def _pixel_stack(self):
        ...
        for account in accounts:
            ...
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.PixelStack, (x, y, 0, 0)
            )
        yield
    ...
    def _brute_force_unstuck(self):
        ...
        for account in accounts:
            ...
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.BruteForceUnstuck, (x, y, 0, 0)
            )
    ...
    def _use_partywide_consumable_message(self, params):
        ...
        for account in accounts:
            ...
            if effect_ids and any(
                GLOBAL_CACHE.Effects.HasEffect(sender_agent_id, effect_id) for effect_id in effect_ids
            ):
                return
    ...
    def _use_local_consumable_models(self, primary_model_id: int, secondary_model_id: int = 0):
        ...
        consumable_effects = [(model_id, single_consumable_effects.get(model_id, "")) for model_id in model_ids]
    ...
    def _use_summoning_stone_message(self):
        ...
        for account in accounts:
            ...
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.UseSummoningStone, (0, 0, 0, 0)
            )
        yield from Routines.Yield.wait(500)
    ...
    def _send_dialog_with_target(self, dialog_id: int, wait_time: int = 3000):
        ...
        for account in accounts:
            ConsoleLog(
                "Messaging",
                f"Ordering {account.AccountEmail} to send dialog {dialog_id} to target: {target}",
                log=False,
            )
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.SendDialogToTarget, (target, dialog_id, 0, 0)
            )
    ...
    def _restock_all_pcons_message(self, quantity: int):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.RestockAllPcons, (quantity, 0, 0, 0)
            )
        yield from Routines.Yield.wait(500)
    ...
    def _restock_conset_message(self, quantity: int):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.RestockConset, (quantity, 0, 0, 0)
            )
        yield from Routines.Yield.wait(500)
    ...
    def _restock_resurrection_scroll_message(self, quantity: int):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.RestockResurrectionScroll, (quantity, 0, 0, 0)
            )
        yield from Routines.Yield.wait(500)
    ...
    def _restock_summoning_stones_message(self, quantity: int):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.RestockSummoningStones, (quantity, 0, 0, 0)
            )
        yield from Routines.Yield.wait(500)
    ...
    def _withdraw_gold_message(self, target_gold: int, deposit_all: bool):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email,
                account.AccountEmail,
                SharedCommandType.WithdrawGold,
                (target_gold, int(deposit_all), 0, 0),
            )
        yield from Routines.Yield.wait(500)
    ...
    def _equip_item_message(self, email: str, model_id: int):
        ...
        GLOBAL_CACHE.ShMem.SendMessage(
            sender_email, email, SharedCommandType.EquipItem, (float(model_id), 0.0, 0.0, 0.0)
        )
        yield from Routines.Yield.wait(500)
    ...
    def _equip_item_on_all_accounts_message(self, char_name_to_model_id: dict):
        ...
        for char_name, model_id in char_name_to_model_id.items():
            ...
            if not email:
                ConsoleLog(
                    "Messaging", f"EquipItemOnAllAccounts: no account found for char '{char_name}', skipping", log=True
                )
                continue
    ...
    def _load_skill_template_message(self, email: str, template: str):
        ...
        GLOBAL_CACHE.ShMem.SendMessage(
            sender_email, email, SharedCommandType.LoadSkillTemplate, (0.0, 0.0, 0.0, 0.0), extra_data
        )
        yield from Routines.Yield.wait(500)
    ...
    def _load_skill_template_on_all_accounts_message(self, char_name_to_template: dict):
        ...
        for char_name, template in char_name_to_template.items():
            ...
            if not email:
                ConsoleLog(
                    "Messaging",
                    f"LoadSkillTemplateOnAllAccounts: no account found for char '{char_name}', skipping",
                    log=True,
                )
                continue
            extra_data = (template, "", "", "")
