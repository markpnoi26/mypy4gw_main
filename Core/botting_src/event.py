...
class Event:
    ...
    def __init__(
        self, parent: "BotConfig", name: str, *, interval_ms: int = 250, callback: Optional[Callable[[], None]] = None
    ):
        self.parent = parent
        self.name = name
    ...
    def run(self):
        ...
        from Core import Routines  # local import to avoid cycles
...
class OnDeathEvent(Event):
    ...
    def should_trigger(self) -> bool:
        from Core import GLOBAL_CACHE  # local import!
    ...
    def should_reset(self) -> bool:
        from Core import GLOBAL_CACHE, Routines, Agent
...
class OnPartyDefeated(Event):
    def should_trigger(self) -> bool:
        from Core import GLOBAL_CACHE  # <- you were missing this import
        from Core import Routines
...
class OnPartyMemberInDanger(Event):
    def should_trigger(self):
        from Core import Routines, GLOBAL_CACHE
...
class OnStuck(Event):
    def __init__(
        self, parent: "BotConfig", name: str = "OnStuckEvent", *, interval_ms: int = 1000, callback=None, active=False
    ):
...
class Events:
    ...
    def __init__(self, parent: "BotConfig"):
        ...
        self.on_party_member_dead_behind = OnPartyMemberDeadBehind(
            parent, "OnPartyMemberDeadBehindEvent", interval_ms=1000
        )
