...
class _MOVE:
    ...
    def _coro_follow_path_to(
        ...
        # --- pause sources ---
        danger_pause = (
            self._config.pause_on_danger_fn if self._config.config_properties.pause_on_danger.is_active() else None
        )
        ...
        if not success_movement:
            if Routines.Checks.Map.MapValid() and (
                Routines.Checks.Party.IsPartyWiped() or GLOBAL_CACHE.Party.IsPartyDefeated()
            ):
    ...
    def _coro_xy_and_exit_map(
        self, x: float, y: float, target_map_id: int = 0, target_map_name: str = "", step_name: str = ""
    ...
    def _coro_follow_auto_path(
        self, points: List[Tuple[float, float]], step_name: str = ""
    ) -> Generator[Any, Any, None]:
        """
        ...
        while idx < len(path_points):
            ...
            while True:
                ...
                if _pause():
                    while _pause():
                        ...
                        if stop_on_wipe and (
                            ...
                            ConsoleLog(
                                "FollowPathAggro", "Party wiped during pause.", Console.MessageType.Warning, log=True
                            )
                            ActionQueueManager().ResetAllQueues()
                            return False
                        t0 = Utils.GetBaseTimestamp()
                ...
                if not Agent.IsDead(Player.GetAgentID()):
                    ...
                    if enemies:
                        ...
                        while True:
                            ...
                            if len(alive) == 0:
                                ConsoleLog(
                                    "FollowPathAggro", "Area clear. Resuming.", Console.MessageType.Success, log=log
                                )
                                break
                            ...
                            chase_path = yield from AutoPathing().get_path(
                                (cur_px, cur_py, 0), (target_x_e, target_y_e, 0)
                            )
                        ...
                        if resume_dist > tolerance:
                            resume_path = yield from AutoPathing().get_path((resume_px, resume_py, 0), (tx, ty, 0))
                            if resume_path:
                                ConsoleLog(
                                    "FollowPathAggro",
                ...
                if cur_dist <= tolerance:
                    ConsoleLog(
                        "FollowPathAggro",
                        f"Reached wp {idx+1}/{len(path_points)}.",
                        Console.MessageType.Success,
                        log=log,
                    )
                    break
    ...
    def _coro_follow_path_and_dialog(
        self, path: List[Tuple[float, float]], dialog_id: int, step_name: str = ""
    ) -> Generator[Any, Any, None]:
    ...
    def _coro_follow_path_and_exit_map(
        self, path: List[Tuple[float, float]], target_map_id: int = 0, target_map_name: str = "", step_name: str = ""
    ) -> Generator[Any, Any, None]:
        yield from self._coro_follow_path(path)
        yield from self.parent.Wait._coro_for_map_load(target_map_id=target_map_id, target_map_name=target_map_name)
    ...
    def ys_follow_path_and_exit_map(
        self, path: List[Tuple[float, float]], target_map_id: int = 0, target_map_name: str = "", step_name: str = ""
    ) -> Generator[Any, Any, None]:
    ...
    def FollowModel(
        self, model_id: int, follow_range: float, exit_condition: Optional[Callable[[], bool]] = lambda: False
    ) -> None:
