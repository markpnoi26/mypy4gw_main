...
class _UI:
    ...
    def _draw_path(
        ...
        snap_to_ground_segments: int = 1,
        floor_offset: float = 0,
    ) -> None:
        from ...DXOverlay import DXOverlay
    ...
    def _draw_texture(
        self,
        texture_path: str,
        size: Tuple[float, float] = (96.0, 96.0),
        tint: Color = Color(255, 255, 255, 255),
        border_col: Color = Color(0, 0, 0, 0),
    ):
        ...
        ImGui.DrawTextureExtended(
            ...
            uv1=(1.0, 1.0),
            tint=tint.to_tuple(),
            border_color=border_col.to_tuple(),
        )
    ...
    def draw_fsm_tree_selector_ranged(self, child_size: Tuple[float, float] = (350, 250)) -> str | None:
        ...
        for sec in sections:
            ...
            header_in_range = (
                sec["header_idx"] >= 0 and self._FSM_FILTER_START <= sec["header_idx"] <= self._FSM_FILTER_END
            )
            children_in_range = [
                (idx, nm) for (idx, nm) in sec["children"] if self._FSM_FILTER_START <= idx <= self._FSM_FILTER_END
            ]
    ...
    def _draw_main_child(
        self, main_child_dimensions: Tuple[int, int] = (350, 275), icon_path: str = "", iconwidth: int = 96
    ) -> None:
        from ...ImGui import ImGui
        ...
        current_header_step, header_for_current, current_step, total_steps, step_name, finished = (
            self._find_current_header_step()
        )
        ...
        if PyImGui.begin_table("bot_header_table", 2, PyImGui.TableFlags.RowBg | PyImGui.TableFlags.BordersOuterH):
            ...
            PyImGui.table_setup_column(
                "titles", PyImGui.TableColumnFlags.WidthFixed, main_child_dimensions[0] - iconwidth
            )
    ...
    def draw_debug_window(self):
        if PyImGui.collapsing_header("Map Navigation"):
            self._config.config_properties.draw_path.set_now(
                "active", PyImGui.checkbox("Draw Path", self._config.config_properties.draw_path.is_active())
            )
            self._config.config_properties.use_occlusion.set_now(
                "active", PyImGui.checkbox("Use Occlusion", self._config.config_properties.use_occlusion.is_active())
            )
            self._config.config_properties.snap_to_ground_segments.set_now(
                "value",
                PyImGui.slider_int(
                    "Snap to Ground Segments",
                    self._config.config_properties.snap_to_ground_segments.get("value"),
                    1,
                    32,
                ),
            )
            self._config.config_properties.floor_offset.set_now(
                "value",
                PyImGui.slider_float(
                    "Floor Offset", self._config.config_properties.floor_offset.get("value"), -10.0, 50.0
                ),
            )
    ...
    def draw_window(
        ...
        if not self._config.ini_key_initialized:
            ini_key = Settings(
                f"{f"BottingClass/bot_{self._config.bot_name}"}/{f"bot_{self._config.bot_name}.ini"}", "account"
            ).name
            if ini_key:
            ...
            if PyImGui.begin_tab_bar(self._config.bot_name + "_tabs"):
                if PyImGui.begin_tab_item("Main"):
                    if PyImGui.begin_child(
                        f"{self._config.bot_name} - Main", main_child_dimensions, True, PyImGui.WindowFlags.NoFlag
                    ):
