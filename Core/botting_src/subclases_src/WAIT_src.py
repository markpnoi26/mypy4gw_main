...
class _WAIT:
    ...
    def _coro_until_model_has_quest(self, model_id: int):
        ...
        wait_function = lambda: (
            ...
            and Agent.HasQuest(Routines.Agents.GetAgentIDByModelID(model_id))
        )
    ...
    def _coro_for_map_to_change(self, target_map_id: int = 0, target_map_name: str = ""):
        ...
        wait_condition = lambda: (
            ...
            and Map.IsMapIDMatch(
                Map.GetMapID(), target_map_id if target_map_id else Map.GetMapIDByName(target_map_name)
            )
        )
