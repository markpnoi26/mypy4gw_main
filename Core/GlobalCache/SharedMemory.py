from Core import ConsoleLog, Party, Player, Agent, Effects, ThrottledTimer, Range
from ..native_src.context.WorldContext import TitleStruct as NAtiveTitleStruct

from Core.Map import Map
from Core.py4gwcorelib_src import Utils
...
class Py4GWSharedMemoryManager:
    ...
    def _attach(self) -> bool:
        ...
        try:
            ...
            ConsoleLog(
                SHMEM_MODULE_NAME,
                f"Failed to attach shared memory: {exc}",
    ...
    def GetAccountDataFromPartyNumber(self, party_number: int, log: bool = False) -> AccountStruct | None:
        ...
        if acc:
            return acc
        ConsoleLog(
            SHMEM_MODULE_NAME, f"Party number {party_number} not found.", PySystem.Console.MessageType.Error, log=False
        )
        return None
    ...
    def IsIntentClaimed(
        ...
        return self.GetAllAccounts().IsIntentClaimed(skill_id, target_agent_id, group_id, exclude_email, now_tick)
