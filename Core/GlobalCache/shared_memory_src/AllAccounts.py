...
#
# Runtime toggle:
#   from Core.GlobalCache.shared_memory_src import AllAccounts as _wb_mod
...
class AllAccounts(Structure):
    ...
    def GetHeroExpiredSlot(self, hero_data: HeroPartyMember) -> int:
        ...
        for index in expired_slots:
            ...
            if (
                account_data.IsHero
                and account_data.AgentData.HeroID == hero_data.hero_id
                and (account_data.AccountEmail == owner_email or account_data.AgentData.OwnerAgentID == owner_id)
            ):
                return index
        return -1
    ...
    def GetPetExpiredSlot(self, pet_data: PetInfo) -> int:
        ...
        for index in expired_slots:
            ...
            if account_data.IsPet and (
                account_data.AccountEmail == owner_email
                or (
                    account_data.AgentData.AgentID == pet_data.agent_id
                    and account_data.AgentData.OwnerAgentID == pet_data.owner_agent_id
                )
            ):
                return index
        return -1
    ...
    def SubmitAccountData(self, account_email: str) -> int:
        ...
        if slot_index == -1:
            ConsoleLog(
                SHMEM_MODULE_NAME, "No empty slot available to submit account data.", PySystem.Console.MessageType.Error
            )
            return -1
        ...
        ConsoleLog(
            SHMEM_MODULE_NAME,
            f"Submitted account data for {account_email} at slot {slot_index}.",
    ...
    def SubmitHeroData(self, hero_data: HeroPartyMember) -> int:
        ...
        ConsoleLog(
            SHMEM_MODULE_NAME,
            f"Submitted hero data for HeroID {hero_data.hero_id} at slot {slot_index}.",
    ...
    def SubmitPetData(self, pet_data: PetInfo) -> int:
        ...
        ConsoleLog(
            SHMEM_MODULE_NAME,
            f"Submitted pet data for AgentID {pet_data.agent_id} at slot {slot_index}.",
            PySystem.Console.MessageType.Info,
        )
        return slot_index
    ...
    def GetHeroSlotByHeroData(self, hero_data: HeroPartyMember) -> int:
        ...
        for i in range(SHMEM_MAX_PLAYERS):
            ...
            if (
                owner_agent_id != 0
                and player.AgentData.OwnerAgentID != 0
                and player.AgentData.OwnerAgentID != owner_agent_id
            ):
                continue
            return i
    ...
    def GetPetSlotByPetData(self, pet_data: PetInfo) -> int:
        ...
        for i in range(SHMEM_MAX_PLAYERS):
            ...
            if player.AccountEmail == owner_email:
                return i
            if (
                player.AgentData.AgentID == pet_data.agent_id
                and player.AgentData.OwnerAgentID == pet_data.owner_agent_id
            ):
                return i
    ...
    def GetHeroesFromPlayers(self, owner_agent_id: int) -> list[AccountStruct]:
        ...
        for i in range(SHMEM_MAX_PLAYERS):
            ...
            if (
                self._is_visible_slot(i)
                and account_data.IsHero
                and account_data.AgentData.OwnerAgentID == owner_agent_id
            ):
    ...
    def GetAllActiveAccountHeroAIPairs(
        self, sort_results: bool = True
    ) -> list[tuple[AccountStruct, HeroAIOptionStruct]]:
    ...
    def GetHeroAIOptionsFromEmail(self, account_email: str) -> HeroAIOptionStruct | None:
        ...
        if index != -1 and self._is_visible_account(index):
            ...
            ConsoleLog(
                SHMEM_MODULE_NAME, f"Account {account_email} not found.", PySystem.Console.MessageType.Error, log=False
            )
            return None
        ...
        if index != -1 and self._is_visible_account(index):
            ...
            if property_name.startswith("Skill_"):
                ...
                if 0 <= skill_index < SHMEM_MAX_NUMBER_OF_SKILLS:
                    ...
                    ConsoleLog(
                        SHMEM_MODULE_NAME, f"Invalid skill index: {skill_index}.", PySystem.Console.MessageType.Error
                    )
                return
            ...
            if hasattr(options, property_name):
                ...
                ConsoleLog(
                    SHMEM_MODULE_NAME,
                    f"Property {property_name} does not exist in HeroAIOptions.",
    ...
    def GetMapsFromPlayers(self):
        ...
        for i in range(SHMEM_MAX_PLAYERS):
            ...
            if self._is_visible_account(i):
                maps.add(
                    (
                        player.AgentData.Map.MapID,
                        player.AgentData.Map.Region,
                        player.AgentData.Map.District,
                        player.AgentData.Map.Language,
                    )
                )
        return list(maps)
    ...
    def GetPartiesFromMaps(self, map_id: int, map_region: int, map_district: int, map_language: int):
        ...
        for i in range(SHMEM_MAX_PLAYERS):
            player = self.AccountData[i]
            if (
                self._is_visible_account(i)
                and player.AgentData.Map.MapID == map_id
                and player.AgentData.Map.Region == map_region
                and player.AgentData.Map.District == map_district
                and player.AgentData.Map.Language == map_language
            ):
    ...
    def GetPlayersFromParty(self, party_id: int, map_id: int, map_region: int, map_district: int, map_language: int):
        ...
        for i in range(SHMEM_MAX_PLAYERS):
            ...
            if (
                self._is_visible_account(i)
                and account_data.AgentData.Map.MapID == map_id
                and account_data.AgentData.Map.Region == map_region
                and account_data.AgentData.Map.District == map_district
                and account_data.AgentData.Map.Language == map_language
                and account_data.AgentPartyData.PartyID == party_id
            ):
    ...
    def SendMessage(
        ...
        if index == -1:
            ConsoleLog(
                SHMEM_MODULE_NAME, f"Receiver account {receiver_email} not found.", PySystem.Console.MessageType.Error
            )
            return -1
        ...
        if not self._can_communicate(sender_email, receiver_email):
            ConsoleLog(
                SHMEM_MODULE_NAME,
                f"Cannot communicate between {sender_email} and {receiver_email} (isolated or different groups).",
        ...
        for i in range(SHMEM_MAX_PLAYERS):
            ...
            packed = [self._str_to_c_wchar_array(normalized_extra_data[j], SHMEM_MAX_CHAR_LEN) for j in range(4)]
    ...
    def GetNextMessage(self, account_email: str) -> tuple[int, SharedMessageStruct | None]:
        ...
        for index in range(SHMEM_MAX_PLAYERS):
            message = self.Inbox[index]
            if (
                message.ReceiverEmail == account_email
                and message.Active
    ...
    def PreviewNextMessage(
        self, account_email: str, include_running: bool = True
    ) -> tuple[int, SharedMessageStruct | None]:
    ...
    def MarkMessageAsRunning(self, account_email: str, message_index: int):
        ...
        if 0 <= message_index < SHMEM_MAX_PLAYERS:
            ...
            ConsoleLog(
                SHMEM_MODULE_NAME, f"Invalid message index: {message_index}.", PySystem.Console.MessageType.Error
            )
