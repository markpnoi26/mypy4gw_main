...
- ``("skill", skill_id)`` — managed via :mod:`Core.Builds.Skills._whiteboard`
- ``("loot", item_id)`` — future
- ``("resurrect", agent_id)`` — future
- etc.

Skill-system callers should keep using the thin wrapper at
``Core.Builds.Skills._whiteboard`` (which pre-fills ``kind="skill"``)
...
def coordinates_via_whiteboard(kind: str, key: int) -> Callable[[F], F]:
    ...
    :func:`Core.Builds.Skills._whiteboard.coordinates_via_whiteboard`)
