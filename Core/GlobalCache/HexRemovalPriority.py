...
_HEX_DEFAULTS: dict[str, HexRemovalEntry] = {
    name: HexRemovalEntry(c, r, m, by_profession=bp or {}) for name, c, r, m, bp in _HEX_DEFAULTS_TABLE
}
...
def get_hex_skill_ids_on_agent(agent_id: int) -> list[int]:
    ...
    try:
        from Core import GLOBAL_CACHE, Routines
    except Exception:
        return []
    ...
    for agent_id in ally_array:
        ...
        try:
            ...
            if shared_buffs:
                ...
                local_effects = GLOBAL_CACHE.Effects.GetBuffs(agent_id) + GLOBAL_CACHE.Effects.GetEffects(agent_id)
        ...
        for skill_id, time_remaining_ms in hex_iter:
            ...
            if HEX_REMOVAL_DEBUG and should_log_detection(agent_id, skill_id):
                rem_str = f"remaining={int(time_remaining_ms)}ms" if time_remaining_ms is not None else "remaining=?"
...
def get_hexed_ally_for_removal(
    ...
    if HEX_REMOVAL_DEBUG:
        ...
        if selected:
            ...
            _log_hex(
                f"picked: {agent_descriptor(selected)} for {skill_label} " f"min_priority={pri_name}{blocked_note}"
            )
        else:
...
def cast_hex_removal_and_track(
    ...
    try:
        ...
        hex_names = ", ".join((GLOBAL_CACHE.Skill.GetName(int(hid)) or f"#{int(hid)}").strip() for hid in pre_hex_ids)
    except Exception:
        hex_names = ", ".join(f"#{int(hid)}" for hid in pre_hex_ids)

    _log_hex(f"casting {skill_name} on {target_name}(#{int(target_agent_id)}) " f"hexes=[{hex_names}]")
    ...
    if cast_result:
        ...
        if len(post_hex_ids) < pre_count:
            try:
                ...
                _log_hex(f"lock_release_error agent#{int(target_agent_id)}: {exc!r}")
            removed_ids = [hid for hid in pre_hex_ids if hid not in set(post_hex_ids)]
            try:
                from Core import GLOBAL_CACHE

                removed_names = ", ".join(
                    (GLOBAL_CACHE.Skill.GetName(int(hid)) or f"#{int(hid)}").strip() for hid in removed_ids
                )
            except Exception:
                removed_names = ", ".join(f"#{int(hid)}" for hid in removed_ids)
            _log_hex(f"removed [{removed_names}] from {target_name}; lock released")
