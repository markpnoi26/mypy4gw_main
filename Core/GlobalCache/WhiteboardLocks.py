...
def _skill_lock_duration_ms(skill_id: int, aftercast_delay: int = 250, minimum_ms: int = 500) -> int:
    ...
    return max(int(minimum_ms), activation_ms + aftercast_ms) + int(SHMEM_INTENT_DEFAULT_PING_BUDGET_MS)
...
def is_minion_lock_blocked(corpse_agent_id: int, now_tick: int | None = None) -> bool:
    ...
    try:
        ...
        if not email:
            return False
        if now_tick is None:
            now_tick = int(GetLiveTimestamp())
        return bool(
            GLOBAL_CACHE.ShMem.IsLockBlocked(
...
def is_buff_target_lock_blocked(
    buffed_ally_agent_id: int, key_id: int = BUFF_TARGET_LOCK_KEY, now_tick: int | None = None
) -> bool:
...
def filter_unlocked_loot_items(item_agent_ids: list[int]) -> list[int]:
    ...
    now_tick = int(GetLiveTimestamp())
    return [
        int(agent_id) for agent_id in item_agent_ids if agent_id and not is_loot_lock_blocked(int(agent_id), now_tick)
    ]
