...
def resolve_key(key: str) -> tuple[str, tuple[int, ...]]:
    ...
    for i, seg in enumerate(parts[1:], 1):
        if seg not in kids:
            raise FrameKeyError("unknown registry key %r: %r has no child %r" % (key, ".".join(parts[:i]), seg))
        node = kids[seg]
        ...
        def walk(prefix: str, anchor: str, tail: list, kids: dict) -> None:
            ...
            for name, node in (kids or {}).items():
                if isinstance(node, int):
                    ...
                    walk(prefix + "." + name, anchor, tail + [node[0]], node[1] if len(node) > 1 else {})
...
class FrameState:
    ...
    __slots__ = ("frame_id", "frame", "is_created", "is_visible", "tick", "_position", "_previous", "_landed", "served")
    ...
    def landed(self) -> bool:
        ...
        if self._landed is None:
            if self.frame is None or not (self.is_created or self.is_visible):
                ...
                self._landed = not _position_unusable(getattr(self.frame, "position", None))
        return self._landed
...
class _FrameTree:
    ...
    def state(self, frame_id: int) -> FrameState:
        ...
        if not fresh.landed and previous is not None and previous.landed and previous.served < self.BUFFER_TICKS:
    ...
    def child_by_parent_hash(self, parent_hash: int, child_offsets: list[int]) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_child_frame_id(parent_hash, child_offsets) or 0))
...
class Frame:
    ...
    @classmethod
    def from_hash(cls, frame_hash: int, codes=(), blackboard: Optional[dict] = None) -> "Frame":
    ...
    def bag_slot(cls, bag: Any, slot: int) -> "Frame":
        ...
        """
        return cls._under(FrameId.InventoryBagsWindow.Content.C0.C0, cls._bag_offset(bag), 2 + int(slot))
    ...
    def inventory_bag_slot(cls, bag: Any, slot: int) -> "Frame":
        ...
        return cls._under(FrameId.InventoryWindow, cls._bag_offset(bag), 2 + int(slot))
    ...
    def storage_slot(cls, bag: Any, slot: int) -> "Frame":
        ...
        return cls._under(FrameId.XunlaiWindow.StorageFrame, cls._bag_offset(bag), 2 + int(slot))
    ...
    def party_list(cls) -> "Frame":
        ...
        return cls(
            ...
            if Map.IsOutpost()
            else FrameId.PartyFormation.Explorable.C0.C0.MembersParentExplorable
        )
    ...
    def party_member(cls, index: int) -> "Frame":
        ...
        parent = (
            ...
            if Map.IsOutpost()
            else FrameId.PartyFormation.Explorable.C0.C0.MembersParentExplorable  # codes [0,0,0,0]
        )
    ...
    def capture_skill(cls, attribute: int, skill_id: int) -> "Frame":
        ...
        return cls._under(FrameId.SkillCaptureDialog, 3, 0, 0, 0, int(attribute), 1, int(skill_id), 0)
    ...
    def label(self) -> str:
        ...
        try:
            return str(PyUIManager.UIManager.get_frame_label_by_frame_id(self._target_id()) or "")
        except Exception:
            return ""
    ...
    def set_visible(self, visible: bool) -> bool:
        return bool(PyUIManager.UIManager.set_frame_visible_by_frame_id(self.frame_id, bool(visible)))

    def set_disabled(self, disabled: bool) -> bool:
        return bool(PyUIManager.UIManager.set_frame_disabled_by_frame_id(self.frame_id, bool(disabled)))
    ...
    def set_opacity(self, opacity: float, fade_time: float = 0.0) -> bool:
        return bool(PyUIManager.UIManager.set_frame_opacity_by_frame_id(self.frame_id, opacity, fade_time))
    ...
    def send_message(self, message_id: int, wparam: int = 0, lparam: int = 0) -> bool:
        return bool(PyUIManager.UIManager.SendFrameUIMessage(self.frame_id, message_id, wparam, lparam))

    def send_message_text(self, message_id: int, text: str) -> bool:
        return bool(PyUIManager.UIManager.SendFrameUIMessageWString(self.frame_id, message_id, text))

    # -- text -------------------------------------------------------------
    def text(self) -> str:
        try:
            return str(PyUIManager.UIManager.get_text_label_decoded_by_frame_id(self._target_id()) or "")
        except Exception:
            return ""

    def encoded(self) -> str:
        try:
            return str(PyUIManager.UIManager.get_text_label_encoded_by_frame_id(self._target_id()) or "")
        except Exception:
            return ""
    ...
    def title(self) -> str:
        ...
        try:
            return str(PyUIManager.UIManager.get_frame_title_by_frame_id(self._target_id()) or "")
        except Exception:
            return ""
    ...
    def is_mouse_over(self) -> bool:
        ...
        return ImGui.is_mouse_in_rect((left, top, right - left, bottom - top), (io.mouse_pos_x, io.mouse_pos_y))
    ...
    def parent_direct(self) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_parent_frame_id_direct(self.frame_id) or 0))
    ...
    def child_native(self, code: int) -> "Frame":
        ...
        return Frame.from_id(int(PyUIManager.UIManager.get_child_frame_by_frame_id(self.frame_id, code) or 0))

    def child_path_native(self, codes: list[int]) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_child_frame_path_by_frame_id(self.frame_id, codes) or 0))

    def child_with_hash(self, name_hash: int) -> "Frame":
        """Child carrying `name_hash`."""
        return Frame.from_id(
            int(PyUIManager.UIManager.get_child_frame_id_from_name_hash(self.frame_id, name_hash) or 0)
        )
    ...
    def first_child(self) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_first_child_frame_id(self.frame_id) or 0))

    def last_child(self) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_last_child_frame_id(self.frame_id) or 0))

    def next_sibling(self) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_next_child_frame_id(self.frame_id) or 0))

    def prev_sibling(self) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_prev_child_frame_id(self.frame_id) or 0))

    def related(self, relation_kind: int, start_after: int = 0) -> "Frame":
        """Native relation walker.  See RELATION_* constants."""
        return Frame.from_id(
            int(PyUIManager.UIManager.get_related_frame_id(self.frame_id, relation_kind, start_after) or 0)
        )

    def item(self, index: int) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_item_frame_id(self.frame_id, index) or 0))

    def tab(self, index: int) -> "Frame":
        return Frame.from_id(int(PyUIManager.UIManager.get_tab_frame_id(self.frame_id, index) or 0))

    def is_ancestor_of(self, other: "Frame") -> bool:
        return bool(PyUIManager.UIManager.is_ancestor_of_by_frame_id(self.frame_id, other.frame_id))
