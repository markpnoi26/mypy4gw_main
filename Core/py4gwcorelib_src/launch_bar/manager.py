...
class LaunchBarManager:
    ...
    def is_action_active(self, action) -> bool:
        ...
        if action == "system_settings":
            try:
                from Core.py4gwcorelib_src.system_settings import is_window_open
    ...
    def do_action(self, action) -> None:
        if action == "browser":
            ...
            try:
                from Core.py4gwcorelib_src.system_settings import toggle_window
            ...
            if PyImGui.begin_tab_bar("launchbar_tabs"):
                for bar in list(self.bar_set.bars):
                    ...
                    if bar.system:
                        ...
                        visible, still_open = PyImGui.begin_tab_item_closable(
                            "%s##tab_%s" % (bar.name, bar.id), True, 0
                        )
                    if visible:
                        self.selected_id = bar.id
    ...
    def _draw_function_picker(self) -> None:
        ...
        if PyImGui.begin_child("##asgflist", (0.0, 220.0), True):
            ...
            for f in self.functions.list_functions():
                if (
                    q
                    and q not in f.name.lower()
                    and q not in f.id.lower()
                    and q not in (f.category or "").lower()
                    and q not in (f.group or "").lower()
                ):
                    continue
