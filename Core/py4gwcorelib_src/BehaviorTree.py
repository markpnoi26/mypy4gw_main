...
class BehaviorTree:
    ...
    class Node(ABC):
        ...
        def _coerce_node(value) -> "BehaviorTree.Node":
            ...
            if (
                ...
                and hasattr(getattr(value, "root"), "get_children")
            ):
                return value.root
        ...
        # ----- PRINT TREE -----
        def print(self, indent: int = 0, is_last: bool = True, prefix: str = "") -> List[str]:
            """
            ...
            line = f"{prefix}{connector}[{self.node_type}] {self.name} " f"(state={state_str}, ticks={self.tick_count})"
            ...
            for idx, child in enumerate(children):
                ...
                lines.extend(child.print(indent=indent + 1, is_last=child_is_last, prefix=child_prefix_base))
        ...
        def draw(self, indent: int = 0) -> None:
            ...
            open_ = PyImGui.tree_node_ex(f"##{self.id}", PyImGui.TreeNodeFlags.SpanFullWidth)  # Hidden ID-only label
            ...
            PyImGui.same_line(0, -1)
            time_elapsed_str = (
                self._format_duration(self.run_last_duration_ms)
                if self.run_last_duration_ms > 0
                else str(self.total_time_ms) + "ms"
            )
    ...
    class ActionNode(Node):
        ...
        def __init__(self, action_fn, aftercast_ms: int = 0, name: str = "ActionNode"):
            super().__init__(
                name=name,
    ...
    class ConditionNode(Node):
        ...
        def _tick_impl(self) -> BehaviorTree.NodeState:
            ...
            if isinstance(result, bool):
                return BehaviorTree.NodeState.SUCCESS if result else BehaviorTree.NodeState.FAILURE

            # ---- CASE 3: invalid return ----
            raise TypeError(f"ConditionNode expected bool or NodeState, got: {type(result).__name__}")
    ...
    class SwitchNode(Node):
        ...
        def _coerce_tree(self, value) -> "BehaviorTree":
            ...
            raise TypeError(f"SwitchNode expected a BehaviorTree or BehaviorTree.Node, got {type(subtree).__name__}.")
    ...
    class RepeaterUntilSuccessNode(Node):
        ...
        def __init__(self, child: "BehaviorTree.Node", timeout_ms: int = 0, name: str = "RepeaterUntilSuccessNode"):
            super().__init__(
                name=name,
                node_type="RepeaterUntilSuccessNode",
                node_category="repeater",
    ...
    class RepeaterUntilFailureNode(Node):
        ...
        def __init__(self, child: "BehaviorTree.Node", timeout_ms: int = 0, name: str = "RepeaterUntilFailureNode"):
            super().__init__(
                name=name,
                node_type="RepeaterUntilFailureNode",
                node_category="repeater",
    ...
    class SubtreeNode(Node):
        ...
        def __init__(
            self,
            subtree_fn: Callable[["BehaviorTree.Node"], "BehaviorTree | BehaviorTree.Node"],
            name: str = "SubtreeNode",
        ):
    ...
    class WaitUntilNode(Node):
        ...
        def __init__(
            self, condition_fn, throttle_interval_ms: int = 100, timeout_ms: int = 0, name: str = "WaitUntilNode"
        ):
        ...
        def _tick_impl(self) -> BehaviorTree.NodeState:
            ...
            # --- BOOL ---
            if isinstance(result, bool):
                state = BehaviorTree.NodeState.SUCCESS if result else BehaviorTree.NodeState.FAILURE
            ...
            raise TypeError(f"WaitUntilNode expected bool or NodeState, got: {type(result).__name__}")
    ...
    class WaitUntilSuccessNode(Node):
        ...
        def __init__(
            self, condition_fn, throttle_interval_ms: int = 100, timeout_ms: int = 0, name: str = "WaitUntilSuccessNode"
        ):
        ...
        def _tick_impl(self) -> BehaviorTree.NodeState:
            ...
            if self._start_time_ms is None:
                self._start_time_ms = now
                self._last_check_time_ms = now
                ConsoleLog("WaitUntilSuccessNode", f"[{self.name}] Init start_time={self._start_time_ms}", log=True)
            ...
            try:
                ...
                ConsoleLog("WaitUntilSuccessNode", f"[{self.name}] ERROR evaluating condition_fn: {repr(e)}", log=True)
                raise
            ...
            # --- Normalize bool ---
            if isinstance(result, bool):
                result = BehaviorTree.NodeState.SUCCESS if result else BehaviorTree.NodeState.FAILURE

            # --- SUCCESS → done ---
            if result == BehaviorTree.NodeState.SUCCESS:
                ConsoleLog("WaitUntilSuccessNode", f"[{self.name}] Returning SUCCESS", log=True)
                self._start_time_ms = None
                self._last_check_time_ms = None
                return BehaviorTree.NodeState.SUCCESS
            ...
            if result in (BehaviorTree.NodeState.FAILURE, BehaviorTree.NodeState.RUNNING):
                ConsoleLog("WaitUntilSuccessNode", f"[{self.name}] Retry (result={result})", log=True)
                return BehaviorTree.NodeState.RUNNING

            # --- INVALID ---
            ConsoleLog("WaitUntilSuccessNode", f"[{self.name}] INVALID return type: {type(result).__name__}", log=True)

            raise TypeError(f"WaitUntilSuccessNode expected bool or NodeState, got: {type(result).__name__}")
    ...
    class WaitUntilFailureNode(Node):
        ...
        def __init__(
            self, condition_fn, throttle_interval_ms: int = 100, timeout_ms: int = 0, name: str = "WaitUntilFailureNode"
        ):
        ...
        def _tick_impl(self) -> BehaviorTree.NodeState:
            ...
            if self._start_time_ms is None:
                self._start_time_ms = now
                self._last_check_time_ms = now
                ConsoleLog("WaitUntilFailureNode", f"[{self.name}] Init start_time={self._start_time_ms}", log=True)
            ...
            try:
                ...
                ConsoleLog("WaitUntilFailureNode", f"[{self.name}] ERROR evaluating condition_fn: {repr(e)}", log=True)
                raise
            ...
            if result == BehaviorTree.NodeState.FAILURE:
                ConsoleLog("WaitUntilFailureNode", f"[{self.name}] Returning SUCCESS (condition FAILED)", log=True)
                self._start_time_ms = None
                self._last_check_time_ms = None
                return BehaviorTree.NodeState.SUCCESS

            # --- Otherwise retry (SUCCESS or RUNNING) ---
            ConsoleLog("WaitUntilFailureNode", f"[{self.name}] Retry (result={result})", log=True)
