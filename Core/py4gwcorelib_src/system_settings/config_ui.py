...
def _draw_chat_commands_monitor() -> None:
    ...
    PyImGui.text_wrapped(
        "Type a command in chat, e.g. '/travel gtob' or '/map toggle'. Unknown " "commands pass through to the game."
    )
    PyImGui.separator()
    if not cmds:
...
def build_window(controller) -> "ImGui.SidebarWindow":
    ...
    for cat in model.CATALOG:
        ...
        if cat.key == "map":
            try:
                from Core.py4gwcorelib_src.map_utilities import config_ui as map_ui
                ...
                for _label in ("Vanquish Tracker", "Instance Timer", "Disable Alcohol Effect"):
                    win.add_section(
                        group, _label, (lambda e=_err: PyImGui.text_colored("Failed to build: %s" % e, ERR_COLOR))
                    )
        if cat.key == "agents":
            ...
            try:
                from Core.py4gwcorelib_src.name_obfuscation import config_ui as no_ui
            ...
            try:
                from Core.py4gwcorelib_src.agent_recolor import config_ui as ar_ui
                ...
                _err = str(exc)
                win.add_section(
                    group, "Agent Recolor", (lambda e=_err: PyImGui.text_colored("Failed to build: %s" % e, ERR_COLOR))
                )
            continue
        if cat.key == "camera":
            try:
                from Core.py4gwcorelib_src.camera_smoothing import config_ui as camera_ui
        ...
        if cat.key == "system":
            try:
                from Core.py4gwcorelib_src.window_renamer import config_ui as renamer_ui
                ...
                _err = str(exc)
                win.add_section(
                    group, "Window Renamer", (lambda e=_err: PyImGui.text_colored("Failed to build: %s" % e, ERR_COLOR))
                )
            continue
        if cat.key == "skills":
            try:
                from Core.py4gwcorelib_src.skillbar_plus import config_ui as sbp_ui
                ...
                _err = str(exc)
                win.add_section(
                    group, "Skillbar +", (lambda e=_err: PyImGui.text_colored("Failed to build: %s" % e, ERR_COLOR))
                )
        if cat.key == "loot":
            ...
            for label, module_path in (
                ("Loot Filter Factory", "Core.py4gwcorelib_src.loot_filter_factory.config_ui"),
                ("Beacons", "Core.py4gwcorelib_src.beacons.config_ui"),
                ("Loot Filters", "Core.py4gwcorelib_src.loot_filters.config_ui"),
                ("Recolor & Beacons", "Core.py4gwcorelib_src.recolor_beacons.config_ui"),
            ):
                try:
                    import importlib
                    ...
                    _err = str(exc)
                    win.add_section(
                        group, label, (lambda e=_err: PyImGui.text_colored("Failed to build: %s" % e, ERR_COLOR))
                    )
            continue
