...
class AutoInventoryHandler:
    ...
    def __init__(self):
        ...
        self.id_model_blacklist: list[int] = (
            []
        )  # Items that should not be identified, even if they match the ID criteria
        ...
        self.item_type_blacklist: list[int] = (
            []
        )  # Item types that should not be salvaged, even if they match the salvage criteria
        self.salvage_blacklist: list[int] = (
            []
    ...
    def _get_inventory_items(self):
        from dev.reference.frenkeyLib.ItemHandling.Items.item_snapshot import ItemSnapshot
        from dev.reference.frenkeyLib.ItemHandling.Items.types import INVENTORY_BAGS
    ...
    def _get_salvage_modes_for_item(
        self,
        item,
        ...
        ordered_modes = (
            material_modes + upgrade_modes
            if (strategy if strategy is not None else self._normalize_salvage_strategy()) == 0
            else upgrade_modes + material_modes
        )
    ...
    def _get_salvage_skip_reason(
        self, item, rarity_filter, allow_unidentified_nonwhite: bool = False, respect_settings: bool = True
    ):
        from ..enums_src.Item_enums import Rarity
    ...
    def _should_keep_salvage_item(self, item, rarity_filter):
        return bool(
            item and item.is_valid and item.is_inventory_item and (not rarity_filter or item.rarity in rarity_filter)
        )
    ...
    def IdentifyItems(
        ...
        from .BehaviorTree import BehaviorTree
        from dev.reference.frenkeyLib.ItemHandling.BTNodes import BTNodes

        target_item_ids = list(
            dict.fromkeys(item_ids if item_ids is not None else self._get_identify_item_ids(rarities))
        )
        identified_items = 0
    ...
    def SalvageItems(
        ...
        target_item_ids = list(
            dict.fromkeys(item_ids if item_ids is not None else [item.id for item in self._get_inventory_items()])
        )
        ...
        for index, item_id in enumerate(target_item_ids, start=1):
            ...
            self._debug_log(
                "Salvage",
                f"Select {self._item_debug_summary(item)} action=salvage modes={[mode.name for mode in modes]}",
            )
            salvaged = False
            for mode in modes:
                if item is None:
                    Console.Log(
                        "AutoSalvage",
                        f"Item with id={item_id} not found in inventory during salvage. Skipping.",
                ...
                if state == BehaviorTree.NodeState.SUCCESS:
                    self._debug_log(
                        "Salvage", f"Result {self._item_debug_summary(item)} mode={mode.name} state=SUCCESS"
                    )
                    salvaged_items += 1
                    salvaged = True
                    ...
                    break
                self._debug_log(
                    "Salvage", f"Result {self._item_debug_summary(item)} mode={mode.name} state={state.name}"
                )
            ...
            if not salvaged:
                ...
                self._debug_log(
                    "Salvage",
                    f"Skip {self._item_debug_summary(item)} reason=all_modes_failed",
                    Console.MessageType.Warning,
                )
                Console.Log(
                    "AutoSalvage", f"No salvage mode succeeded for item_id={item_id}.", Console.MessageType.Warning
                )
        ...
        for bag_id in range(Bags.Backpack, Bags.Bag2 + 1):
            ...
            for item_id in item_array:
                ...
                if not deposited and is_trophy and self.deposit_trophies and is_white:
                    self._debug_log(
                        "Deposit", f"Select {self._item_debug_summary(item)} action=deposit reason=white_trophy"
                    )
                    GLOBAL_CACHE.Inventory.DepositItemToStorage(item_id)
                    yield from Routines.Yield.wait(350)
                    deposited = True
                ...
                if not deposited and is_material and self.deposit_materials:
                    self._debug_log(
                        "Deposit", f"Select {self._item_debug_summary(item)} action=deposit reason=material"
                    )
                    GLOBAL_CACHE.Inventory.DepositItemToStorage(item_id)
                    yield from Routines.Yield.wait(350)
                    deposited = True
                ...
                if not deposited and is_gold and self.deposit_golds and not is_usable and not is_trophy:
                    self._debug_log(
                        "Deposit",
                        f"Select {self._item_debug_summary(item)} action=deposit reason=gold_nonusable_nontrophy",
                    )
                    GLOBAL_CACHE.Inventory.DepositItemToStorage(item_id)
                    yield from Routines.Yield.wait(350)
                    deposited = True
                ...
                if (
                    (not deposited)
                    and (model_id in event_items)
                    and self.deposit_event_items
                    and model_id not in self.deposit_event_items_blacklist
                ):
                    self._debug_log(
                        "Deposit", f"Select {self._item_debug_summary(item)} action=deposit reason=event_item"
                    )
                    GLOBAL_CACHE.Inventory.DepositItemToStorage(item_id)
                    yield from Routines.Yield.wait(350)
                    deposited = True
