...
def draw_general() -> None:
    ...
    if PyImGui.checkbox("Recolor & Beacons enabled##rb_master", enabled) != enabled:
        ...
        if enabled:
            mark._clear_output()
    ImGui.show_tooltip("This feature's own switch. Loot Filters has a separate one -- neither " "depends on the other.")

    PyImGui.separator()
    PyImGui.text("Profile")
    PyImGui.text_colored(
        "Profiles are made in the Loot Filter Factory. This feature runs its own, "
        "independently of the one Loot Filters runs.",
        GRAY,
    )
    ...
    if PyImGui.checkbox("Blank loot assigned to someone else##rb_blank_un", blank) != blank:
        ...
        mark.save()
    ImGui.show_tooltip(
        "It is not yours to take, so it need not be on screen. Visual only -- it "
    ...
    status = mark.status()
    PyImGui.text_colored(
        "%d marking filter(s)  -  %d item(s) recoloured  -  %d beacon(s), "
        "%d particle(s)" % (status["filters"], status["recoloured"], status["beacons"], status["particles"]),
        GRAY,
    )
...
def draw_outcomes() -> None:
    ...
    PyImGui.text_colored(
        "The profile's order settles only what cannot hold two values: when several "
        "filters match one drop, the topmost supplies the colour and the preset. "
        "Reorder them in the Factory.",
        GRAY,
    )
    PyImGui.separator()
    ...
    for position, rule in enumerate(rules):
        ...
        header = "%d. %s  %s###rb_rule_%s" % (
            position + 1,
            rule.name,
            "(%s)" % marks if marks else "(no marking)",
            rule.id,
        )
        ...
        ImGui.show_tooltip(
            "A fully transparent label: the drop disappears from view. Visual only -- "
...
def draw_budget() -> None:
    ...
    PyImGui.text_wrapped(
        "Beacons are the expensive part of this feature. These are your controls; "
    ...
    distance = PyImGui.slider_float("Beacon distance##rb_dist", float(config.beacon_distance), 100.0, 5000.0)
    if abs(distance - config.beacon_distance) > 1e-6:
        config.beacon_distance = distance
        mark.live.beacon_distance = distance
        mark.save()
    ImGui.show_tooltip("Beyond this, no beacon. Recolouring is unaffected -- a distant drop can " "still be blanked.")
    ...
    if PyImGui.checkbox("Cheap shape for distant beacons##rb_cheap", cheap) != cheap:
        ...
        mark.save()
    ImGui.show_tooltip(
        "Past the distance below, draw the beam only -- no particles, no rings. The "
        "beam is what carries at range; the rest is what it costs."
    )
    if config.cheap_distant:
        cheap_at = PyImGui.slider_float("Cheap beyond##rb_cheapd", float(config.cheap_distance), 100.0, 5000.0)
    ...
    status = mark.status()
    PyImGui.text_colored(
        "Right now: %d beacon(s), %d live particle(s)" % (status["beacons"], status["particles"]),
    ...
    if worst:
        PyImGui.text_colored(
            "Your heaviest preset is about %.0f particles per beacon, so %d of them "
            "would be roughly %.0f." % (worst, config.max_beacons, worst * config.max_beacons),
            GRAY,
        )
