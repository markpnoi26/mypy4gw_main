...
def names() -> list[str]:
    ...
    """
    seen: dict[str, None] = {}
    for name in [RARITIES, ITEM_TYPES, MATERIALS, SALVAGES_TO, DYES, NICHOLAS] + list(catalog_data.categories()):
...
def groups_for(surface: str, config) -> list[tuple[str, list[tuple[int, str, int | None]]]]:
    ...
    if surface == ITEM_TYPES:
        ...
        return [
            (group, [(value, display, None) for display, value in entries])
    ...
    if not group_names:
        return [
            (band, [(e.model_id, e.name, e.model_id) for e in rows])
            for band, rows in catalog_data.alphabetical_bands(entries)
        ]
    return [
        (name, [(e.model_id, e.name, e.model_id) for e in catalog_data.in_group(surface, name)]) for name in group_names
    ]
...
def tooltip_for(surface: str):
    ...
    def item_tooltip(entry_id: int) -> str:
        ...
        parts = [
            entry.name,
            entry.category if not entry.group else "%s / %s" % (entry.category, entry.group),
...
def nicholas_rows(config, today):
    ...
    for selection in config.nicholas:
        ...
        label = ("This week: %s" % item) if selection.week is None else ("%s: %s" % (week.isoformat(), item))
