...
def draw_entries(key: str, entries, selected: set[int], apply, layout: str, tooltip=None) -> None:
    ...
    for index, (entry_id, name, model) in enumerate(entries):
        ...
        if model is None:
            ...
            new_state = ImGui.image_toggle_button(
                "##%s_%s" % (key, entry_id), _texture(model), on, width=_TILE, height=_TILE
            )
            if new_state != on:
                apply(entry_id, not on)
        PyImGui.end_group()
...
def draw_section(
    key: str, title: str, groups, selected: set[int], apply, layout: str, tooltip=None, default_open: bool = False
) -> None:
