...
def draw_general(loot: LootFilters) -> None:
    ...
    if PyImGui.checkbox("Respect the cross-account loot lock##lf_lock", lock) != lock:
        ...
        loot.save()
    ImGui.show_tooltip(
        "On: skip unassigned drops another of your accounts has claimed. Off: offer "
...
def _draw_verdict(loot: LootFilters, agent_id: int, item_id: int, source: str) -> None:
    ...
    ImGui.show_tooltip(
        "Writes the full report to the Py4GW console: everything the game reports "
...
def draw_debug(loot: LootFilters) -> None:
    ...
    if PyImGui.collapsing_header("Live configuration###dbg_cfg"):
        PyImGui.text_colored(
            "master switch: %s" % ("ON" if config.enabled else "OFF"), GOOD if config.enabled else WARN
        )
        PyImGui.text("profile: %s" % (config.profile or "(none)"))
        PyImGui.text(
            "rarities: "
            + "  ".join(
                "%s=%s" % (label, "on" if config.rarity_enabled(key) else "off") for key, label in surfaces.RARITY_KEYS
            )
        )
        for label, values in (
            ("hand list (models)", config.enabled_model_ids),
            ...
            PyImGui.text_colored("%-22s %d" % (label, len(values)), WARN if values and "script" in label else GRAY)
    ...
    PyImGui.text_colored("ACCEPTED LOOT ARRAY: %d" % len(accepted), GOOD if accepted else GRAY)
    ImGui.show_tooltip(
        "Exactly what GetLootArray returns. A drop being taken while NOT on this "
    ...
    try:
        ...
        nearby = AgentArray.Filter.ByDistance(AgentArray.GetItemArray(), Player.GetXY(), float(_state["dbg_range"]))
        ...
        for candidate in nearby:
            try:
                ...
                if agent_item is None:
                    continue
                loot.print_diagnostics(candidate, agent_item.item_id, Item.GetModelID(agent_item.item_id))
            except Exception:
                continue
    ImGui.show_tooltip(
        "Dumps the full report for every ground item in range. If a drop vanishes "
    ...
    for agent_id in nearby:
        ...
        breakdown = "\n".join(
            ("[x] " if holds else "[ ] ") + reason for reason, holds in loot.explain(agent_id, item_id, model_id)
        )
...
def draw_why(loot: LootFilters) -> None:
    ...
    **Hover any item in your bags** and it is evaluated live; with nothing hovered it falls back to
    ...
    PyImGui.text_wrapped(
        "Hover an item in your inventory, or target a drop on the ground. "
...
def draw_rarities(loot: LootFilters) -> None:
    ...
    for index, (key, label) in enumerate(surfaces.RARITY_KEYS):
        ...
        if key == "gold_coins":
            ImGui.show_tooltip(
                "An independent switch, not an alternative to the rarities - " "combine it with any of them."
            )


def draw_catalog(loot: LootFilters) -> None:
    _live_banner(loot)
    PyImGui.text_colored("A category contains groups. Trophies is banded alphabetically as it draws.", GRAY)
    PyImGui.separator()
...
def draw_materials(loot: LootFilters) -> None:
    _live_banner(loot)
    PyImGui.text_wrapped(
        "Wanting a material when it drops. Separate from 'Salvages to', which wants "
...
def draw_salvages_to(loot: LootFilters) -> None:
    _live_banner(loot)
    PyImGui.text_wrapped(
        "Take anything that salvages into these. This is what makes the large "
        "un-listed salvageable population reachable without enumerating it."
    )
    PyImGui.text_colored(
        "Every material is offered; the bundled salvage data informs the tooltip and " "never limits the choice.", GRAY
    )
    PyImGui.separator()
...
def draw_dyes(loot: LootFilters) -> None:
    _live_banner(loot)
    PyImGui.text_wrapped(
        "Every dye shares one model id, so a dye is chosen by colour and matched on " "item type plus colour."
    )
    PyImGui.separator()
...
def draw_nicholas(loot: LootFilters) -> None:
    ...
    _live_banner(loot)
    PyImGui.text_wrapped(
        "Nicholas is opt in -- nothing here is watched until you switch it on. " "Several weeks may be active at once."
    )
    PyImGui.separator()
    ...
    if PyImGui.checkbox("Follow the current week##nick_rolling", following) != following:
        _set(
            [s for s in loot.persisted.nicholas if s != rolling]
            if following
            else list(loot.persisted.nicholas) + [rolling]
        )
        return
    ImGui.show_tooltip(
        "Rolls over on its own as the week changes. Untick it and no week is " "watched unless you pinned one below."
    )
    ...
    try:
        picked = datetime.date(
            int(_state["nick_year"]),
            max(1, min(12, int(_state["nick_month"]))),
...
def draw_item_types(loot: LootFilters) -> None:
    ...
    _live_banner(loot)
    PyImGui.text_wrapped(
        "Tick a type to NEVER take it. This is a veto: it beats every other rule, " "exactly like the model blacklist."
    )
    PyImGui.text_colored(
        "Turning a rarity off is not the same thing -- that only removes rarity as "
        "a reason, and a hand list, a salvage target or a filter can still take the "
        "item. This is the only control that stops it outright.",
        GRAY,
    )
    PyImGui.separator()
...
def draw_blacklist(loot: LootFilters) -> None:
    ...
    _live_banner(loot)
    PyImGui.text_wrapped(
        "The blacklist inhibits every other rule. A blacklisted item is never taken, " "no matter what else wants it."
    )
    PyImGui.separator()
    ...
    if loot.live.blacklist_item_ids:
        PyImGui.separator()
        PyImGui.text_colored(
            "%d drop(s) suppressed this session (cleared on map change)" % len(loot.live.blacklist_item_ids), GRAY
        )
...
def draw_quick_access(loot: LootFilters) -> None:
    ...
    ImGui.show_tooltip(
        "A separate window you keep open while playing. It holds only what you add "
    ...
    for name in surfaces.names():
        ...
        if PyImGui.checkbox("%s##qa_pick_%s" % (name, name), on) != on:
            quick_access.set_surfaces([s for s in surfaces.names() if (s in chosen) != (s == name)])
