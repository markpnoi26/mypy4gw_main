...
class LootConfig:
    ...
    def copy(self) -> "LootConfig":
        ...
        return LootConfig(
            ...
            loot_whites=self.loot_whites,
            loot_blues=self.loot_blues,
            loot_purples=self.loot_purples,
            loot_golds=self.loot_golds,
            loot_greens=self.loot_greens,
            loot_gold_coins=self.loot_gold_coins,
    ...
    def diff(self, other: "LootConfig") -> list[str]:
        ...
        if self.enabled != other.enabled:
            out.append("looting: %s (saved: %s)" % ("on" if self.enabled else "off", "on" if other.enabled else "off"))
        for label, name in (
            ("whites", "white"),
            ("blues", "blue"),
            ("purples", "purple"),
            ...
            if mine != theirs:
                out.append("%s: %s (saved: %s)" % (label, "on" if mine else "off", "on" if theirs else "off"))
