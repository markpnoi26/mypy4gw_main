...
class LootFilters:
    ...
    def on_hand_list(self, item_id: int, model_id: int) -> bool:
        ...
        **A model id does not identify an item.** The game reuses the number across item types::
        ...
        category = (
            ...
            else ("Materials" if material_data.material(model_id) is not None else "")
        )
    ...
    def diagnostics(self, agent_id: int, item_id: int, model_id: int) -> list[str]:
        ...
        read(
            "owner",
            lambda: (
                __import__("Core.Agent", fromlist=["Agent"]).Agent.GetItemAgentOwnerID(agent_id)
                if agent_id
                else "(bag item)"
            ),
        )
        ...
        lines.append(
            "  rarities                   white=%s blue=%s purple=%s gold=%s green=%s coins=%s"
            % (
                config.loot_whites,
                config.loot_blues,
                config.loot_purples,
        ...
        for label, values in (
            ...
            lines.append(
                "  %-26s %d entries%s" % (label, len(values), "  <-- CONTAINS THIS MODEL" if model_id in values else "")
            )
        ...
        lines.append("-- VERDICT: %s --" % ("WANTED" if self.wants(agent_id, item_id, model_id) else "NOT WANTED"))
        return lines
    ...
    def explain(self, agent_id: int, item_id: int, model_id: int) -> list[tuple[str, bool]]:
        ...
        try:
            ...
            from Core.GlobalCache.WhiteboardLocks import is_loot_lock_blocked
        ...
        out.append(
            (
                (
                    "item type %s is vetoed (vetoes everything else)" % self.type_name(item_id)
                    if type_vetoed
                    else "item type is vetoed"
                ),
                type_vetoed,
            )
        )
        ...
        if on_list:
            ...
            out.append(
                (
                    "hand list has %s, but this item is not that kind (%s) -- the model id is "
                    "shared across item types" % (self.describe_model(model_id), self.hand_list_kind(model_id)),
                    False,
                )
            )
        else:
        ...
        if config.salvage_target_ids:
            ...
            for candidate in common + rare:
                if candidate in config.salvage_target_ids:
                    ...
                    break
        out.append(
            ("salvages into %s" % salvage_hit if salvage_hit else "salvages into something you want", bool(salvage_hit))
        )
        ...
        rarity_names = (
            ("loot_whites", "white", Item.Rarity.IsWhite),
        ...
        for attribute, label, check in rarity_names:
            try:
                ...
                is_that = False
            if is_that:
                out.append(("rarity: it is %s, and %s are on" % (label, label), bool(getattr(config, attribute))))
                break
    ...
    def register(self) -> None:
        ...
        try:
            ...
            PyCallback.PyCallback.Register(
                _CB_NAME, PyCallback.Phase.Data, self._pass, priority=99, context=PyCallback.Context.Update
            )
