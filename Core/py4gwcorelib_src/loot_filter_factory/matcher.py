...
def evaluate(rule: Rule, item_id: int, model_id: int | None = None) -> tuple[bool, list[tuple[str, bool]]]:
    ...
    if rule.name_contains:
        ...
        results.append(
            (
                "name contains %s" % " / ".join(rule.name_contains),
    ...
    if rule.dye_colors:
        ...
        results.append(("dye colour", dye_data.is_dye(item_id) and dye_data.color_of(item_id) in rule.dye_colors))
    ...
    if rule.min_damage is not None:
        ...
        try:
            ...
            ok = bool(
                Item.Mods.HasMod(
                    item_id, ModifierIdentifier.Damage, lambda low: True, lambda high: high >= rule.min_damage
                )
            )
        except Exception:
            ok = False
    ...
    if rule.damage_types:
        try:
            ...
            ok = any(
                Item.Mods.HasMod(item_id, ModifierIdentifier.DamageTypeProperty, DamageType(d))
