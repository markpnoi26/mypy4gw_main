...
def _notify() -> None:
    ...
    * the module is loaded but the name is wrong -- a bug, and a silent one. It shipped exactly
    ...
    for path, attribute in (
        ("Core.py4gwcorelib_src.loot_filters", "LootFilters"),
        ("Core.py4gwcorelib_src.recolor_beacons", "RecolorBeacons"),
    ):
        try:
    ...
    if PyImGui.begin_table("tbl_%s" % key, 2):
        ...
        if PyImGui.begin_child("all_%s" % key, (250.0, 220.0), True, PyImGui.WindowFlags.NoFlag):
            for display, value in sorted(source, key=lambda e: e[0].lower()):
                ...
                if PyImGui.selectable(
                    "%s (%d)##%s_a_%d" % (display, value, key, value), False, PyImGui.SelectableFlags.NoFlag, (0.0, 0.0)
                ):
                    result = result + (value,)
        PyImGui.end_child()
        ...
        if PyImGui.begin_child("sel_%s" % key, (250.0, 220.0), True, PyImGui.WindowFlags.NoFlag):
            ...
            for value in result:
                ...
                if PyImGui.selectable(
                    "%s (%d)##%s_r_%d" % (display, value, key, value), False, PyImGui.SelectableFlags.NoFlag, (0.0, 0.0)
                ):
    ...
    * ``_two_pane_picker`` -- every ``ModelID`` member, including the ones never catalogued. A flat
    ...
    for category in catalog_data.categories():
        ...
        if not PyImGui.collapsing_header(
            "%s  (%d/%d)###cur_%s_%s" % (category, picked_here, len(entries), key, category)
        ):
            continue
        groups = catalog_data.groups(category)
        # Trophies has no groups: its A..W split is banded at RENDER time, never in the data.
        bands = (
            [(name, catalog_data.in_group(category, name)) for name in groups]
            if groups
        ...
        for band, rows in bands:
            ...
            if not PyImGui.collapsing_header(
                "%s  (%d/%d)###cur_%s_%s_%s" % (band, on_count, len(band_ids), key, category, band)
            ):
                continue
            ...
            for position, entry in enumerate(rows):
                ...
                new_state = ImGui.image_toggle_button(
                    "##cur_%s_%d" % (key, entry.model_id), _texture(entry.model_id), on, width=48, height=48
                )
                if new_state != on:
                    result = (result + (entry.model_id,)) if not on else tuple(i for i in result if i != entry.model_id)
...
def _item_type_groups(key: str, chosen: tuple[int, ...]) -> tuple[int, ...]:
    ...
    for group, entries in item_type_data.grouped():
        ...
        if not PyImGui.collapsing_header("%s  (%d/%d)###ity_%s_%s" % (group, on_count, len(ids), key, group)):
            continue
...
def _attribute_source() -> list[tuple[str, int]]:
    ...
    try:
        ...
        return sorted(
            ((" ".join(w.capitalize() for w in a.name.split("_")), int(a.value)) for a in Attribute if a.value >= 0),
            key=lambda e: e[0],
        )
    except Exception:
        return []
...
def _damage_type_source() -> list[tuple[str, int]]:
    ...
    try:
        ...
        return [(d.name, int(d.value)) for d in DamageType if d.value >= 0 and not d.name.startswith("unknown")]
    except Exception:
        return []
...
def _draw_criteria(rule: Rule, index: int) -> Rule | None:
    ...
    values = {
        ...
        "model_ids": rule.model_ids,
        "dye_colors": rule.dye_colors,
        "salvages_into": rule.salvages_into,
    ...
    if PyImGui.collapsing_header("Rarity  (%d)###rar_%s" % (len(rarities), rid)):
        PyImGui.text_colored(
            "Any of the ticked ones. Combine with anything else -- this is what "
            "makes 'gold weapons at req 9' or 'beacon every green' possible.",
            GRAY,
        )
    ...
    if PyImGui.collapsing_header("Name contains  (%d)###name_%s" % (count, rid)):
        ...
        for position, part in enumerate(rule.name_contains):
            if PyImGui.small_button("remove##nm_%s_%d" % (rid, position)):
                values["name_contains"] = tuple(p for i, p in enumerate(rule.name_contains) if i != position)
                changed = True
            PyImGui.same_line(0, 6)
            PyImGui.text(part)
    ...
    if want_damage:
        ...
        if typed != (min_damage or 10):
            ...
            changed = True
        ImGui.show_tooltip(
            "Damage is a RANGE -- 'Chaos Dmg: 11-21'. This compares its top end, "
    ...
    return Rule(
        ...
        item_types=values["item_types"],
        model_ids=values["model_ids"],
        name_contains=values["name_contains"],
        rarities=rarities,
        dye_colors=values["dye_colors"],
        salvages_into=values["salvages_into"],
        max_requirement=max_req,
        requirement_attribute=req_attribute,
        min_value=min_value,
        min_damage=min_damage,
        damage_types=damage_types,
        mark_recolor=rule.mark_recolor,
        mark_color=rule.mark_color,
        mark_blank=rule.mark_blank,
        mark_beacon=rule.mark_beacon,
        mark_preset=rule.mark_preset,
    )
...
def _draw_preview(rule: Rule) -> None:
    ...
    PyImGui.text_colored(
        "ITEM MATCHES" if verdict else "ITEM DOES NOT MATCH",
        (0.45, 0.8, 0.45, 1.0) if verdict else (0.85, 0.4, 0.4, 1.0),
    )
    PyImGui.same_line(0, 8)
    PyImGui.text_colored("(%s of %d conditions)" % ("all" if rule.mode == MATCH_ALL else "any", len(breakdown)), GRAY)
    for label, passed in breakdown:
        PyImGui.text_colored(("  [x] " if passed else "  [ ] ") + label, (0.45, 0.8, 0.45, 1.0) if passed else GRAY)
...
def draw_filters() -> None:
    ...
    for index, rule in enumerate(list(current)):
        ...
        if PyImGui.small_button("Duplicate##dup_%s" % rule.id):
            ...
            current.insert(
                index + 1,
                Rule.from_dict({**copy.to_dict(), "id": store.next_id(current), "name": rule.name + " (copy)"}),
            )
            _commit_rules()
            return
        PyImGui.same_line(0, 6)
...
def draw_profiles() -> None:
    ...
    for index, profile in enumerate(list(current)):
        if not PyImGui.collapsing_header("%s  (%d)###prof_%s" % (profile.name, len(profile.filter_ids), profile.name)):
            continue
