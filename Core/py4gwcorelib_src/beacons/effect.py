...
def _beam_quads(x, y, gz, height, base_w, top_w, base_rgba, mid_rgba, top_rgba, alpha_mul, num_quads, mid_frac):
    ...
    for i in range(count):
        ...
        rows = [
            ((x - ax * half, y - ay * half, gz - height * f, col), (x + ax * half, y + ay * half, gz - height * f, col))
...
def _base_disc(x, y, cz, profile, radius, rgba, alpha_mul, segments: int = 36):
    ...
    for i in range(segments + 1):
        ...
        rim.append((x + math.cos(angle) * radius, y + math.sin(angle) * radius, _prof_z(profile, angle), edge_c))
    verts = []
...
class BeaconRenderer:
    ...
    def _ground_profile(self, x: float, y: float, radius: float, lift: float):
        ...
        if cached is not None:
            return cached
        points = [
            (x + math.cos(i / GROUND_SAMPLES * _TAU) * radius, y + math.sin(i / GROUND_SAMPLES * _TAU) * radius)
    ...
    def _show(self, key, x: float, y: float, preset, alpha: float) -> None:
        ...
        if int(params.get("beam_shape", 1)) == 1:
            ...
            verts += _cone(x, y, gz, params["height"], base_r, top_r, stops, beam_a, mid_frac, segments)
        else:
            verts = _beam_quads(
                x,
                y,
                ...
                colors["beam_top"],
                beam_a,
                int(params.get("beam_quads", 2)),
                params["beam_mid_frac"],
            )
        ...
        renderer.draw_shaded_3d(
            _base_disc(x, y, gz - lift, profile, radius, colors["disc"], disc_a), ground_additive, True
        )
        ...
        for k in range(rings):
            ...
            if ring_a > 0.02:
                renderer.draw_shaded_3d(
                    _ring(x, y, profile, ring_r, radius * float(params["ring_thickness"]), colors["disc"], ring_a),
                    ground_additive,
                    True,
                )
