...
EMITTER_GROUPS: tuple[tuple[str, tuple[tuple[str, float, float], ...]], ...] = (
    ...
    (
        ...
        (
            ...
            ("grav_z", -600.0, 600.0),
            ("drag", 0.0, 5.0),
            ("turbulence", 0.0, 500.0),
        ),
    ),
    (
        "orbit",
        (
            ("orbit_radius", 0.0, 150.0),
            ("orbit_radius_var", 0.0, 80.0),
            ("orbit_radius_end", -1.0, 150.0),
            ("orbit_spin", -8.0, 8.0),
            ("orbit_rise", -100.0, 250.0),
            ("orbit_height", 10.0, 1200.0),
        ),
    ),
...
EMITTER_DEFAULTS: dict[str, float] = {
    ...
    "turbulence": 0.0,
    "orbit_radius": 40.0,
    "orbit_radius_var": 15.0,
    "orbit_radius_end": -1.0,
    "orbit_spin": 2.0,
    "orbit_rise": 40.0,
    "orbit_height": 250.0,
    "spawn_radius": 0.0,
...
class Emitter:
    ...
    def copy(self) -> "Emitter":
        return Emitter(
            self.name, self.enabled, self.mode, self.additive, list(self.color), self.muted, dict(self.params)
        )
    ...
    def from_dict(cls, raw: dict) -> "Emitter":
        ...
        return cls(
            str(raw.get("name", "Emitter")),
            bool(raw.get("enabled", False)),
            int(raw.get("mode", ORBITAL)),
            bool(raw.get("additive", False)),
            [float(c) for c in color][:4],
            False,
            params,
        )
...
class BeaconPreset:
    ...
    colors: dict[str, list[float]] = field(default_factory=lambda: {k: list(v) for k, v in COLOR_DEFAULTS.items()})
    ...
    def estimated_live_particles(self) -> float:
        ...
        return sum(
            e.params.get("rate", 0.0) * e.params.get("life", 0.0) for e in self.emitters if e.enabled and not e.muted
        )
                ...
                if key in stored:
                    ...
                    try:
                        params[key] = (
                            ...
                            else (int(value) if isinstance(default, int) else float(value))
                        )
                    except (TypeError, ValueError):
                        pass
...
def base_preset() -> BeaconPreset:
    ...
    return BeaconPreset(
        BASE_NAME,
        dict(BEACON_DEFAULTS),
        {k: list(v) for k, v in COLOR_DEFAULTS.items()},
        [
            make_emitter(
                "Fireflies",
                enabled=True,
