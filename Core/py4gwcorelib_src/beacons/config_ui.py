...
def _draw_preset_bar(preset: BeaconPreset) -> None:
    ...
    if protected:
        PyImGui.text_colored(
            "The base beacon. It cannot be deleted -- there is always something to " "fall back to.", GRAY
        )
    ...
    if PyImGui.button("Randomise##bp_rand"):
        ...
        _save()
    ImGui.show_tooltip(
        "A starting point rather than a blank slate. Colours and shape only; " "the emitters are left alone."
    )
    ...
    if PyImGui.button("Export##bp_export"):
        _dialog().open_save(
            "Export beacon preset", valid_types=".json", start_path="%s.json" % preset.name, tag="export"
        )
    ImGui.show_tooltip("Write this preset to a file you choose. Sharing is by file, never the " "clipboard.")
    PyImGui.same_line(0, 4)
...
def _randomise(preset: BeaconPreset) -> None:
    ...
    for group_params in (BEACON_GROUPS[0][1], BEACON_GROUPS[1][1]):
        for param in group_params:
            if param.kind == COLOR:
                preset.set(param.key, [random.random(), random.random(), random.random(), preset.color(param.key)[3]])
            elif param.kind == INT:
...
def _draw_preview(preset: BeaconPreset) -> None:
    ...
    PyImGui.text_colored(
        "%.0f particles/sec  -  about %.0f alive per beacon" % (per_second, estimated),
    ...
    if _state["preview"]:
        ...
        PyImGui.text_colored("drawing %s  -  %d live particle(s)" % (label, renderer.live_particles()), GOOD)
...
def draw_editor() -> None:
    preset = _current()
    if preset is None:
        PyImGui.text_colored("No presets. This should not happen -- the base beacon is guaranteed.", WARN)
        return
...
def draw_presets() -> None:
    ...
    for index, preset in enumerate(presets):
        ...
        PyImGui.same_line(0, 8)
        PyImGui.text_colored(
            "%d emitter(s), %.0f particles/sec%s"
            % (
                len(preset.emitters),
                preset.particles_per_second(),
                "  -  protected" if store.is_protected(preset.name) else "",
            ),
            GRAY,
        )
