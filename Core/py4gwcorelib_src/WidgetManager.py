...
import PyImGui
from Core.HotkeyManager import HOTKEY_MANAGER, HotKey
from Core.ImGui_src.Style import Style
from Core.ImGui_src.types import Alignment, StyleTheme
...
def _get_profiling():
    ...
    if _profiling_registry is None:
        from Core.py4gwcorelib_src.Profiling import ProfilingRegistry
...
class WidgetTreeNode:
    ...
    def get_child(self, name: str) -> "WidgetTreeNode":
        if name not in self.children:
            self.children[name] = WidgetTreeNode(name=name, depth=self.depth + 1, parent=self)
...
class WidgetCatalog:
    ...
    def _matches_keywords(cls, widget: "Widget", keywords: Iterable[str]) -> bool:
        ...
        haystacks = [((field or "").lower(), cls._normalize(field or "")) for field in search_fields if field]
    ...
    def load_module(self) -> bool:
        ...
        if not os.path.isfile(self.script_path):
            PySystem.Console.Log(
                "WidgetManager", f"Widget script not found: {self.script_path}", PySystem.Console.MessageType.Error
            )
            return False
        ...
        try:
            ...
            PySystem.Console.Log(
                "WidgetManager",
                f"Failed to load widget module '{self.folder_script_name}': {e}",
        ...
        if self.module:
            ...
            self.minimal = (
                getattr(self.module, "minimal", None) if callable(getattr(self.module, "minimal", None)) else None
            )
            self.on_enable = (
                getattr(self.module, "on_enable", None) if callable(getattr(self.module, "on_enable", None)) else None
            )
            self.on_disable = (
                getattr(self.module, "on_disable", None) if callable(getattr(self.module, "on_disable", None)) else None
            )

            self.name = (
                getattr(self.module, 'MODULE_NAME', "") if hasattr(self.module, 'MODULE_NAME') else self.cleaned_name()
            )
            self.category = (
                getattr(self.module, 'MODULE_CATEGORY', "")
                if hasattr(self.module, 'MODULE_CATEGORY')
                else (self.widget_path.split('/')[0] if self.widget_path else "")
            )  # get first folder after Widgets
            self.tags = (
                getattr(self.module, 'MODULE_TAGS', [])
                if hasattr(self.module, 'MODULE_TAGS')
                else [folder for folder in self.widget_path.split('/') if folder]
            )
            self.aliases = [
                str(alias).strip() for alias in getattr(self.module, 'MODULE_ALIASES', []) if str(alias).strip()
            ]
            self.image = os.path.join(
                base_path,
                (
                    getattr(self.module, 'MODULE_ICON', "")
                    if hasattr(self.module, 'MODULE_ICON')
                    else "Textures\\missing_texture.png"
                ),
            )

            self.optional = (
                getattr(self.module, 'OPTIONAL', True)
                if hasattr(self.module, 'OPTIONAL')
                else self.category not in ["System", "Py4GW"]
            )  # System and Py4GW widgets are non-optional by default, all others are optional by default
        ...
        if self.__enabled:
            if self.module is not None:
                try:
                    ...
                    PySystem.Console.Log(
                        "WidgetManager",
                        f"Error during on_disable of widget {self.folder_script_name}: {str(e)}",
                        PySystem.Console.MessageType.Error,
                    )
                    PySystem.Console.Log(
                        "WidgetManager", f"Stack trace: {traceback.format_exc()}", PySystem.Console.MessageType.Error
                    )
    ...
    def enable(self):
        ...
        if self.enabled:
            ...
            try:
                ...
                PySystem.Console.Log(
                    "WidgetManager",
                    f"Error during on_enable of widget {self.folder_script_name}: {str(e)}",
                    PySystem.Console.MessageType.Error,
                )
                PySystem.Console.Log(
                    "WidgetManager", f"Stack trace: {traceback.format_exc()}", PySystem.Console.MessageType.Error
...
class WidgetHandler:
    ...
    def _draw_confirmation_modal(self):
        ...
        if PyImGui.begin_popup_modal(
            ...
            if widget:
                ImGui.text_colored(
                    "Warning - This widget is required for core functionality!", (1.0, 0.2, 0.2, 1.0), font_size=16
                )
                PyImGui.separator()
    ...
    def _load_widget_module(self, folder: str, filename: str):
        ...
        try:
            ...
            self.config_vars.append(
                WidgetConfigVars(widget_id=widget_id, section=f"Widget:{widget_id}", var_name=f"{widget_id}__enabled")
            )
            self.config_vars.append(
                WidgetConfigVars(widget_id=widget_id, section=f"Widget:{widget_id}", var_name=f"{widget_id}__optional")
            )
        ...
        for widget_name, widget_info in self.widgets.items():
            ...
            if widget_info.update is not None:
                try:
                    ...
                    PySystem.Console.Log(
                        "WidgetHandler",
                        f"Error executing widget {widget_name}: {str(e)}",
                        PySystem.Console.MessageType.Error,
                    )
                    PySystem.Console.Log(
                        "WidgetHandler", f"Stack trace: {traceback.format_exc()}", PySystem.Console.MessageType.Error
                    )
        ...
        for widget_name, widget_info in self.widgets.items():
            ...
            if widget_info.minimal is not None:
                try:
                    ...
                    PySystem.Console.Log(
                        "WidgetHandler",
                        f"Error executing minimal of widget {widget_name}: {str(e)}",
                        PySystem.Console.MessageType.Error,
                    )
                    PySystem.Console.Log(
                        "WidgetHandler", f"Stack trace: {traceback.format_exc()}", PySystem.Console.MessageType.Error
    ...
    def execute_configuring_widgets(self):
        ...
        for widget_info in self.widgets.values():
            ...
            try:
                ...
                PySystem.Console.Log(
                    "WidgetHandler",
                    f"Error executing widget {widget_info.folder_script_name}: {str(e)}",
                    PySystem.Console.MessageType.Error,
                )
                PySystem.Console.Log(
                    "WidgetHandler", f"Stack trace: {traceback.format_exc()}", PySystem.Console.MessageType.Error
    ...
    def _force_heroai_player_options(self, enabled: bool):
        try:
            ...
            PySystem.Console.Log(
                "WidgetHandler", f"Failed to force HeroAI player options: {exc}", PySystem.Console.MessageType.Warning
            )
