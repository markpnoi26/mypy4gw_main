...
class UIManager:
    ...
    def GetDialogButtons(debug: bool = False) -> list:
        ...
        valid = [f for f in FrameTree.frames_at_path(NPC_DIALOG_HASH, DIALOG_CHILD_OFFSET) if _is_button(f)]
        if valid:
    ...
    def ClickDialogButton(choice: int, debug: bool = False) -> bool:
        ...
        target = buttons[idx]
        if debug:
            ConsoleLog("DialogHelper", f"Clicking dialog choice #{choice} -> frame {target}", Console.MessageType.Info)
        target.click()
        return True
    ...
    def GetDialogButtonCount(debug: bool = False) -> int:
        ...
        if debug:
            ...
            ConsoleLog("DialogHelper", f"Dialog button count {count}", Console.MessageType.Info)
    ...
    def GetDialogButtonFrames(debug: bool = False) -> list[tuple[int, tuple[int, int, int, int]]]:
        ...
        if debug:
            ConsoleLog("DialogHelper", f"Detected dialog button frames: {valid_frames}", Console.MessageType.Info)
        ...
        result = [(f, f.rect) for f in FrameTree.descendants(root) if f.is_visible and f.template_type == 1]
...
class XunlaiStorageWindow:
    ...
    def GetMaterialSlotFrames() -> list[Frame]:
        ...
        for material, slot in XunlaiStorageWindow.MATERIAL_FRAME_OFFSETS.items():
            frames.append(Frame.material_slot(slot, max_tabs=XunlaiStorageWindow.MAX_TABS, raw_slot=True))
