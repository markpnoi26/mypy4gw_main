...
from Core.py4gwcorelib_src import Console
...
class HotKey:
    def __init__(
        ...
        key: Key,
        name: str = "",
        modifiers: ModifierKey = ModifierKey.NoneKey,
    ):
        self.key = key
...
class HotkeyManager:
    ...
    def register_hotkey(
        ...
        self.hotkeys[identifier] = HotKey(
            identifier=identifier, name=name, callback=callback, key=key, modifiers=modifiers
        )
                ...
                for hotkey in hot_keys.values():
                    if hotkey.matches(key, modifiers):
                        ...
                        try:
                            ConsoleLog(
                                "HotkeyManager",
                                f"Executing hotkey '{hotkey.name}' with identifier '{hotkey.identifier}' for hotkey '{hotkey.format_hotkey()}'",
                                message_type=Console.Console.MessageType.Debug,
                                log=log,
                            )
                            hotkey.callback()
                            ...
                            ConsoleLog(
                                "HotkeyManager",
                                f"Error while executing hotkey '{hotkey.name}' with identifier '{hotkey.identifier}' for hotkey '{hotkey.format_hotkey()}': {e}",
                                message_type=Console.Console.MessageType.Error,
                                log=log,
                            )
