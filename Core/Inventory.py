...
class Inventory:
    ...
    def IdentifyFirst():
        ...
        PySystem.Console.Log(
            "IdentifyFirst", f"Identified item with Item ID: {unid_item_id} using ID Kit ID: {id_kit_id}"
        )
        return True
    ...
    @staticmethod
    def _frame_by_alias(frame_label: str) -> Frame:
        """Alias lookup that yields an inert handle rather than raising."""
        try:
    ...
    def find_material_confirm_yes_frame() -> int:
        ...
        for dialog_frame_id, (screen_frame_id, _, dialog_template) in frames.items():
            ...
            if not matches(
                icon_frame_id, Inventory.MATERIAL_CONFIRM_ICON_OFFSET, Inventory.MATERIAL_CONFIRM_ICON_TEMPLATE
            ):
                continue
            if not matches(
                no_frame_id, Inventory.MATERIAL_CONFIRM_NO_OFFSET, Inventory.MATERIAL_CONFIRM_BUTTON_TEMPLATE
            ):
                continue
            if not matches(
                yes_frame_id, Inventory.MATERIAL_CONFIRM_YES_OFFSET, Inventory.MATERIAL_CONFIRM_BUTTON_TEMPLATE
            ):
                continue
    ...
    @staticmethod
    def _salvage_option_container() -> Frame:
    ...
    def _pick_salvage_choice_click_entry(
        ...
        def _priority(entry: SalvageChoiceEntry) -> tuple[int, float, int, float, int, int]:
            ...
            priority_bucket = (
                0
                if template_type == 1 and subtree_depth > 0
                else 1 if template_type == 1 else 2 if subtree_depth > 0 else 3
            )
            return (
                priority_bucket,
    ...
    def _get_salvage_choice_dialog_options(
        ...
        direct_children = [
            cast(VisibleFrameEntry, dict(entry)) for entry in visible_entries_by_parent.get(option_parent_id, [])
        ]

        direct_positive_entries = [entry for entry in direct_children if int(entry.get("offset", -1)) >= 1]
        ...
        for container_entry in container_candidates:
            ...
            positive_nested_children = [
                nested_entry for nested_entry in nested_children if int(nested_entry.get("offset", -1)) >= 1
            ]
        ...
        option_entries.sort(
            key=lambda item: (int(item.get("offset", -1)), float(item.get("top", 0.0)), int(item["frame_id"]))
        )
    ...
    def HandleSalvageChoiceDialog(
        ...
        dialog_frame_id = Inventory._salvage_dialog()
        ...
        if Inventory.IsSalvageChoiceMaterialConfirmVisible():
            return (
                yield from Inventory._wait_for_salvage_choice_dialog_close(
        ...
        visible_entries_by_parent = Inventory._build_visible_frame_entry_map()
        option_parent_id, option_parent_children, option_entries = Inventory._get_salvage_choice_dialog_options(
            visible_entries_by_parent
        )
        raw_child_summary = (
            ", ".join(
        ...
        for direct_entry in option_parent_children:
            ...
            positive_nested_children = [entry for entry in nested_children if int(entry.get("offset", -1)) >= 1]
            if not positive_nested_children:
                continue
            ...
            nested_container_summary.append(f"frame_id={int(direct_entry.get('frame_id', 0))}: {nested_child_summary}")
        ...
        for order, entry in enumerate(option_entries, start=1):
            entry["order"] = order
            path_root_frame_ids = (
                list(entry["path_root_frame_ids"]) if "path_root_frame_ids" in entry else [int(entry["frame_id"])]
            )
        ...
        option_summaries = ", ".join(
            Inventory._format_salvage_choice_option(entry, len(option_entries)) for entry in option_entries
        )
        Inventory._salvage_choice_debug_log(
            debug_enabled,
            log_module,
        ...
        selected_frame = Frame.from_id(int(selected_entry["frame_id"]))
        if not selected_frame.is_usable:
            Inventory._salvage_choice_debug_log(
                debug_enabled,
                log_module,
                f"{item_prefix}safety exit: selected option {selected_frame} is no longer clickable.",
            )
        ...
        ActionQueueManager().AddAction(queue_name, selected_frame.mouse_click_action, 0, 0)
        queue_drained = yield from Routines.Yield.Items._wait_for_empty_queue(
            queue_name,
            timeout_ms=queue_wait_timeout_ms,
            poll_ms=poll_ms,
        ...
        Inventory._salvage_choice_debug_log(
            debug_enabled,
            log_module,
            f"{item_prefix}dialog still visible after option click; clicking confirm {confirm_frame}.",
        )
        ActionQueueManager().AddAction(queue_name, confirm_frame.click)
        queue_drained = yield from Routines.Yield.Items._wait_for_empty_queue(
            queue_name,
            timeout_ms=queue_wait_timeout_ms,
            poll_ms=poll_ms,
    ...
    def DepositItemToStorage(item_id, Anniversary_panel=True):
        ...
        def GetBags():
            ...
            # Dynamically calculate the total capacity using PyInventory.Bag
            total_capacity = sum(PyInventory.Bag(bag_enum.value, bag_enum.name).GetSize() for bag_enum in possible_bags)
