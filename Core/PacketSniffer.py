...
class PacketSniffer:
    ...
    def _decode_stoc_packet(self, header: int, size: int, raw: bytes) -> str:
        ...
        if header == 0x009F and len(raw) >= 16:
            ...
            fields = {
                'type': GENERIC_VALUE_IDS.get(value_id or 0, value_id),
                'agent_id': self._u32(raw, 8),
                'value': self._u32(raw, 12),
            }
        elif header == 0x00A0 and len(raw) >= 20:
            fields = {
                'type': GENERIC_VALUE_IDS.get(self._u32(raw, 4) or 0, self._u32(raw, 4)),
                'target_id': self._u32(raw, 8),
                'caster_id': self._u32(raw, 12),
                'value': self._u32(raw, 16),
            }
        elif header == 0x00A2 and len(raw) >= 16:
            fields = {
                'type': self._u32(raw, 4),
                'agent_id': self._u32(raw, 8),
                'value': round(self._f32(raw, 12) or 0.0, 4),
            }
        elif header == 0x00A3 and len(raw) >= 20:
            fields = {
                'type': self._u32(raw, 4),
                'target_id': self._u32(raw, 8),
                'cause_id': self._u32(raw, 12),
                'value': round(self._f32(raw, 16) or 0.0, 4),
            }
        elif header == 0x00E4 and len(raw) >= 16:
            fields = {
                'agent_id': self._u32(raw, 4),
                'skill_id': self._u32(raw, 8),
                'skill_instance': self._u32(raw, 12),
            }
            ...
            fields = {
                'hero_id': self._u32(raw, 4),
                'profession': self._u32(raw, 8),
                'arg3': self._u32(raw, 12),
                'arg4': self._u32(raw, 16),
                'arg5': self._u32(raw, 20),
            }
        elif header == 0x0127 and len(raw) >= 180:
            fields = {
                'invited_name': self._utf16le_fixed(raw, 4, 20),
                'current_name': self._utf16le_fixed(raw, 44, 20),
                'invited_by': self._utf16le_fixed(raw, 84, 20),
                'context_info': self._utf16le_fixed(raw, 124, 28),
                'minutes_since_login': self._u32(raw, 176),
                'join_date': self._u32(raw, 180),
                'status': self._u32(raw, 184),
                'member_type': self._u32(raw, 188),
            }
        elif header == 0x0121 and len(raw) >= 128:
            fields = {
                'guild_id': self._u32(raw, 4),
                'name': self._utf16le_fixed(raw, 24, 32),
                'tag': self._utf16le_fixed(raw, 88, 6),
                'features': self._u32(raw, 100),
                'territory': self._u32(raw, 104),
                'faction': self._u32(raw, 132),
                'factions_count': self._u32(raw, 136),
                'qualifier_points': self._u32(raw, 140),
                'rating': self._u32(raw, 144),
                'rank': self._u32(raw, 148),
            }
        elif header in {0x0029, 0x002B, 0x002E} and len(raw) >= 16:
            fields = {
                'agent_id': self._u32(raw, 4),
                'x': round(self._f32(raw, 8) or 0.0, 4),
                'y': round(self._f32(raw, 12) or 0.0, 4),
            }
    ...
    def _decode_ctos_packet(self, header: int, size: int, raw: bytes) -> str:
        ...
        if header == 0x23 and len(raw) >= 12:
            ...
            fields = {
                'x': round(self._f32(raw, 4) or 0.0, 4),
                'y': round(self._f32(raw, 8) or 0.0, 4),
                'z_plane_or_unk': self._u32(raw, 12),
                'dir_x': round(self._f32(raw, 16) or 0.0, 4),
                'dir_y': round(self._f32(raw, 20) or 0.0, 4),
                'flags': self._u32(raw, 24),
            }
            ...
            fields = {
                'skill_id': self._u32(raw, 4),
                'type': self._u32(raw, 8),
                'target_id': self._u32(raw, 12),
                'flags': self._u32(raw, 16),
            }
