...
from Core import SpiritModelID
from Core.enums import SPIRIT_BUFF_MAP, Weapon
...
def _GetAppropiateTarget(
    ...
    def _GetEnemyCastingSpell(combat_distance):
        return (
            GetEnemyCastingSpell(combat_distance)
            if multibox
            else Routines.Targeting.GetEnemyCastingSpell(combat_distance)
        )
    ...
    def _GetEnemyConditioned(combat_distance):
        return (
            GetEnemyConditioned(combat_distance)
            if multibox
            else Routines.Targeting.GetEnemyConditioned(combat_distance)
        )
    ...
    def _GetEnemyDegenHexed(combat_distance):
        return (
            GetEnemyDegenHexed(combat_distance) if multibox else Routines.Targeting.GetEnemyDegenHexed(combat_distance)
        )
    ...
    def _GetEnemyKnockedDown(combat_distance):
        return (
            GetEnemyKnockedDown(combat_distance)
            if multibox
            else Routines.Targeting.GetEnemyKnockedDown(combat_distance)
        )

    def _TargetLowestAllyCaster(filter_skill_id=0):
        return (
            TargetLowestAllyCaster(filter_skill_id=filter_skill_id)
            if multibox
            else Routines.Targeting.TargetLowestAllyCaster(filter_skill_id=filter_skill_id)
        )

    def _TargetLowestAllyMartial(filter_skill_id=0):
        return (
            TargetLowestAllyMartial(filter_skill_id=filter_skill_id)
            if multibox
            else Routines.Targeting.TargetLowestAllyMartial(filter_skill_id=filter_skill_id)
        )

    def _TargetLowestAllyMelee(filter_skill_id=0):
        return (
            TargetLowestAllyMelee(filter_skill_id=filter_skill_id)
            if multibox
            else Routines.Targeting.TargetLowestAllyMelee(filter_skill_id=filter_skill_id)
        )

    def _TargetLowestAllyRanged(filter_skill_id=0):
        return (
            TargetLowestAllyRanged(filter_skill_id=filter_skill_id)
            if multibox
            else Routines.Targeting.TargetLowestAllyRanged(filter_skill_id=filter_skill_id)
        )

    def _TargetLowestAllyEnergy(other_ally=False, filter_skill_id=0):
        return (
            TargetLowestAllyEnergy(other_ally=other_ally, filter_skill_id=filter_skill_id)
            if multibox
            else Routines.Targeting.TargetLowestAllyEnergy(other_ally=other_ally, filter_skill_id=filter_skill_id)
        )

    def _TargetLowestAlly(other_ally=False, filter_skill_id=0):
        return (
            TargetLowestAlly(other_ally=other_ally, filter_skill_id=filter_skill_id)
            if multibox
            else Routines.Targeting.TargetLowestAlly(other_ally=other_ally, filter_skill_id=filter_skill_id)
        )
...
def _AreCastConditionsMet(
    ...
    if skills[slot].custom_skill_data.Conditions.UniqueProperty:
        ...
        if (
            skills[slot].skill_id == unique_skills.energy_drain
            or skills[slot].skill_id == unique_skills.energy_tap
            or skills[slot].skill_id == unique_skills.ether_lord
        ):
        ...
        if (
            skills[slot].skill_id == unique_skills.plague_sending
            or skills[slot].skill_id == unique_skills.plague_signet
            or skills[slot].skill_id == unique_skills.plague_touch
        ):
            return Agent.IsConditioned(Player.GetAgentID())

        if (
            skills[slot].skill_id == unique_skills.golden_fang_strike
            or skills[slot].skill_id == unique_skills.golden_fox_strike
            or skills[slot].skill_id == unique_skills.golden_lotus_strike
            or skills[slot].skill_id == unique_skills.golden_phoenix_strike
            or skills[slot].skill_id == unique_skills.golden_skull_strike
        ):
        ...
        if (
            skills[slot].skill_id == unique_skills.dwaynas_kiss
            or skills[slot].skill_id == unique_skills.unnatural_signet
            or skills[slot].skill_id == unique_skills.toxic_chill
        ):
        ...
        if (
            skills[slot].skill_id == unique_skills.empathic_removal
            or skills[slot].skill_id == unique_skills.iron_palm
            or skills[slot].skill_id == unique_skills.melandrus_resilience
            or skills[slot].skill_id == unique_skills.necrosis
            or skills[slot].skill_id == unique_skills.peace_and_harmony
            or skills[slot].skill_id == unique_skills.purge_signet
            or skills[slot].skill_id == unique_skills.resilient_weapon
        ):
            return Agent.IsHexed(vTarget) or Agent.IsConditioned(vTarget)

        if (
            skills[slot].skill_id == unique_skills.gaze_from_beyond
            or skills[slot].skill_id == unique_skills.spirit_burn
            or skills[slot].skill_id == unique_skills.signet_of_ghostly_might
        ):
            return True if Routines.Agents.GetNearestSpirit(Range.Spellcast.value) != 0 else False

        if skills[slot].skill_id == unique_skills.comfort_animal or skills[slot].skill_id == unique_skills.heal_as_one:
        ...
        if skills[slot].skill_id == unique_skills.ursan_force:
            return HasEffect_fn(Player.GetAgentID(), 228) and not HasEffect_fn(
                Player.GetAgentID(), unique_skills.ursan_force
            )
    ...
    if skills[slot].custom_skill_data.SkillType == SkillType.PetAttack.value:
        ...
        pet_attack_list = [
            GLOBAL_CACHE.Skill.GetID("Bestial_Mauling"),
    ...
    if Conditions.AlliesInRange != 0:
        player_pos = Player.GetXY()
        ally_array = ally_array = Routines.Agents.GetFilteredAllyArray(
            player_pos[0], player_pos[1], Conditions.AlliesInRangeArea, other_ally=True
        )
    ...
    if Conditions.SpiritsInRange != 0:
        player_pos = Player.GetXY()
        ally_array = ally_array = Routines.Agents.GetFilteredSpiritArray(
            player_pos[0], player_pos[1], Conditions.SpiritsInRangeArea
        )
    ...
    if Conditions.MinionsInRange != 0:
        player_pos = Player.GetXY()
        ally_array = ally_array = Routines.Agents.GetFilteredMinionArray(
            player_pos[0], player_pos[1], Conditions.MinionsInRangeArea
        )
...
def _IsReadyToCast(
    ...
    if expertise_exists:
        energy_cost = Routines.Checks.Skills.apply_expertise_reduction(
            energy_cost, expertise_level, skills[slot].skill_id
        )
...
def _IsOOCSkill(slot, skills) -> bool:
    ...
    if (
        skill_type == SkillType.Form.value
        or skill_type == SkillType.Preparation.value
        or skill_nature == SkillNature.Healing.value
        or skill_nature == SkillNature.Hex_Removal.value
        or skill_nature == SkillNature.Condi_Cleanse.value
        or skill_nature == SkillNature.EnergyBuff.value
        or skill_nature == SkillNature.Resurrection.value
    ):
        return True
    ...
    class Autocombat:
        ...
        class _SkillData:
            def __init__(self, slot):
                ...
                self.skillbar_data = GLOBAL_CACHE.SkillBar.GetSkillData(
                    slot
                )  # Fetch additional data from the skill bar
                self.custom_skill_data = SkillManager.Autocombat.custom_skill_data_handler.get_skill(
                    self.skill_id
                )  # Retrieve custom skill data
        ...
        def GetAppropiateTarget(self, slot):
            ...
            if target_allegiance == Skilltarget.Enemy:
                ...
                if self.skills[slot].custom_skill_data.Nature == SkillNature.EnergyBuff.value:
                    v_target = Routines.Targeting.TargetLowestAllyEnergy(
                        other_ally=True, filter_skill_id=self.skills[slot].skill_id
                    )
                    # print("Energy Buff Target: ", RawAgentArray().get_name(v_target))
                else:
                    v_target = Routines.Targeting.TargetLowestAlly(
                        other_ally=True, filter_skill_id=self.skills[slot].skill_id
                    )
            elif target_allegiance == Skilltarget.Self:
                v_target = Player.GetAgentID()
        ...
        def AreCastConditionsMet(self, slot, vTarget):
            ...
            if self.skills[slot].custom_skill_data.Conditions.UniqueProperty:
                """check all UniqueProperty skills"""
                if (
                    self.skills[slot].skill_id == self.unique_skills.energy_drain
                    or self.skills[slot].skill_id == self.unique_skills.energy_tap
                    or self.skills[slot].skill_id == self.unique_skills.ether_lord
                ):
                ...
                if (
                    self.skills[slot].skill_id == self.unique_skills.plague_sending
                    or self.skills[slot].skill_id == self.unique_skills.plague_signet
                    or self.skills[slot].skill_id == self.unique_skills.plague_touch
                ):
                    return Agent.IsConditioned(Player.GetAgentID())

                if (
                    self.skills[slot].skill_id == self.unique_skills.golden_fang_strike
                    or self.skills[slot].skill_id == self.unique_skills.golden_fox_strike
                    or self.skills[slot].skill_id == self.unique_skills.golden_lotus_strike
                    or self.skills[slot].skill_id == self.unique_skills.golden_phoenix_strike
                    or self.skills[slot].skill_id == self.unique_skills.golden_skull_strike
                ):
                    return Agent.IsEnchanted(Player.GetAgentID())
                ...
                if (
                    self.skills[slot].skill_id == self.unique_skills.dwaynas_kiss
                    or self.skills[slot].skill_id == self.unique_skills.unnatural_signet
                    or self.skills[slot].skill_id == self.unique_skills.toxic_chill
                ):
                    return Agent.IsHexed(vTarget) or Agent.IsEnchanted(vTarget)
                ...
                if (
                    self.skills[slot].skill_id == self.unique_skills.empathic_removal
                    or self.skills[slot].skill_id == self.unique_skills.iron_palm
                    or self.skills[slot].skill_id == self.unique_skills.melandrus_resilience
                    or self.skills[slot].skill_id == self.unique_skills.necrosis
                    or self.skills[slot].skill_id == self.unique_skills.peace_and_harmony
                    or self.skills[slot].skill_id == self.unique_skills.purge_signet
                    or self.skills[slot].skill_id == self.unique_skills.resilient_weapon
                ):
                    return Agent.IsHexed(vTarget) or Agent.IsConditioned(vTarget)

                if (
                    self.skills[slot].skill_id == self.unique_skills.gaze_from_beyond
                    or self.skills[slot].skill_id == self.unique_skills.spirit_burn
                    or self.skills[slot].skill_id == self.unique_skills.signet_of_ghostly_might
                ):
                    return True if Routines.Agents.GetNearestSpirit(Range.Spellcast.value) != 0 else False

                if (
                    self.skills[slot].skill_id == self.unique_skills.comfort_animal
                    or self.skills[slot].skill_id == self.unique_skills.heal_as_one
                ):
                    LessLife = Agent.GetHealth(vTarget) < Conditions.LessLife
                    dead = Agent.IsDead(vTarget)
                    return LessLife or dead
                ...
                if self.skills[slot].skill_id == self.unique_skills.ursan_force:
                    return self.HasEffect(Player.GetAgentID(), 228) and not self.HasEffect(
                        Player.GetAgentID(), self.unique_skills.ursan_force
                    )
            ...
            if Conditions.EnemyCount != 0:
                ...
                enemy_array = Routines.Agents.GetFilteredEnemyArray(
                    player_pos[0], player_pos[1], Conditions.EnemiesInRange
                )
                if len(enemy_array) >= Conditions.EnemyCount:
                    number_of_features += 1
                else:
        ...
        def IsReadyToCast(self, slot):
            ...
            energy_cost = Routines.Checks.Skills.GetEnergyCostWithEffects(
                self.skills[slot].skill_id, Player.GetAgentID()
            )

            if self.expertise_exists:
                energy_cost = Routines.Checks.Skills.apply_expertise_reduction(
                    energy_cost, self.expertise_level, self.skills[slot].skill_id
                )
            ...
            target_allegiance = self.skills[slot].custom_skill_data.TargetAllegiance
            if target_allegiance != Skilltarget.NonWeaponSpelledAlly.value and self.HasEffect(
                v_target, self.skills[slot].skill_id
            ):
                self.in_casting_routine = False
                return False, v_target
            ...
            if attack_speed == 0:
                match weapon_type:
                    ...
                    case Weapon.Staff3.value:
                        attack_speed = 1.75
                    case _:
                        attack_speed = 1.75  # safe default
            ...
            if self.game_throttle_timer.IsExpired():
                ...
                if self.auto_attack_timer.IsExpired():
                    ...
                    if (
                        ...
                        and not Agent.IsCasting(player_id)  # and
