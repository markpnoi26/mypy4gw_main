...
class RestorationMagic:
    ...
    def _count_allies_suffering_degen(self) -> int:
        ...
        from Core import AgentArray, GLOBAL_CACHE
    ...
    def Spirit_Light(
        ...
        *,
        ...
        threshold: float = (
            health_threshold if health_threshold is not None else float(spirit_light.Conditions.LessLife or 0.60)
        )
        threshold = max(0.0, min(1.0, threshold))
    ...
    def Spirit_Transfer(
        ...
        *,
        ...
        threshold: float = (
            health_threshold if health_threshold is not None else float(spirit_transfer.Conditions.LessLife or 0.50)
        )
        threshold = max(0.0, min(1.0, threshold))
