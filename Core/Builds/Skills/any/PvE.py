...
class PvE:
    ...
    def Air_of_Superiority(self) -> BuildCoroutine:
        from Core import Player, GLOBAL_CACHE
    ...
    def Cry_of_Pain(self, require_mesmer_hex: bool = False) -> BuildCoroutine:
        from Core import Agent, GLOBAL_CACHE
        ...
        *,
        ...
        target_agent_id = Routines.Targeting.PickClusteredTarget(
            ...
            preferred_condition=lambda agent_id: (
                Agent.GetHealth(agent_id) > 0.3 and (Agent.IsHexed(agent_id) or Agent.IsConditioned(agent_id))
            ),
            filter_radius=Range.Spellcast.value,
        )
    ...
    def Technobabble(self) -> BuildCoroutine:
        from Core import Agent, AgentArray, GLOBAL_CACHE, Player
        ...
        if not target_agent_id:
            ...
            enemy_array = AgentArray.Filter.ByCondition(
                enemy_array,
                lambda agent_id: (
                    Agent.IsAlive(agent_id) and (Agent.IsHexed(agent_id) or Agent.IsConditioned(agent_id))
                ),
            )
            if enemy_array:
                target_agent_id = min(
    ...
    def Vampirism(self) -> BuildCoroutine:
        from Core import Agent, AgentArray, Player, SpiritModelID
        ...
        return (
            yield from self.build.CastSpiritSkillID(
    ...
    def _get_spirit_form_agent_ids(self) -> set[int]:
        ...
        for account in GLOBAL_CACHE.ShMem.GetAllAccountData() or []:
            ...
            try:
                if any(
                    b.SkillId == self._SPIRIT_FORM_SKILL_ID for b in account.AgentData.Buffs.Buffs if b.SkillId != 0
                ):
    ...
    def _get_best_rod_target(self) -> int:
        ...
        allies = AgentArray.Filter.ByCondition(
            ...
            and ((Agent.GetXY(aid)[0] - me_x) ** 2 + (Agent.GetXY(aid)[1] - me_y) ** 2) ** 0.5
            <= Range.Spellcast.value * 1.4,
        )
        if not allies:
            return 0
    ...
    def Ghostly_Fury(self, is_active: bool = True) -> BuildCoroutine:
        ...
        if not target:
            ...
            if enemies:
                enemies.sort(
                    key=lambda aid: ((Agent.GetXY(aid)[0] - me_x) ** 2 + (Agent.GetXY(aid)[1] - me_y) ** 2) ** 0.5
                )
