...
from Core import AgentArray, GLOBAL_CACHE, Player, Range, Routines, SpiritModelID, ThrottledTimer, Utils
...
class NoAttribute:
    ...
    def Breath_of_the_Great_Dwarf(self) -> BuildCoroutine:
        ...
        def _party_has_burning() -> bool:
            ...
            return any(Routines.Checks.Agents.HasEffect(agent_id, burning_id) for agent_id in (ally_array or []))
    ...
    def Great_Dwarf_Weapon(self) -> BuildCoroutine:
        ...
        return (
            yield from self.build.CastSkillIDAndRestoreTarget(
                skill_id=great_dwarf_weapon_id, target_agent_id=target_agent_id, log=False, aftercast_delay=250
            )
        )
    ...
    def You_Move_Like_a_Dwarf(
        ...
        *,
        ...
        if not target_agent_id and _has_enough_energy():
            ...
            melee_candidates = [aid for aid in enemy_array if Agent.IsMelee(aid)]
            if melee_candidates:
    ...
    def Finish_Him(self) -> BuildCoroutine:
        ...
        if not target_agent_id:
            candidates = [aid for aid in enemy_array if not _has_cracked_armor(aid) and not _has_deep_wound(aid)]
            if candidates:
                target_agent_id = sorted(
                    candidates,
        ...
        if not target_agent_id:
            candidates = [aid for aid in enemy_array if _has_cracked_armor(aid) ^ _has_deep_wound(aid)]
            if candidates:
                target_agent_id = sorted(
                    candidates,
                    key=lambda aid: -Agent.GetMaxHealth(aid),
    ...
    def I_Am_Unstoppable(
        ...
        *,
        ...
        had_recent_knockdown = (
            self._iau_last_kd_ms > 0 and (now_ms - self._iau_last_kd_ms) <= self._iau_recent_kd_window_ms
        )
        ...
        should_cast = (
            ...
            or (min_adjacent_enemies > 0 and int(contact_count or 0) >= min_adjacent_enemies)
        )
        if not should_cast:
            return False
    ...
    def Save_Yourselves_kurzick(self) -> BuildCoroutine:
        ...
        return (
            yield from self._cast_protective_party_shout(
    ...
    def Summon_Spirits(self) -> BuildCoroutine:
        ...
        owned_spirits = AgentArray.Filter.ByCondition(
            owned_spirits,
            lambda agent_id: (
                Agent.IsAlive(agent_id) and Agent.IsSpawned(agent_id) and Agent.GetOwnerID(agent_id) == self_agent_id
            ),
        )
        ...
        if close_to_aggro:
            ...
            combat_pull = any(
                Utils.Distance(player_xy, Agent.GetXY(spirit_id)) > combat_threshold for spirit_id in owned_spirits
            )
        else:
            combat_pull = False
        ...
        if not close_to_aggro:
            ...
            travel_pull = any(
                Utils.Distance(player_xy, Agent.GetXY(spirit_id)) > travel_threshold for spirit_id in owned_spirits
            )
        else:
            travel_pull = False
    ...
    def _summon_spirits(self, skill_id: int) -> BuildCoroutine:
        ...
        player_xy = Player.GetXY()
        spirit_distances = [Utils.Distance(player_xy, Agent.GetXY(spirit_id)) for spirit_id in spirits]
        if in_aggro:
            # In combat, keep spirits tight: pull them once any core spirit leaves Nearby.
            should_reposition = any(
                Range.Nearby.value < distance <= Range.Compass.value for distance in spirit_distances
            )
