...
from Core import AgentArray, GLOBAL_CACHE, Range, Routines
...
class SmitingPrayers:
    ...
    def Ray_of_Judgment(self, *, exclude_target_id: int = 0) -> BuildCoroutine:
        ...
        if ready_slot == primary_slot:
            ...
            if self.build._is_whiteboard_skill(ray_of_judgment_id) and self.build._whiteboard_is_claimed(
                ray_of_judgment_id, target_agent_id
            ):
                return False
    ...
    def Reversal_of_Damage(self) -> BuildCoroutine:
        ...
        def _melee_count(ally_id: int) -> int:
            ...
            if cached is not None:
                return cached
            count = sum(1 for enemy_id in _enemies_around(ally_id) if Routines.Checks.Agents.IsMelee(enemy_id))
