...
class HealingPrayers:
    ...
    def Healing_Burst(self) -> BuildCoroutine:
        ...
        healing_burst_candidate_health_threshold: float = max(
            0.0, min(1.0, float(healing_burst.Conditions.LessLife or 0.80))
        )
        ...
        def _score_healing_burst_target(anchor_agent_id: int, ally_array: list[int]) -> tuple[float, float]:
            ...
            cluster_bonus = max(0.0, cluster_missing - anchor_missing) * 100.0 + max(0, nearby_injured - 1) * 15.0
