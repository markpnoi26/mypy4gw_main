...
class DBMgr:
    ...
    def _get_setup_scripts_for_database(cls, name: str, db_path: Path) -> list[Path]:
        ...
        if name == _PRIMARY_ALIAS:
            return sorted(
                path
                for path in setup_dir.glob("*.sql")
    ...
    def _ensure_tx(self) -> None:
        ...
        if self._tx_conn is None:
            raise RuntimeError(
                f"No active transaction on '{self._name}'. " f"Call BeginTransaction() first, then use commit=False."
            )
    ...
    def _recover_from_corruption(self) -> None:
        ...
        self._log(
            f"Database '{self._name}' is corrupt and no backup was found. " f"Creating a clean replacement.",
            "Warning",
        )
    ...
    def _retry_on_lock(self, sql: str, params: tuple, fetch: bool) -> Any:
        ...
        try:
            ...
            if "locked" in str(e).lower():
                raise sqlite3.OperationalError(f"Database '{self._name}' is locked after 3 retries.") from e
            raise
    ...
    def Register(self, name: str, filename: Union[str, Path]) -> Path:
        ...
        if self.Has(name):
            raise ValueError(
                f"Database '{name}' is already registered. " f"Use Unregister('{name}') to remove it first."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Database path '{path}' escapes the DBMgr data directory " f"'{DBMgr._require_data_path()}'."
            )
    ...
    def Insert(
        ...
        if len(fields) != len(values):
            raise ValueError(f"Insert field/value count mismatch: " f"{len(fields)} fields vs {len(values)} values.")
        if not fields:
    ...
    def Merge(
        ...
        for f in fields:
            self._validate_identifier(f)
        if len(fields) != len(values):
            raise ValueError(f"Merge field/value count mismatch: " f"{len(fields)} fields vs {len(values)} values.")
        if not fields:
        ...
        if non_pk_fields:
            ...
            sql = f"INSERT OR IGNORE INTO {table} ({cols}) VALUES ({places})"
    ...
    def EndTransaction(self, action: str) -> None:
        ...
        if action not in ("COMMIT", "ROLLBACK"):
            raise ValueError(f"Invalid action '{action}'. Use 'COMMIT' or 'ROLLBACK'.")
        if self._tx_conn is None:
            raise RuntimeError(f"No active transaction on '{self._name}'. " f"Call BeginTransaction() first.")
    ...
    def Backup(self, target_folder: Union[str, Path]) -> Path:
        ...
        if self._tx_conn is not None:
            raise RuntimeError(
                f"Cannot backup '{self._name}' while a transaction is active. " f"End the transaction first."
            )
    ...
    def Export(self, target_path: Union[str, Path]) -> None:
        ...
        """
        if self._tx_conn is not None:
            raise RuntimeError(f"Cannot export '{self._name}' while a transaction is active.")

        target_path = Path(target_path).resolve()
        if target_path.exists():
            raise FileExistsError(
                f"Export target '{target_path}' already exists. " f"Delete it first or choose a different path."
            )
    ...
    def Import(self, source_path: Union[str, Path]) -> None:
        ...
        if "tables" not in data or not isinstance(data["tables"], dict):
            raise ValueError("Import JSON must contain a 'tables' key with a dictionary " "of table name → rows.")
    ...
    def Vacuum(self) -> None:
        ...
        """
        if self._tx_conn is not None:
            raise RuntimeError(f"Cannot vacuum '{self._name}' while a transaction is active.")
