...
class GWUI:
    ...
    def CreatePanel(window_id: int, width: float = 0.0, height: float = 0.0, child_index: int = 0) -> int:
        ...
        try:
            return int(
                PyUIManager.UIManager.create_content_panel_by_frame_id(
                    int(window_id), float(width), float(height), int(child_index)
                )
                or 0
            )
        except Exception:
    ...
    def CreateButtonList(window_id: int, child_index: int = 0) -> int:
        ...
        list_id = int(PyUIManager.UIManager.create_scrollable_content_by_frame_id(window_id, child_index, 0x20000) or 0)
        if list_id:
            try:
    ...
    def SetChecked(checkbox_id: int, checked: bool) -> bool:
        ...
        try:
            return bool(PyUIManager.UIManager.set_checkbox_checked_by_frame_id(checkbox_id, bool(checked)))
        except Exception:
            return False
    ...
    def CreateHyperlink(hyperlink_list_id: int, caption: str, insert_index: int = 0) -> int:
        ...
        return int(
            PyUIManager.UIManager.add_clickable_text_button_to_selectable_list(hyperlink_list_id, str(caption), code, 0)
            or 0
        )
    ...
    def GetClickedHyperlink(hyperlink_list_id: int) -> int:
        ...
        return int(PyUIManager.UIManager.get_frame_list_selection_by_frame_id(hyperlink_list_id) or 0)
    ...
    def SetHyperlinkColor(text_button_id: int, color_abgr: int) -> bool:
        ...
        try:
            return bool(
                PyUIManager.UIManager.set_text_button_color_by_frame_id(text_button_id, int(color_abgr) & 0xFFFFFFFF)
            )
        except Exception:
            return False
    ...
    def SetHyperlinkText(text_button_id: int, caption: str) -> bool:
        ...
        try:
            return bool(PyUIManager.UIManager.set_text_button_text_by_frame_id(text_button_id, str(caption)))
        except Exception:
            return False
    ...
    def CreateProgressBar(
        window_id: int, x: float = 10.0, y: float = 10.0, width: float = 160.0, height: float = 0.0
    ) -> int:
    ...
    def CreateTabs(window_id: int, child_index: int = 0) -> int:
        ...
        return int(PyUIManager.UIManager.create_tabs_frame_by_frame_id(window_id, 0x40000, child_index) or 0)
    ...
    def AddTab(tabs_id: int, caption: str, index: int, body_text: str = "") -> int:
        ...
        return int(
            PyUIManager.UIManager.add_tab_to_page_by_frame_id(tabs_id, str(caption), int(index), 0, str(body_text)) or 0
        )
