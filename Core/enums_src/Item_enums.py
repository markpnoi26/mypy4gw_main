...
ArmorType: TypeAlias = Literal[
    ItemType.Headpiece, ItemType.Chestpiece, ItemType.Gloves, ItemType.Leggings, ItemType.Boots, ItemType.Salvage
]
...
class ItemAction(IntEnum):
    ...
    Ignore = (
        auto()
    )  # Ignore the item, no actions will be performed on it. This can be useful to exclude certain items from being processed by other rules.
    ...
    Hold = (
        auto()
    )  # Hold the item in the inventory, no other actions will be performed on it. This can be useful to exclude certain items from being processed by other rules.
    ...
    TradeToPlayer = (
        auto()
    )  # Open the trade window with a specific player and offer the item. The player name should be specified in the rule's parameters.
