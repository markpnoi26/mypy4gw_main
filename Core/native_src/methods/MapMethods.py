...
class MapMethods:
    ...
    def Travel(map_id: int, region: int = 0, district_number: int = 0, language: int = 0) -> bool:
        ...
        return UIManager.SendUIMessageRaw(
            UIMessage.kTravel,
            ctypes.addressof(
                TravelStruct(map_id=map_id, region=region, language=language, district_number=district_number)
            ),
            False,
        )
    ...
    def TravelGH(key: GHKey | None = None) -> bool:
        ...
        return UIManager.SendUIMessageRaw(UIMessage.kGuildHall, ctypes.addressof(gh_key), 0, False)
