...
def _resolve_ctlbtn_proc() -> int:
    ...
    addr = Scanner.Find(
        b"\x55\x8b\xec\x83\xec\x30\x53\x8b\x5d\x08\x56\x57" b"\x8b\x7d\x0c\x8b\x43\x04\x8b\x53\x08\x48\x83\xf8\x5e",
        ...
        def _action():
            ...
            if enable_click and DIALOG_SUBCLASS_TYPE_ADDR:
                if FrameNewSubclass_Func.is_valid():
                    ...
                    print(
                        f"[ButtonMethods] FrameNewSubclass called on parent {parent_frame_id} — click support enabled"
                    )
