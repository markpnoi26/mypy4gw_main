...
from Core.enums_src.GameData_enums import Ailment, Attribute, DamageType, Profession, Reduced_Ailment
from Core.enums_src.Item_enums import BowType, ItemType, Rarity
from Core.native_src.internals import string_table
from Core.mods_types import ItemBaneSpecies
...
class GWStringEncoded:
    ...
    def __init__(
        ...
        fallback: str,
        placeholder_bytes: bytes = bytes(),
        placeholder_replacement: Optional[list[str]] = None,
    ):
...
class GWEncoded:
    ...
    HEALTH_MINUS_75 = bytes(
        [0x7E, 0xA, 0xA, 0x1, 0x52, 0xA, 0x1, 0x0, 0x1, 0x1, 0x4B, 0x1, 0x1, 0x0]
    )  # 0x4B = 75 in little endian, 0x1 = minus, 0x0 = health
    HEALTH_MINUS_NUM = bytes(
        [0x7E, 0xA, 0xA, 0x1, 0x52, 0xA, 0x1, 0x0, 0x1, 0x1]
    )  # 0x4B = 75 in little endian, 0x1 = minus, 0x0 = health
    ...
    WHILE_HEALTH_ABOVE_BYTES = (
        WHILE_ABOVE_BYTES + SEPARATING_BYTES + HEALTH_BYTES + bytes([0x1, 0x1, 0x32, 0x1])
    )  # next byte is the health threshold
    WHILE_HEALTH_BELOW_BYTES = (
        WHILE_BELOW_BYTES + SEPARATING_BYTES + HEALTH_BYTES + bytes([0x1, 0x1, 0x32, 0x1])
    )  # next byte is the health threshold
    ...
    DAMAGE_PLUS_PERCENT = bytes(
        [
            *ITEM_BONUS,
            *PLUS_PERCENT_TEMPLATE,
            *DAMAGE_TEXT,
            0x1,
            0x0,
            0x1,
    ...
    def _requires_attribute_level(attribute: Attribute = Attribute.None_, attribute_level: int = 0) -> bytes:
        return bytes(
            [
                *GWEncoded.REQUIRES_NUM1_STR1,
                *GWEncoded.ATTRIBUTE_NAMES.get(attribute, GWEncoded.ItemsAtrribute),
                0x1,
                0x0,
                0x1,
                0x1,
                *GWEncoded._encode_string_table_number(attribute_level),
            ]
        )
    ...
    def _gold_amount_bytes(amount: int) -> bytes:
        return (
            bytes([*GWEncoded.NUM1_GOLD, 0x1, 0x1]) + GWEncoded._encode_string_table_number(amount) + bytes([0x1, 0x0])
        )

    @staticmethod
    def _platinum_amount_bytes(amount: int) -> bytes:
        return (
            bytes([*GWEncoded.NUM1_PLATINUM, 0x1, 0x1])
            + GWEncoded._encode_string_table_number(amount)
            + bytes([0x1, 0x0])
        )
    ...
    def _formatted_currency_amount_bytes(amount: int) -> tuple[bytes, bytes]:
        ...
        return GWEncoded._platinum_amount_bytes(platinum_amount) if platinum_amount > 0 else b"", (
            GWEncoded._gold_amount_bytes(gold_amount) if gold_amount > 0 else b""
        )
    ...
    def _append_line_with_fallback(base: GWStringEncoded, line_bytes: bytes, fallback_suffix: str) -> GWStringEncoded:
        ...
        return GWEncoded._encoded(
            GWEncoded._join_segments(base.encoded, line_bytes), f"{base.fallback}{separator}{fallback_suffix}"
        )
