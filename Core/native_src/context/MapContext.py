...
from ctypes import Structure, POINTER, c_uint32, c_float, c_void_p, c_wchar, c_uint8, c_uint16, cast, sizeof
...
class SpawnEntryStruct(Structure):
    ...
    def snapshot(self) -> SpawnPoint:
        return SpawnPoint(x=float(self.x), y=float(self.y), angle=float(self.angle), tag=self.tag)
...
class MapStaticDataStruct(Structure):
    ...
    _fields_ = [
        ...
        (
            "trapezoid_count",
            c_uint32,
        ),  # +0x084  GWCA: nextTrapezoidId. Starts at 0, incremented per trap — equals total count.
...
class MapContext:
    ...
    def _update_ptr():
        ...
        MapContext._cached_ctx = cast(ptr, POINTER(MapContextStruct)).contents
