...
class AccAgentContextStruct(Structure):
    ...
    _fields_ = [
        ...
        (
            "agent_summary_info_array",
            GW_Array,
        ),  # +0x0098 Array<AgentSummaryInfo> elements are of size 12. {ptr, func, ptr}
        ("h00A8_array", GW_Array),  # +0x00A8 Array<void *>
        ("h00B8_array", GW_Array),  # +0x00B8 Array<void *>
        (
            "rand1",
            c_uint32,
        ),  # +0x00C8 Number seems to be randomized quite a bit o.o seems to be accessed by textparser.cpp
...
class AccAgentContext:
    ...
    def _update_ptr():
        ...
        if not ptr:
            ...
            return
        AccAgentContext._cached_ctx = cast(ptr, POINTER(AccAgentContextStruct)).contents
