...
class GuildHistoryEvent(Structure):
    ...
    _fields_ = [
        ...
        (
            "name_enc",
            c_wchar * 256,
        ),  # Name of added/kicked person, then the adder/kicker, they seem to be in the same array
    ]
...
class GuildContext:
    ...
    def _update_ptr():
        ...
        if not ptr:
            ...
            return
        GuildContext._cached_ctx = cast(ptr, POINTER(GuildContextStruct)).contents
