...
class EquipmentItemsUnion:
    items: tuple[ItemData, ItemData, ItemData, ItemData, ItemData, ItemData, ItemData, ItemData, ItemData]
    weapon: ItemData
    offhand: ItemData
    chest: ItemData
    legs: ItemData
...
class EquipmentStruct(Structure):
    ...
    _fields_ = [
        ...
        (
            "left_hand_map",
            c_uint8,
        ),  # 0x0020 Weapon1     None = 9, Bow = 0, Hammer = 0, Focus = 1, Daggers = 0, Scythe = 0
        (
            "right_hand_map",
            c_uint8,
        ),  # 0x0021 Weapon2     None = 9, Sword = 0, Spear = 0, Staff = 0, Daggers = 0, Axe = 0, Zepter = 0, Bundle
...
class AgentLivingStruct(AgentStruct):
    ...
    _fields_ = [
        ...
        (
            "weapon_attack_speed",
            c_float,
        ),  # The base attack speed in float of last attacks weapon. 1.33 = axe, sWORD, daggers etc.
        ...
        (
            "allegiance",
            c_uint8,
        ),  # Constants::Allegiance; 0x1 = ally/non-attackable, 0x2 = neutral, 0x3 = enemy, 0x4 = spirit/pet, 0x5 = minion, 0x6 = npc/minipet
        (
            "weapon_type",
            c_uint16,
        ),  # 1=bow, 2=axe, 3=hammer, 4=daggers, 5=scythe, 6=spear, 7=sWORD, 10=wand, 12=staff, 14=staff
...
class AgentArrayStruct(Structure):
    ...
    def GetAgentByID(self, agent_id: int) -> Optional["AgentStruct"]:
        ...
        agent = cast(agent_ptr, POINTER(AgentStruct)).contents
...
class AgentArray:
    ...
    def _update_ptr():
        ...
        if not ptr:
            ...
            return
        AgentArray._cached_ctx = cast(ptr, POINTER(AgentArrayStruct)).contents
