...
class WorldContextStruct(Structure):
    ...
    _fields_ = [
        ...
        (
            "learnable_character_skills_array",
            GW_Array,
        ),  # Array<uint32_t> #// populated at skill trainer and when using signet of capture
        ("unlocked_character_skills_array", GW_Array),  # Array<uint32_t>
        (
            "duplicated_character_skills_array",
            GW_Array,
        ),  # Array<DupeSkill> // When res signet is bought more than once, its mapped into this array. Used in skill window.
    ...
    def _is_valid_attribute(attribute: AttributeStruct) -> bool:
        return (
            attribute.level_base > 0
            or attribute.level > 0
            or attribute.decrement_points > 0
            or attribute.increment_points > 0
        )
...
class WorldContext:
    ...
    def _update_ptr():
        ...
        if not ptr:
            ...
            return
        WorldContext._cached_ctx = cast(ptr, POINTER(WorldContextStruct)).contents
