...
def _validate_step_type(value: Any, *, recipe_name: str, step_idx: int) -> str:
    ...
    if step_type not in CANONICAL_STEP_TYPES:
        raise UnknownRecipeStepType(f"Recipe {recipe_name!r} step {step_idx + 1} has unknown type {step_type!r}.")
    return step_type
...
def _interact_and_dialog_tree(
    step: dict[str, Any], context: JsonBTCompilerContext, dialog_id: str | int
) -> BehaviorTree:
    log = _bool(step.get("log"), False)
    target = _choice(step, "target", _choice(step, "kind", "npc"))
...
def _interact_delay_tree(step: dict[str, Any]) -> BehaviorTree:
    ...
    return (
        BT.Player.Wait(delay_ms, log=False)
        if delay_ms > 0
        else BehaviorTree(BehaviorTree.SucceederNode(name="NoInteractDelay"))
    )
...
def _dialog_delay_tree(step: dict[str, Any]) -> BehaviorTree:
    ...
    return (
        BT.Player.Wait(delay_ms, log=False)
        if delay_ms > 0
        else BehaviorTree(BehaviorTree.SucceederNode(name="NoDialogDelay"))
    )
...
def _target_named_agent(kind: str, key: str, *, max_dist: float) -> BehaviorTree:
    ...
    def _target() -> BehaviorTree.NodeState:
        from Core import AgentArray, Player
...
def _target_nearest_gadget(max_dist: float, *, log: bool = False) -> BehaviorTree:
    def _target() -> BehaviorTree.NodeState:
        from Core import Agent, AgentArray, Player
...
def _optional_item_by_model_interact_tree(
    step: dict[str, Any], point: tuple[float, float] | None, *, log: bool
) -> BehaviorTree:
    model_id = _int(step.get("model_id", step.get("id")), 0)
    max_dist = _float(step.get("max_dist"), 4500.0)
...
def _use_consumables(step: dict[str, Any]) -> BehaviorTree.NodeState:
    ...
    from Core.routines_src.behaviourtrees_src.botting_consumables import consumable_specs
    from Core.routines_src.behaviourtrees_src.botting_consumables import normalize_consumable_mode
    from Core.routines_src.behaviourtrees_src.botting_consumables import send_consumable_to_accounts
    from Core.routines_src.behaviourtrees_src.botting_consumables import should_skip_local_consumable_for_non_leader
    from Core.routines_src.behaviourtrees_src.botting_consumables import use_local_consumable
