...
class BTRecipeRunner:
    ...
    def _install_planner_steps(self, *, reset: bool) -> None:
        planner_steps: list[tuple[str, Callable[[], object]]] = [
            (step.planner_step_name, self._make_step_builder(step)) for step in self._runtime_steps
        ]
    ...
    def _debug_progress(self) -> None:
        ...
        if view is None:
            return
        if self._last_debug_phase_index == view.phase_index + 1 and self._last_debug_step_index == view.step_index:
            return
