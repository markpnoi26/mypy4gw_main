...
from Core.enums_src.GameData_enums import Ailment
from Core.enums_src.GameData_enums import Attribute
from Core.enums_src.GameData_enums import DamageType
from Core.enums_src.GameData_enums import Reduced_Ailment
from Core.enums_src.Item_enums import BowType
from Core.enums_src.Item_enums import ItemType
from Core import mods_upgrades
from Core.mods_types import ItemBaneSpecies
from Core.mods_types import ItemUpgrade
from Core.mods_types import ItemUpgradeId
from Core.mods_types import ModifierIdentifier
...
def render_mod(item_id: int, mod: DecodedMod) -> str:
    ...
    if ident in (
        int(ModifierIdentifier.HalvesCastingTimeItemAttribute),
    ...
    if tmpl:
        return (
            tmpl.replace("{value}", str(vals[0]) if vals else "")
            .replace("{sub}", sub_s)
    ...
    for dm in decode_item(item_id):
        if dm.upgrade_id:  # a barcode carrier word
            ...
            if rendered:
                ...
                tag = "(UNHANDLED)"
        out.append(
            "0x%03X %-24s a1=%-3d a2=%-3d  %s" % (dm.identifier, effect_name(dm.identifier)[:24], dm.arg1, dm.arg2, tag)
        )
    return out
