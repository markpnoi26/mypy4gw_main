...
from Core.native_src.context.MapContext import PathingTrapezoid
...
def _build_trap_bsp(traps: list, sorted_indices: list, depth: int = 0):
    ...
    return _BspSplit(split_y, _build_trap_bsp(traps, above, depth + 1), _build_trap_bsp(traps, below, depth + 1))
...
class TrapezoidBSP:
    ...
    def __init__(self, trapezoids: List[PathingTrapezoid]):
        if not trapezoids:
            ...
            sorted_indices = sorted(range(len(trapezoids)), key=lambda i: (trapezoids[i].YT + trapezoids[i].YB) * 0.5)
...
class NavMesh:
    ...
    def get_adjacent_side(self, a: PathingTrapezoid, b: PathingTrapezoid) -> Optional[str]:
        if abs(a.YB - b.YT) < 1.0:
            return 'bottom_top'
        if abs(a.YT - b.YB) < 1.0:
            return 'top_bottom'
        if abs(a.XBR - b.XBL) < 1.0:
            return 'right_left'
        if abs(a.XBL - b.XBR) < 1.0:
            return 'left_right'
        return None
    ...
    def create_portal(self, box1: AABB, box2: AABB, side: Optional[str]) -> bool:
        ...
        def is_close(a, b):
            return abs(a - b) < _PORTAL_TOLERANCE
    ...
    def touching(
        self, a: AABB, b: AABB, vert_tol: float = _PORTAL_VERT_TOL, horiz_tol: float = _PORTAL_HORIZ_TOL
    ) -> bool:
    ...
    def _create_cross_layer_portals_from_snapshots(self, pathing_maps):
        ...
        for plane_idx, pmap in enumerate(pathing_maps):
            for portal in pmap.portals:
                ...
                layer_pair = (
                    min(portal.left_layer_id, portal.right_layer_id),
    ...
    def has_line_of_sight(
        self, p1: Tuple[float, float], p2: Tuple[float, float], margin: float = 100, step_dist: float = 200.0
    ) -> bool:
    ...
    def smooth_path_by_los(
        self, path: List[Tuple[float, float]], margin: float = 100, step_dist: float = 200.0
    ...
    def load_from_file(pathing_maps, map_id: int, folder: str) -> "NavMesh":
        ...
        PySystem.Console.Log(
            "NavMesh",
            f"Loaded NavMesh for map {map_id} with {len(nav.portal_graph)} connections and {len(nav.trapezoids)} trapezoids.",
...
class AStar:
    ...
    def search(self, start_pos: Tuple[float, float], goal_pos: Tuple[float, float]) -> bool:
        ...
        if start_id is None or goal_id is None:
            PySystem.Console.Log(
                "A-Star", f"Invalid start or goal trapezoid: {start_id}, {goal_id}", PySystem.Console.MessageType.Error
            )
            return False
        ...
        PySystem.Console.Log(
            "A-Star", f"Path not found from {start_id} to {goal_id}", PySystem.Console.MessageType.Warning
        )
        return False
...
PATHING_MAP_GROUPS = [
    ...
    [name_to_map_id["The Mouth of Torment"], name_to_map_id["The Ruptured Heart"]],
]
...
class AutoPathing:
    ...
    def get_path(
        ...
        path_planner.reset()
        path_planner.plan(
            start_x=start[0], start_y=start[1], start_z=start[2], goal_x=goal[0], goal_y=goal[1], goal_z=goal[2]
        )
