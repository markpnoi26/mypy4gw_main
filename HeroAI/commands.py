...
class Command:
    ...
    def __init__(
        ...
        name: str,
        icon: str,
        command_function: Callable[[list[AccountStruct]], None] | None,
        tooltip: str = "",
        description: str = "",
        map_types: list[str] = ["Explorable", "Outpost"],
    ) -> None:
        self.name = name
        self.icon = icon
...
class HeroAICommands:
    ...
    def __init__(self):
        ...
        self.PixelStack = Command(
            "Pixel Stack", IconsFontAwesome5.ICON_COMPRESS_ARROWS_ALT, self.__pixel_stack_command, "Pixel Stack Team"
        )
        self.InteractWithTarget = Command(
            "Interact With Target",
            IconsFontAwesome5.ICON_HAND_POINT_RIGHT,
            self.__interact_with_target_command,
            "Interact with current target",
        )
        self.UnlockChest = Command(
            "Unlock Chest", IconsFontAwesome5.ICON_KEY, self.__unlock_chest_command, "Unlock chest with current target"
        )
        self.TakeDialogWithTarget = Command(
            "Dialog With Target",
            IconsFontAwesome5.ICON_COMMENT_DOTS,
            self.__talk_and_dialog_with_target_command,
            "Take dialog with current target",
        )
        self.OpenConsumables = Command(
            "Open Consumables",
            IconsFontAwesome5.ICON_CANDY_CANE,
            self.__open_consumables_commands,
            "Open/Close Consumables Configuration Window",
        )
        self.FlagHeroes = Command(
            "Flag Heroes",
            IconsFontAwesome5.ICON_FLAG,
            self.__flag_heroes_command,
            "Flag all heroes",
            map_types=["Explorable"],
        )
        self.UnflagHeroes = Command(
            "Unflag Heroes",
            IconsFontAwesome5.ICON_CIRCLE_XMARK,
            self.__unflag_heroes_command,
            "Unflag all heroes",
            map_types=["Explorable"],
        )
        self.Resign = Command(
            "Resign",
            IconsFontAwesome5.ICON_SKULL,
            self.__resign_command,
            "Resign all accounts",
            map_types=["Explorable"],
        )
        self.DonateFaction = Command(
            "Donate Faction", IconsFontAwesome5.ICON_DONATE, self.__donate_faction_command, "Donate faction to guild"
        )
        self.PickUpLoot = Command(
            "Pick up loot", IconsFontAwesome5.ICON_COINS, self.__pick_up_loot_command, "Pick up loot from ground"
        )
        self.CombatPrep = Command(
            "Prepare for Combat",
            IconsFontAwesome5.ICON_SHIELD_ALT,
            self.__combat_prep_command,
            "Use Combat Preparations",
            map_types=["Explorable"],
        )
        self.DisbandParty = Command(
            "Disband Party",
            IconsFontAwesome5.ICON_SIGN_OUT_ALT,
            self.__leave_party_command,
            "Make all heroes leave party",
            map_types=["Outpost"],
        )
        self.FormParty = Command(
            "Form Party",
            IconsFontAwesome5.ICON_USERS,
            self.__invite_all_command,
            "Invite all heroes to party",
            map_types=["Outpost"],
        )
        self.TravelAltsToLeaderMap = Command(
            "Travel Alt Accounts to Leader's Map",
            IconsFontAwesome5.ICON_MAP_MARKED_ALT,
            self.__travel_alts_to_leader_map_command,
            "Send all alt accounts to the leader's map",
            map_types=["Outpost"],
        )
        self.LeavePartyAndTravelGH = Command(
            "Leave & Travel to GH",
            IconsFontAwesome5.ICON_HOME,
            self.__leave_party_and_travel_gh_command,
        ...
        for account in accounts:
            ...
            if same_map:
                ...
                GLOBAL_CACHE.ShMem.SendMessage(
                    sender_email, account.AccountEmail, SharedCommandType.SendDialog, (button_number, 0, 0, 0)
                )
    ...
    def __leave_party_command(self, accounts: list[AccountStruct]):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.LeaveParty, (0, 0, 0, 0)
            )
    ...
    def __leave_party_and_travel_gh_command(self, accounts: list[AccountStruct]):
        ...
        for account in accounts:
            ...
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.TravelToGuildHall, (0, 0, 0, 0)
            )
    ...
    def __combat_prep_command(self, accounts: list[AccountStruct]):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.UseSkillCombatPrep, (1, 0, 0, 0)
            )
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.UseSkillCombatPrep, (2, 0, 0, 0)
            )
    ...
    def __donate_faction_command(self, accounts: list[AccountStruct]):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.DonateToGuild, (0, 0, 0, 0)
            )
    ...
    def __invite_all_command(self, accounts: list[AccountStruct]):
        ...
        def SetWaitingActions(delay_ms: int):
            ...
            for _ in range(delays):
                GLOBAL_CACHE._ActionQueueManager.AddAction(
                    "ACTION", lambda: None
                )  # Adding a no-op to ensure spacing between invites
    ...
    def __pixel_stack_command(self, accounts: list[AccountStruct]):
        ...
        for account in accounts:
            ...
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.PixelStack, (player_x, player_y, 0, 0)
            )
    ...
    def __unlock_chest_command(self, accounts: list[AccountStruct]):
        ...
        GLOBAL_CACHE.ShMem.SendMessage(
            sender_email, lowest_party_index_account.AccountEmail, SharedCommandType.OpenChest, (target_id, 1, 0, 0)
        )
    ...
    def __interact_with_target_command(self, accounts: list[AccountStruct]):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.InteractWithTarget, (target_id, 0, 0, 0)
            )
    ...
    def __talk_and_dialog_with_target_command(self, accounts: list[AccountStruct]):
        ...
        for account in accounts:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.TakeDialogWithTarget, (target_id, 0, 0, 0)
            )
