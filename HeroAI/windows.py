...
from Core import UIManager, ModelID, GLOBAL_CACHE
from Core import Agent, Player
from Core import Routines, ActionQueueManager, Key, Keystroke, ThrottledTimer
...
class HeroAI_FloatingWindows:
    ...
    def draw_Targeting_floating_buttons(cached_data: CacheData):
        ...
        for agent_id in enemy_array:
            ...
            # 1. Draw the "O" layer
            if ImGui.floating_button(f"O", name=f"O_{agent_id}", x=o_x, y=o_y, width=o_w, height=o_h, font_size=30):
                clicked = True

            # 2. Draw the "+" layer with its own dimensions
            if ImGui.floating_button(
                f"+", name=f"plus_{agent_id}", x=plus_x, y=plus_y, width=plus_w, height=plus_h, font_size=45
            ):
                clicked = True
    ...
    def _handle_settings():
        if HeroAI_FloatingWindows.SETTINGS_THROTTLE.IsExpired():
            ...
            if not HeroAI_FloatingWindows.settings.ensure_initialized():
                ...
                info = HeroAI_FloatingWindows.settings.get_hero_panel_info(
                    HeroAI_FloatingWindows.command_panel_window.window_name
                )
        ...
        if not HeroAI_FloatingWindows.settings.ShowPanelOnlyOnLeaderAccount or own_data.AgentPartyData.IsPartyLeader:
            if HeroAI_FloatingWindows.settings.ShowHeroPanels:
                ...
                if HeroAI_FloatingWindows.settings.CombinePanels:
                    ...
                    open = HeroAI_FloatingWindows.hero_windows[combined_identifier].begin(
                        True, PyImGui.WindowFlags.AlwaysAutoResize
                    )
                ...
                for account in accounts:
                    ...
                    if (
                        account.AccountEmail == Player.GetAccountEmail()
                        and not HeroAI_FloatingWindows.settings.ShowLeaderPanel
                    ):
                        continue
    ...
    def show_ui(cached_data: CacheData):
        ...
        show_ui = (
            ...
            and not Map.Pregame.InCharacterSelectScreen()
        ...
        if show_ui:
            ...
            if HeroAI_FloatingWindows.settings.ShowCommandPanel and (
                own_data.AgentPartyData.IsPartyLeader
                or not HeroAI_FloatingWindows.settings.ShowCommandPanelOnlyOnLeaderAccount
            ):
...
class HeroAI_Windows:
                ...
                if PyImGui.begin_tab_item(GLOBAL_CACHE.Skill.GetName(skill.skill_id)):
                    ...
                    if PyImGui.tree_node(f"Combat debug"):
                        ...
                        PyImGui.text_colored(
                            f"IsReadyToCast: {is_ready_to_cast}", Utils.TrueFalseColor(is_ready_to_cast)
                        )
                        if PyImGui.tree_node(f"IsReadyToCast: {is_ready_to_cast}"):
                            ...
                            skillbar_recharge = cached_data.combat_handler.skills[
                                HeroAI_Windows.skill_slot
                            ].skillbar_data.recharge
                            ...
                            if ordered_skill:
                                ...
                                are_cast_conditions_met = cached_data.combat_handler.AreCastConditionsMet(
                                    HeroAI_Windows.skill_slot, v_target
                                )
                                ...
                                PyImGui.text_colored(
                                    f"CastingSkill: {casting_skill}", Utils.TrueFalseColor(not casting_skill != 0)
                                )
                                PyImGui.text_colored(
                                    f"SkillBar Casting: {skillbar_casting}",
                                    Utils.TrueFalseColor(not skillbar_casting != 0),
                                )
                                PyImGui.text_colored(
                                    f"SkillBar recharge: {skillbar_recharge}",
                                    Utils.TrueFalseColor(skillbar_recharge == 0),
                                )
                                PyImGui.text_colored(
                                    f"Energy: {current_energy} / Cost {energy_cost}",
                                    Utils.TrueFalseColor(current_energy >= energy_cost),
                                )
                                PyImGui.text_colored(
                                    f"Current HP: {current_hp} / Target HP: {target_hp} / Health Cost: {health_cost}",
                                    Utils.TrueFalseColor(health_cost == 0 or current_hp >= health_cost),
                                )
                                PyImGui.text_colored(
                                    f"Adrenaline Required: {adrenaline_required}",
                                    Utils.TrueFalseColor(
                                        adrenaline_required == 0 or (adrenaline_a >= adrenaline_required)
                                    ),
                                )
                                PyImGui.text_colored(
                                    f"Current Overcast: {current_overcast} / Overcast Target: {overcast_target} / Skill Overcast: {skill_overcast}",
                                    Utils.TrueFalseColor(current_overcast >= overcast_target or skill_overcast == 0),
                                )

                                PyImGui.text_colored(
                                    f"AreCastConditionsMet: {are_cast_conditions_met}",
                                    Utils.TrueFalseColor(are_cast_conditions_met),
                                )
                                PyImGui.text_colored(
                                    f"SpiritBuffExists: {spirit_buff_exists}",
                                    Utils.TrueFalseColor(not spirit_buff_exists),
                                )
    ...
    def DrawFlags(cached_data: CacheData):
        ...
        active_account_option_pairs: list[tuple[AccountStruct, HeroAIOptionStruct]] = (
            shmem.GetAllActiveAccountHeroAIPairs(sort_results=False)
        )
        ...
        if HeroAI_Windows.capture_hero_flag:
            ...
            if mouse_clicked:
                ...
                if 0 < capture_index <= hero_count:
                    ...
                    options: HeroAIOptionStruct | None = (
                        options_by_party[hero_ai_index] if 0 <= hero_ai_index < MAX_NUM_PLAYERS else None
                    )
                    if options:
                        if capture_index == 0:
                            options.AllFlag.x = x
    ...
    def DrawPlayersDebug(cached_data: CacheData):
        ...
        if PyImGui.button("Submit"):
            ...
            account.AgentData.Energy.Pips = Utils.calculate_energy_pips(
                account.AgentData.Energy.Max, account.AgentData.Energy.Regen
            )
    ...
    def DrawGameOptionsDebug(cached_data: CacheData):
        ...
        row += [cached_data.global_options.Skills[j] for j in range(NUMBER_OF_SKILLS)]
        ...
        for i in range(MAX_NUM_PLAYERS):
            ...
            row = [i, options.Following, options.Avoidance, options.Looting, options.Targeting, options.Combat]

            row += [options.Skills[j] for j in range(NUMBER_OF_SKILLS)]
    ...
    def DrawFollowDebug(cached_data: CacheData):
        ...
        Overlay().BeginDraw()
        if show_area_rings:
            player_x, player_y, player_z = Agent.GetXYZ(
                Player.GetAgentID()
            )  # cached_data.data.player_xyz # needs to be live
            ...
            Overlay().DrawPoly3D(
                ...
                player_z,
                Range.Touch.value / 2,
                Utils.RGBToColor(255, 255, 0, 128),
                numsegments=segments,
                thickness=2.0,
            )
            Overlay().DrawPoly3D(
                ...
                player_z,
                Range.Touch.value,
                Utils.RGBToColor(255, 200, 0, 128),
                numsegments=segments,
                thickness=2.0,
            )
            Overlay().DrawPoly3D(
                ...
                Utils.RGBToColor(255, 150, 0, 128),
                numsegments=segments,
                thickness=2.0,
            )
            Overlay().DrawPoly3D(
                ...
                player_z,
                Range.Nearby.value,
                Utils.RGBToColor(255, 100, 0, 128),
                numsegments=segments,
                thickness=2.0,
            )
            Overlay().DrawPoly3D(
            ...
            Overlay().DrawPoly3D(
                ...
                Utils.RGBToColor(255, 12, 0, 128),
                numsegments=segments,
                thickness=2.0,
            )

        if show_hero_follow_grid:
            leader_x, leader_y, leader_z = Agent.GetXYZ(
                GLOBAL_CACHE.Party.GetPartyLeaderID()
            )  # cached_data.data.party_leader_xyz #needs to be live

            for index, angle in enumerate(hero_formation):
                if index == 0:
                    continue
                angle_on_hero_grid = Agent.GetRotationAngle(GLOBAL_CACHE.Party.GetPartyLeaderID()) + Utils.DegToRad(
                    angle
                )
            ...
            for i in range(MAX_NUM_PLAYERS):
                ...
                if account and account.IsSlotActive:
                    ...
                    Overlay().DrawText3D(
                        ...
                        target_y,
                        z_coord - 130,
                        f"{DistanceFromWaypoint(target_x, target_y):.1f}",
                        color=Utils.RGBToColor(255, 255, 255, 128),
                        autoZ=False,
                        centered=True,
                        scale=2.0,
    ...
    def DrawOptions(cached_data: CacheData):
        cached_data.ui_state_data.show_classic_controls = PyImGui.checkbox(
            "Show Classic Controls", cached_data.ui_state_data.show_classic_controls
        )
        try:
            from HeroAI.ui_base import HeroAI_BaseUI
    ...
    def DrawMessagingOptions(cached_data: CacheData):
        ...
        if ImGui.colored_button(
            f"{IconsFontAwesome5.ICON_TIMES}##commands_resign",
        ...
        if PyImGui.button(f"{IconsFontAwesome5.ICON_COMMENT_DOTS}##commands_takedialog"):
            ...
            for account in accounts:
                ...
                ConsoleLog("Messaging", f"Ordering {account.AccountEmail} to interact with target: {target}")
                GLOBAL_CACHE.ShMem.SendMessage(
                    sender_email, account.AccountEmail, SharedCommandType.TakeDialogWithTarget, (target, 0, 0, 0)
                )
        ImGui.show_tooltip("Get Dialog")
        PyImGui.separator()
        if PyImGui.collapsing_header("PCons"):
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["Celerity"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["Celerity"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["Celerity"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton("##Esence_unique_name", HeroAI_Windows.ButtonColors["Celerity"].texture_path, 32, 32):
                _post_pcon_message(
                    (
                        ...
                        GLOBAL_CACHE.Skill.GetID("Essence_of_Celerity_item_effect"),
                        0,
                        0,
                    )
                )
            ...
            PyImGui.same_line(0, -1)
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["GrailOfMight"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["GrailOfMight"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["GrailOfMight"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##Grail_unique_name", HeroAI_Windows.ButtonColors["GrailOfMight"].texture_path, 32, 32
            ):
                _post_pcon_message(
            ...
            PyImGui.push_style_color(
                ...
                HeroAI_Windows.ButtonColors["ArmorOfSalvation"].button_color.to_tuple_normalized(),
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["ArmorOfSalvation"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["ArmorOfSalvation"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##Armor_unique_name", HeroAI_Windows.ButtonColors["ArmorOfSalvation"].texture_path, 32, 32
            ):
                _post_pcon_message(
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["CandyCane"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["CandyCane"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["CandyCane"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##CandyCane_unique_name", HeroAI_Windows.ButtonColors["CandyCane"].texture_path, 32, 32
            ):
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button,
                HeroAI_Windows.ButtonColors["BirthdayCupcake"].button_color.to_tuple_normalized(),
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["BirthdayCupcake"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["BirthdayCupcake"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##BirthdayCupcake_unique_name", HeroAI_Windows.ButtonColors["BirthdayCupcake"].texture_path, 32, 32
            ):
                _post_pcon_message(
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["GoldenEgg"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["GoldenEgg"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["GoldenEgg"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##GoldenEgg_unique_name", HeroAI_Windows.ButtonColors["GoldenEgg"].texture_path, 32, 32
            ):
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["CandyCorn"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["CandyCorn"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["CandyCorn"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##CandyCorn_unique_name", HeroAI_Windows.ButtonColors["CandyCorn"].texture_path, 32, 32
            ):
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["Alcohol"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["Alcohol"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive, HeroAI_Windows.ButtonColors["Alcohol"].active_color.to_tuple_normalized()
            )
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["CandyApple"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["CandyApple"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["CandyApple"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##CandyApple_unique_name", HeroAI_Windows.ButtonColors["CandyApple"].texture_path, 32, 32
            ):
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["PumpkinPie"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["PumpkinPie"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["PumpkinPie"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##PumpkinPie_unique_name", HeroAI_Windows.ButtonColors["PumpkinPie"].texture_path, 32, 32
            ):
                _post_pcon_message(
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["DrakeKabob"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["DrakeKabob"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["DrakeKabob"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##DrakeKabob_unique_name", HeroAI_Windows.ButtonColors["DrakeKabob"].texture_path, 32, 32
            ):
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["SkalefinSoup"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["SkalefinSoup"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["SkalefinSoup"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##SkalefinSoup_unique_name", HeroAI_Windows.ButtonColors["SkalefinSoup"].texture_path, 32, 32
            ):
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["PahnaiSalad"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["PahnaiSalad"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["PahnaiSalad"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##PahnaiSalad_unique_name", HeroAI_Windows.ButtonColors["PahnaiSalad"].texture_path, 32, 32
            ):
                _post_pcon_message(
            ...
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.Button, HeroAI_Windows.ButtonColors["WarSupplies"].button_color.to_tuple_normalized()
            )  # On color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonHovered,
                HeroAI_Windows.ButtonColors["WarSupplies"].hovered_color.to_tuple_normalized(),
            )  # Hover color
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.ButtonActive,
                HeroAI_Windows.ButtonColors["WarSupplies"].active_color.to_tuple_normalized(),
            )
            if ImGui.ImageButton(
                "##WarSupplies_unique_name", HeroAI_Windows.ButtonColors["WarSupplies"].texture_path, 32, 32
            ):
