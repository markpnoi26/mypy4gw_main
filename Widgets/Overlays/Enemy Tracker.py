...
from Core import GLOBAL_CACHE, Py4GW, Range, Map
...
MODULE_ALIASES = ['System/Enemy Tracker.py']
...
class EnemyTracker:
    ...
    def _load_data(self) -> None:
        try:
            ...
            PySystem.Console.Log(
                EnemyTrackerConfig.MODULE_NAME, f"Failed to load data: {exc}", PySystem.Console.MessageType.Warning
            )
    ...
    def _save_data_if_needed(self, force: bool = False) -> None:
        ...
        try:
            ...
            PySystem.Console.Log(
                EnemyTrackerConfig.MODULE_NAME, f"Failed to save data: {exc}", PySystem.Console.MessageType.Warning
            )
    ...
    def export_bundle_to(self, path: str) -> bool:
        try:
            ...
            PySystem.Console.Log(
                EnemyTrackerConfig.MODULE_NAME, f"Export failed: {exc}", PySystem.Console.MessageType.Warning
            )
            return False
    ...
    def import_bundle_from(self, path: str) -> bool:
        try:
            ...
            PySystem.Console.Log(
                EnemyTrackerConfig.MODULE_NAME, f"Import failed: {exc}", PySystem.Console.MessageType.Warning
            )
            return False
    ...
    def _record_names(self, key: str, language: str = EnemyTrackerConfig.DEFAULT_NAME_LANGUAGE) -> list[str]:
        ...
        if (
            EnemyTrackerConfig.DEFAULT_NAME_LANGUAGE in self.vars.name_records
            and key in self.vars.name_records[EnemyTrackerConfig.DEFAULT_NAME_LANGUAGE]
        ):
...
def _ensure_ini() -> bool:
    ...
    EnemyTrackerConfig.MAIN_INI_KEY = Settings(
        f"{EnemyTrackerConfig.INI_PATH}/{EnemyTrackerConfig.MAIN_INI_FILENAME}", "account"
    ).name
    EnemyTrackerConfig.FLOATING_INI_KEY = Settings(
        f"{EnemyTrackerConfig.INI_PATH}/{EnemyTrackerConfig.FLOATING_INI_FILENAME}", "account"
    ).name
...
def scanner_main():
    try:
        ...
        PySystem.Console.Log(
            EnemyTrackerConfig.MODULE_NAME, f"Scanner error: {exc}", PySystem.Console.MessageType.Error
        )
        raise
