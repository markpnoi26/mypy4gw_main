...
from Core import GLOBAL_CACHE, Player, Map, Agent, Effects, Inventory, Party
from Core import ActionQueueManager
from Core import CombatPrepSkillsType
from Core import Console
from Core import ConsoleLog
from Core.py4gwcorelib_src.loot_filters import LootFilters
import PyImGui
import PySystem
from Core import Range, TitleID
from Core import Routines
from Core import Utils, ImGui, Color, ColorPalette
from Core import SharedCommandType
from Core import UIManager
from Core import AutoPathing
from Core.GlobalCache.WhiteboardLocks import post_loot_lock, clear_loot_lock
...
def _process_pending_widget_enables() -> None:
    ...
    try:
        ...
        ConsoleLog(
            MODULE_NAME, f"Deferred EnableWidget('{widget_name}') failed: {exc}", Console.MessageType.Error, False
        )
...
def get_inventory_count(sender_email: str, model_id_min: int, model_id_max: int) -> int:
    return _inventory_cache().get((sender_email, int(model_id_min), int(model_id_max)), -1)


def reset_inventory_count(sender_email: str, model_id_min: int, model_id_max: int) -> None:
    _inventory_cache().pop((sender_email, int(model_id_min), int(model_id_max)), None)
...
def _send_message_to_recipients(
    ...
    if recipient == "All other accounts":
        account_emails = [str(account.AccountEmail or "").strip() for account in GLOBAL_CACHE.ShMem.GetAllAccountData()]
...
def _draw_message_queue(messages: list[tuple[int, SharedMessageStruct]]) -> None:
    ...
    for index, message in messages:
        ...
        account_matches = (
            ...
            or (selected_account == "Current account" and receiver == Player.GetAccountEmail())
...
def _draw_message_history() -> None:
    ...
    _message_history_filter_text = PyImGui.input_text("Search##messaging_history_search", _message_history_filter_text)
    PyImGui.same_line()
...
def RestoreHeroAISnapshot(account_email: str):
    ...
    if not account_snapshots:
        ...
        ConsoleLog(
            MODULE_NAME,
            "No Hero AI snapshot found, enabling all options as fallback.",
...
def MerchantItems(index: int, message: SharedMessageStruct):
    ...
    if mode == "report_salvage_kits":
        ...
        try:
            ...
            ConsoleLog(
                MODULE_NAME,
                f"MerchantItems: salvage kits in inventory = {salvage_kits_in_inv}.",
                Console.MessageType.Info,
                False,
            )
        finally:
    ...
    try:
        ...
        if not ok:
            ConsoleLog(
                MODULE_NAME,
                "MerchantItems: merchant NPC not found, skipping kit buy",
                Console.MessageType.Warning,
                False,
            )
            return
...
def MerchantMaterials(index: int, message: SharedMessageStruct):
    ...
    try:
        ...
        if mode == "sell":
            ...
            ConsoleLog(
                MODULE_NAME, f"MerchantMaterials deposit metrics: {deposit_metrics}", Console.MessageType.Info, False
            )
            ...
            ConsoleLog(
                MODULE_NAME, f"MerchantMaterials buy_ectoplasm metrics: {ecto_metrics}", Console.MessageType.Info, False
            )
            ...
            if leftover_ids:
                ...
                if not ok:
                    ConsoleLog(
                        MODULE_NAME,
                        "MerchantMaterials sell_merchant_leftovers: merchant NPC not found, skipping sell",
                        Console.MessageType.Warning,
                        False,
                    )
                else:
                    ...
                    ConsoleLog(
                        MODULE_NAME,
                        f"MerchantMaterials sell_merchant_leftovers: sold {len(leftover_ids)} stacks",
                        Console.MessageType.Info,
                        False,
                    )
            else:
                ConsoleLog(
                    MODULE_NAME,
                    "MerchantMaterials sell_merchant_leftovers: no leftover stacks, skipping",
                    Console.MessageType.Info,
                    False,
                )
            ...
            if rare_model_ids:
                ...
                ConsoleLog(
                    MODULE_NAME,
                    f"MerchantMaterials sell_rare_mats: sold {sold_total} unit(s)",
                    Console.MessageType.Info,
                    False,
                )
            ...
            if scroll_model_ids:
                ...
                if sell_ids:
                    ...
                    if not ok:
                        ConsoleLog(
                            MODULE_NAME,
                            "MerchantMaterials sell_scrolls: merchant NPC not found, skipping sell",
                            Console.MessageType.Warning,
                            False,
                        )
                    else:
                        ...
                        ConsoleLog(
                            MODULE_NAME,
                            f"MerchantMaterials sell_scrolls: sold {len(sell_ids)} scroll(s)",
                            Console.MessageType.Info,
                            False,
                        )
                else:
                    ConsoleLog(
                        MODULE_NAME,
                        "MerchantMaterials sell_scrolls: no scrolls in inventory, skipping",
                        Console.MessageType.Info,
                        False,
                    )
            ...
            if sell_ids:
                ...
                if not ok:
                    ConsoleLog(
                        MODULE_NAME,
                        "MerchantMaterials sell_nonsalvageable_golds: merchant NPC not found, skipping sell",
                        Console.MessageType.Warning,
                        False,
                    )
                else:
                    ...
                    ConsoleLog(
                        MODULE_NAME,
                        f"MerchantMaterials sell_nonsalvageable_golds: sold {len(sell_ids)} item(s)",
                        Console.MessageType.Info,
                        False,
                    )
            else:
                ConsoleLog(
                    MODULE_NAME,
                    "MerchantMaterials sell_nonsalvageable_golds: no items in inventory, skipping",
                    Console.MessageType.Info,
                    False,
                )
        else:
...
def MerchantRules(index: int, message: SharedMessageStruct):
    ...
    if widget is None:
        ConsoleLog(
            MODULE_NAME,
            "Merchant Rules widget is not available for shared message handling.",
            Console.MessageType.Warning,
            False,
        )
        GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
...
def _shared_effect_ids_for_receiver() -> set[int]:
    ...
    return {int(getattr(buff, "SkillId", 0) or 0) for buff in buffs if int(getattr(buff, "SkillId", 0) or 0) > 0}
...
def UsePcon(index: int, message: SharedMessageStruct):
    ...
    if morale_model_ids & _PARTY_WIDE_MORALE_MODELS:
        ...
        if (
            morale_target is not None
            and shared_party_min_morale is not None
            and shared_party_min_morale >= morale_target
        ):
            ConsoleLog(
                MODULE_NAME,
                (
    ...
    try:
        ...
        if cascade:
            ...
            if account_data is not None:
                ConsoleLog(
                    MODULE_NAME,
                    f"Current account party position: {account_data.AgentPartyData.PartyPosition}",
                    Console.MessageType.Info,
                )
                ...
                queued_accounts = [
                    ...
                    if on_same_map_and_party(account)
                    and account.AgentPartyData.PartyPosition > account_data.AgentPartyData.PartyPosition
                ]
                ...
                sorted_by_party_index = (
                    sorted(
                        [
                            acc
                            for acc in queued_accounts
                            if Utils.Distance((acc.AgentData.Pos.x, acc.AgentData.Pos.y), chest_pos) < 2500.0
                        ],
                ...
                if sorted_by_party_index:
                    ...
                    ConsoleLog(
                        MODULE_NAME,
                        f"Cascading OpenChest to next party member: {next_account.AgentData.CharacterName} ({next_account.AccountEmail})",
                        Console.MessageType.Info,
                    )
...
def SetWindowGeometry(index: int, message: SharedMessageStruct):
    ...
    if sender_data is None:
        GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
        return
    PySystem.Console.set_window_geometry(
...
def UseItem(index: int, message: SharedMessageStruct):
    ...
    if count < 1:
        ConsoleLog(
            MODULE_NAME, f"UseItem: blocked (model_id {model_id} not in inventory).", Console.MessageType.Warning
        )
        GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
        return
    if effect_id > 0 and _local_has_effect(effect_id):
        ConsoleLog(
            MODULE_NAME, f"UseItem: effect {effect_id} already active, skipping.", Console.MessageType.Info, False
        )
        GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
        return
    ...
    for _ in range(repeat):
        if effect_id > 0 and _local_has_effect(effect_id):
            ConsoleLog(
                MODULE_NAME, f"UseItem: effect {effect_id} active mid-loop, stopping.", Console.MessageType.Info, False
            )
            break
    ...
    ConsoleLog(
        MODULE_NAME,
        f"UseItem: executed (requested={repeat}, used={used}, model_id={model_id}).",
        Console.MessageType.Info,
        False,
    )
    GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
...
def EnableWidget(index: int, message: SharedMessageStruct):
    ...
    GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
    ConsoleLog(
        MODULE_NAME, f"EnableWidget('{widget_name}') message processed and finished.", Console.MessageType.Info, False
    )
...
def DisableWidget(index: int, message: SharedMessageStruct):
    ...
    GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
    ConsoleLog(
        MODULE_NAME, f"DisableWidget('{widget_name}') message processed and finished.", Console.MessageType.Info, False
    )
...
def RestockResurrectionScroll(index: int, message: SharedMessageStruct):
    ...
    GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
    ConsoleLog(
        MODULE_NAME, "RestockResurrectionScroll message processed and finished.", Console.MessageType.Info, False
    )
    ...
    try:
        if mode == "report_inventory_count":
            ...
            if range_start > 0 and range_end >= range_start:
                try:
                    count = sum(
                        int(GLOBAL_CACHE.Inventory.GetModelCount(mid)) for mid in range(range_start, range_end + 1)
                    )
                except Exception as exc:
                    ConsoleLog(
                        MODULE_NAME,
...
def EquipItem(index: int, message: SharedMessageStruct):
    ...
    ConsoleLog(
        MODULE_NAME, f"EquipItem: equipped item_id {item_id} (model {model_id}).", Console.MessageType.Info, False
    )
    GLOBAL_CACHE.ShMem.MarkMessageAsFinished(message.ReceiverEmail, index)
