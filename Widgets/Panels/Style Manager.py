...
from Core import IconsFontAwesome5, ImGui, Color
from Core.ImGui_src.WindowModule import WindowModule
from Core.ImGui_src.Style import Style
from Core.ImGui_src.types import Alignment, ControlAppearance, StyleColorType, StyleTheme
...
MODULE_ALIASES = ['System/Style Manager.py']
...
window_height = ini_handler.get_int(MODULE_NAME + str(" Config"), "height", 500)
...
selected_theme = StyleTheme[py4_gw_ini_handler.get_str("settings", "style_theme", StyleTheme.Py4GW.name)]

force_theme_override = py4_gw_ini_handler.get_bool("settings", "force_theme_override", False)
...
themes = [
    theme.name.replace("_", " ") + (f" (Textured)" if theme in ImGui.Textured_Themes else "") for theme in StyleTheme
]
...
textures = [
    ("Textures/Item Models/17081-Battle_Commendation.png", ControlAppearance.Default, True),
    ("Textures/Item Models/00514-Molten_Heart.png", ControlAppearance.Primary, True),
    ("Textures/Item Models/00035-Bag.png", ControlAppearance.Danger, True),
    ("Textures/Item Models/30855-Bottle_of_Grog.png", ControlAppearance.Default, False),
]
...
def draw_icon_button(theme: StyleTheme):
    ...
    PyImGui.same_line(0, 5)
    ImGui.icon_button(IconsFontAwesome5.ICON_SYNC, disabled=True)


def draw_icon_toggle_button(theme: StyleTheme):
    preview.icon_toggle_button_1 = ImGui.toggle_icon_button(
        (IconsFontAwesome5.ICON_SYNC) + " With Text" + "##toggle_icon_button" + theme.name, preview.icon_toggle_button_1
    )
    PyImGui.same_line(0, 5)
    preview.icon_toggle_button_2 = ImGui.toggle_icon_button(
        (IconsFontAwesome5.ICON_TOGGLE_ON if preview.icon_toggle_button_2 else IconsFontAwesome5.ICON_TOGGLE_OFF)
        + "##toggle_icon_button"
        + theme.name,
        preview.icon_toggle_button_2,
    )
    PyImGui.same_line(0, 5)
    preview.icon_toggle_button_3 = ImGui.toggle_icon_button(
        (IconsFontAwesome5.ICON_EYE if preview.icon_toggle_button_3 else IconsFontAwesome5.ICON_EYE_SLASH)
        + "##toggle_icon_button"
        + theme.name,
        preview.icon_toggle_button_3,
    )
...
def draw_image_toggle(theme: StyleTheme):
    preview.image_toggle_button_1 = ImGui.image_toggle_button(
        ("On" if preview.image_toggle_button_1 else "Off") + "##ImageToggle_1" + theme.name,
        texture_path=textures[0][0],
        v=preview.image_toggle_button_1,
    )
    PyImGui.same_line(0, 5)
    preview.image_toggle_button_2 = ImGui.image_toggle_button(
        ("On" if preview.image_toggle_button_2 else "Off") + "##ImageToggle_2" + theme.name,
        texture_path=textures[1][0],
        v=preview.image_toggle_button_2,
    )
    PyImGui.same_line(0, 5)
    preview.image_toggle_button_3 = ImGui.image_toggle_button(
        ("On" if preview.image_toggle_button_3 else "Off") + "##ImageToggle_3" + theme.name,
        texture_path=textures[2][0],
        v=preview.image_toggle_button_3,
        disabled=True,
    )
...
def draw_image_button(theme: StyleTheme):
    for texture, appearance, enabled in textures:
        ImGui.image_button(
            "Image Button" + "##" + theme.name + texture, texture, appearance=appearance, disabled=not enabled
        )
        PyImGui.same_line(0, 5)


def draw_combo(theme: StyleTheme):
    preview.combo = ImGui.combo("Combo##" + theme.name, preview.combo, ["Option 1", "Option 2", "Option 3"])


def draw_checkbox(theme: StyleTheme):
    preview.checkbox_2 = ImGui.checkbox("##Checkbox 2" + "##" + theme.name, preview.checkbox_2)
    PyImGui.same_line(0, 5)
    preview.checkbox = ImGui.checkbox("Checkbox" + "##" + theme.name, preview.checkbox)


def draw_radio_button(theme: StyleTheme):
    preview.radio_button = ImGui.radio_button("Option 1##Radio Button 1" + "##" + theme.name, preview.radio_button, 0)
    preview.radio_button = ImGui.radio_button("Option 2##Radio Button 2" + "##" + theme.name, preview.radio_button, 1)
    preview.radio_button = ImGui.radio_button("Option 3##Radio Button 3" + "##" + theme.name, preview.radio_button, 2)


def draw_slider(theme: StyleTheme):
    preview.slider_int = ImGui.slider_int("Slider Int##" + theme.name, preview.slider_int, 0, 100)
    preview.slider_float = ImGui.slider_float("Slider Float##" + theme.name, preview.slider_float, 0.0, 100.0)


def draw_input(theme: StyleTheme):
    changed, preview.search_value = ImGui.search_field("Search##" + theme.name, preview.search_value)
    preview.input_text_value = ImGui.input_text("Text##" + theme.name, preview.input_text_value)
    preview.input_float_value = ImGui.input_float("Float##" + theme.name, preview.input_float_value)
    preview.input_int_value = ImGui.input_int("Int##3" + theme.name, preview.input_int_value, 0, 10000, 0)
    preview.input_int_value = ImGui.input_int("Int Buttons##2" + theme.name, preview.input_int_value)
    ...
    if theme_compare_window.begin():
        ...
        if window_size[1] > 100:
            ...
            if ImGui.begin_table("Control Preview#Header", 4, PyImGui.TableFlags.BordersOuterH, 0, 30):
                ...
                PyImGui.table_setup_column("Guild Wars", PyImGui.TableColumnFlags.WidthStretch)
                PyImGui.table_setup_column("Minimalus", PyImGui.TableColumnFlags.WidthStretch)
            ...
            if ImGui.begin_table(
                "Theme Compare Control Preview",
                len(comparing_themes) + 1,
                PyImGui.TableFlags.ScrollX | PyImGui.TableFlags.ScrollY,
            ):
                PyImGui.table_setup_column("Control", PyImGui.TableColumnFlags.WidthFixed, 150)
                PyImGui.table_setup_column("ImGui", PyImGui.TableColumnFlags.WidthStretch)
                PyImGui.table_setup_column("Guild Wars", PyImGui.TableColumnFlags.WidthStretch)
...
def on_enable():
    ...
    PySystem.Console.Log(
        MODULE_NAME, f"Enabled Style Manager with theme: {selected_theme.name}", PySystem.Console.MessageType.Info
    )
...
def DrawWindow():
    ...
    if window_module.begin():
        ...
        if PyImGui.begin_child(
            "Theme Buttons", (0, 110), True, PyImGui.WindowFlags.NoScrollbar | PyImGui.WindowFlags.NoScrollWithMouse
        ):
            if PyImGui.begin_child(
                ...
                if value != ImGui.Selected_Style.Theme.value:
                    ...
                    set_theme(theme)
                    py4_gw_ini_handler.set("settings", "style_theme", ImGui.Selected_Style.Theme.name)

                PyImGui.same_line(0, 5)
                PyImGui.set_cursor_pos_y(PyImGui.get_cursor_pos_y() + (2 if style.Theme is StyleTheme.Minimalus else 0))
                theme_compare = ImGui.checkbox("##show_theme_compare", theme_compare)
                ImGui.show_tooltip("Show Theme Compare window")
            ...
            ImGui.show_tooltip(
                "Leave empty to save over the selected theme. Enter a name to save a new preset (Styles/<name>)."
            )
        ...
        def table_separator_header(
            title: str,
            font_size: int = 20,
            font_family: str = "Regular",
            ...
            if ImGui.begin_tab_item("Style Customization"):
                if PyImGui.is_rect_visible((0, 10)):
                    ...
                    if PyImGui.begin_table("Style Variables", 3, PyImGui.TableFlags.ScrollY):
                        PyImGui.table_setup_column("Variable", PyImGui.TableColumnFlags.WidthFixed, 250)
                        ...
                        for enum, var in ImGui.Selected_Style.StyleVars.items():
                            ...
                            changed = (
                                org_style.StyleVars[enum].value1 != var.value1
                                or org_style.StyleVars[enum].value2 != var.value2
                            )
                        ...
                        colors = {
                            **ImGui.Selected_Style.Colors,
                            **ImGui.Selected_Style.CustomColors,
                            **ImGui.Selected_Style.TextureColors,
                        }
                            ...
                            if org_color:
                                ...
                                if show_button:
                                    if ImGui.icon_button(IconsFontAwesome5.ICON_UNDO + "##" + col_name, 25, 25):
                                        col.set_rgba(*org_color.rgb_tuple)
