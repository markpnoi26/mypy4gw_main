...
Thin host: all behaviour lives in ``Core.py4gwcorelib_src.system_settings``. As a System
...
# (under Core), so Python caches them in sys.modules — a widget reload re-runs THIS file but
...
for _name in [
    ...
    if m.startswith("Core.py4gwcorelib_src.system_settings")
    or m.startswith("Core.py4gwcorelib_src.name_obfuscation")
    or m.startswith("Core.py4gwcorelib_src.agent_recolor")
    or m.startswith("Core.py4gwcorelib_src.window_renamer")
    or m.startswith("Core.py4gwcorelib_src.map_utilities")
]:
    del sys.modules[_name]

from Core.py4gwcorelib_src.system_settings import get_controller

OPTIONAL = False

MODULE_ALIASES = ['System/System Settings.py']
...
def draw() -> None:
    ...
    try:
        if not _applied:
            ...
            try:
                from Core.py4gwcorelib_src.skillbar_plus import get_controller as _sbp_get

                _sbp_get().register()
            except Exception as skillbar_error:
                PySystem.Console.Log(
                    MODULE_NAME, "Skillbar+ boot failed: %s" % skillbar_error, PySystem.Console.MessageType.Error
                )
            try:
                from Core.py4gwcorelib_src.camera_smoothing import get_controller as _camera_get

                _camera_get().register()
            except Exception as camera_error:
                PySystem.Console.Log(
                    MODULE_NAME, "Camera smoothing boot failed: %s" % camera_error, PySystem.Console.MessageType.Error
                )
            try:
                from Core.py4gwcorelib_src.window_renamer import get_controller as _renamer_get

                _renamer_get().register()
            except Exception as renamer_error:
                PySystem.Console.Log(
                    MODULE_NAME, "Window Renamer boot failed: %s" % renamer_error, PySystem.Console.MessageType.Error
                )
            try:
                from Core.py4gwcorelib_src.map_utilities import get_controller as _map_utils_get

                _map_utils_get().register()
            except Exception as map_utils_error:
                PySystem.Console.Log(
                    MODULE_NAME, "Map Utilities boot failed: %s" % map_utils_error, PySystem.Console.MessageType.Error
                )
            # Also register the persisted name-obfuscation alias set (global/multi-account) at boot.
            try:
                from Core.py4gwcorelib_src.name_obfuscation import get_controller as _no_get
            ...
            try:
                from Core.py4gwcorelib_src.agent_recolor import get_controller as _ar_get
            ...
            try:
                ...
                from Core.py4gwcorelib_src.loot_filters import quick_access

                LootFilters().register()
                quick_access.register()
            except Exception as loot_error:
                PySystem.Console.Log(
                    MODULE_NAME, "Loot Filters boot failed: %s" % loot_error, PySystem.Console.MessageType.Error
                )
            try:
                from Core.py4gwcorelib_src.recolor_beacons import RecolorBeacons

                RecolorBeacons().register()
            except Exception as mark_error:
                PySystem.Console.Log(
                    MODULE_NAME, "Recolor & Beacons boot failed: %s" % mark_error, PySystem.Console.MessageType.Error
                )
