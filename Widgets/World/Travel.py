    ...
    if config_module.begin():
        ...
        if PyImGui.begin_tab_bar("##TravelConfigTabs"):
                        ...
                        for i, id in enumerate(widget_config.favorites):
                            ...
                            if outpost:
                                ...
                                PySystem.Console.Log(
                                    MODULE_NAME,
                                    f"Favorite outpost {id} not found in outposts.",
            ...
            if PyImGui.begin_tab_item("General"):
                ...
                ImGui.text_aligned(
                    f"Travel History Length ({widget_config.history_length})",
                    width=100,
                    height=24,
    ...
    match (ImGui.get_style().Theme):
        ...
        case Style.StyleTheme.Minimalus:
            PyImGui.draw_list_add_rect_filled(
                ...
                (
                    Utils.RGBToColor(48, 48, 48, 150)
                    if ImGui.is_mouse_in_rect(button_rect)
                    else Utils.RGBToColor(0, 0, 0, 150)
                ),
                0,
                0,
            )
            ...
            PyImGui.draw_list_add_rect(
                ...
                button_rect[1] + button_rect[3] - 1,
                (
                    Utils.RGBToColor(255, 255, 255, 75)
                    if ImGui.is_mouse_in_rect(button_rect)
                    else Utils.RGBToColor(200, 200, 200, 50)
                ),
                0,
                0,
                1,
        ...
        case Style.StyleTheme.Py4GW:
            PyImGui.draw_list_add_rect_filled(
                ...
                button_rect[1] + button_rect[3] - 1,
                (
                    Utils.RGBToColor(51, 76, 102, 255)
                    if ImGui.is_mouse_in_rect(button_rect)
                    else Utils.RGBToColor(26, 38, 51, 255)
                ),
                4,
                0,
            )
...
def DrawWindow():
    ...
    try:
        ...
        button_rect = (
            widget_config.button_position[0],
            widget_config.button_position[1],
            widget_config.button_size,
            widget_config.button_size,
        )
        ...
        win_open = PyImGui.begin(
            "##TravelButton",
            PyImGui.WindowFlags.NoTitleBar | PyImGui.WindowFlags.NoResize | PyImGui.WindowFlags.NoScrollbar,
        )
        ...
        if win_open:
            ...
            icon_rect = (
                button_rect[0] + (button_rect[2] - button_size) / 2,
                button_rect[1] + (button_rect[3] - button_size) / 2,
                button_size,
                button_size,
            )
        ...
        if window_y + window_module.window_size[1] > screen_height:
            window_y = (
                screen_height
                - window_module.window_size[1]
                - (ImGui.get_style().Theme is Style.StyleTheme.Guild_Wars and 10 or 0)
            )
        ...
        if window_module.begin():
            ...
            changed, search = ImGui.search_field(
                "##Search Outpost", search_outpost, "Search ...", PyImGui.InputTextFlags.AutoSelectAll
            )
            if changed and search != search_outpost:
                ...
                filtered_outposts = [
                    (id, outpost)
                    for id, outpost in outposts.items()
                    if not search
                    or search in outpost.lower()
                    or search in generate_initials(outpost).lower()
                    or any(search in alias for alias in outpost_aliases.get(id, []))
                ]
                ...
                filtered_outposts.sort(
                    key=lambda item: (
                        0 if item[0] in priority_outposts else 1,
                        priority_outposts.get(item[0], ""),
                        item[1].lower(),
                    )
                )
                ...
                filtered_history = (
                    [
                        (id, outpost)
                        for id, outpost in travel_history
                        if not search or search in outpost.lower() or search in generate_initials(outpost).lower()
                    ]
                    if widget_config.show_travel_history
                    else []
                )
            ...
            items_height = max(
                ...
                min(
                    300,
                    (
                        (len(filtered_outposts) * 20 if search else 0)
                        + (
                            len(filtered_history) * 20 + (20 if search and filtered_outposts else 0)
                            if filtered_history
                            else 0
                        )
                    ),
                ...
                if PyImGui.begin_child("##OutpostList", (0, items_height), False, PyImGui.WindowFlags.NoFlag):
                    ...
                    for i, (id, outpost) in enumerate(filtered_history):
                        ...
                        if PyImGui.is_item_clicked(0) or (is_selected and PyImGui.is_key_pressed(Key.Enter.value)):
                            ConsoleLog(
                                MODULE_NAME,
                                f"Traveling to outpost {outpost} ({id}) from history.",
                    ...
                    if filtered_outposts and search:
                        for i in range(travel_history_len, len(filtered_outposts) + travel_history_len):
                            ...
                            if PyImGui.is_item_clicked(0) or (is_selected and PyImGui.is_key_pressed(Key.Enter.value)):
                                ConsoleLog(
                                    MODULE_NAME,
                                    f"Traveling to outpost {outpost} ({id}) from search results.",
                                    PySystem.Console.MessageType.Info,
                                )
                                click_select_outpost(io, id, i)
                                traveled = True
                    ...
                    if click_timer.IsExpired():
                        max_index = (
                            (len(filtered_outposts) if search and filtered_outposts else 0) + travel_history_len - 1
                        )
...
def click_select_outpost(io: PyImGui.ImGuiIO, id, i):
    ...
    if io.key_shift:
        ...
        filtered_history = [
            (id, outpost)
            for id, outpost in travel_history
            if not search_outpost
            or search_outpost.lower() in outpost.lower()
            or search_outpost.lower() in generate_initials(outpost).lower()
        ]
...
def TravelToOutpost(outpost_id):
    ...
    if not is_traveling:
        if outpost_id != Map.GetMapID():
            ConsoleLog(
                MODULE_NAME,
                f"Traveling to outpost: {outposts[outpost_id]} ({outpost_id})",
