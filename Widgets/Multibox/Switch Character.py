...
from Core import ProfessionShort, Profession, Campaign
...
from Core import ProfessionTextureMap
from Core import GWContext
import traceback
from Core.FrameTree import Frame, FrameId

MODULE_ALIASES = ['Guild Wars/Switch Character.py']
...
class RerollCharacter:
    ...
    def _get_target_index(self):
        ...
        if found_index != -99 and pregame:
            ...
            ConsoleLog(
                "Reroll",
                f"Found '{target_name}' at index {found_index}. Current selection: {self.last_known_index}",
    ...
    def _navigate_char_select(self):
        ...
        if distance != 0:
            ...
            ConsoleLog(
                "Reroll",
                f"Navigating {'Right' if distance > 0 else 'Left'} (Current: {current_index}, Target: {self.target_index})",
    ...
    def _update(self):
        ...
        if self.state == self.STATE_IDLE or self.state == self.STATE_TIMED_OUT or self.state == self.STATE_ERROR:
            return
        ...
        if PyImGui.begin_child("characterList2", (300, 270), True, PyImGui.WindowFlags.NoFlag):
            if PyImGui.begin_table("CharTable", 4, PyImGui.TableFlags.RowBg | PyImGui.TableFlags.BordersInnerV):
                ...
                for index, character in enumerate(characters):
                    ...
                    primary_texture = (
                        f"Textures\\Profession_Icons\\{ProfessionTextureMap.get(character.primary, 'unknown')}"
                    )
                    secondary_texture = (
                        f"Textures\\Profession_Icons\\{ProfessionTextureMap.get(character.secondary, 'unknown')}"
                    )
...
def is_in_character_select():
    ...
    in_char_select = (
        not in_load_screen and any(f.exists for f in (cs_base, cs_c0, cs_c1)) and not GLOBAL_CACHE.Party.IsPartyLoaded()
    )
