...
def _reload_core():
    """Widget reload does NOT reload Core submodules (they stay in sys.modules), so
    ...
    import importlib
    from Core import mods_core, mods_upgrades
...
def _write():
    ...
    try:
        PySystem.Console.Log(
            MODULE_NAME, "parity -> %s (%d items)" % (OUT, len(_items)), PySystem.Console.MessageType.Success
        )
    except Exception:
        pass
