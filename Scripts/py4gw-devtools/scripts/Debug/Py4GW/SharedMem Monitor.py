...
from Core.GlobalCache.shared_memory_src.TitlesStruct import TitleUnitStruct
from Core.GlobalCache.shared_memory_src.FactionStruct import FactionStruct
...
def tooltip():
    ...
    PyImGui.bullet_text(
        "Account Analytics: Real-time display of Faction points, Luxon/Kurzick standing, and Title progress"
    )
    PyImGui.bullet_text(
        "Character Discovery: View available characters and account-wide metadata stored in shared memory"
    )
    PyImGui.bullet_text("Live Vitals: Instant tracking of Agent health, energy, experience, and level data")
    PyImGui.bullet_text(
        "Data Visualization: Includes specialized tabs for Factions, Player attributes, and Experience metrics"
    )
...
def begin_striped_table(label: str, columns: int = 2, width: float = 0.0):
    ...
    PyImGui.push_style_color(
        PyImGui.ImGuiCol.TableRowBgAlt, Color(64, 64, 64, 128).to_tuple_normalized()
    )  # slightly darker
...
def draw_account_info(player: AccountStruct):
    ...
    if begin_striped_table("AccountInfoTable", 2):
        ...
        if PyImGui.tree_node(f"Heroes ({num_heroes})"):
            ...
            for hero in heroes:
                PyImGui.text(
                    f"{hero.AgentData.CharacterName} (HeroID: {hero.AgentData.HeroID}) Slot: {hero.SlotNumber}"
                )
            PyImGui.tree_pop()
        ...
        if PyImGui.tree_node(f"Pets ({num_pets})"):
            ...
            for pet in pets:
                PyImGui.text(
                    f"{pet.AgentData.CharacterName} (PlayerID: {pet.AgentData.AgentID}) Slot: {pet.SlotNumber}"
                )
            PyImGui.tree_pop()
...
class TitleData:
    ...
    def _draw_title(self, title: TitleStruct, managed: bool):
        ...
        PyImGui.push_style_color(
            PyImGui.ImGuiCol.PlotHistogram, ColorPalette.GetColor("midnight_violet").to_tuple_normalized()
        )
    ...
    def draw_content(self):
        ...
        if PyImGui.collapsing_header("Missions Completed", PyImGui.TreeNodeFlags.NoFlag):
            ...
            self.show_details["missions_not_completed"] = PyImGui.checkbox(
                "Show Not Completed Missions", self.show_details.get("missions_not_completed", False)
            )

            expanded = self._expand_bit_array(self.missions_completed, self._cache_expanded["missions_completed"])

            PyImGui.text(
                f"entries:{len(self.missions_completed)} -{len(self.missions_completed)*32} total missions tracked."
            )
            ...
            for map_id, completed in expanded:
                ...
                if show or completed:
                    ...
                    PyImGui.text_colored(
                        f"MapID {map_id} - {self._get_map_name(map_id)} - {status}", Utils.TrueFalseColor(completed)
                    )
        ...
        if PyImGui.collapsing_header("Missions Bonus Completed", PyImGui.TreeNodeFlags.NoFlag):
            ...
            expanded = self._expand_bit_array(self.missions_bonus, self._cache_expanded["missions_bonus"])
        ...
        if PyImGui.collapsing_header("Missions Completed HM", PyImGui.TreeNodeFlags.NoFlag):
            ...
            expanded = self._expand_bit_array(self.missions_completed_hm, self._cache_expanded["missions_completed_hm"])

            PyImGui.text(
                f"entries:{len(self.missions_completed_hm)} -{len(self.missions_completed_hm)*32} total missions tracked."
            )
        ...
        if PyImGui.collapsing_header("Missions Bonus Completed HM", PyImGui.TreeNodeFlags.NoFlag):
            ...
            self.show_details["missions_bonus_hm_not_completed"] = PyImGui.checkbox(
                "Show Not Completed Bonus (HM)", self.show_details.get("missions_bonus_hm_not_completed", False)
            )

            expanded = self._expand_bit_array(self.missions_bonus_hm, self._cache_expanded["missions_bonus_hm"])

            PyImGui.text(
                f"entries:{len(self.missions_bonus_hm)} -{len(self.missions_bonus_hm)*32} total missions tracked."
            )
        ...
        if PyImGui.collapsing_header("Unlocked Character Skills", PyImGui.TreeNodeFlags.NoFlag):
            ...
            self.show_details["show_locked_skills"] = PyImGui.checkbox(
                "Show Locked Skills", self.show_details.get("show_locked_skills", False)
            )

            expanded = self._expand_bit_array(self.unlocked_character_skills, self._cache_expanded["unlocked_skills"])

            PyImGui.text(
                f"entries:{len(self.unlocked_character_skills)} -{len(self.unlocked_character_skills)*32} total skills tracked."
            )
            ...
            for skill_id, unlocked in expanded:
                if show or unlocked:
                    PyImGui.text_colored(
                        f"Skill ID {skill_id} - {self._get_skill_name(skill_id)}", Utils.TrueFalseColor(unlocked)
                    )
...
class ExperienceData:
    ...
    def draw_content(self):
        ...
        if PyImGui.begin_table("ExperienceOuter", 1, PyImGui.TableFlags.SizingStretchProp):
            ...
            if PyImGui.begin_table("ExperienceHeader", 3, PyImGui.TableFlags.SizingStretchProp):
            ...
            avail_width = PyImGui.get_content_region_avail()[0]
            PyImGui.push_style_color(
                PyImGui.ImGuiCol.PlotHistogram, ColorPalette.GetColor("dark_green").to_tuple_normalized()
            )
    ...
    if PyImGui.begin(f"{MODULE_NAME}"):
        if PyImGui.collapsing_header("Shared Memory Info"):
            ...
            ImGui.show_tooltip(
                "\n".join(
                    [
                        f"{i}. | Slot:{acc.SlotNumber} {acc.AccountEmail} | {acc.AgentData.CharacterName}"
                        for i, acc in enumerate(SMM.GetAllAccounts().AccountData)
                        if SMM.GetAllAccounts()._is_slot_active(i)
                    ]
                )
            )
        ...
        if PyImGui.begin_child("AccountsInfoChild", (new_width - 20, 0), True, PyImGui.WindowFlags.NoFlag):
                        ...
                        if PyImGui.begin_tab_bar("##AccountDetails"):
                            ...
                            if PyImGui.begin_tab_item("Available Characters"):
                                if PyImGui.begin_child(
                                    "AvailableCharsChild", (0, 0), False, PyImGui.WindowFlags.NoFlag
                                ):
                            ...
                            if PyImGui.begin_tab_item("Agent"):
                                if PyImGui.begin_child(
                                    "ExperienceDataChild", (0, 0), False, PyImGui.WindowFlags.NoFlag
                                ):
