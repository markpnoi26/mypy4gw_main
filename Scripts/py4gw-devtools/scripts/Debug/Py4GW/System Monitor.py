...
import PyImGui
import PyProfiler
from Core import UIManager, ThrottledTimer, Color, ColorPalette, ImGui
from Core import ProfilingRegistry, SimpleProfiler, WidgetHandler
...
def _draw_sparkline(id_str: str, values: list[float], height: float, line_col: tuple) -> None:
    ...
    if implot.begin_plot(id_str, -1.0, height, implot.Flags_CanvasOnly | implot.Flags_NoInputs):
        ...
        implot.plot_shaded("##fill", values, vmin - pad, 1.0, 0.0, implot.make_spec(fill_col=line_col, fill_alpha=0.18))
        implot.plot_line("##line", values, 1.0, 0.0, implot.make_spec(line_col=line_col, line_weight=1.6))
        implot.end_plot()
...
def _print_flame_summary() -> None:
    ...
    for func_key, (_cc, nc, tt, ct, _callers) in sorted_funcs[:25]:
        print(
            f"  - {_flame_names.get(func_key, str(func_key))}: "
...
def _draw_flame() -> None:
    ...
    if implot.begin_plot("##flame", 600.0, height, implot.Flags_CanvasOnly | implot.Flags_NoLegend):
        ...
        for i, cell in enumerate(_flame_cells):
            ...
            spec = implot.make_spec(
                fill_col=_FLAME_PALETTE[cidx],
                fill_alpha=1.0 if is_hl else 0.95,
                flags=implot.ItemFlags_NoLegend | implot.ItemFlags_NoFit,
            )
            implot.plot_shaded_between(
                f"##fc{i}", [float(x0), float(x1)], [float(d), float(d)], [float(d) + 1.0, float(d) + 1.0], spec
            )
            if is_hl:  # white outline on every cell of the hovered function
                border = implot.make_spec(
                    line_col=_FLAME_HL,
                    line_weight=2.0,
                ...
                implot.plot_line_xy(
                    f"##hl{i}",
                    [float(x0), float(x1), float(x1), float(x0)],
                    [float(d), float(d), float(d) + 1.0, float(d) + 1.0],
                    border,
                )
            width = x1 - x0
            if width > 0.05:
    ...
    if visible:
        ...
        if _prof_active is not None and _prof_target == selected_widget:
            ...
            PyImGui.progress_bar(
                done / _prof_total if _prof_total else 0.0, -1.0, 0.0, f"capturing {done}/{_prof_total}"
            )
    ...
    if PyImGui.begin_table("top_widgets_v2", 3, table_flags):
        ...
        for i, (name, ms) in enumerate(system_info.widgets[:15]):  # top 15
            ...
            if PyImGui.selectable(
                f"{name}##row{i}", selected_widget == name, PyImGui.SelectableFlags.SpanAllColumns, (0.0, 0.0)
            ):
                picked = name
