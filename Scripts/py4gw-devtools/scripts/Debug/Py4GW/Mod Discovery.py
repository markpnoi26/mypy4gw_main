        ...
        for m in mods:
            try:
                if m.GetIdentifier() == UPGRADE_IDENTIFIER:
                    uid = m.GetArg()
                    found.append(
                        {"upgrade_id": uid, "item_id": iid, "model_id": model, "type": itype, "known": uid in known}
                    )
...
def _write():
    ...
    lines.append("=" * 78)
    lines.append(
        "items scanned with upgrades: %d   distinct upgrade_ids on items: %d"
        % (len({f["item_id"] for f in _found}), len(unk_ids))
    )
    lines.append("known (in the 390): %d   UNKNOWN (missing from the 390): %d" % (len(known_ids), len(unk_only)))
    lines.append("")
    ...
    for f in sorted(unknown, key=lambda x: x["upgrade_id"]):
        ...
        lines.append("  upgrade_id 0x%X  (type %d, model %d)  on item: %r" % (uid, f["type"], f["model_id"], nm))
    lines.append("")
    ...
    try:
        PySystem.Console.Log(
            MODULE_NAME, "scanned; %d unknown mods -> %s" % (len(unk_only), OUT), PySystem.Console.MessageType.Success
        )
    except Exception:
        pass
