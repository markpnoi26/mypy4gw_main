...
def _write_named():
    lines = [
        "GAME MOD TABLE — resolved names (from the client string table)",
    ...
    for u in _mod_unlocks:
        ...
        lines.append(
            "%-4d 0x%-7X %-34s %-40s [%s]"
            % (u.get("i", 0), u.get("upgrade_id", 0), _clean(name)[:34], _clean(desc)[:40], codes)
        )
...
def _write_all():
    ...
    try:
        PySystem.Console.Log(
            MODULE_NAME, "wrote resolved tables (%d+%d pending)" % (p1, p2), PySystem.Console.MessageType.Success
        )
    except Exception:
        pass
