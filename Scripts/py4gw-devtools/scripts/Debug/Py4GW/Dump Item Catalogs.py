...
from Core.native_src.internals.encoded_strings import GWEncoded
...
def _write_all():
    ...
    for name in ("colors", "attributes", "descriptions"):
        rows = [[i, ent(name)[i][0], _clean(_resolve(int(ent(name)[i][0], 16)))] for i in sorted(ent(name), key=int)]
    ...
    for i in sorted(ent("pvp_unlocks"), key=int):
        ...
        mrows.append([idx, "0x%X" % m.get("upgrade_id", 0), _clean(_decode_enc(n)), _clean(_decode_enc(d)), codes])
    _wcsv("pvp_unlocks.csv", ["index", "upgrade_id", "name", "description", "codes"], mrows)

    _last = CATDIR
    try:
        PySystem.Console.Log(MODULE_NAME, "wrote complete catalogs to " + CATDIR, PySystem.Console.MessageType.Success)
    except Exception:
        pass
