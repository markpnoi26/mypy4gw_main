...
def _write():
    global _last_path
    lines = [
        "GAME MOD TABLE — names composed by the game (native get_pvp_unlock_name_enc)",
        "=" * 96,
    ...
    for r in _rows:
        ...
        lines.append(
            "%-4d 0x%-7X %-36s %-44s [%s]" % (idx, m.get("upgrade_id", 0), _clean(name)[:36], _clean(desc)[:44], codes)
        )
    ...
    try:
        PySystem.Console.Log(
            MODULE_NAME, "wrote %s (%d pending)" % (OUT_PATH, pending), PySystem.Console.MessageType.Success
        )
    except Exception:
        pass
