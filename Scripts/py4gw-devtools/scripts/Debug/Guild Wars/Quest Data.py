...
from Core import ImGui, ColorPalette, Color, TITLE_TIERS, TITLE_NAME, GLOBAL_CACHE, TITLE_CATEGORIES, Utils
from Core import Routines, Map
import PyImGui
import Py4GW
...
def _request_names_coroutine_parallel(quest_ids: List[int]):
    ...
    remaining = set(q for q in quest_ids if not name_requested_map.get(q, False) or force_refresh)
...
def _request_quest_data_coroutine(quest_id: int):
    ...
    while True:
        ...
        if name_ready and obj_ready and loc_ready and npc_ready and desc_ready:
            print(
                f"Completed FULL fetch for quest ID {quest_id} {GLOBAL_CACHE.Quest.GetQuestName(quest_id)} in {time.time() - start:.2f} seconds."
            )
            break
...
def draw_window():
    ...
    if PyImGui.begin(MODULE_NAME):
        ...
        if active_quest not in full_quest_requested_list or force_refresh:
            ...
            if force_refresh:
                GLOBAL_CACHE.Coroutines.append(_request_names_coroutine(GLOBAL_CACHE.Quest.GetQuestLogIds()))
        ...
        if PyImGui.begin_child("QuestTreeChild", (new_width - 20, 250), True, PyImGui.WindowFlags.NoFlag):
            ...
            for location, quest_ids in ordered_groups:
                ...
                sorted_qids = sorted(
                    quest_ids,
                    key=lambda qid: (
                        GLOBAL_CACHE.Quest.GetQuestName(qid) if GLOBAL_CACHE.Quest.IsQuestNameReady(qid) else str(qid)
                    ).lower(),
                )
                ...
                if opened:
                    for qid in sorted_qids:
                        ...
                        if qid == active_quest:
                            ...
                            rect_max = (child_pos[0] + child_size[0] - padding_x, text_pos[1] + max_height + v_margin)
        ...
        if PyImGui.begin_child("AccountInfoChild", (new_width - 20, 0), False, PyImGui.WindowFlags.NoFlag):
            ...
            PyImGui.text_colored(
                f"Is Current Mission Quest: {is_current}", Utils.TrueFalseColor(quest.is_current_mission_quest)
            )
...
def tooltip():
    ...
    PyImGui.bullet_text(
        "Rich Text Rendering: Custom markup system that highlights @Quest names and @Warnings in color."
    )
