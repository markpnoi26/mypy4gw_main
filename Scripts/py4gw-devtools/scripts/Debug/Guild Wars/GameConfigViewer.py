...
class Preferences:
    ...
    _PREF_MAP = {
        ...
        "DoubleTapForwardToRunBackwardsToFlip": (
            PrefType.BOOL,
            FlagPreference.DoubleTapForwardToRunBackwardsToFlip,
            None,
            False,
        ),
        ...
        "DoNotShowSkillTipsOnEffectMonitor": (
            PrefType.BOOL,
            FlagPreference.DoNotShowSkillTipsOnEffectMonitor,
            None,
            False,
        ),
        ...
        "UseHighResolutionTexturesInOutposts": (
            PrefType.BOOL,
            FlagPreference.UseHighResolutionTexturesInOutposts,
            None,
            False,
        ),
    ...
    if PyImGui.begin(MODULE_NAME, PyImGui.WindowFlags.AlwaysAutoResize):
        if PyImGui.begin_tab_bar("##tabs"):
            if PyImGui.begin_tab_item("General"):
                ...
                PyImGui.text(f"Lock Compass Rotation:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('LockCompassRotation')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('LockCompassRotation')),
                )
                PyImGui.text(f"Disable Mouse Walking:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('DisableMouseWalking')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('DisableMouseWalking')),
                )
                ...
                PyImGui.text(f"Invert Mouse Control of Camera:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('InvertMouseControlOfCamera')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('InvertMouseControlOfCamera')),
                )
                PyImGui.text(f"Double Tap Forward to Run Backwards to Flip:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('DoubleTapForwardToRunBackwardsToFlip')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('DoubleTapForwardToRunBackwardsToFlip')),
                )
                PyImGui.text(f"Double Click to Interact:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('DoubleClickToInteract')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('DoubleClickToInteract')),
                )
                PyImGui.text(f"Always Show Nearby Names in PvP:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('AlwaysShowNearbyNamesPvP')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('AlwaysShowNearbyNamesPvP')),
                )
                PyImGui.text(f"Do Not Close Windows on Escape:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('DoNotCloseWindowsOnEscape')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('DoNotCloseWindowsOnEscape')),
                )
                PyImGui.text(f"Do Not Show Skill Tips on Skill Bars:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('DoNotShowSkillTipsOnSkillBars')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('DoNotShowSkillTipsOnSkillBars')),
                )
                PyImGui.text(f"Do Not Show Skill Tips on Effect Monitor:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('DoNotShowSkillTipsOnEffectMonitor')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('DoNotShowSkillTipsOnEffectMonitor')),
                )
                PyImGui.text(f"Show Text in Skill Floaters:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('ShowTextInSkillFloaters')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('ShowTextInSkillFloaters')),
                )
                PyImGui.text(f"Auto Target Foes:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('AutoTargetFoes')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('AutoTargetFoes')),
                )
                PyImGui.text(f"Auto Target NPCs:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('AutoTargetNPCs')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('AutoTargetNPCs')),
                )
                PyImGui.text(f"Fade Distant Name Tags:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('FadeDistantNameTags')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('FadeDistantNameTags')),
                )
                PyImGui.text(f"Concise Skill Descriptions:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('ConciseSkillDescriptions')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('ConciseSkillDescriptions')),
                )
                PyImGui.text(f"Whispers From Friends Etc Only:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('WhispersFromFriendsEtcOnly')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('WhispersFromFriendsEtcOnly')),
                )
                PyImGui.text(f"Show Chat Timestamps:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('ShowChatTimestamps')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('ShowChatTimestamps')),
                )
                PyImGui.text(f"Show Collapsed Bags:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('ShowCollapsedBags')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('ShowCollapsedBags')),
                )
                PyImGui.text(f"Item Rarity Border:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('ItemRarityBorder')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('ItemRarityBorder')),
                )
                PyImGui.text(f"Always Show Ally Names:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('AlwaysShowAllyNames')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('AlwaysShowAllyNames')),
                )
                PyImGui.text(f"Always Show Foe Names:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('AlwaysShowFoeNames')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('AlwaysShowFoeNames')),
                )

                PyImGui.end_tab_item()
            if PyImGui.begin_tab_item("Graphics"):
                PyImGui.text("Resolution:")
                PyImGui.same_line(0, -1)
                PyImGui.text(
                    f"{GwPreferences.GetWithEnum('WindowSizeX')[1]} x {GwPreferences.GetWithEnum('WindowSizeY')[1]}"
                )
                PyImGui.same_line(0, -1)
                ...
                PyImGui.text(
                    f"Fullscreen Gamma: {GwPreferences.GetWithEnum('FullscreenGamma')[1]} (weird number but ok)"
                )
                PyImGui.text(f"Wait For V-Sync:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('WaitForVSync')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('WaitForVSync')),
                )
                ...
                PyImGui.text(f"Enhanced Draw Distance:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('EnhancedDrawDistance')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('EnhancedDrawDistance')),
                )
                PyImGui.text(f"Use High Resolution Textures in Outposts:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('UseHighResolutionTexturesInOutposts')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('UseHighResolutionTexturesInOutposts')),
                )
            ...
            if PyImGui.begin_tab_item("Sound"):
                ...
                PyImGui.text(f"Mute When Guild Wars Is In Background:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('MuteWhenGuildWarsIsInBackground')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('MuteWhenGuildWarsIsInBackground')),
                )
                ...
                PyImGui.text(f"Optimize For Stereo:")
                PyImGui.same_line(0, -1)
                PyImGui.text_colored(
                    f"{GwPreferences.GetWithEnum('OptimizeForStereo')[1]}",
                    Utils.TrueFalseColor(GwPreferences.Get('OptimizeForStereo')),
                )
