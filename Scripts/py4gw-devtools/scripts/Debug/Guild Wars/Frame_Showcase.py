...
# ========================================================================
import Py4GW
from Core import UIManager, Color, Utils
...
class ShowcaseConfig:
    ...
    not_created_color: Tuple[float, float, float, float] = field(
        default_factory=lambda: Utils.RGBToNormal(150, 150, 150, 255)
    )
    not_visible_color: Tuple[float, float, float, float] = field(
        default_factory=lambda: Utils.RGBToNormal(180, 0, 0, 255)
    )
    no_hash_color: Tuple[float, float, float, float] = field(
        default_factory=lambda: Utils.RGBToNormal(150, 0, 150, 255)
    )
    identified_color: Tuple[float, float, float, float] = field(
        default_factory=lambda: Utils.RGBToNormal(200, 180, 0, 255)
    )
    ...
    def draw(self):
        ...
        if self.root:
            if self._active_filter:
                if not self.root._matches_search() and not any(c._matches_search() for c in self.root.children):
...
class UIManagerAPITester:
    ...
    def render(self):
        ...
        if self._section_header("Navigation (GetFrameByID, GetFrameIDByLabel/Hash, etc.)"):
            ...
            self._run_button(
                "GetChildFrameByFrameId",
                (lambda fid, c: Frame.from_id(fid).child_native(c).frame_id),
                self._input_frame_id,
                self._input_offset,
            )
            ...
            self._run_button(
                "GetChildFramePathByFrameId",
                (lambda fid, offs: Frame.from_id(fid).child_path_native(offs).frame_id),
                self._input_frame_id,
                offs,
            )
            PyImGui.text("GetChildFrameIdFromNameHash(parent_id, name_hash) -> int")
            self._run_button(
                "GetChildFrameIdFromNameHash",
        ...
        if self._section_header("Tree Walkers (First/Last/Next/Prev Child, Parent)"):
            ...
            self._run_button(
                "GetFirstChildFrameID", (lambda fid: Frame.from_id(fid).first_child().frame_id), self._input_frame_id
            )
            PyImGui.text("GetLastChildFrameID(parent_id) -> int")
            self._run_button(
                "GetLastChildFrameID", (lambda fid: Frame.from_id(fid).last_child().frame_id), self._input_frame_id
            )
            PyImGui.text("GetNextChildFrameID(frame_id) -> int")
            self._run_button(
                "GetNextChildFrameID", (lambda fid: Frame.from_id(fid).next_sibling().frame_id), self._input_frame_id
            )
            PyImGui.text("GetPrevChildFrameID(frame_id) -> int")
            self._run_button(
                "GetPrevChildFrameID", (lambda fid: Frame.from_id(fid).prev_sibling().frame_id), self._input_frame_id
            )
            PyImGui.text("GetParentFrameID(frame_id) -> int")
            self._run_button(
                "GetParentFrameID", (lambda fid: Frame.from_id(fid).parent_id_native()), self._input_frame_id
            )
            PyImGui.text("GetParentFrameIdDirect(frame_id) -> int")
            self._run_button(
                "GetParentFrameIdDirect",
                (lambda fid: Frame.from_id(fid).parent_direct().frame_id),
                self._input_frame_id,
            )
            ...
            self._run_button(
                ...
                self._input_frame_id,
                self._input_relation_kind,
                self._input_start_after,
            )
        ...
        if self._section_header("Properties (Layer, Code, Opacity, State, UserParam, Title)"):
            ...
            self._run_button(
                "GetStateBit",
                (lambda fid, b: Frame.from_id(fid).state_bit(b)),
                self._input_frame_id,
                self._input_state_bit,
            )
        ...
        if self._section_header("Geometry (MinSize, NativeSize, ClientBorder, ClipRect, PositionEx)"):
            ...
            self._run_button(
                "GetFrameClientBorder", (lambda fid: Frame.from_id(fid).client_border()), self._input_frame_id
            )
        ...
        if self._section_header("Visibility / State (SetVisible, SetDisabled, ShowFrame, Exists)"):
            ...
            self._run_button(
                "SetVisible",
                (lambda fid, v: Frame.from_id(fid).set_visible(v)),
                self._input_frame_id,
                self._input_visible,
            )
            ...
            self._run_button(
                ...
                self._input_frame_id,
                self._input_disabled,
            )
            PyImGui.text("ShowFrame(frame_id, show) -> bool")
            self._run_button(
                "ShowFrame", (lambda fid, sh: Frame.from_id(fid).show(sh)), self._input_frame_id, self._input_show
            )
        ...
        if self._section_header("Messages (SendUIMessage, SendFrameUIMessage, etc.)"):
            ...
            self._run_button(
                ...
                self._input_msgid,
                [self._input_wparam, self._input_lparam],
                False,
            )
            ...
            self._run_button(
                "SendFrameUIMessage",
                (lambda fid, m, w, l=0: Frame.from_id(fid).send_message(m, w, l)),
                self._input_frame_id,
                self._input_msgid,
                self._input_wparam,
                self._input_lparam,
            ...
            self._run_button(
                "SendFrameUIMessageWString",
                (lambda fid, m, t: Frame.from_id(fid).send_message_text(m, t)),
                self._input_frame_id,
                self._input_msgid,
                self._input_str_val or "test",
            )
        ...
        if self._section_header("IO Events (Register, Unregister, GetIOEvents, IsMouseOver)"):
            ...
            self._run_button(
                "RegisterFrameIOEventCallback", UIManager.RegisterFrameIOEventCallback, self._input_frame_id
            )
            PyImGui.text("UnregisterFrameIOEventCallback(frame_id)")
            self._run_button(
                "UnregisterFrameIOEventCallback", UIManager.UnregisterFrameIOEventCallback, self._input_frame_id
            )
        ...
        if self._section_header("Preferences (Get/Set Enum, Int, String, Bool)"):
            ...
            self._run_button(
                "SetStringPreference", UIManager.SetStringPreference, self._input_pref, self._input_str_val
            )
        ...
        if self._section_header("Windows (GetPosition, IsVisible, SetVisible, SetPosition)"):
            ...
            self._run_button(
                "SetWindowPosition",
                UIManager.SetWindowPosition,
                self._input_window_id,
                [self._input_pos_left, self._input_pos_top, self._input_pos_right, self._input_pos_bottom],
            )
        ...
        if self._section_header("Interaction (FrameClick, TestMouseAction, TestMouseClickAction)"):
            ...
            self._run_button(
                "TestMouseAction",
                (lambda fid, s, w, l=0: Frame.from_id(fid).mouse_action(s, w, l)),
                self._input_frame_id,
                0,
                self._input_wparam,
                self._input_lparam,
            ...
            self._run_button(
                "TestMouseClickAction",
                (lambda fid, s, w, l=0: Frame.from_id(fid).mouse_click_action(s, w, l)),
                self._input_frame_id,
                0,
                self._input_wparam,
                self._input_lparam,
        ...
        if self._section_header("Coordinates (GetFrameCoords, GetContentFrameCoords, GetFrameCoordsByHash)"):
            ...
            self._run_button(
                "GetContentFrameCoords", (lambda fid: Frame.from_id(fid).content_coords()), self._input_frame_id
            )
    ...
    def render(self):
        ...
        if self._show_io_events:
            ...
            if PyImGui.begin_child("LogIOEvents", size=(0, 200), border=True):
                from Core.UIManager import UIManager as UM
...
class VisualToolkit:
    ...
    def render(self):
        ...
        if PyImGui.collapsing_header("Drawing Tools", PyImGui.TreeNodeFlags.DefaultOpen):
            ...
            _config.draw_thickness = PyImGui.slider_float(
                "Outline Thickness##vtk_thick", _config.draw_thickness, 0.5, 10.0
            )
...
class FrameInspector:
    ...
    def render(self, ini_key: str, title: str):
        ...
        if PyImGui.begin_child(
            f"finsp_child_{self}", size=(0, 0), border=True, flags=PyImGui.WindowFlags.HorizontalScrollbar
        ):
    ...
    def _render_position(self):
        ...
        fields = [
            ...
            ("Bottom", p.bottom),
            ("Right", p.right),
            ("Content Top", p.content_top),
            ("Content Left", p.content_left),
            ("Content Bottom", p.content_bottom),
            ("Content Right", p.content_right),
            ("Unknown", p.unknown),
            ("Scale Factor", p.scale_factor),
            ("Viewport Width", p.viewport_width),
            ("Viewport Height", p.viewport_height),
            ("Screen Top", p.screen_top),
            ("Screen Left", p.screen_left),
            ("Screen Bottom", p.screen_bottom),
            ("Screen Right", p.screen_right),
            ("Top on Screen", p.top_on_screen),
            ("Left on Screen", p.left_on_screen),
            ("Bottom on Screen", p.bottom_on_screen),
            ("Right on Screen", p.right_on_screen),
            ("Width on Screen", p.width_on_screen),
            ("Height on Screen", p.height_on_screen),
            ("Viewport Scale X", p.viewport_scale_x),
            ("Viewport Scale Y", p.viewport_scale_y),
        ]
        headers = ["Field", "Value"]
    ...
    def _render_raw_fields(self):
        ...
        data = [self._raw_field_row(name, value) for name, value in frame.fields().items()]
...
class FrameShowcase:
    ...
    def render(self):
        ...
        expanded, open_ = ImGui.begin_with_close(
            "Frame Showcase — UIManager Explorer", None, PyImGui.WindowFlags.NoFlag
        )
