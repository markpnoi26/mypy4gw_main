...
def draw_multi_table(table_id: str, headers: list[str], rows: list[list[str | int | float]]):
    """
        ...
        if PyImGui.begin_tab_bar("MainTabBar"):
            ...
            if PyImGui.begin_tab_item("UI Message Listener"):
                if PyImGui.begin_child("UIMessageChild", (1000, 800), True, 0):
                    ...
                    for i, (tick, message_id, incoming, is_frame_message, frame_id, w_bytes, lbytes) in enumerate(
                        frame_logs
                    ):
                        ts = tick_to_timestamp(tick)
                        message_str = (
                            UIMessage(message_id).name
                            if message_id in UIMessage._value2member_map_
                            else hex(message_id)
                        )
                        rows.append(
                            [
                    ...
                    if PyImGui.button("print logs to console"):
                        for i, (tick, message_id, incoming, is_frame_message, frame_id, w_bytes, lbytes) in enumerate(
                            ...
                            print(
                                f"[{i}] {ts} | Incoming: {incoming} | IsFrameMsg: {is_frame_message} | FrameID: {frame_id} | MsgID: {message_str} | WParam: {w_bytes} | LParam: {lbytes}"
                            )
