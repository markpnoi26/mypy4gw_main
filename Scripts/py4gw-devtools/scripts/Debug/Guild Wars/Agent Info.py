from Core import PyImGui, Agent
from Core import ImGui
from Core import Routines
from Core import Allegiance
from Core import Color
from typing import Tuple
from Core import AgentArray, Agent, Player
from Core.native_src.context.AgentContext import AgentStruct, AgentLivingStruct, AgentItemStruct, AgentGadgetStruct
...
window_module = ImGui.WindowModule(
    MODULE_NAME, window_name="Agent Info Viewer", window_size=(0, 0), window_flags=PyImGui.WindowFlags.AlwaysAutoResize
)
...
def DrawMainWindow():
        ...
        if Agent.IsLiving(_AGENT_ID):
            if PyImGui.collapsing_header("Living Agent Data"):
                ...
                if PyImGui.begin_table(f"livingfields##livingfields{_AGENT_ID}", 3, flags):
                    ...
                    PyImGui.table_next_column()
                    PyImGui.text(
                        f"Allegiance: {Agent.GetAllegiance(_AGENT_ID)[0]} ({Agent.GetAllegiance(_AGENT_ID)[1]})"
                    )
                    PyImGui.table_next_row()
                    PyImGui.table_next_column()
                    PyImGui.text(
                        f"Weapon Type: {Agent.GetWeaponType(_AGENT_ID)[0]} ({Agent.GetWeaponType(_AGENT_ID)[1]})"
                    )
                    PyImGui.table_next_column()
                    ...
                    PyImGui.text_colored(
                        "Can Be Viewed In Party Window", _colored_bool(Agent.CanBeViewedInPartyWindow(_AGENT_ID))
                    )
                    PyImGui.table_next_column()
    ...
    if PyImGui.begin(window_module.window_name, window_module.window_flags):
        if PyImGui.begin_child(
            "NearestAgents Info", size=(600, 330), border=True, flags=PyImGui.WindowFlags.HorizontalScrollbar
        ):
        ...
        if PyImGui.begin_child(
            "InfoGlobalArea", size=(600, 500), border=True, flags=PyImGui.WindowFlags.HorizontalScrollbar
        ):
