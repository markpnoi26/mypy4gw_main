...
----------------------------------------------------------------
```python
from Core import CombatEvents, Player, Skillbar
...
-------------------
```python
from Core import CombatEvents, Agent
...
----------------------------------------------------
```python
from Core import CombatEvents, Skill
...
from Core import *
from Core.CombatEvents import CombatEvents as CombatEvents, EventType
...
def on_damage(target_id: int, source_id: int, damage_fraction: float, skill_id: int):
    ...
    if state.damage_tracker_running:
        ...
        if state.damage_track_agent_id is None or target_id == state.damage_track_agent_id:
            ...
            state.damage_received_from_source[source_id] = (
                state.damage_received_from_source.get(source_id, 0) + actual_dmg
            )
...
def draw_damage_tracker_tab():
    ...
    if not state.callbacks_registered:
        PyImGui.text_colored(
            "Note: Register callbacks in Event Log tab for damage tracking to work!", (255, 200, 100, 255)
        )
...
def draw_skill_recharges_tab():
    ...
    if PyImGui.begin_child("SkillRechargesChild", (0, 400), True, 0):
                    ...
                    for skill_id in sorted(observed):
                        ...
                        if Agent.IsSkillOnCooldown(agent_id, skill_id):
                            ...
                            if is_estimated:
                                ...
                                PyImGui.text_colored(
                                    f"    {skill_name}: ~{remaining/1000:.1f}s remaining (estimated)",
                                    (255, 200, 100, 255),
                                )
                            else:
                                # Purple for actual server data
                                PyImGui.text_colored(
                                    f"    {skill_name}: {remaining/1000:.1f}s remaining", (180, 130, 255, 255)
                                )
                        else:
        ...
        if PyImGui.collapsing_header("Debug: Raw Recharge Data"):
            try:
                from Core.CombatEventQueue_src.helpers import _recharges, _tracked_agents
                import ctypes
    ...
    if PyImGui.begin_tab_bar("HistoryTabs"):
        ...
        if PyImGui.begin_tab_item("Skill Events"):
            if PyImGui.begin_child(
                "SkillHistoryChild", size=(0, 300), border=True, flags=PyImGui.WindowFlags.HorizontalScrollbar
            ):
                try:
        ...
        if PyImGui.begin_tab_item("Damage Events"):
            if PyImGui.begin_child(
                "DamageHistoryChild", size=(0, 300), border=True, flags=PyImGui.WindowFlags.HorizontalScrollbar
            ):
                try:
...
def draw_debug_tab():
    ...
    if PyImGui.begin_child("DebugChild", (0, 550), True, 0):
        ...
        if PyImGui.collapsing_header("System Capabilities", PyImGui.TreeNodeFlags.DefaultOpen):
            try:
                from Core.CombatEventQueue_src.helpers import _disabled, _recharges, _stances, _tracked_agents
            ...
            try:
                ...
                if not healing_events:
                    ...
                    for _, _, target_id, skill_id, source_id, heal_frac in healing_events[-10:]:
                        ...
                        PyImGui.text(
                            f"    {source_name} -> {target_name}: {actual_heal:.0f} ({skill_name}) frac={heal_frac:.4f}"
                        )
            except Exception as e:
                PyImGui.text_colored(f"Error: {e}", (255, 0, 0, 255))
                ...
                if _recharges:
                    ...
                    for agent_id, skills in _recharges.items():
                        for skill_id, data in skills.items():
                            ...
                            PyImGui.text(
                                f"    {get_agent_name(agent_id)}: {get_skill_name(skill_id)} {remaining}ms{est_marker}"
                            )
                            shown += 1
                        if shown >= 10:
                            break
