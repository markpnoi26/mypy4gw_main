...
from Core import Map, Agent, Player, GLOBAL_CACHE, Color, ImGui
from Core import ProfessionShort, Campaign, Routines, Utils
from Core.native_src.context.WorldContext import AttributeStruct
from Core.native_src.context.AvailableCharacterContext import AvailableCharacterStruct
import PyImGui
import PyPlayer
import PyAgent
import PySkillbar
...
from dev.reference.ApoSource.account_data_src.rank_data_src import RankData
from dev.reference.ApoSource.account_data_src.faction_data_src import FactionData
from dev.reference.ApoSource.account_data_src.experience_data_src import ExperienceData
from dev.reference.ApoSource.account_data_src.title_data_src import TitleData
from dev.reference.ApoSource.account_data_src.quest_data_src import QuestData
...
class PlayerData:
    ...
    def draw_content(self):
        ...
        if PyImGui.collapsing_header("Missions Completed", PyImGui.TreeNodeFlags.NoFlag):
            ...
            PyImGui.text(
                f"entries:{len(self.missions_completed)} -{len(self.missions_completed) * bits_per_entry} total missions tracked."
            )
        ...
        if PyImGui.collapsing_header("Missions Bonus Completed", PyImGui.TreeNodeFlags.NoFlag):
            ...
            bits_per_entry = 32
            PyImGui.text(
                f"entries:{len(self.missions_bonus)} -{len(self.missions_bonus) * bits_per_entry} total missions tracked."
            )
        ...
        if PyImGui.collapsing_header("Missions Completed HM", PyImGui.TreeNodeFlags.NoFlag):
            ...
            bits_per_entry = 32
            PyImGui.text(
                f"entries:{len(self.missions_completed_hm)} -{len(self.missions_completed_hm) * bits_per_entry} total missions tracked."
            )
        ...
        if PyImGui.collapsing_header("Missions Bonus Completed HM", PyImGui.TreeNodeFlags.NoFlag):
            ...
            bits_per_entry = 32
            PyImGui.text(
                f"entries:{len(self.missions_bonus_hm)} -{len(self.missions_bonus_hm) * bits_per_entry} total missions tracked."
            )
        ...
        if PyImGui.collapsing_header("Unlocked Character Skills", PyImGui.TreeNodeFlags.NoFlag):
            ...
            PyImGui.text(
                f"entries:{len(self.unlocked_character_skills)} -{len(self.unlocked_character_skills) * bits_per_entry} total skills tracked."
            )
...
class AccountInfo:
    ...
    class AccountData:
        def __init__(
            ...
            account_name: str = "",
            account_email: str = "",
            available_characters: List[AvailableCharacterStruct] = [],
        ):
        ...
        if PyImGui.begin_tab_bar("AccountInfoTabs"):
            ...
            if PyImGui.begin_tab_item("Quest Log"):
                if PyImGui.begin_child("QuestLogChild", (0, 0), False, PyImGui.WindowFlags.NoFlag):
                    self.fetch_and_handle_quests = PyImGui.checkbox(
                        "Fetch and Handle Quests", self.fetch_and_handle_quests
                    )
                    if self.fetch_and_handle_quests:
