...
from Core.ImGui_src.IconsFontAwesome5 import IconsFontAwesome5 as Icons
...
class RouteBuilderWidget:
    ...
    def _draw_trapezoids(self, index: int, layer, clip_rect: tuple, color: Color) -> None:
        ...
        for tl_x, tl_y, tr_x, tr_y, br_x, br_y, bl_x, bl_y, area in quads:
            ...
            PyImGui.draw_list_add_quad_filled(
                tl_x + ox, tl_y + oy, tr_x + ox, tr_y + oy, br_x + ox, br_y + oy, bl_x + ox, bl_y + oy, col
            )
    ...
    def _draw_main_area(self) -> None:
        ...
        if PyImGui.begin_child("LeftCol", (canvas_w, avail_h), border=False):
            # Canvas
            canvas_flags = (
                PyImGui.WindowFlags.NoScrollbar | PyImGui.WindowFlags.NoScrollWithMouse | PyImGui.WindowFlags.NoMove
            )
            if PyImGui.begin_child("CanvasChild", (canvas_w, canvas_h), border=True, flags=canvas_flags):
                ...
                clip_rect = (origin[0], origin[1], child_pos[0] + child_max[0], child_pos[1] + child_max[1])
    ...
    def _draw_waypoints_on_canvas(self, child_pos: tuple) -> None:
        ...
        for i in range(n_wps - 1):
            ...
            PyImGui.draw_list_add_line(
                screen_pos[i][0], screen_pos[i][1], screen_pos[i + 1][0], screen_pos[i + 1][1], lc, 2.0
            )
        ...
        for i in range(n_wps):
            ...
            if not force_draw:
                for pr in occupied:
                    if (
                        label_rect[0] < pr[2]
                        and label_rect[2] > pr[0]
                        and label_rect[1] < pr[3]
                        and label_rect[3] > pr[1]
                    ):
                        overlaps = True
                        break
