...
def _draw_confirmation_popup():
    ...
    ImGui.text_wrapped(
        "This will close the current Guild Wars client in a way to be able to rejoin the same instance if retried within a few minutes. You have to login with the same character for this to work!"
    )
    PyImGui.spacing()
    ImGui.text_colored(
        "Do you really want to close the Guild Wars client?", color=color_red, font_style="Bold", font_size=14
    )
...
def tooltip():
    ...
    ImGui.text_colored("Warning!", color=color_red, font_size=18)
    ImGui.text_wrapped(
        "This widget will close the current Guild Wars client in a way to be able to rejoin the same instance if retried within a few minutes. You have to login with the same character for this to work!"
    )
