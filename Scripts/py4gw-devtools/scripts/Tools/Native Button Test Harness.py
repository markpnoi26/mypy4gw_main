...
from Core.GWUI import GWUI
from Core.native_src.methods.ButtonMethods import (
    ButtonMethods,
    CtlBtnProc_Callback,
    FrameCreate_Func,
    CtlBtnSetTextLiteral_Func,
)
from Core.native_src.internals.native_function import NativeFunction
...
def _verify_scanners() -> dict:
    ...
    try:
        valid = (
            FrameCreate_Func.is_valid() if hasattr(FrameCreate_Func, 'is_valid') else (FrameCreate_Func.address != 0)
        )
    ...
    try:
        valid = (
            CtlBtnSetTextLiteral_Func.is_valid()
            if hasattr(CtlBtnSetTextLiteral_Func, 'is_valid')
            else (CtlBtnSetTextLiteral_Func.address != 0)
        )
...
def _create_window() -> None:
    ...
    _log(
        f"Creating window: label='{WINDOW_LABEL}' title='{WINDOW_TITLE}' "
...
def _create_button() -> None:
    ...
    if not scanner["all_valid"]:
        ...
        return
    _log(
        f"  scanners: all valid âœ“ (CtlBtnProc=0x{CtlBtnProc_Callback.func_ptr:08X}, "
        ...
        try:
            ...
            if LAST_BUTTON_ID == 0:
                ...
                try:
                    ...
                    _log(
                        f"[call #{call_id}] Button frame: created={bool(frame.is_created)}, "
                        f"visible={bool(frame.is_visible)}, "
                        f"parent_id={int(frame.parent_id)}, "
                        f"child_offset=0x{int(frame.code):X}"
                    )
...
def _dump_scanner_status() -> None:
    ...
    for name, info in status.items():
        ...
        if isinstance(info, dict):
            _log(
                f"  {name}: valid={info.get('valid')}, addr={info.get('address','?')}, "
