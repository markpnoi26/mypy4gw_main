...
def _coro_get_path(start_x: float, start_y: float, start_zplane: int, end_x: float, end_y: float, end_zplane: int):
    ...
    normalized: list[tuple[float, float, int]] = (
        [(float(x), float(y), int(z)) for (x, y, z) in path_points] if path_points else []
    )
...
def draw_window():
    ...
    if draw_path_in_3d_world:
        ...
        def _draw_point_in_3d_world(x, y, z, index, color: Color = Color(0, 0, 255, 200)):
            ...
            Overlay().DrawText3D(
                x, y, real_z - 100, f"{index}", Color(255, 255, 255, 255).to_color(), False, False, 4.2
            )
                ...
                if key not in AUTO_SEG_CACHE:
                    if key not in AUTO_SEG_PENDING:
                        ...
                        GLOBAL_CACHE.Coroutines.append(
                            _coro_get_path(start[0], start[1], start[2], end[0], end[1], end[2])
                        )
