from Core import GLOBAL_CACHE, Console, ConsoleLog, UIManager, UIMessage, Color, ImGui
import PyImGui
import struct
from Core.native_src.internals.prototypes import Prototypes
from Core.native_src.internals.native_function import NativeFunction, ScannerSection
from Core.native_src.internals.gw_array import GW_Array_View, GW_Array
from Core.native_src.context.PreGameContext import (
    PreGameContext,
...
def Travel(map_id: int, region: int, district_number: int, language: int) -> bool:
    return UIManager.SendUIMessage(UIMessage.kTravel, [map_id, region, language, district_number], False)
...
def Travel_struct(map_id, region, district_number, language):
    ...
    return UIManager.SendUIMessageRaw(UIMessage.kTravel, ctypes.addressof(t), 0, False)
...
def probe_login_character_offsets(
    ...
    for off in range(0, max_offset, 2):
        try:
            buf = cast(elem_ptr + off, POINTER(c_wchar * 20)).contents

            raw = "".join(buf).split("\x00", 1)[0]

            if raw:
                # sanitize for imgui
                safe = raw.encode("utf-8", errors="replace").decode("utf-8", errors="replace").replace("\x00", "")
...
def draw_dword_probe_table(table_id: str, label: str, values):
    flags = PyImGui.TableFlags.BordersInnerV | PyImGui.TableFlags.RowBg | PyImGui.TableFlags.SizingStretchProp
