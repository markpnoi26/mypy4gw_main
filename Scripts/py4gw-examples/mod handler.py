...
_MODIFIER_MAP[8376] = ModifierInfo(
    ...
    representation=lambda arg, arg1, arg2: (f"{_MODIFIER_MAP[8760].format(None, None, arg2)}\n" f"Energy -{arg2}"),
)
...
_MODIFIER_MAP[10248] = ModifierInfo(
    ...
    max_value={10248: 20},
    modifier_value_arg=ModifierValue.Arg1,
    representation=lambda arg, arg1, arg2: (f"Halves casting time on spells of item's attribute (Chance: {arg1}%)"),
)
...
def ShowOffhandItemdescription():
    try:
        ...
        PySystem.Console.Log(
            module_name, f"Error in ShowItemComparisonWindow: {str(e)}", PySystem.Console.MessageType.Error
        )
        raise
    ...
    try:
        ...
        if PyImGui.begin(f"Compare Items", window_module.window_flags):
            ...
            if PyImGui.collapsing_header("Modifiers"):
                ...
                all_identifiers = {mod.GetIdentifier() for mod in modifiers1}.union(
                    {mod.GetIdentifier() for mod in modifiers2}
                )
                ...
                for identifier in all_identifiers:
                    ...
                    if mod_data:
                        ...
                        data = [
                            (
                                f"Item Type:",
                                f"{item1_type_id} - {item1_type_name}",
                                f"{item2_type_id} - {item2_type_name}",
                            ),
                    ...
                    if mod_data:
                        ...
                        and item1_arg == item2_arg
                        and item1_arg1 == item2_arg1
                        and item1_arg2 == item2_arg2
                    ):
                        PyImGui.push_style_color(
                            PyImGui.ImGuiCol.Text, (0.0, 1.0, 0.0, 1.0)
                        )  # Green for identical mods
                    else:
