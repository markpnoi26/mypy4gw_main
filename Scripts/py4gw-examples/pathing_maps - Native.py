        ...
        for trapezoid in layer.trapezoids:
            try:
                ...
                PySystem.Console.Log(
                    module_name,
                    f"Error processing trapezoid {trapezoid.id}: {str(e)}",
...
def draw_minimap_layer(layer, width=500, height=500):
    ...
    if layer.zplane not in precomputed_geometry:
        PySystem.Console.Log(
            module_name, f"No precomputed geometry for z-plane {layer.zplane}.", PySystem.Console.MessageType.Warning
        )
        return
...
def draw_minimap_layers(layers, width=500, height=500):
    ...
    if not layers or not precomputed_geometry:
        PySystem.Console.Log(
            module_name,
            "No layers or precomputed geometry available for drawing.",
            PySystem.Console.MessageType.Warning,
        )
        return
    ...
    if PyImGui.begin("Minimap", True):
        ...
        for layer in layers:
            if layer.zplane not in precomputed_geometry:
                PySystem.Console.Log(
                    module_name,
                    f"No precomputed geometry for z-plane {layer.zplane}.",
                    PySystem.Console.MessageType.Warning,
                )
                continue
...
def log_all_pathing_maps():
    ...
    if pathing_map is None:
        PySystem.Console.Log(
            module_name, "No pathing maps to log. Fetch them first.", PySystem.Console.MessageType.Warning
        )
        return
    ...
    for i, layer in enumerate(pathing_map):
        ...
        for y_node in layer.y_nodes:
            PySystem.Console.Log(
                module_name,
                f"    YNode {y_node.id}: Position={y_node.pos}, " f"Left={y_node.left_id}, Right={y_node.right_id}",
                PySystem.Console.MessageType.Debug,
            )
    ...
    try:
        if PyImGui.begin(module_name):
            ...
            if map_boundaries is not None:
                PyImGui.text(
                    f"Map Boundaries: {map_boundaries.x_min}, {map_boundaries.x_max}, {map_boundaries.y_min}, {map_boundaries.y_max}"
                )
                ...
                if show_map:
                    if draw_all_layers:
                        ...
                        if zplane < len(pathing_map):
                            ...
                            PySystem.Console.Log(
                                module_name,
                                f"No corresponding layer found for z-plane {zplane}.",
                                PySystem.Console.MessageType.Warning,
                            )
