...
bot_vars = BotVariables(
    window_info=ImGui.WindowModule(
        "Boreal Bot Tutorial", window_name="Control Panel", window_flags=PyImGui.WindowFlags.AlwaysAutoResize
    ),
    fsm=FSM("MainRoutine"),
    total_timer=Timer(),
    run_timer=Timer(),
...
def IsSkillBarLoaded(log_action=True):
    ...
    if primary_profession != "Assassin" and secondary_profession != "Assassin":
        ...
        PySystem.Console.Log(
            bot_vars.window_info.module_name,
    ...
    if log_action:
        PySystem.Console.Log(
            bot_vars.window_info.module_name, "Skill bar loaded successfully.", PySystem.Console.MessageType.Info
        )
    return True
...
bot_vars.sell_to_vendor_fsm.AddState(
    name="Target Merchant", execute_fn=lambda: Routines.Targeting.TargetMerchant(), transition_delay_ms=1000
)
bot_vars.sell_to_vendor_fsm.AddState(
    name="Interact with Merchant",
...
bot_vars.fsm.AddState(
    name="Are we in boreal station?",
    execute_fn=lambda: Routines.Transition.TravelToOutpost(
        boreal_station
    ),  # it will run this block of code (default is run code once)
    exit_condition=lambda: Routines.Transition.HasArrivedToOutpost(
        boreal_station
    ),  # will check this condition until met
...
bot_vars.fsm.AddSubroutine(
    name="Inventory Handling", sub_fsm=bot_vars.sell_to_vendor_fsm, condition_fn=lambda: IsInventoryHandlingNeeded()
)
...
def main():
    ...
    try:
        ...
        PySystem.Console.Log(
            bot_vars.window_info.module_name, f"ImportError encountered: {str(e)}", PySystem.Console.MessageType.Error
        )
        PySystem.Console.Log(
            bot_vars.window_info.module_name,
        ...
        PySystem.Console.Log(
            bot_vars.window_info.module_name, f"ValueError encountered: {str(e)}", PySystem.Console.MessageType.Error
        )
        PySystem.Console.Log(
            bot_vars.window_info.module_name,
            f"Stack trace: {traceback.format_exc()}",
        ...
        PySystem.Console.Log(
            bot_vars.window_info.module_name, f"TypeError encountered: {str(e)}", PySystem.Console.MessageType.Error
        )
        PySystem.Console.Log(
            bot_vars.window_info.module_name,
            f"Stack trace: {traceback.format_exc()}",
