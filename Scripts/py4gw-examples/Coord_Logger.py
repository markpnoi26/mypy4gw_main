...
def GetPlayerCoords():
    ...
    if not player_xy or (abs(new_coord[0] - player_xy[-1][0]) >= 300 or abs(new_coord[1] - player_xy[-1][1]) >= 300):
...
def CoordLogger(log_type, log_name, data, header="GWMapper"):
    ...
    try:
        ...
        PySystem.Console.Log(
            "GWMapper", f"{log_type.capitalize()} data logged to {file_path}", PySystem.Console.MessageType.Info
        )
    except Exception as e:
        # Log any error that occurs
        PySystem.Console.Log("GWMapper", f"Failed to log {log_type} data: {str(e)}", PySystem.Console.MessageType.Error)
...
def CoordMapper():
    ...
    try:
        ...
        PySystem.Console.Log(
            "GWMapper", f"Directory created or already exists: {save_dir}", PySystem.Console.MessageType.Info
        )
    except Exception as e:
    ...
    if enemy_xy:
        ...
        for agent_id, coords in enemy_xy.items():
            if coords:
                ...
                PySystem.Console.Log(
                    "Coordinate Logger",
                    f"Skipping empty coordinates for enemy {agent_id}",
    ...
    if ally_xy:
        ...
        for agent_id, coords in ally_xy.items():
            if coords:
                ...
                PySystem.Console.Log(
                    "Coordinate Logger",
                    f"Skipping empty coordinates for ally {agent_id}",
                    PySystem.Console.MessageType.Error,
                )
    ...
    if cart_xy:
        PySystem.Console.Log(
            "Coordinate Logger", "Adding cartography coordinates...", PySystem.Console.MessageType.Info
        )
        fig.add_trace(
            go.Scatter(
    ...
    try:
        ...
        if PyImGui.begin("GWMapper", PyImGui.WindowFlags.NoCollapse):
            ...
            if PyImGui.collapsing_header("Advanced Settings"):
                player_update_distance = PyImGui.slider_int(
                    "Player Log Update Distance", player_update_distance, 1, 2500
                )
            ...
            if PyImGui.collapsing_header("How to Use"):
                PyImGui.text_wrapped(
                    "Click to activate, while active the following coordinates will update: Player, Enemy, and Ally."
                )
                PyImGui.text_wrapped(
                    "Click Add Carto Coord to add a cartography coordinate at the players current position"
                )
                PyImGui.text_wrapped(
                    "Click Add Custom Coord to add a cartography coordinate at the players current position, you can also change the name"
                )
                PyImGui.text_wrapped("Click Write Coords to File to log all coordinates since last clear. ")
                PyImGui.text_wrapped(
                    "Click Clear All Coords to clear all coordinates, you can also clear coordinate types specifically"
                )
                PyImGui.text_wrapped("Advanced Settings: ")
                PyImGui.text_wrapped(
                    "Player Log Update Distance: How far the player has to travel in order to log a coordinate. "
                )
                PyImGui.text_wrapped(
                    "Enemy Log Update Distance: How far the enemy has to travel in order to log a coordinate. "
                )
