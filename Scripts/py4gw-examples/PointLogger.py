        ...
        if PyImGui.begin(module_name):
            ...
            if started:
                if timer.HasElapsed(polling_time * 1000):
                    PySystem.Console.Log(
                        module_name, f"({int(player_x)}, {int(player_y)}),", PySystem.Console.MessageType.Info
                    )
                    timer.Reset()
            ...
            if PyImGui.button("print new path"):
                for coord in new_path:
                    PySystem.Console.Log(
                        module_name, f"({int(coord[0])}, {int(coord[1])}),", PySystem.Console.MessageType.Info
                    )
