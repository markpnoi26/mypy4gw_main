...
def record_current_enemies():
    ...
    enemy_agent_ids = Routines.Agents.GetFilteredEnemyArray(player_x, player_y, Range.SafeCompass.value)
    ...
    for agent_id in enemy_agent_ids:
        ...
        if name not in recorded_agents:
            recorded_agents[name] = {"model_id": model_id, "agent_id": agent.agent_id}
            added += 1
