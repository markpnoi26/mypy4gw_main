...
from Core.native_src.internals.types import PointOrPath
from Core.native_src.internals.types import PointPath
from Core.Player import Player
from Core.py4gwcorelib_src.BehaviorTree import BehaviorTree
from Core.py4gwcorelib_src.Settings import Settings
from Core.routines_src.Agents import Agents as RoutinesAgents
from Core.routines_src.behaviourtrees_src.constants import *
from Core.routines_src.Checks import Checks
from dev.reference.ApoSource.ApoBottingLib import wrappers as BT
...
MONASTERY_COMMON_BUY_BY_PROFESSION: dict[str, BuyList] = {
    ...
    "Monk": [
        (ModelID.Bolt_Of_Cloth.value, 1),
        (ModelID.Pile_Of_Glittering_Dust.value, 1),
        (ModelID.Wood_Plank.value, 1),
    ],
    "Necromancer": [
        (ModelID.Tanned_Hide_Square.value, 1),
        (ModelID.Pile_Of_Glittering_Dust.value, 1),
        (ModelID.Bone.value, 1),
    ],
    "Mesmer": [
        (ModelID.Bolt_Of_Cloth.value, 1),
        (ModelID.Pile_Of_Glittering_Dust.value, 1),
...
OTHER_MASTER_COORDS_BY_PROFESSION: dict[str, list[PointOrPath]] = {
    profession: [
        coords for other_profession, coords in MASTER_COORDS_BY_PROFESSION.items() if other_profession != profession
    ]
...
def _take_quest_for_profession_from_master(
    node: BehaviorTree.Node, profession: str, COORDS: PointOrPath
) -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            ...
            BT.Subtree(
                ...
                subtree_fn=lambda node: _take_quest_for_profession_from_master(
                    node,
                    "Warrior",
...
def RangerPrimaryStarterQuestsPart1() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            ...
            BT.HandleAutoQuest(
                pos=None,
                use_npc_model_or_enc_str=SUJUN_ENC_STR,
                buttons=[0, 0],
                require_quest_marker=True,
            ),
...
def RangerPrimaryStarterQuestsPart2() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            # track down zho
            BT.Subtree(
                name="Get To Kinya Province If Ranger",
                subtree_fn=_get_to_kynya_if_ranger,
            ),
            BT.HandleAutoQuest(
                ZHO_COORDS,
                buttons=[0, 0],
            ...
            BT.Subtree(
                name="Cancel Skill Reward Window If Not Ranger",
                subtree_fn=_cancel_skill_reward_window_if_not_ranger,
            ),
...
def MonkPrimaryStarterQuestsPart2() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            BT.Subtree(
                name="Get To Kinya Province If Monk",
                subtree_fn=_get_to_kynya_if_monk,
            ),
...
def NecromancerPrimaryStarterQuestsPart2() -> BehaviorTree:
    ...
    KILLSPOTS_COORDS = [(5934.45, -12796.59), (11069.31, -13898.07), (12615.04, -10692.58), (9007.76, -9230.19)]
    ...
    return BT.Sequence(
        ...
        children=[
            BT.Subtree(
                name="Get To Panjian If Necromancer",
                subtree_fn=_get_to_panjian_if_necromancer,
            ),
            BT.Subtree(
...
def MesmerPrimaryStarterQuestsPart2() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            BT.Subtree(
                name="Exit To Kinya If Mesmer",
                subtree_fn=_exit_to_kinya_if_mesmer,
            ),
...
def ElementalistPrimaryStarterQuestsPart2() -> BehaviorTree:
    ...
    def _get_to_panjian_if_elementalist(node: BehaviorTree.Node) -> BehaviorTree:
        ...
        return BT.Sequence(
            ...
            map_id_or_name=TSUMEI_VILLAGE,
            map_prep=PrepareForBattle(),
            children=[BT.MoveAndExitMap(FROM_TSUMEI_VILLAGE_TO_PANJIAN_PENINSULA, target_map_id=PANJIAN_PENINSULA)],
        )
    ...
    return BT.Sequence(
        ...
        children=[
            BT.Subtree(
                name="Get To Panjian If Elementalist",
                subtree_fn=_get_to_panjian_if_elementalist,
            ),
            BT.Subtree(
            ...
            BT.HandleAutoQuest(
                pos=None,
                use_npc_model_or_enc_str=KAI_YING_ENC_STR,
                buttons=[0, 0],
                require_quest_marker=True,
            ),
        ],
...
def AssassinPrimaryStarterQuestsPart2() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            BT.Subtree(
                name="Get To Kinya If Assassin",
                subtree_fn=_get_to_kynya_if_assassin,
            ),
            ...
            BT.Wait(1000),
            BT.HandleAutoQuest(
                None, use_npc_model_or_enc_str=PANAKU_MODEL_ID, require_quest_marker=True, buttons=[0, 0]
            ),
        ],
    )
...
def RitualistPrimaryStarterQuestsPart2() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            BT.Subtree(
                name="Get To Panjian If Ritualist",
                subtree_fn=_get_to_panjian_if_ritualist,
            ),
            BT.Subtree(
...
def An_Audience_WithMasterTogo() -> BehaviorTree:
    return BT.Sequence(
        ...
        children=[
            ...
            BT.MoveAndExitMap(
                (-3480, 9460),
                target_map_name="Linnok Courtyard",
            ),
            ...
            BT.MoveAndExitMap(
                (-3762, 9471),
                target_map_id=SHING_JEA_MONASTERY,
            ),
        ],
    )
...
def Talk_With_Masters() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            ...
            BT.GetValuesByProfession(
                profession_values=OTHER_MASTER_COORDS_BY_PROFESSION, target_key="other_master_coords"
            ),
...
def R_Mo_Me_A_secondaries_and_Appearance_of_the_naga() -> BehaviorTree:
    ...
    def _escort_complete() -> bool:
        ...
        return (
            wulk_agent_id != 0 and Agent.HasQuest(wulk_agent_id) and not Checks.Agents.InDanger(aggro_area=Range.Spirit)
        )
    ...
    return BT.Sequence(
        ...
        children=[
            ...
            BT.HandleAutoQuest(
                pos=None, use_npc_model_or_enc_str=WULK_ENC_STR, require_quest_marker=False
            ),  # revenge of the yeti
            BT.FollowModel(
                WULK_ENC_STR, exit_condition=_escort_complete, exit_by_area=((-13717.04, 7714.01), Range.Earshot.value)
            ),
...
def Attribute_Points_Quest_1() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            BT.HandleQuest(
                lost_treasure_quest_id, [(15775.29, 18832.91), (14363.00, 19499.00)], 0x815A01, mode=BT.Questmode.Accept
            ),
            PrepareForBattle(),
...
def The_Threat_Grows_CashCrops_Togos_Utimatum() -> BehaviorTree:
    ...
    return BT.Sequence(
        ...
        children=[
            ...
            BT.HandleQuest(
                quest_id=the_threat_grows_quest_id,
                pos=None,
                dialog_id=0x815407,
