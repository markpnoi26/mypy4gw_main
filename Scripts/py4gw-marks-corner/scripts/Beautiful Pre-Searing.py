...
from dev.reference.ApoSource.beautiful_pre_searing_src.acquire_belt_pouch import AcquireBeltPouch
from dev.reference.ApoSource.beautiful_pre_searing_src.acquire_weapon import AcquireWeapon
from dev.reference.ApoSource.beautiful_pre_searing_src.debug import dump_tree_diagnostics
from dev.reference.ApoSource.beautiful_pre_searing_src.farming_routines import *
from dev.reference.ApoSource.beautiful_pre_searing_src.getting_started import GetGettingStartedSequence
from dev.reference.ApoSource.beautiful_pre_searing_src.globals import ITEMS_BLACKLIST
from dev.reference.ApoSource.beautiful_pre_searing_src.globals import *
from dev.reference.ApoSource.beautiful_pre_searing_src.helpers import *
from dev.reference.ApoSource.beautiful_pre_searing_src.outpost_unlock import *
from dev.reference.ApoSource.beautiful_pre_searing_src.tree_builder import CommonMapExit
from dev.reference.ApoSource.beautiful_pre_searing_src.tree_builder import ensure_botting_tree
from dev.reference.ApoSource.beautiful_pre_searing_src.unlock_pet import UnlockPet
                    ...
                    BehaviorTree.SequenceNode(
                        ...
                        children=[
                            BehaviorTree.ConditionNode(
                                ...
                                condition_fn=lambda: _has_charr_at_the_gate_quest()
                                and Quest.IsQuestCompleted(CHARR_AT_THE_GATE_QUEST_ID),
                            ),
...
def _draw_welcome_tab() -> None:
    ...
    if ImGui.collapsing_header("About this script", flags=0):
        ImGui.text_wrapped(
            "This script is a comprehensive collection of quests and activities for the Pre-Searing area."
        )
        ImGui.text_wrapped(
            "Use the Controls tab for all routines. Sections are grouped under compact collapsing headers."
        )
        ImGui.separator()
...
def _draw_controls_tab(botting_tree: BottingTree) -> None:
    ...
    if ImGui.collapsing_header("Quests"):
        ImGui.text("Quests")
        _draw_help_marker(
            "Standalone quest routines that do not belong under the broader setup, unlock, or farm sections."
        )
...
def _draw_getting_started_tab(botting_tree: BottingTree) -> None:
    ...
    if ImGui.collapsing_header("1. Prepare Your Character"):
        ImGui.text_wrapped(
            "This routine will complete important early quests that unlock essential features and quality of life improvements for the rest of the content in Pre-Searing."
        )
        ImGui.separator()
    ...
    if ImGui.collapsing_header("2. Capture Pet and Unlock Secondary Profession"):
        ImGui.text_wrapped(
            "This routine will capture the pet from the first quest and complete the necessary steps to unlock the secondary profession."
        )
        ImGui.text_wrapped(
            "This is a separate routine because it involves a lot of waiting for the pet capture to succeed, which can take a long time and is not required to be done early on."
        )
        ImGui.separator()
    ...
    if ImGui.collapsing_header("3. Acquire a Weapon"):
        ...
        ImGui.text_wrapped(
            "Nevermore Flatbow is the best weapon to acquire in Pre-Searing, we will attempt to acquire it"
        )
        ImGui.text_wrapped(
            "If Nevermore is not available, we will fall back to Farming material for Buying a bow with a collector"
        )
        ImGui.separator()
    ...
    if ImGui.collapsing_header("4. Acquire a Belt Pouch"):
        ...
        ImGui.text_wrapped(
            "Belt pouches are a very useful item that increase your inventory space, and the one available in Pre-Searing is very easy to acquire"
        )
        ImGui.separator()
...
def _draw_outpost_unlocks_tab(botting_tree: BottingTree) -> None:
    ...
    if ImGui.collapsing_header("5. Unlock Wizard's Folly"):
        ImGui.text_wrapped(
            "This routine travels from Ashford Abbey through Wizard's Folly and into Foible's Fair to unlock the outpost path."
        )
        ImGui.separator()
    ...
    if ImGui.collapsing_header("6. Unlock Barradin State"):
        ImGui.text_wrapped(
            "This routine travels from Ascalon City through Green Hills County and into Barradin State to unlock the outpost path."
        )
        ImGui.separator()
    ...
    if ImGui.collapsing_header("7. Unlock Fort Ranik"):
        ImGui.text_wrapped(
            "This routine travels from Ashford Abbey through Regent Valley and into Fort Ranik to unlock the outpost path."
        )
        ImGui.separator()
...
def _draw_farming_tab(botting_tree: BottingTree) -> None:
    ...
    if ImGui.collapsing_header("1. Skale Fin Farm"):
        ...
        ImGui.text_wrapped(
            "Update the start map, kill path, model id, and target quantity in this script if you want to customize the farm."
        )
        ImGui.separator()
...
def _draw_debug_tab() -> None:
    ...
    if ImGui.collapsing_header("Draw Move Path Debug Options"):
        ...
        draw_move_current_waypoint_radius = PyImGui.slider_float(
            "Current Waypoint Radius", draw_move_current_waypoint_radius, 20.0, 120.0
        )
