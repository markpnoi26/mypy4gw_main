...
def withdraw_cons_materials_from_inventory():
    ...
    for model_id, req_amount in PER_CONSET.items():
        available = GLOBAL_CACHE.Inventory.GetModelCountInStorage(model_id) + GLOBAL_CACHE.Inventory.GetModelCount(
            model_id
        )
    ...
    for model_id, req_amount in PER_CONSET.items():
        ...
        if needed == 0:
            ConsoleLog(
                "Withdraw", f"Already have enough {model_name} ({already_have}/{total_needed}) — skipping withdraw."
            )
            continue
    ...
    for model_id, required_qty in required_mats.items():
        ...
        if qty < required_qty:
            PySystem.Console.Log(
                MODULE_NAME,
                f"Not enough {model_name} to craft {crafts}x {target_name}. " f"Required: {required_qty}, Found: {qty}",
