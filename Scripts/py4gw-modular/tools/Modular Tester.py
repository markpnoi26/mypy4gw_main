...
from Core.botting_tree_src.ui import BottingTreeUIMovePathMixin
from Core.modular import BTRecipeRunner
from Core.modular import RecipeSpec
from Core.modular.paths import modular_data_root
from Core.modular.widget_runtime import guarded_widget_main
...
def _draw_recipe_overview() -> None:
    ...
    if PyImGui.begin_table(
        "##modular_tester_metrics", 3, PyImGui.TableFlags.SizingStretchSame | PyImGui.TableFlags.NoSavedSettings
    ):
        PyImGui.table_next_row()
...
def _draw_step_detail() -> None:
    ...
    PyImGui.spacing()
    if PyImGui.begin_table(
        "##modular_tester_step_detail", 2, PyImGui.TableFlags.SizingStretchProp | PyImGui.TableFlags.RowBg
    ):
