...
from Core.enums_src.Hero_enums import HeroType
import PyGameThread
...
TEXTURE = os.path.join(
    PySystem.Console.get_projects_path(), "Widgets", "Automation", "Helpers", "Elite Skills", "elite_skills_capture.png"
)
...
def should_skip_skill(skill_id: int) -> Tuple[bool, str]:
    ...
    if skill and not can_access_skill_map(skill):
        return (
            True,
            f"Skill {skill_id} ({GLOBAL_CACHE.Skill.GetName(skill_id)}) map not accessible: {Map.GetMapName(skill.start_map)}",
        )
...
def find_py4gw_root(start_path: str) -> Optional[str]:
    ...
    while True:
        ...
        py4gw_indicators = ["Core", "Widgets", "Examples", "py4gw.exe", "Py4GW.py"]
...
def LoadSecondaryBuild(profession: Profession):
    ...
    if not isinstance(primary_builds, dict):
        ConsoleLog(
            "Build", f"ERROR: Expected dict for {current_primary.value}, got {type(primary_builds).__name__}", log=True
        )
        yield
        return
    ...
    if template and len(template) > 10:
        try:
            ...
            if profession == current_primary:
                ConsoleLog(
                    "Build",
                    f"Loaded {current_primary.value}/{current_primary.value} build with Signet of Capture",
                    log=True,
                )
            else:
...
def DisableHeroAI():
    ...
    global bot
    bot.ResetHeroAICombatState(
        active=False, following=False, avoidance=False, looting=False, targeting=False, combat=False, skills=False
    )
...
def ClickSkillFrame(skill_id: int):
    ...
    ConsoleLog(
        "ClickSkillFrame",
        f"Looking for skill frame {skill_id} with offset {skill_id_offset}, got frame ID: {skill_frame}",
        log=True,
    )
    ...
    if skill_frame.exists:
        ...
        ConsoleLog(
            "ClickSkillFrame",
            f"ERROR: Skill frame {skill_id} not found! Check if Signet of Capture UI is open",
            log=True,
        )
...
class EliteSkillsGUI:
    ...
    def _set_property_active_now(self, prop_name: str, active: bool) -> None:
        try:
            if (
                hasattr(self.bot, "Properties")
                and hasattr(self.bot.Properties, "exists")
                and self.bot.Properties.exists(prop_name)
            ):
        ...
        try:
            if hasattr(self.bot, "config") and hasattr(self.bot.config, "config_properties"):
                if hasattr(self.bot.config.config_properties, "pause_on_danger"):
                    self.bot.config.config_properties.pause_on_danger.set_now(
                        "active", bool(self.settings.get("pause_on_danger", True))
                    )
                if hasattr(self.bot.config.config_properties, "halt_on_death"):
                    self.bot.config.config_properties.halt_on_death.set_now(
                        "active", bool(self.settings.get("halt_on_death", False))
                    )
        except:
            pass
    ...
    def draw_header(self):
        ...
        if self.skill_chain:
            ...
            available_skills = [
                s
                for s in ELITE_SKILLS
                if not is_skill_unlocked(s.skill_id) and can_learn_skill(s.skill_id) and can_access_skill_map(s)
            ]
    ...
    def draw_settings_window(self):
        ...
        if PyImGui.begin("Elite Skills Settings", True, PyImGui.WindowFlags.AlwaysAutoResize):
            ...
            self.settings["auto_deposit_gold"] = PyImGui.checkbox(
                "Auto Deposit Gold", self.settings["auto_deposit_gold"]
            )
    ...
    def draw_controls(self):
        ...
        available_skills = [
            s
            for s in ELITE_SKILLS
            if s.profession == self.current_profession
    ...
    def add_skill_to_chain(self, skill: 'EliteSkill'):
        ...
        if skill.step_name not in self.skill_chain:
            ...
            ConsoleLog(
                "SkillChain", f"Added {skill.display_name} to chain ({len(self.skill_chain)} skills total)", log=True
            )
        else:
    ...
    def _advance_chain(self):
        ...
        if current_step:
            ...
            if skill and self._is_skill_captured(skill.skill_id):
                ...
                ConsoleLog(
                    "SkillChain",
                    f"Skill {skill.display_name if skill else current_step} not captured, will retry. Unlocked: {unlocked_check}, SkillBar: {skillbar_check}",
                    log=True,
                )
    ...
    def start_skill_chain(self, skills: List['EliteSkill']):
        ...
        if first_step and self._build_single_skill(first_step):
            ...
            if self.skill_chain:
                next_skills = [
                    s
                    for s in (next((e for e in ELITE_SKILLS if e.step_name == sn), None) for sn in self.skill_chain)
                    if s is not None
                ]
                if next_skills:
    ...
    def _start_next_batch_skill(self):
        ...
        ConsoleLog(
            "BatchCapture",
            f"Capturing skill {self.current_batch_index + 1}/{len(self.batch_skills)}: {skill.display_name}",
            log=True,
        )
        self.selected_skill = skill
        self._start_capture()
        ...
        try:
            ...
            if self.selected_skill.start_map > 0:
                if not CheckMapUnlocked(self.selected_skill.start_map, self.selected_skill.display_name):
                    ConsoleLog(
                        "Capture", f"Cannot start {self.selected_skill.display_name}: Outpost not unlocked", log=True
                    )
                    self.capture_running = False
                    return
        ...
        try:
            ...
            if capture_done:
                ...
                if gui.selected_skill and not gui._is_skill_captured(gui.selected_skill.skill_id):
                    ...
                    print(
                        f"Capture completed for {gui.selected_skill.display_name if gui.selected_skill else 'unknown skill'}"
                    )
        except:
            pass
