...
class QueuedRun:
    ...
    def __init__(self, region: str, run_name: str, display: str, outpost_id: int, outpost_path: list, segments: list):
        self.region = region
...
def bot_routine(bot: Botting) -> None:
    ...
    for run_idx, run in enumerate(_queued_runs):
        ...
        bot.States.AddCustomState(lambda idx=run_idx: _set_current_index(idx), f"SetRunIndex_{run_idx}")
        ...
        for seg_i, entry in enumerate(run.segments):
            ...
            if seg_path:
                next_map_id = run.segments[seg_i + 1]["map_id"] if seg_i + 1 < len(run.segments) else entry["map_id"]
...
def _draw_settings():
    ...
    runs = sorted(
        ...
        key=lambda name: int(re.search(r"_(\d+)_", name).group(1)) if re.search(r"_(\d+)_", name) else 0,
    )
