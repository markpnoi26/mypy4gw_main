import Py4GW
from Core import Routines, Item, Botting, ActionQueueManager, ConsoleLog, Player, GLOBAL_CACHE, Agent, HeroType
from Core.Builds.CombatAutomatorExcluded.SF_Assassin_Barbarous import SF_Assassin_Barbarous
from Core.Builds.CombatAutomatorExcluded.SF_Derv_Barbarous import SF_Derv_Barbarous
from Core.Builds.CombatAutomatorExcluded.BuildHelpers import BuildDangerHelper, DangerTable
from dev.reference.Sasemoi.bot_helpers import BotStuckHelper
from dev.reference.Sasemoi.utils.inventory_utils import (
    filter_valuable_inscription_type,
    get_unidentified_items,
    filter_valuable_weapon_type,
    filter_valuable_rune_type,
)
from dev.reference.Sasemoi.bot_helpers.bot_mystic_healing_support import MysticHealingSupport
...
hero_list = [HeroType.Gwen, HeroType.MOX, HeroType.Melonni]
...
bot = Botting(
    ...
    custom_build=SF_Derv_Barbarous(
        build_danger_helper=BuildDangerHelper(
            cripple_kd_table=barbarous_cripple_kd_table, spellcast_table=barbarous_spellcast_table
        )
    ),
...
stuck_helper = BotStuckHelper(config={"log_enabled": False, "movement_timeout_handler": lambda: HandleStuck()})
...
def AssessLootManagement():
    ...
    if should_manage_inventory:
        ConsoleLog(
            "Inventory Check",
            f"Free Inventory Slots: {free_slots} - Managing Inventory before next run",
