import Py4GW
from Core import Routines, Item, Botting, ActionQueueManager, Agent, ConsoleLog, GLOBAL_CACHE, ItemArray, Bags, Player
from Core.Builds.CombatAutomatorExcluded.SF_Assassin_HP import SF_Assassin_Hells_Precipice
...
# Danger Tables
Pongmei_cripple_kd_table: DangerTable = (([3768, 3774, 4096, 4076, 3787, 4248], "Pongmei KD/Cripple"),)
Pongmei_spellcast_table: DangerTable = (([3768, 3774, 4096, 4076, 3787, 4248], "Pongmei Casters"),)
...
bot = Botting(
    ...
    custom_build=SF_Assassin_Hells_Precipice(
        build_danger_helper=BuildDangerHelper(
            cripple_kd_table=Pongmei_cripple_kd_table, spellcast_table=Pongmei_spellcast_table
        )
    ),
...
def SetupDangerTables():
    if isinstance(bot.config.build_handler, SF_Assassin_Hells_Precipice):
        ...
        build_danger_helper.update_tables(
            cripple_kd_table=Pongmei_cripple_kd_table, spellcast_table=Pongmei_spellcast_table
        )
...
def ManageInventory(bot: Botting):
    ...
    if should_manage_inventory:
        ...
        ConsoleLog(
            "Item Info Test",
            f"Valuable Unidentified Items Count: {len(valuable_loot)}",
            PySystem.Console.MessageType.Info,
        )
