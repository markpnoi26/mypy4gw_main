...
from Core import UIManager, Color, ImGui
...
from dev.reference.frenkeyLib.SulfurousRunner.settings import Settings
from dev.reference.frenkeyLib.SulfurousRunner.pathing import search_paths, update_waypoints
from dev.reference.frenkeyLib.SulfurousRunner.ui import draw_configure, draw_flags, draw_paths
from dev.reference.frenkeyLib.SulfurousRunner.globals import Global
import PyImGui
...
def main():
    ...
    if draw_throttle.IsExpired():
        if settings.draw_flags:
            draw_flags(g.waypoints.get(Map.GetMapID(), []), settings.flag_color, settings.use_flag_collision)

        if settings.draw_paths:
            draw_paths(g.paths, settings.path_color, g.closest_waypoint, settings.use_path_collision)
