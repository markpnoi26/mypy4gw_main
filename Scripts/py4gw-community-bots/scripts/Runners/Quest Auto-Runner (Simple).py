...
MAINTAIN_CONSUMABLES = [
    ...
    },
    {
        ...
        "model_id": _model_id_value("Pumpkin_Cookie", 0),
        "skills": ["Pumpkin_Cookie_skill"],
    },
    {
        "key": "pumpkin_pie",
    ...
    },
    {
        ...
        "skills": ["Honeycomb_skill", "Honeycomb_item_effect", "Honeycomb"],
    },
]
...
def _coro_upkeep_consumables():
    while True:
        ...
        for key in keys:
            ...
            if model_id <= 0:
                DebugLog(
                    f"Consumable '{spec.get('label', key)}' has no valid model_id in this build; skipping.",
            ...
            try:
                if (
                    hasattr(bot, "helpers")
                    and hasattr(bot.helpers, "Multibox")
                    and hasattr(bot.helpers.Multibox, "_use_consumable_message")
                ):
                    skill_id = 0
    ...
    def load_quest_state():
        try:
            ...
            if coords is None:
                ConsoleLog(
                    BOT_NAME,
                    "Quest has no valid marker. Make sure the quest is active and has a green star.",
            ...
            ConsoleLog(
                BOT_NAME,
                f"Quest marker: ({quest_info['marker_x']:.0f}, {quest_info['marker_y']:.0f})",
    ...
    def navigate_state():
        ...
        while True:
            ...
            if next_marker is None:
                ConsoleLog(
                    BOT_NAME, "Arrived at quest marker (no further marker detected).", Console.MessageType.Success
                )
                break
