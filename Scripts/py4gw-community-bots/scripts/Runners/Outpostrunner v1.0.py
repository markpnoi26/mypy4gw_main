from Core import *
from dev.reference.aC_Scripts.OutpostRunner.runner_singleton import runner_fsm
from dev.reference.aC_Scripts.OutpostRunner.map_loader import get_regions, get_runs
from dev.reference.aC_Scripts.OutpostRunner.StatsManager import RunInfo
from Core import IconsFontAwesome5
from dev.reference.aC_Scripts.OutpostRunner.Build_Manager_Addon import BodyBlockDetection
from dev.reference.aC_Scripts.OutpostRunner import Build_Manager
from dev.reference.aC_Scripts.Consumable import ConsumablesSelector
        ...
        if not Build_Manager.FREESTYLE_MODE and not fsm_active:
            ...
            if not runs_raw:
                ...
                if PyImGui.button("Add All in Region"):
                    ...
                    ConsoleLog(
                        "OutpostRunner", f"Added all runs from region {selected_region}", Console.MessageType.Debug
                    )
                ...
                for r in stats.runs:
                    if r.finished:
                        ...
                        PyImGui.text_colored(
                            f"Done: {format_time(r.duration)} {IconsFontAwesome5.ICON_CHECK}", green_color
                        )
                        if r.failures >= 1:
                            PyImGui.same_line(0, 5)
...
ATTRIBUTES = [("Deadly Arts", 3), ("Shadow Arts", 12), ("Mysticism", 12)]
        ...
        for skill_id in BUILD_SKILLS:
            ...
            if full_texture_path and os.path.exists(full_texture_path):
                ...
                PySystem.Console.Log(
                    "BuildViewer",
                    f"Missing texture for skill {skill_id} -> {full_texture_path}",
        ...
        if PyImGui.button("Finish"):
            ...
            try:
                with open(filename, "w") as f:
                    f.write("from Core.enums import outpost_name_to_id, explorable_name_to_id\n\n")
...
def log_path():
    ...
    if current_map_id != state["last_map_id"]:
        ...
        if not new_name or current_map_id == 0 or (x == 0 and y == 0):
            ...
            ConsoleLog(
                "Logger", f"Detected map change: {state['last_map_id']} -> {current_map_id}", Console.MessageType.Info
            )
