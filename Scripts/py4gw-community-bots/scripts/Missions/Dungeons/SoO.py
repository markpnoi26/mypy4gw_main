...
_BDS_ICON_PATH = os.path.join(
    PySystem.Console.get_projects_path(), "Widgets", "Automation", "Bots", "Missions", "Dungeons", "bds.png"
)
...
def farm_bds_routine(bot: Botting) -> None:
    ...
    bot.States.AddCustomState(
        _reset_l3_boss_route_flag, "Reset L3 boss route flag"
    )  # ICI CE TROUVE LE SHRINE DONC SI JE MEURT APRES JE REVIENS ICI DONC MAIS SI JE MEURT AU BOSS C4EST ICI QUE JE REVIS
...
def _masked_email(key: str) -> str:
    ...
    all_keys = sorted(
        set(list(_bds_drops.keys()) + list(_session_bds.keys()) + list(_gb_drops.keys()) + list(_session_gb.keys()))
    )
...
def _check_shandra_inside_dungeon(bot: Botting) -> Generator:
    ...
    for _attempt in range(_max_attempts):
        state = _shandra_quest_state()
        ConsoleLog(
            BOT_NAME, f"[Shandra] Inside dungeon state: {state} (attempt {_attempt + 1}/{_max_attempts})", log=True
        )
        ...
        if not ok:
            ConsoleLog(
                BOT_NAME, "[Shandra] Failed to re-enter SoO Level 1 â€” retrying", PySystem.Console.MessageType.Warning
            )
            continue
    ...
    ConsoleLog(
        BOT_NAME,
        f"[HARD STOP] Shandra quest never became active after {_max_attempts} attempts â€” stopping bot.",
...
def _record_run_end() -> Generator:
    ...
    if _t_run_start > 0 and _t_l2_start > 0 and _t_l3_start > 0:
        ...
        if l3_time < _l3_fastest:
            _l3_fastest = l3_time
        if l3_time > _l3_slowest:
            _l3_slowest = l3_time

        ConsoleLog(
            BOT_NAME, f"[Run Timing] Total: {run_time:.0f}s  L1: {l1_time:.0f}s  L2: {l2_time:.0f}s  L3: {l3_time:.0f}s"
        )
...
def _draw_bds_stats() -> None:
    ...
    if PyImGui.begin_table("##bds_overview_layout", 2, layout_flags):
        ...
        if PyImGui.begin_table("##bds_alltime", 3, tbl_flags):
            ...
            PyImGui.table_set_column_index(1)
            _rtext(str(alltime_total_bds))
            PyImGui.table_set_column_index(2)
            _rtext(_runs_per_drop(_total_runs, alltime_total_bds))
            PyImGui.end_table()
...
def _interact_brazier(
    label: str, result: list, max_dist: float = 220.0, attempts: int = BRAZIER_INTERACT_ATTEMPTS
) -> Generator:
...
def run_brazier_sequence(points: list[tuple[float, float]], interact_dist: float = 200.0) -> None:
    bot.States.AddCustomState(lambda p=points, d=interact_dist: _brazier_sequence_gen(p, d), "Brazier sequence")
...
def _on_party_wipe(bot: "Botting"):
    ...
    SHRINES_BY_MAP = {
        SoO_lvl1: [("Secure return - L1", 8503.9, 12143.5), ("Secure return 1 - L1", 15953.0, 11902.0)],
        SoO_lvl2: [("Secure return - L2", -14076.0, -19457.0)],
        SoO_lvl3: [("Secure return 1 - L3", 17544.0, 18810.0), ("Secure return boss - L3", -9686.32, 2632)],
    }
...
def _draw_merchant_settings() -> None:
    ...
    if _merchant_enabled:
        ...
        new_rare = PyImGui.checkbox(
            "Sell Diamond & Onyx to Rare Material Trader##bds_rare_mats", _merchant_sell_rare_mats
        )
        ...
        new_ectos = PyImGui.checkbox(
            "Buy Glob of Ectoplasm when storage over threshold##bds_ectos", _merchant_buy_ectos
        )
