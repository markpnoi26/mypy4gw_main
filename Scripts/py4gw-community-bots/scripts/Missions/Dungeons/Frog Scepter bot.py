...
from Core.routines_src.Yield import Utils
from Core.routines_src.Yield import Yield
from Widgets.Panels.Messaging import get_inventory_count, reset_inventory_count
...
_FROGGY_ICON_PATH = os.path.join(
    PySystem.Console.get_projects_path(), "Widgets", "Automation", "Bots", "Missions", "Dungeons", "Frog Scepter.png"
)
...
def farm_froggy_routine(bot: Botting) -> None:
    ...
    path_2 = [(5097.64, 2204.33), (1228.15, 54.49), (-140.87, 2741.86), (1228.15, 54.49), (141.23, -1965.14)]
    bot.Templates.Aggressive()
...
def _masked_email(key: str) -> str:
    ...
    all_keys = sorted(
        set(
            list(_froggy_drops.keys())
            + list(_session_froggy.keys())
            + list(_gb_drops.keys())
            + list(_session_gb.keys())
        )
    )
...
def _write_local_salvage_kit_count() -> None:
    from Core.enums_src.Model_enums import ModelID as _ModelID
    ...
    for item_id in item_array:
        ...
        while stack_qty > 0:
            quoted = yield from Routines.Yield.Merchant._wait_for_quote(
                GLOBAL_CACHE.Trading.Trader.RequestSellQuote, item_id, timeout_ms=750, step_ms=10
            )
            if quoted <= 0:
                break
...
def _coro_deposit_crafting_materials_to_storage(selected_models: set[int]) -> Generator:
    ...
    if not GLOBAL_CACHE.Inventory.IsStorageOpen():
        ConsoleLog(
            BOT_NAME,
            "[Merchant] Storage not open; skipping crafting material deposit",
            PySystem.Console.MessageType.Warning,
        )
        yield
        return
...
def _coro_sell_scrolls(mx: float, my: float) -> Generator:
    ...
    item_array = GLOBAL_CACHE.ItemArray.GetItemArray(bag_list)
    sell_ids = [
        int(item_id) for item_id in item_array if int(GLOBAL_CACHE.Item.GetModelID(item_id)) in _SCROLL_MODEL_IDS
    ]
    if not sell_ids:
        ConsoleLog(BOT_NAME, "[Merchant] No scrolls to sell in bags 1-4")
        storage_hits = [(mid, GLOBAL_CACHE.Inventory.GetModelCountInStorage(mid)) for mid in _SCROLL_MODEL_IDS]
        storage_hits = [(mid, cnt) for mid, cnt in storage_hits if cnt > 0]
        if storage_hits:
            ConsoleLog(
                BOT_NAME, f"[Merchant] WARNING: scrolls found in STORAGE (InventoryPlus deposited them): {storage_hits}"
            )
        yield
        return
...
def _disable_merchant_widgets() -> Generator:
    ...
    from Core.py4gwcorelib_src.WidgetManager import get_widget_handler as _get_wh
...
def _check_tekks_inside_dungeon(bot: Botting) -> Generator:
    ...
    for _attempt in range(_max_attempts):
        state = _tekks_quest_state()
        ConsoleLog(
            BOT_NAME, f"[tekks] Inside dungeon state: {state} (attempt {_attempt + 1}/{_max_attempts})", log=True
        )
        ...
        if not ok:
            ConsoleLog(
                BOT_NAME, "[tekks] Failed to re-enter SoO Level 1 â€” retrying", PySystem.Console.MessageType.Warning
            )
            continue
    ...
    ConsoleLog(
        BOT_NAME,
        f"[HARD STOP] tekks quest never became active after {_max_attempts} attempts â€” stopping bot.",
...
def _interact_with_tekks(bot: Botting, dialog_id: int, tolerance: float = 220.0, max_move_retries: int = 10):
    ...
    ConsoleLog(
        BOT_NAME, f"[tekks] Found {npc_name} at ({x:.0f},{y:.0f}) dist={distance:.0f} agent_id={agent_id}", log=True
    )
    ...
    for attempt in range(1, max_move_retries + 1):
        ...
        ConsoleLog(BOT_NAME, f"[tekks] Move attempt {attempt}/{max_move_retries} toward ({x:.0f},{y:.0f})", log=True)
    ...
    for account in GLOBAL_CACHE.ShMem.GetAllAccountData():
        if account.AccountEmail != sender_email:
            GLOBAL_CACHE.ShMem.SendMessage(
                sender_email, account.AccountEmail, SharedCommandType.SendDialogToTarget, (agent_id, dialog_id, 0, 0)
            )
...
def _reenable_merchant_widgets() -> Generator:
    ...
    if _pending:
        ConsoleLog(
            BOT_NAME,
            f"[Merchant] EnableWidget timeout â€” {len(_pending)} message(s) unconfirmed. Proceeding.",
            PySystem.Console.MessageType.Warning,
        )
...
def _gh_merchant_setup(leave_party: bool = True) -> Generator:
    ...
    def _wait_for_alt_dispatch_completion(
        ...
        pending: dict[tuple[str, int], None] = {
            (acc_email, msg_index): None for acc_email, msg_index in message_refs if int(msg_index) >= 0
        }
        if not pending:
            return
    ...
    if leave_party:
        ...
        for acc in GLOBAL_CACHE.ShMem.GetAllAccountData():
            if acc.AccountEmail != _my_email:
                GLOBAL_CACHE.ShMem.SendMessage(
                    _my_email, acc.AccountEmail, SharedCommandType.LeaveParty, (0, 0, 0, 0), ("", "", "", "")
                )
    ...
    _expected_gh_alts = len([acc for acc in GLOBAL_CACHE.ShMem.GetAllAccountData() if acc.AccountEmail != _my_email])
    ...
    yield from _wait_for_alt_dispatch_completion(
        "travel_gh", _gh_refs, SharedCommandType.TravelToGuildHall, timeout_ms=10_000
    )
    ...
    while time.time() < _arrival_deadline:
        ...
        _in_gh = sum(
            1 for acc in _accounts if acc.AccountEmail != _my_email and int(acc.AgentData.Map.MapID) == _gh_map
        )
        ...
        ConsoleLog(
            BOT_NAME,
            f"[Merchant] GH arrival timeout â€” {_in_gh}/{_expected_gh_alts} alts at GH. Proceeding.",
            PySystem.Console.MessageType.Warning,
        )
    ...
    while _find_npc_xy_by_name("Merchant") is None:
        if time.time() > _npc_deadline:
            ConsoleLog(
                BOT_NAME,
                "[Merchant] Merchant NPC not found after 20s â€” proceeding anyway",
                PySystem.Console.MessageType.Warning,
            )
            break
        yield from Routines.Yield.wait(500)
    ...
    if _merchant_store_consumable_materials:
        ...
        yield from _wait_for_alt_dispatch_completion(
            "deposit_materials", deposit_refs, SharedCommandType.MerchantMaterials
        )
    ...
    if _merchant_sell_materials:
        if mat_xy:
            ...
            yield from _wait_for_alt_dispatch_completion(
                "sell_materials", sell_mat_refs, SharedCommandType.MerchantMaterials
            )
        else:
    ...
    if merchant_xy:
        ...
        yield from _wait_for_alt_dispatch_completion(
            "sell_scrolls", sell_scroll_refs, SharedCommandType.MerchantMaterials
        )
    ...
    if _merchant_buy_ectos and rare_xy:
        rx, ry = rare_xy
        ConsoleLog(
            BOT_NAME, f"[Merchant] Dispatching buy_ectoplasm to all alts (threshold={_merchant_ecto_threshold:,})"
        )
        ...
        if leader_storage > _merchant_ecto_threshold:
            ...
            ConsoleLog(
                BOT_NAME,
                f"[Merchant] Leader storage ({leader_storage:,}) at/below threshold â€” skipping leader ecto buy",
            )
        yield from _wait_for_alt_dispatch_completion(
            "buy_ectoplasm", buy_ecto_refs, SharedCommandType.MerchantMaterials
        )
...
def _resign_all_to_outpost_before_merchant() -> Generator:
    ...
    for acc in GLOBAL_CACHE.ShMem.GetAllAccountData():
        if acc.AccountEmail != my_email:
            GLOBAL_CACHE.ShMem.SendMessage(
                my_email, acc.AccountEmail, SharedCommandType.Resign, (0, 0, 0, 0), ("", "", "", "")
            )
...
def _summon_and_invite_party(settle_ms: int = 1000) -> Generator:
    ...
    while time.time() < _ld_deadline:
        ...
        ConsoleLog(
            BOT_NAME,
            "[Party] Leader ShMem did not update in time â€” proceeding anyway",
            PySystem.Console.MessageType.Warning,
        )
    ...
    if not _ld:
        ConsoleLog(
            BOT_NAME, "[Party] Could not read leader ShMem â€” skipping invite", PySystem.Console.MessageType.Warning
        )
        yield
        return
    ...
    _expected_alt_count = len([acc for acc in GLOBAL_CACHE.ShMem.GetAllAccountData() if acc.AccountEmail != _my_email])
    ...
    ConsoleLog(
        BOT_NAME,
        f"[Party] Waiting for {_expected_alt_count} alt(s) to arrive on map {_live_map} "
    ...
    while time.time() < _arrival_deadline:
        ...
        _arrived = sum(
            ...
            if (
                acc.AccountEmail != _my_email
                and int(acc.AgentData.Map.MapID) == _live_map
                and acc.AgentData.Map.Region == _ld.AgentData.Map.Region
                and acc.AgentData.Map.Language == _ld.AgentData.Map.Language
                and acc.AgentData.Map.District == _ld.AgentData.Map.District
            )
        )
        ...
        ConsoleLog(
            BOT_NAME,
            f"[Party] Arrival timeout â€” {_arrived}/{_expected_alt_count} on correct district. Proceeding.",
            PySystem.Console.MessageType.Warning,
        )
    ...
    for _attempt in range(3):
        ...
        if _actual >= _expected_size:
            ConsoleLog(
                BOT_NAME, f"[Party] Party full ({_actual}/{_expected_size}) after attempt {_attempt + 1}", log=True
            )
            yield
            return
        ...
        ConsoleLog(
            BOT_NAME,
            f"[Party] Party incomplete ({_actual}/{_expected_size}) â€” retrying",
            PySystem.Console.MessageType.Warning,
        )
        yield from Routines.Yield.wait(1500)
    ...
    ConsoleLog(
        BOT_NAME,
        f"[Party] Still incomplete after 3 attempts ({_actual}/{_expected_size}) â€” proceeding",
        PySystem.Console.MessageType.Warning,
    )
    yield
...
def _gh_merchant_setup_if_inventory_full() -> Generator:
    ...
    ConsoleLog(
        BOT_NAME,
        f"[Merchant] Inventory nearly full ({free_slots} free slot) â€” resigning to outpost then triggering GH merchant run",
    )
...
def _ensure_ini_initialized() -> bool:
    ...
    _merchant_alt_wait_ms = max(
        0, min(_MAX_ALT_SETTLE_WAIT_MS, _settings_ini.get_int(_M, "alt_wait_ms", _DEFAULT_ALT_SETTLE_WAIT_MS))
    )
...
def _record_run_end() -> Generator:
    ...
    if _t_run_start > 0 and _t_l2_start > 0:
        ...
        if run_time < _fastest_run:
            _fastest_run = run_time
        if run_time > _slowest_run:
            _slowest_run = run_time

        _l1_total_time += l1_time
        if l1_time < _l1_fastest:
            _l1_fastest = l1_time
        if l1_time > _l1_slowest:
            _l1_slowest = l1_time

        _l2_total_time += l2_time
        if l2_time < _l2_fastest:
            _l2_fastest = l2_time
        if l2_time > _l2_slowest:
            _l2_slowest = l2_time
...
def _take_dungeon_entry_snapshot() -> Generator:
    ...
    for acc in alt_accounts:
        ...
        if not responded:
            ...
            ConsoleLog(
                BOT_NAME, f"[FROGGY Stats] FROGGY snapshot timeout for: {name}", PySystem.Console.MessageType.Warning
            )
        ...
        if not responded:
            name = acc.AgentData.CharacterName or acc.AccountEmail
            ConsoleLog(
                BOT_NAME, f"[FROGGY Stats] GB snapshot timeout for: {name}", PySystem.Console.MessageType.Warning
            )
...
def _record_drops_after_loot() -> Generator:
    ...
    if alt_accounts:
        ...
        for acc in alt_accounts:
            ...
            if not responded:
                name = acc.AgentData.CharacterName or acc.AccountEmail
                ConsoleLog(
                    BOT_NAME, f"[FROGGY Stats] FROGGY count timeout for: {name}", PySystem.Console.MessageType.Warning
                )
            ...
            if not responded:
                name = acc.AgentData.CharacterName or acc.AccountEmail
                ConsoleLog(
                    BOT_NAME, f"[FROGGY Stats] GB count timeout for: {name}", PySystem.Console.MessageType.Warning
                )
    ...
    ConsoleLog(
        BOT_NAME, f"[FROGGY Stats] Run complete. FROGGY={total_froggy_this_run} GB={total_gb_this_run}", log=True
    )
    _save_settings()
    yield
...
def _draw_froggy_stats() -> None:
    ...
    if PyImGui.begin_table("##froggy_overview_layout", 2, layout_flags):
        ...
        if PyImGui.begin_table("##froggy_session", 2, tbl_flags):
            ...
            PyImGui.table_next_row(0, _ROW_H)
            PyImGui.table_set_column_index(0)
            _rtext(str(_session_runs))
            PyImGui.table_set_column_index(1)
            _rtext(str(session_total_froggy))
            PyImGui.end_table()
        ...
        if PyImGui.begin_table("##froggy_alltime", 3, tbl_flags):
            ...
            PyImGui.table_next_row(0, _ROW_H)
            PyImGui.table_set_column_index(0)
            _rtext(str(_total_runs))
            PyImGui.table_set_column_index(1)
            _rtext(str(alltime_total_froggy))
            PyImGui.table_set_column_index(2)
            _rtext(_runs_per_drop(_total_runs, alltime_total_froggy))
            PyImGui.end_table()
    ...
    if PyImGui.begin_table("##froggy_timings", 5, tbl_flags):
        ...
        for label, current, is_live, total, fastest, slowest in timing_rows:
            ...
            if is_live:
                PyImGui.table_set_column_index(1)
                _rtext_colored(_fmt_time(current), live)
            else:
                PyImGui.table_set_column_index(1)
                _rtext(_fmt_time(current))
            PyImGui.table_set_column_index(2)
            _rtext(_avg_time(total, _total_runs))
            PyImGui.table_set_column_index(3)
            _rtext(_fmt_time(fastest))
            PyImGui.table_set_column_index(4)
            _rtext(_fmt_time(slowest))
    ...
    if PyImGui.begin_table("##froggy_drops", 4, tbl_flags):
        ...
        for acct in all_accounts:
            ...
            PyImGui.table_next_row(0, _ROW_H)
            PyImGui.table_set_column_index(0)
            _ltext(_masked_email(acct))
            PyImGui.table_set_column_index(1)
            _rtext(str(s_count))
            PyImGui.table_set_column_index(2)
            _rtext(str(a_count))
            PyImGui.table_set_column_index(3)
            _rtext(_runs_per_drop(_total_runs, a_count))
        ...
        _ltext("Total")
        PyImGui.table_set_column_index(1)
        _rtext_colored(str(session_total), gold)
        PyImGui.table_set_column_index(2)
        _rtext_colored(str(alltime_acct_total), gold)
        PyImGui.table_set_column_index(3)
        _rtext_colored(_runs_per_drop(_total_runs, alltime_acct_total), gold)
...
def debug_nearby_gadgets(max_print: int = 10) -> Generator:
    ...
    for i, a in enumerate(gadgets[:max_print]):
        ...
        try:
            ConsoleLog(
                BOT_NAME, f"[BOGROOT] #{i} aid={aid} gadget_id={int(g.gadget_id)} extra_type={int(g.extra_type)}"
            )
        except Exception:
...
def _draw_merchant_settings() -> None:
    ...
    if _merchant_enabled:
        ...
        new_rare = PyImGui.checkbox(
            "Sell Diamond & Onyx to Rare Material Trader##froggy_rare_mats", _merchant_sell_rare_mats
        )
        if new_rare != _merchant_sell_rare_mats:
            _merchant_sell_rare_mats = new_rare
            _save_settings()

        new_ectos = PyImGui.checkbox(
            "Buy Glob of Ectoplasm when storage over threshold##froggy_ectos", _merchant_buy_ectos
        )
...
def _draw_froggy_window_with_stats_tab() -> None:
    ...
    if not bot.config.ini_key_initialized:
        bot.config.ini_key = Settings(
            f"{f"BottingClass/bot_{bot.config.bot_name}"}/{f"bot_{bot.config.bot_name}.ini"}", "account"
        ).name
        ...
        if PyImGui.begin_tab_bar(bot.config.bot_name + "_tabs"):
            if PyImGui.begin_tab_item("Main"):
                if PyImGui.begin_child(
                    f"{bot.config.bot_name} - Main", main_child_dimensions, True, PyImGui.WindowFlags.NoFlag
                ):
