...
import PySystem
import PyImGui
from Core import Key, Keystroke, Map, CHAR_MAP, Player
from Core import (
    GLOBAL_CACHE,
    Agent,
    Routines,
...
def custom_delete_character(character_name: str, timeout_ms: int = 45000):
    ...
    try:
        ...
        if delete_button.exists:
            ...
            final_delete = Frame(FrameId.ScreenFrame.Child.CharacterSelectFrame.DeleteCharacterFrame.DeleteButton)
...
def LogoutAndDeleteState():
    ...
    ConsoleLog("Reroll", f"Target='{char_name}' prof='{primary_prof}' camp='{campaign_name}'", Console.MessageType.Info)
    ...
    if not success:
        ConsoleLog(
            "Reroll", "Custom character deletion failed, falling back to framework method", Console.MessageType.Warning
        )
        try:
    ...
    if char_name in names:
        ...
        ConsoleLog(
            "Reroll", f"Original name still locked. Using generated name '{final_name}'.", Console.MessageType.Warning
        )
    else:
    ...
    if not success:
        ConsoleLog(
            "Reroll", "Custom character creation failed, falling back to framework method", Console.MessageType.Warning
        )
...
def RndTravelState(map_id: int, use_districts: int = 4):
    ...
    lang = language[idx]
    ConsoleLog(
        "RndTravel", f"MoveMap(map_id={map_id}, region={reg}, district=0, language={lang})", Console.MessageType.Info
    )
