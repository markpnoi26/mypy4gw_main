...
from Core.enums_src.Map_enums import name_to_map_id
...
class ConsSettings:
    ...
    _active: dict[str, bool] = {p: bool(_ini.get_bool(BOT_NAME, f"cons_{p}_active", True)) for p, _, _, _ in _CONS_DEFS}
    _restock: dict[str, int] = {p: int(_ini.get_int(BOT_NAME, f"cons_{p}_restock", dr)) for p, _, _, dr in _CONS_DEFS}
...
def _get_adapter():
    ...
    if _heroai_adapter_instance is None:
        from dev.reference.sch0l0ka.adapter.uw_heroai_adapter import UWHeroAIAdapter
...
class EnterSettings:
    ...
    EntryPoint: str = str(
        _ini.get_str(BOT_NAME, "enter_entrypoint", DEFAULT_UW_ENTRYPOINT_KEY) or DEFAULT_UW_ENTRYPOINT_KEY
    )
...
def _restart_main_loop(bot_instance: Botting, reason: str) -> None:
    ...
    try:
        if target:
            ...
            ConsoleLog(
                BOT_NAME,
                "[WIPE] MAIN_LOOP header missing, restarting from first state.",
        ...
        ConsoleLog(
            BOT_NAME,
            f"[WIPE] Header '{target}' not found, restarting from first state.",
    ...
    def _set_team_flags() -> None:
        ...
        for acct in all_accounts:
            ...
            if email and email.lower() not in known:
                ...
                ConsoleLog(
                    BOT_NAME, f"[Imprisoned] Auto-assigned '{email}' to right team.", PySystem.Console.MessageType.Info
                )
        ...
        for email in left_emails:
            ...
            ConsoleLog(
                BOT_NAME, f"[Imprisoned] Left  [{cb_idx}] {email} \u2192 ({x},{y})", PySystem.Console.MessageType.Info
            )
            cb_idx += 1
            left_pt += 1
        ...
        for email in right_emails:
            ...
            assignments.append((email, cb_idx, float(x), float(y)))
            ConsoleLog(
                BOT_NAME, f"[Imprisoned] Right [{cb_idx}] {email} \u2192 ({x},{y})", PySystem.Console.MessageType.Info
            )
            cb_idx += 1
            right_pt += 1
        ...
        for email in right_overflow:
            ...
            ConsoleLog(
                BOT_NAME,
                f"[Imprisoned] Overflow→Left  [{cb_idx}] {email} \u2192 ({x},{y})",
        ...
        for email in left_overflow:
            ...
            ConsoleLog(
                BOT_NAME,
                f"[Imprisoned] Overflow→Right [{cb_idx}] {email} \u2192 ({x},{y})",
                PySystem.Console.MessageType.Info,
            )
            cb_idx += 1
            right_pt += 1
        ...
        ConsoleLog(
            BOT_NAME, f"[Imprisoned] Flagged {len(assignments)} account(s) total.", PySystem.Console.MessageType.Info
        )
...
def EnqueueDialogUntilQuestActive(
    ...
    def _coro_ensure_quest_active() -> Generator[Any, Any, None]:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            if int(Quest.GetActiveQuest()) == target_quest_id:
                _append_debug_watchdog_log(f"Quest {target_quest_id} confirmed before retry {attempt}.")
                return
            ...
            if int(Quest.GetActiveQuest()) == target_quest_id:
                _append_debug_watchdog_log(f"Quest {target_quest_id} accepted after retry {attempt}.")
                return
        ...
        _append_debug_watchdog_log(f"Quest {target_quest_id} still not active after retries.")
...
def _move_with_unstuck(
    ...
    def _coro():
        ...
        from Core.Pathing import AutoPathing, AStar, AStarNode, densify_path2d
        from Core.EnemyBlacklist import EnemyBlacklist
        ...
        while True:
            ...
            bl = EnemyBlacklist()
            avoid_pts = [
                Agent.GetXY(eid) for eid in AgentArray.GetEnemyArray() if bl.is_blacklisted(eid) and Agent.IsAlive(eid)
            ]

            enemy_nearby = any(math.hypot(px - ax, py - ay) <= 500.0 for ax, ay in avoid_pts)
...
def FocusKeeperOfSouls(bot_instance: Botting):
    ...
    def _focus_logic():
        enemies = [
            e for e in AgentArray.GetEnemyArray() if Agent.IsAlive(e) and Agent.GetModelID(e) == KeeperOfSoulsModelID
        ]
...
def _coro_skeleton_dhuum_watchdog(bot: Botting):
    ...
    while True:
        ...
        skeletons = [
            e
            for e in AgentArray.GetEnemyArray()
            if Agent.IsAlive(e)
...
def _coro_dhuum_spirit_form_watchdog(bot: Botting):
    ...
    while True:
        ...
        for account in GLOBAL_CACHE.ShMem.GetAllAccountData() or []:
            ...
            try:
                has_spirit_form = any(
                    b.SkillId == _SPIRIT_FORM_SKILL_ID for b in account.AgentData.Buffs.Buffs if b.SkillId != 0
                )
            except Exception:
            ...
            try:
                ...
                if email not in _saved_flag_positions:
                    ...
                    _append_debug_watchdog_log(
                        f"Spirit Form detected -> {email} saved=({cur[0]:.0f}, {cur[1]:.0f})"
                        if cur
                        else f"Spirit Form detected -> {email} (no saved flag)"
                    )
...
def Enter_UW(bot_instance: Botting):
    from Core.routines_src.behaviourtrees_src.botting_interaction import add_use_item_state
    from Core.routines_src.behaviourtrees_src.botting_movement import add_wait_map_change_state
    from Core.routines_src.behaviourtrees_src.botting_multibox import add_leave_party_state
    ...
    def _travel_to_entrypoint() -> None:
        import random
        from Core.enums_src.Region_enums import District
    ...
    bot_instance.Wait.UntilCondition(lambda: Routines.Checks.Map.MapValid() and Map.GetMapID() == _enter_ep[1])
...
def enable_default_party_behavior(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(
        lambda: _toggle_move_to_party_member_if_dead(True), "Enable MoveToPartyMemberIfDead"
    )
...
def Clear_the_Chamber(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().set_party_leader(Player.GetAccountEmail()), "Set Party Leader"
    )
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        dialog_id=0x806501,
        quest_id=UWQuestID.ClearTheChamber,
        model_id=UWNpcModelID.LostSoul,
        step_name="Take Clear the Chamber quest",
    )
    bot_instance.Multibox.SendDialogToTarget(0x806501)
    bot_instance.States.AddCustomState(lambda: _get_adapter().set_combat_enabled(True), "Enable Combat")
    bot_instance.Move.XY(769, 6564, "Prepare to clear the chamber")
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().set_forced_state(BehaviorState.CLOSE_TO_AGGRO),
    ...
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().set_forced_state(None),
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        dialog_id=0x806D01,
        quest_id=UWQuestID.RestoringGrenthsMonuments,
        model_id=UWNpcModelID.ReaperOfTheLabyrinth,
        step_name="Take Restore Monuments quest",
    )
    bot_instance.Wait.ForTime(3000)
...
def Pass_The_Mountains(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(
        lambda: _record_quest_done("Pass the Mountains"), "Record Pass the Mountains done"
    )
...
def Deamon_Assassin(bot_instance: Botting):
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806801,
        int(UWQuestID.DemonAssassin),
        int(UWNpcModelID.ReaperOfTheTwinSerpentMountains),
        "take Deamon Assassin quest",
    )
...
def Restore_Planes(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(
        lambda: __import__("Core.EnemyBlacklist", fromlist=["EnemyBlacklist"])
        .EnemyBlacklist()
        .add_name("banished dream rider"),
    ...
    bot_instance.States.AddCustomState(
        lambda: __import__("Core.EnemyBlacklist", fromlist=["EnemyBlacklist"])
        .EnemyBlacklist()
        .remove_name("banished dream rider"),
...
def The_Four_Horsemen(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(
        lambda: _toggle_move_to_party_member_if_dead(False), "Disable MoveToPartyMemberIfDead"
    )
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().set_forced_state(BehaviorState.CLOSE_TO_AGGRO),
        "Force Close_to_Aggro",
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806A01,
        int(UWQuestID.TheFourHorsemen),
        int(UWNpcModelID.ReaperOfTheChaosPlanes),
        "take Foure Horsemen quest",
    )
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().set_forced_state(None),
        "Release Close_to_Aggro",
...
def Terrorweb_Queen(bot_instance: Botting):
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806B01,
        int(UWQuestID.TerrorwebQueen),
        int(UWNpcModelID.ReaperOfTheChaosPlanes),
        "take Terrorweb Queen quest",
    )
...
def Imprisoned_Spirits(bot_instance: Botting):
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806901,
        int(UWQuestID.ImprisonedSpirits),
        int(UWNpcModelID.ReaperOfTheBonePits),
        "take Imprisoned Spirits quest",
    )
    ...
    bot_instance.Wait.UntilCondition(lambda: time.monotonic() - _is_timer[0] >= 28.0)
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().clear_flags(),
        "Clear Flags",
    )
    ...
    bot_instance.Wait.UntilCondition(lambda: time.monotonic() - _is_timer[0] >= 90.0)
    ...
    bot_instance.States.AddCustomState(
        lambda: _record_quest_done("Imprisoned Spirits"), "Record Imprisoned Spirits done"
    )
...
def Restore_Vale(bot_instance: Botting):
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806C01,
        int(UWQuestID.EscortOfSouls),
        int(UWNpcModelID.ReaperOfTheLabyrinth),
        "take Escort of Souls quest",
    )
...
def Wrathfull_Spirits(bot_instance: Botting):
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806E01,
        int(UWQuestID.WrathfulSpirits),
        int(UWNpcModelID.ReaperOfTheLabyrinth),
        "take Wrathfull Spirits quest",
    )
...
def Unwanted_Guests(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(lambda: _get_adapter().set_following_enabled(False), "Disable Following")
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().set_forced_state(BehaviorState.CLOSE_TO_AGGRO), "Force Close_to_Aggro"
    )
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806701,
        int(UWQuestID.UnwantedGuests),
        int(UWNpcModelID.ReaperOfTheLabyrinth),
        "take Unwanted Guests quest",
    )
...
def Servants_of_Grenth(bot_instance: Botting):
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x806601,
        int(UWQuestID.ServantsOfGrenth),
        int(UWNpcModelID.ReaperOfTheBonePits),
        "take Servants of Grenth quest",
    )
    bot_instance.States.AddCustomState(
        lambda: _get_adapter().set_forced_state(None),
        "Release Close_to_Aggro",
    ...
    bot_instance.States.AddCustomState(
        lambda: _record_quest_done("Servants of Grenth"), "Record Servants of Grenth done"
    )
...
def Dhuum(bot_instance: Botting):
    ...
    def _flag_sacrifice_accounts() -> None:
        ...
        if not sacrifice_emails:
            ConsoleLog(
                BOT_NAME,
                "[Dhuum] No sacrifice accounts configured in Dhuum settings.",
                PySystem.Console.MessageType.Warning,
            )
            return
        ...
        if not cb_flagged_emails:
            ConsoleLog(
                BOT_NAME, "[Dhuum] No sacrifice accounts found in shared memory.", PySystem.Console.MessageType.Warning
            )
            return
    ...
    def _flag_survivor_accounts() -> None:
        ...
        sacrifice_offset = sum(1 for account in all_accounts if str(account.AccountEmail) in sacrifice_emails)
    ...
    def _coro_follow_king_to_destination():
        ...
        while time.time() < deadline:
            king_id = next(
                (
                    a
                    for a in AgentArray.GetAgentArray()
                    if Agent.IsValid(a) and int(Agent.GetModelID(a)) == _KING_MODEL_ID
                ),
                None,
            )
        ...
        ConsoleLog(
            BOT_NAME,
            "[Dhuum] Timed out waiting for the King - continuing anyway.",
            PySystem.Console.MessageType.Warning,
        )
    ...
    EnqueueDialogUntilQuestActive(
        bot_instance,
        0x846901,
        int(UWQuestID.TheNightmareCometh),
        int(UWNpcModelID.KingFrozenwind),
        "Take The Nightmare Cometh quest",
    )
    ...
    def _enough_spiritforms() -> bool:
        ...
        for acct in GLOBAL_CACHE.ShMem.GetAllAccountData() or []:
            if getattr(acct.AgentData.Map, "MapID", 0) != UW_MAP_ID:
                continue
            try:
                if any(b.SkillId == _SPIRIT_FORM_SKILL_ID_CHECK for b in acct.AgentData.Buffs.Buffs if b.SkillId != 0):
                    count += 1
            except Exception:
                pass
            if count >= threshold:
    ...
    def _loot_underworld_chest():
        ...
        if chest_id is None:
            ConsoleLog(
                BOT_NAME, "[Dhuum] Underworld Chest not found for looting!", PySystem.Console.MessageType.Warning
            )
            return
        ...
        current_map_id = Map.GetMapID()
        all_accounts = GLOBAL_CACHE.ShMem.GetAllAccountData()
        same_map_accounts = [acc for acc in all_accounts if acc.AgentData.Map.MapID == current_map_id]

        ConsoleLog(
            BOT_NAME,
            f"[Dhuum] Looting chest with {len(same_map_accounts)} account(s)",
            PySystem.Console.MessageType.Info,
        )
        ...
        for account in same_map_accounts:
            ...
            if msg_index < 0:
                ConsoleLog(
                    BOT_NAME,
                    f"[Dhuum] Failed to send InteractWithTarget to {email}",
                    PySystem.Console.MessageType.Warning,
                )
            else:
                ConsoleLog(
                    BOT_NAME, f"[Dhuum] Sent InteractWithTarget (chest) to {email}", PySystem.Console.MessageType.Info
                )
            yield from Routines.Yield.wait(5000)
    ...
    def _coro_merchant_rules_refill():
        ...
        if followers:
            PySystem.Console.Log(
                BOT_NAME,
                f"Sent EnableWidget '{_MR_WIDGET_NAME}' to {len(followers)} follower(s).",
                PySystem.Console.MessageType.Info,
            )
            yield from Routines.Yield.wait(1000)
        ...
        if not Map.IsGuildHall():
            PySystem.Console.Log(
                BOT_NAME, "Traveling to Guild Hall for MerchantRules.", PySystem.Console.MessageType.Info
            )
            Map.TravelGH()
        ...
        if followers:
            PySystem.Console.Log(
                BOT_NAME,
                f"Sending TravelToGuildHall to {len(followers)} follower(s).",
                PySystem.Console.MessageType.Info,
            )
            for account in followers:
        ...
        PySystem.Console.Log(
            BOT_NAME,
            f"Waiting for all accounts to arrive in Guild Hall (map {leader_map_id}).",
            PySystem.Console.MessageType.Info,
        )
        elapsed = 0
        all_arrived = False
        while elapsed < _GH_TRAVEL_TIMEOUT_MS:
            ...
            all_arrived = all(int(acc.AgentData.Map.MapID) == leader_map_id for acc in all_accounts_fresh)
            if all_arrived:
                break
            yield from Routines.Yield.wait(_POLL_MS)
            elapsed += _POLL_MS
        ...
        if not all_arrived:
            PySystem.Console.Log(
                BOT_NAME,
                "Not all accounts reached the Guild Hall. Continuing anyway.",
                PySystem.Console.MessageType.Warning,
            )
        ...
        PySystem.Console.Log(
            BOT_NAME, "Starting MerchantRules Execute Here (leader).", PySystem.Console.MessageType.Info
        )
        ...
        if widget.execution_running:
            PySystem.Console.Log(
                BOT_NAME, "MerchantRules leader execution timed out.", PySystem.Console.MessageType.Warning
            )
        elif widget.last_error:
            PySystem.Console.Log(
                BOT_NAME, f"MerchantRules leader error: {widget.last_error}", PySystem.Console.MessageType.Warning
            )
        else:
            PySystem.Console.Log(
                BOT_NAME, "MerchantRules leader execution completed.", PySystem.Console.MessageType.Info
            )
        ...
        from Core.routines_src.behaviourtrees_src.botting_multibox import outbound_messages_done
        ...
        if not all_done:
            PySystem.Console.Log(
                BOT_NAME,
                "MerchantRules follower execution timed out on some accounts.",
                PySystem.Console.MessageType.Warning,
            )
        else:
            PySystem.Console.Log(
                BOT_NAME,
                "MerchantRules follower execution completed on all accounts.",
                PySystem.Console.MessageType.Info,
            )
...
def _enqueue_cons_restock(bot_instance: Botting) -> None:
    ...
    _inactive_pcon_qtys: dict[str, int] = {
        p: ConsSettings.get_restock(p) for p, _, _, _ in _CONS_DEFS if not ConsSettings.is_active(p)
    }
...
def _draw_help():
    ...
    PyImGui.text_wrapped(
        "For the Imprisoned Spirits quest, 1 or 2 durable damage dealers are recommended for the left team. You need to figure out which ones."
    )
...
def _draw_inventory_settings() -> None:
    ...
    PyImGui.separator()
    PyImGui.text_wrapped(
        "Travels all accounts to the Guild Hall" "Configure buy/sell/deposit rules " "in the MerchantRules widget."
    )
    PyImGui.separator()
...
def _draw_imprisoned_spirits_settings() -> None:
    PyImGui.text_wrapped("Assign each multibox account to the Left or Right team for the Imprisoned Spirits quest.")
    PyImGui.separator()
    ...
    if PyImGui.begin(f"Armor Setup: {_armor_edit_char}##armor_edit", win_flags):
        ...
        if PyImGui.begin_table("##armor_edit_tbl", 3, tbl_flags, 0.0, 0.0):
            ...
            for slot in sorted(_ARMOR_SLOT_NAMES):
                ...
                _armor_edit_normal[slot_str] = _input_int_val(PyImGui.input_int(f"##n{slot}", cur_n, 0, 0, 0), cur_n)
                PyImGui.table_next_column()
                cur_s = _armor_edit_sac.get(slot_str, 0)
                _armor_edit_sac[slot_str] = _input_int_val(PyImGui.input_int(f"##s{slot}", cur_s, 0, 0, 0), cur_s)
...
def _draw_quest_settings():
    ...
    if new_mode != BotSettings.BotMode:
        ...
        ConsoleLog(
            BOT_NAME,
            f"[Settings] Bot mode switched to '{new_mode}' — FSM will rebuild on next Start.",
            PySystem.Console.MessageType.Info,
        )
...
def _draw_debug_settings():
    ...
    try:
        ...
        for account in accounts:
            ...
            has_buff = any(b.SkillId == _SPIRIT_FORM_SKILL_ID for b in account.AgentData.Buffs.Buffs if b.SkillId != 0)
            if not has_buff:
                continue
            found_any = True
...
def _on_party_wipe(bot: "Botting"):
    ...
    while True:
        ...
        if not Routines.Checks.Map.MapValid():
            ConsoleLog(
                BOT_NAME,
                "[WIPE] Returned to outpost after wipe, restarting run...",
                PySystem.Console.MessageType.Warning,
            )
            yield from Routines.Yield.wait(3000)
        ...
        if not Agent.IsDead(player_id):
            ConsoleLog(
                BOT_NAME, "[WIPE] Player resurrected in instance, resuming...", PySystem.Console.MessageType.Info
            )
    ...
    try:
        ...
        if bot.config.fsm_running and Routines.Checks.Map.MapValid():
            ...
            if _entered_dungeon and Map.GetMapID() == 138:
                ConsoleLog(
                    BOT_NAME,
                    "[WIPE] Watchdog: back in outpost (map 138) without wipe callback — restarting.",
                    PySystem.Console.MessageType.Warning,
                )
