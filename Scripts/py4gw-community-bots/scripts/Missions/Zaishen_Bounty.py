...
def bot_routine(bot: Botting) -> None:
    ...
    for b_idx, bounty in enumerate(_queued_bounties):
        ...
        bot.States.AddCustomState(lambda idx=b_idx: _set_current_index(idx), f"SetBountyIndex_{b_idx}")
        ...
        if has_outpost_path and has_explorable:
            if transit_count > 0:
                ...
                for i in range(transit_count):
                    ...
                    bot.States.AddCustomState(
                        lambda bi=b_idx, ti=i, tc=t_coord: _set_section_header(_section_headers[bi][ti], tc[0], tc[1]),
            ...
            bot.UI.PrintMessageToConsole(
                BotSettings.BOT_NAME, f"No outpost exit path. Executing transit paths from outpost."
            )
            for i in range(transit_count):
                t_coord = _get_first_path_coord(bounty.transit_paths[i])
                bot.States.AddCustomState(
        ...
        bot.States.AddCustomState(
            lambda bi=b_idx, bc=bp_coord: _set_section_header(_section_headers[bi][-1], bc[0], bc[1]),
        ...
        if is_last:
            ...
            if _loop_queue:
                ...
                bot.States.AddCustomState(lambda h=first_bounty_header: _do_loop_jump(bot, h), f"DoLoopJump")
        else:
            bot.UI.PrintMessageToConsole(
                BotSettings.BOT_NAME, f"Bounty SUCCESS: {bounty.display}. Moving to next Bounty."
            )
            bot.Multibox.ResignParty()
            bot.Wait.ForTime(1000)
            bot.Wait.UntilOnOutpost()
...
def _on_party_wipe(bot: "Botting"):
    ...
    if Map.IsOutpost():
        ...
        ConsoleLog("on_party_wipe", f"Resurrected in outpost. Re-executing bounty. Jumping to: {target}")
        if bot.config.FSM.current_state:
            bot.config.FSM.current_state.reset()
...
def _draw_settings():
    ...
    PyImGui.separator()
    bounties = sorted([f[:-3] for f in os.listdir(BOUNTIES_DIR) if f.endswith(".py")])
