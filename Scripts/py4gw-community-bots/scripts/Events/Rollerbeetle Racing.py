...
FSM_vars.state_machine.AddState(
    name="Waiting for Map Ready", exit_condition=lambda: Map.IsMapReady(), transition_delay_ms=1000
)
...
FSM_vars.wait_for_dishonorable_removal.AddState(
    ...
    exit_condition=lambda: not DoesPlayerHaveEffect(
        SKILL_DISHONORABLE
    ),  # Wait until the player no longer has Dishonorable effect
    transition_delay_ms=1000,
)
...
FSM_vars.state_machine.AddSubroutine(
    ...
    condition_fn=lambda: not Map.GetMapID() == ROLLERBEETLE_RACING_OUTPOST_ID
    or (Map.GetMapID() == ROLLERBEETLE_RACING_OUTPOST_ID and Map.IsOutpost() and Map.IsMapReady()),
)
...
FSM_vars.state_machine.AddState(
    name="Waiting for Race to Start",
    execute_fn=lambda: set_waiting_routine(),
    transition_delay_ms=28100,
    exit_condition=lambda: end_waiting_routine(),
)

FSM_vars.state_machine.AddState(name="Starting Race", execute_fn=lambda: set_running_routine())
...
FSM_vars.state_machine.AddState(name="Updating Stats", execute_fn=lambda: UpdateBotStats(), transition_delay_ms=1000)

FSM_vars.state_machine.AddState(
    name="Waiting for Map Load", exit_condition=lambda: Map.IsMapLoading(), transition_delay_ms=1000
)
    ...
    try:
        ...
        if PyImGui.begin(bot_vars.window_module.window_name, bot_vars.window_module.window_flags):
            ...
            if IsBotStarted():
                ...
                if not Map.GetMapID() == ROLLERBEETLE_RACING_OUTPOST_ID and not Map.IsMapUnlocked(
                    ROLLERBEETLE_RACING_OUTPOST_ID
                ):
            ...
            average_medals = (
                (bot_vars.current_medals - bot_vars.initial_medals) / bot_vars.run_count
                if bot_vars.run_count > 0
                else 0
            )
...
def UseSkills():
    ...
    if (
        not DoesPlayerHaveEffect(SKILL_SUPER_ROLLERBEETLE)
        and not DoesPlayerHaveEffect(SKILL_ROLLERBEETLE_DASH)
        and not DoesPlayerHaveEffect(SKILL_RAM)
        and skill_1_data.recharge == 0
    ):
        SkillBar.UseSkill(1)
        return
