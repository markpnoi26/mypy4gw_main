...
def _find_best_target():
    my_pos = Player.GetXY()
    enemies = [
        e for e in AgentArray.GetEnemyArray() if Agent.IsAlive(e) and Utils.Distance(my_pos, Agent.GetXY(e)) <= 5000
    ]
    if not enemies:
        return None
    best_target = None
...
def isAgentInDanger(agentId, aggro_area=Range.Spellcast, aggressive_only=False):
    ...
    enemy_array = AgentArray.Filter.ByCondition(
        enemy_array, lambda agent_id: Utils.Distance(Agent.GetXY(agentId), Agent.GetXY(agent_id)) <= aggro_area.value
    )
...
def draw_window(bot: Botting):
    ...
    if BotSettings.STATS_FOR_NERDS:
        ...
        if PyImGui.begin("Snowball Dominance Stats"):
            ...
            wr_color = (
                (0, 255, 0, 255) if win_rate >= 80 else (255, 255, 0, 255) if win_rate >= 50 else (255, 50, 50, 255)
            )
            ...
            gold_color = (
                (255, 150, 0, 255) if current_gold >= BotSettings.GOLD_THRESHOLD_DEPOSIT else (255, 215, 0, 255)
            )
