...
from Core.Builds.Any.KeiranThackerayEOTN import KeiranThackerayEOTN
from Core.Builds.Any.HeroAI import HeroAI_Build
from Core.ImGui_src.types import Alignment
from Core.enums_src.UI_enums import UIMessage
...
def QuestLoop(quest_id, quest_x, quest_y, quest_dialog, mode="accept", quest_npc=0, multi=0):
    ...
    if success_condition():
        ...
        ConsoleLog(
            label,
            f"The bot attempted and failed to {action_inf} quest #{quest_id}. The bot has stopped.",
...
_MAX_ARMOR_DATA = {
    ...
    "Monk": {  # Shinjea armor â€” crafter: Ryoko
        ...
        "buy_rare": [
            (ModelID.Roll_Of_Parchment.value, 5),
            (ModelID.Vial_Of_Ink.value, 4),
            (ModelID.Bolt_Of_Linen.value, 28),
        ],
        "pieces": [
...
def VerifyMaxArmorMaterials(bot: Botting) -> bool:
    ...
    if missing:
        for model_id, current, needed in missing:
            ConsoleLog(
                "CraftMaxArmor",
                f"Missing material {model_id}: have {current}, need {needed}.",
...
def DoCraftMaxArmor(bot: Botting):
    if any(item_id == 0 for item_id, _, _ in _get_max_armor_data()["pieces"]):
        ConsoleLog(
            "CraftMaxArmor", "Missing armor piece mapping for current profession.", PySystem.Console.MessageType.Error
        )
        bot.helpers.Events.on_unmanaged_fail()
        return False
...
def DoCraftShortbow(bot: Botting):
    for weapon_id, mats, qtys in _SHORTBOW_DATA["pieces"]:
        ...
        if not result:
            ConsoleLog(
                "DoCraftShortbow", f"Failed to craft shortbow ({weapon_id}).", PySystem.Console.MessageType.Error
            )
            bot.helpers.Events.on_unmanaged_fail()
            return False
        yield
        ...
        if not result:
            ConsoleLog(
                "DoCraftShortbow", f"Failed to equip shortbow ({weapon_id}).", PySystem.Console.MessageType.Error
            )
            bot.helpers.Events.on_unmanaged_fail()
            return False
        yield
...
def destroy_starter_armor_and_useless_items() -> Generator[Any, Any, None]:
    ...
    if primary == "Assassin":
        starter_armor = [7251, 7249, 7250, 7252, 7248]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Ritualist":
        starter_armor = [11332, 11330, 11331, 11333, 11329]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Warrior":
        starter_armor = [10174, 10172, 10173, 10175, 10171]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Ranger":
        starter_armor = [10623, 10621, 10622, 10624, 10620]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Monk":
        starter_armor = [9725, 9723, 9724, 9726, 9722]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Elementalist":
        starter_armor = [9324, 9322, 9323, 9325, 9321]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Mesmer":
        starter_armor = [8026, 8024, 8025, 8054, 8023]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Necromancer":
        starter_armor = [8863, 8861, 8862, 8864, 8860]  # Head  # Chest  # Gloves  # Pants  # Boots
...
def Unlock_Xunlai_Storage(bot: Botting) -> None:
    ...
    path_to_xunlai: List[Tuple[float, float]] = [
        (-4958, 9472),
        (-5465, 9727),
        (-4791, 10140),
        (-3945, 10328),
...
def Attribute_Points_Quest_1(bot: Botting):
    ...
    exit_function = lambda: (
        ...
        and Agent.HasQuest(Routines.Agents.GetAgentIDByModelID(3093))
    )
    bot.Move.FollowModel(
        3093, follow_range=(Range.Area.value), exit_condition=exit_function
    )  # Guard model id updated 20.12.2025 GW Reforged
...
def _ensure_bonus_bow(bot: Botting):
    ...
    if (
        BotSettings.CUSTOM_BOW_ID != 0
        or Routines.Checks.Inventory.IsModelInInventory(BotSettings.CRAFTED_BOW_ID)
        or Routines.Checks.Inventory.IsModelEquipped(BotSettings.CRAFTED_BOW_ID)
    ):
        yield
        return
...
def Unlock_Eye_Of_The_North_Pool(bot: Botting):
    ...
    bot.Dialogs.WithModel(
        6021, 0x0000008A
    )  # Gwen dialog to obtain Keiran's bow. Model id updated 20.12.2025 GW Reforged
    ...
    def _level_check_entry():
        ...
        if level >= 20:
            ...
            if not target_step:
                ConsoleLog(
                    "FarmUntil20",
                    f"[Farm] Target state not found: {NEXT_STEP_AFTER_FARM}",
                    PySystem.Console.MessageType.Error,
                )
                yield
                return
    ...
    def _enter_ab_quest(bot: Botting):
        ...
        while not PyDialog.PyDialog.is_dialog_active():
            if time.time() > deadline:
                ConsoleLog(
                    MODULE_NAME, "[FarmAB] Timed out waiting for Keiran's dialog", PySystem.Console.MessageType.Warning
                )
                return
        ...
        ConsoleLog(
            MODULE_NAME, f"[FarmAB] base={hex(base_id)} offset={hex(_AB_DIALOG_OFFSET)} -> sending {hex(target_id)}"
        )
    ...
    def _level_check_and_loop():
        ...
        if level < 20:
            ...
            if not target_step:
                ConsoleLog(
                    "FarmUntil20",
                    f"[Farm] Target state not found: {FARM_PREPARE_STEP}",
                    PySystem.Console.MessageType.Error,
                )
                yield
                return
...
def Attribute_Points_Quest_2(bot: Botting):
    ...
    wait_function = lambda: (not (Routines.Checks.Agents.InDanger(aggro_area=Range.Spirit)))
...
def To_Lions_Arch(bot: Botting):
    ...
    bot.Map.Travel(target_map_id=KAINENG_CENTER_MAP_ID)
    auto_path_list = [(3049.35, -2020.75), (2739.30, -3710.67), (-648.30, -3493.72), (-1661.91, -636.09)]
    bot.Move.FollowAutoPath(auto_path_list)
...
def To_Kamadan(bot: Botting):
    ...
    bot.Move.FollowAutoPath(auto_path_list)
    bot.Wait.ForTime(10000)
    auto_path_list = [(1352.94, -3694.75), (2547.49, -3667.82), (2541.67, -2582.88)]  # Critical part, high aggro area
    bot.Move.FollowAutoPath(auto_path_list)
    bot.Wait.ForTime(10000)
    bot.Move.XY(1990.27, -1636.21)
    bot.Wait.ForTime(15000)
    auto_path_list = [(2651.48, -3750.63), (3355.63, -2151.82)]  # Critical part, high aggro area
    bot.Move.FollowAutoPath(auto_path_list)
    bot.Wait.ForTime(10000)
...
def Unlock_Remaining_Secondary_Professions(bot: Botting):
    ...
    if primary == "Warrior":
        ...
        bot.Dialogs.WithModel(
            201, 0x684
        )  # Elementalist trainer - Model ID 201. Model id updated 20.12.2025 GW Reforged
...
WAYPOINTS: dict[int, WaypointData] = {
    ...
    1: WaypointData(
        label="Forming A Party", MapID=242, step_name="[H]Quest: Forming A Party_2", section="Shing Jea Island"
    ),
    ...
    14: WaypointData(
        ...
        MapID=KAINENG_CENTER_MAP_ID,
        step_name="[H]To Marketplace_21",
        section="Factions Mainland",
    ),
    15: WaypointData(label="Craft Max Armor", MapID=KAINENG_CENTER_MAP_ID, step_name="[H]Craft max armor_23"),
    16: WaypointData(
        label="Quest: The Search For A Cure",
        MapID=KAINENG_CENTER_MAP_ID,
        step_name="[H]Quest: The Search For A Cure_25",
    ),
    17: WaypointData(
        label="Quest: A Master's Burden", MapID=KAINENG_CENTER_MAP_ID, step_name="[H]Quest: A Master's Burden_26"
    ),
    ...
    19: WaypointData(
        label="To Eye of the North", MapID=675, step_name="[H]To Eye of the North_28", section="Eye of the North"
    ),
    ...
    27: WaypointData(
        label="To Lion's Arch",
        MapID=KAINENG_CENTER_MAP_ID,
        step_name="[H]To Lion's Arch_38",
        section="Prophecies / NF / GToB",
    ),
    ...
    30: WaypointData(
        label="Unlock Secondary Professions", MapID=449, step_name="[H]Unlock remaining secondary professions_42"
    ),
...
def on_party_wipe(bot: "Botting"):
    ...
    fsm.AddManagedCoroutine(
        f"{fsm.get_state_name_by_number(current_step)}_OPD", on_party_wipe_coroutine(bot, target_name)
    )
...
def _draw_texture():
    ...
    level = Agent.GetLevel(Player.GetAgentID())
    path = os.path.join(
        PySystem.Console.get_projects_path(), "Sources", "ApoSource", "textures", "factions_leveler_art.png"
    )
    ...
    if level <= 5:
        ImGui.DrawTextureExtended(
            texture_path=path, size=size, uv0=(0.0, 0.0), uv1=(0.25, 1.0), tint=tint, border_color=border_col
        )
    elif level <= 8:
        ImGui.DrawTextureExtended(
            texture_path=path, size=size, uv0=(0.25, 0.0), uv1=(0.5, 1.0), tint=tint, border_color=border_col
        )
    elif level <= 9:
        ImGui.DrawTextureExtended(
            texture_path=path, size=size, uv0=(0.5, 0.0), uv1=(0.75, 1.0), tint=tint, border_color=border_col
        )
    else:
        ImGui.DrawTextureExtended(
            texture_path=path, size=size, uv0=(0.75, 0.0), uv1=(1.0, 1.0), tint=tint, border_color=border_col
        )
...
def _draw_readme():
    ...
    PyImGui.text_colored(
        "  - Do not manually move the character or interact with the game while the bot is running.",
        (255, 100, 100, 255),
    )
...
def tooltip():
    ...
    PyImGui.text_wrapped(
        "This bot levels a Factions character from 1 to 20, completing key quests and unlocking important content along the way. It manages gold and consumables automatically, allowing you to sit back and watch your character grow."
    )
    PyImGui.spacing()
