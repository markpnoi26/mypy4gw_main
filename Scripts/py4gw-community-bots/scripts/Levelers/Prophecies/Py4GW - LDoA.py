...
import time, Py4GW
import traceback
from Core import Key, Map, ImGui, Botting, ActionQueue, Agent
...
bot_vars.window_module = ImGui.WindowModule(
    module_name, window_name, (300, 300), (0, 0), PyImGui.WindowFlags.AlwaysAutoResize
)
...
dull_carapaces_coordinate_list_2 = [
    (9780, -9295),
    (12204, -8720),
    (9406, -11301),
    (6718, -11119),
...
# GARGOYLE SKULLS
gargoyle_skull_coordinate_list = [
    (-6865, 16314),
    (-6110, 15653),
    (-3609, 18598),
    (319, 17870),
...
# GRAWL NECKLACES
grawl_necklace_coordinate_list = [
    (-7907, 1420),
    (-7604, -174),
    (-1761, 302),
    (1534, 427),
...
# UNNATURAL SEEDS
unnatural_seeds_coordinate_list = [
    (-7910, 1418),
    (-7555, -1752),
    (-8062, -3071),
    (-12424, -3125),
...
# NICHOLAS GIFTS
nicholas_sandford_coordinate_list = [
    (22237, 4061),
    (17213, 2817),
    (16320, 3621),
    (16757, 8255),
...
ranger_skill_coordinate_list_2 = [
    (1618, 7084),
    (2605, 9654),
    (4034, 10546),
    (7977, 9735),
...
def handle_map_path(map_pathing):
    ...
    if (
        FSM_vars.current_target_id is None
        or not Agent.IsAlive(FSM_vars.current_target_id)
        or not FSM_vars.current_target_id == FSM_vars.last_target_id
    ):
...
def handle_loot():
    ...
    agent_to_item_map = {agent_id: Agent.GetItemAgentItemID(agent_id) for agent_id in item_array}
    ...
    filtered_agent_ids = [agent_id for agent_id, item_id in agent_to_item_map.items() if item_id in filtered_items]
...
def handle_loot_baked_husk():
    ...
    try:
        ...
        if len(filtered_agent_ids) > 0:
            ...
            if (
                hasattr(FSM_vars, 'loot_target_start_time')
                and FSM_vars.loot_target_start_time > 0
                and time.time() - FSM_vars.loot_target_start_time > 10.0
            ):
                PySystem.Console.Log(
                    "Baked Husk Loot", "Loot target timeout - giving up", PySystem.Console.MessageType.Warning
                )
                FSM_vars.current_loot_target = None
        ...
        if hasattr(FSM_vars, 'current_loot_target'):
            FSM_vars.current_loot_target = None
        PySystem.Console.Log(
            "Baked Husk Loot", f"Error in loot handling: {str(e)}", PySystem.Console.MessageType.Warning
        )
...
def comprehensive_loot_sweep():
    ...
    try:
        ...
        PySystem.Console.Log(
            "Baked Husk Loot", f"Error in comprehensive sweep: {str(e)}", PySystem.Console.MessageType.Error
        )
    ...
    try:
        ...
        if FSM_vars.last_position == current_pos:
            if FSM_vars.position_stuck_time == 0.0:
                ...
                PySystem.Console.Log(
                    "Movement", "Bot appears stuck - resetting movement", PySystem.Console.MessageType.Warning
                )
                FSM_vars.movement_handler.reset()
                FSM_vars.current_loot_target = None
...
def handle_gadget_interaction():
    ...
    if not hasattr(FSM_vars, 'lever_targeted_logged'):
        PySystem.Console.Log(
            "Gadget Interaction", "Targeting lever with semicolon key", PySystem.Console.MessageType.Info
        )
    ...
    if not FSM_vars.has_interacted or (current_time - FSM_vars.last_interaction_time >= 3):
        if not hasattr(FSM_vars, 'lever_activated_logged'):
            PySystem.Console.Log(
                "Gadget Interaction", "Activating lever with spacebar", PySystem.Console.MessageType.Info
            )
...
FSM_vars.state_machine_warrior.AddState(
    name="COMMAND BONUS",
    execute_fn=lambda: Player.SendChatCommand(text_bonus),
...
FSM_vars.state_machine_warrior.AddState(
    name="EQUIP WAND",
    execute_fn=lambda: equipitem(6508, agent_id),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(
    name="EQUIP SHIELD",
    execute_fn=lambda: equipitem(6514, agent_id),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(
    name="GOING NEAR TOWN CRIER",
...
FSM_vars.state_machine_warrior.AddState(
    name="CHECK NPC", execute_fn=lambda: handle_npc_interaction(), transition_delay_ms=2000, run_once=True
)
FSM_vars.state_machine_warrior.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805001", 16)),
    transition_delay_ms=300,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(
    name="GOING NEAR SIR TYDUS",
...
FSM_vars.state_machine_warrior.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x805007", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DD01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(
    name="GOING OUT ASCALON",
    ...
    or (Map.IsMapReady() and Map.IsExplorable() and Party.IsPartyLoaded()),
    run_once=False,
)
FSM_vars.state_machine_warrior.AddState(
    name="WAITING EXPLORABLE MAP",
...
FSM_vars.state_machine_warrior.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DD07", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805501", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
...
FSM_vars.state_machine_warrior.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.warrior_quest_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_warrior.AddState(
    name="GOING BACK TO ASCALON",
...
FSM_vars.state_machine_warrior.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805507", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
# END WARRIOR ROUTINE
...
FSM_vars.state_machine_warrior.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804703", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(
    name="TAKING SKILLS",
    execute_fn=lambda: Player.SendDialog(int("0x804701", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior.AddState(name="USING IMP STONE", execute_fn=lambda: useitem(30847), run_once=True)
FSM_vars.state_machine_warrior.AddState(
    name="SECOND MAP PATH",
    execute_fn=lambda: warrior_handle_map_path(FSM_vars.leveling_pathing),
...
FSM_vars.state_machine_warrior.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802E01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
...
# START COMMON ROUTINE PART ONE
FSM_vars.state_machine_ranger.AddState(
    name="COMMAND BONUS",
    execute_fn=lambda: Player.SendChatCommand(text_bonus),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_ranger.AddState(
    name="EQUIP BOW",
    execute_fn=lambda: equipitem(5831, agent_id),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
    run_once=True,
)
...
FSM_vars.state_machine_ranger.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DE01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_ranger.AddState(
...
FSM_vars.state_machine_ranger.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DE07", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_ranger.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x805601", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
...
FSM_vars.state_machine_ranger.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.ranger_quest_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_ranger.AddState(
...
FSM_vars.state_machine_ranger.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.artemis_pathing_1, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_ranger.AddState(
...
FSM_vars.state_machine_ranger.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x805607", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
# END RANGER ROUTINE
...
FSM_vars.state_machine_ranger.AddState(
    name="SECOND MAP PATH",
    execute_fn=lambda: hamnet_handle_map_path(FSM_vars.leveling_pathing),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.leveling_pathing, FSM_vars.movement_handler),
    run_once=False,
)
FSM_vars.state_machine_ranger.AddState(
...
# START COMMON ROUTINE PART ONE
FSM_vars.state_machine_monk.AddState(
    name="COMMAND BONUS",
    execute_fn=lambda: Player.SendChatCommand(text_bonus),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_monk.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DC01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk.AddState(
...
FSM_vars.state_machine_monk.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DC07", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x805401", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.monk_quest_pathing_1, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_monk.AddState(
...
FSM_vars.state_machine_monk.AddState(
    name="GOING BACK TO CIGLO",
    execute_fn=lambda: handle_map_path(FSM_vars.monk_quest_pathing_2),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.monk_quest_pathing_2, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_monk.AddState(
...
FSM_vars.state_machine_monk.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x805407", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
# END MONK ROUTINE
...
FSM_vars.state_machine_monk.AddState(
    name="SECOND MAP PATH",
    execute_fn=lambda: early_handle_map_path(FSM_vars.leveling_pathing),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.leveling_pathing, FSM_vars.movement_handler),
    run_once=False,
)
FSM_vars.state_machine_monk.AddState(
...
FSM_vars.state_machine_necromancer.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DA01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necromancer.AddState(
...
FSM_vars.state_machine_necromancer.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DA07", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necromancer.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805201", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
...
FSM_vars.state_machine_necromancer.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.necromancer_quest_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_necromancer.AddState(
...
FSM_vars.state_machine_necromancer.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805207", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
...
# START COMMON ROUTINE PART ONE
FSM_vars.state_machine_mesmer.AddState(
    name="COMMAND BONUS",
    execute_fn=lambda: Player.SendChatCommand(text_bonus),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_mesmer.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80D901", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_mesmer.AddState(
...
FSM_vars.state_machine_mesmer.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80D907", 16)),
    transition_delay_ms=300,
    run_once=True,
)
FSM_vars.state_machine_mesmer.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805101", 16)),
    transition_delay_ms=300,
    run_once=True,
)
...
FSM_vars.state_machine_mesmer.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.mesmer_quest_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_mesmer.AddState(
...
FSM_vars.state_machine_mesmer.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.sebedoh_pathing_1, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_mesmer.AddState(
...
FSM_vars.state_machine_mesmer.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x805107", 16)),
    transition_delay_ms=300,
    run_once=True,
)
# END MESMER ROUTINE
...
# START COMMON ROUTINE PART ONE
FSM_vars.state_machine_elementalist.AddState(
    name="COMMAND BONUS",
    execute_fn=lambda: Player.SendChatCommand(text_bonus),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_elementalist.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DB01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_elementalist.AddState(
...
FSM_vars.state_machine_elementalist.AddState(
    name="TAKING REWARD",
    execute_fn=lambda: Player.SendDialog(int("0x80DB07", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_elementalist.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805301", 16)),
    transition_delay_ms=300,
    run_once=True,
)
...
FSM_vars.state_machine_elementalist.AddState(
    name="GOING TO KILL",
    execute_fn=lambda: handle_map_path_loot(FSM_vars.elementalist_quest_pathing),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.elementalist_quest_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_elementalist.AddState(
    name="CHECK ITEM", execute_fn=lambda: handle_item_interaction(), transition_delay_ms=2000, run_once=True
)
FSM_vars.state_machine_elementalist.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
...
FSM_vars.state_machine_elementalist.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805307", 16)),
    transition_delay_ms=300,
    run_once=True,
)
...
FSM_vars.state_machine_lvl2_10.AddState(
    name="GOING OUT ASCALON",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.ascalon_pathing_1, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.ascalon_pathing_1, FSM_vars.movement_handler)
    or (Map.IsMapReady() and Map.IsExplorable() and Party.IsPartyLoaded()),
...
FSM_vars.state_machine_lvl2_10.AddState(
    name="COUNTER",
    execute_fn=lambda: increment_run_counter(),
    exit_condition=lambda: Party.IsPartyLoaded() and Map.IsMapReady(),
    transition_delay_ms=1000,
    run_once=True,
)
FSM_vars.state_machine_lvl2_10.AddSubroutine(
    name="CHECK QUEST STATUS",
    sub_fsm=FSM_vars.state_machine_ResetQuest,
    condition_fn=lambda: Quest.IsQuestCompleted(bot_vars.CharrAtTheGate),
)
FSM_vars.state_machine_lvl2_10.AddState(
    name="RESET CHECK QUEST ROUTINE", execute_fn=lambda: FSM_vars.state_machine_ResetQuest.reset()
)

FSM_vars.state_machine_ResetQuest.AddState(
    name="ABANDON QUEST",
    execute_fn=lambda: Quest.AbandonQuest(bot_vars.CharrAtTheGate),
    transition_delay_ms=1000,
    run_once=True,
)
FSM_vars.state_machine_ResetQuest.AddState(
...
FSM_vars.state_machine_lvl11_20.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.foible_map),
    exit_condition=lambda: Map.GetMapID() == bot_vars.foible_map
    or Map.IsMapReady()
    and LDoA_IsOutpost()
...
FSM_vars.state_machine_lvl11_20.AddState(
    name="GOING OUT IN DANGEROUS LANDS",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.foible_pathing, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.foible_pathing, FSM_vars.movement_handler)
    or (Map.IsMapReady() and Map.IsExplorable() and Party.IsPartyLoaded()),
    run_once=False,
)
FSM_vars.state_machine_lvl11_20.AddState(
...
FSM_vars.state_machine_lvl11_20.AddState(
    name="LUCKILY THERE IS A PRIEST",
    execute_fn=lambda: early_handle_map_path(FSM_vars.bandit_pathing),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.bandit_pathing, FSM_vars.movement_handler)
    or Survivor_Hamnet(),
    run_once=False,
)
FSM_vars.state_machine_lvl11_20.AddState(
    name="FIRE IMP IS FIRING",
    execute_fn=lambda: set_killing_routine(),
...
FSM_vars.state_machine_TamePet.AddState(
    name="WALKING TO REGENT VALLEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.tamepet_pathing_1, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_TamePet.AddState(
    name="SECOND MAP PATH",
    execute_fn=lambda: handle_map_path(FSM_vars.tamepet_pathing_2),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.tamepet_pathing_2, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_TamePet.AddState(
    name="CHECK QUEST", execute_fn=lambda: handle_quest_interaction(), transition_delay_ms=1000, run_once=True
)
FSM_vars.state_machine_TamePet.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804C03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_TamePet.AddState(
    name="TAKING SKILLS",
    execute_fn=lambda: Player.SendDialog(int("0x804C01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_TamePet.AddState(
    name="TAKING SKILLS", execute_fn=lambda: Player.SendDialog(int("0x85", 16)), transition_delay_ms=1500, run_once=True
)
FSM_vars.state_machine_TamePet.AddState(
    name="WALKING TO REGENT VALLEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.tamepet_pathing_3, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.tamepet_pathing_3, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_TamePet.AddState(
    name="CHECK QUEST", execute_fn=lambda: InteractPet(), transition_delay_ms=5000, run_once=True
)
FSM_vars.state_machine_TamePet.AddState(
    name="TAME PET", execute_fn=lambda: UseSkillByIDOnNearestNeutral(411), transition_delay_ms=20000, run_once=True
)
FSM_vars.state_machine_TamePet.AddState(
    name="GOING BACK TO ASHFORD ABBEY",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.abbey_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=1000,
    run_once=True,
)
...
FSM_vars.state_machine_abbey.AddState(
    name="GOING BACK TO ASHFORD ABBEY",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_abbey.AddState(
    name="WALKING TO ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.abbey_pathing, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.abbey_pathing, FSM_vars.movement_handler)
    or (Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded()),
    run_once=False,
)
FSM_vars.state_machine_abbey.AddState(
    name="WAITING OUTPOST MAP",
...
FSM_vars.state_machine_foible.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.goingout_ashfordabbey, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.goingout_ashfordabbey, FSM_vars.movement_handler
...
FSM_vars.state_machine_foible.AddState(
    name="WALKING TO WIZARD'S FOLLY",
...
FSM_vars.state_machine_ranik.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.goingout_ashfordabbey, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.goingout_ashfordabbey, FSM_vars.movement_handler
...
FSM_vars.state_machine_barradin.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_barradin.AddState(
    name="WALKING TO BARRADIN ESTATE",
    execute_fn=lambda: Routines.Movement.FollowPath(
        FSM_vars.barradin_coordinate_list_two_pathing, FSM_vars.movement_handler
    ),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.barradin_coordinate_list_two_pathing, FSM_vars.movement_handler
    ),
...
FSM_vars.state_machine_grandtour.AddState(
    name="GOING BACK TO ASHFORD ABBEY",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_warrior_req.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_warrior_req.AddState(
    name="WALKING TO QUEST",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.warrior_skill_pathing_1, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.warrior_skill_pathing_1, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_warrior_req.AddState(
...
FSM_vars.state_machine_warrior_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804B03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804B01", 16)),
    transition_delay_ms=3000,
    run_once=True,
)
FSM_vars.state_machine_warrior_req.AddState(
    name="GOING TO BARRADIN",
...
FSM_vars.state_machine_warrior_req.AddState(
    name="WALKING TO QUEST",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.warrior_skill_pathing_2, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_warrior_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x803C03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x803C01", 16)),
    transition_delay_ms=3000,
    run_once=True,
)
FSM_vars.state_machine_warrior_req.AddState(
...
FSM_vars.state_machine_warrior_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802803", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802801", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior_req.AddState(
    name="PRESS ESC",
    execute_fn=lambda: Keystroke.PressAndRelease(Key.Escape.value),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_warrior_req.AddState(
...
FSM_vars.state_machine_warrior_noreq.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_warrior_noreq.AddState(
    name="TAKING QUEST", execute_fn=lambda: Player.SendDialog(int("0x85", 16)), transition_delay_ms=1500, run_once=True
)
FSM_vars.state_machine_warrior_noreq.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
...
FSM_vars.state_machine_ranger_req.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_ranger_req.AddState(
    name="WALKING TO REGENT VALLEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.ranger_skill_pathing_1, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_ranger_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802A03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_ranger_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802A01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_ranger_req.AddState(
...
FSM_vars.state_machine_ranger_req.AddState(
    name="WALKING TO REGENT VALLEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.ranger_skill_pathing_2, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_ranger_req.AddState(
    name="SECOND MAP PATH",
    execute_fn=lambda: handle_map_path(FSM_vars.ranger_skill_pathing_3),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.ranger_skill_pathing_3, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_ranger_req.AddState(
...
FSM_vars.state_machine_ranger_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805803", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_ranger_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805801", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_ranger_req.AddState(
...
FSM_vars.state_machine_ranger_noreq.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_monk_req.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_monk_req.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.monk_skill_pathing_1, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_monk_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805703", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x805701", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk_req.AddState(
...
FSM_vars.state_machine_monk_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804A03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804A01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk_req.AddState(
...
FSM_vars.state_machine_monk_req.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.monk_skill_pathing_2, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_monk_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804D03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804D01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_monk_req.AddState(
...
FSM_vars.state_machine_monk_noreq.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_necro_req.AddState(
    name="GOING BACK TO ASCALON",
    execute_fn=lambda: LDoA_TravelToOutpost(bot_vars.ascalon_map),
    exit_condition=lambda: Map.IsMapReady() and LDoA_IsOutpost() and Party.IsPartyLoaded(),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_necro_req.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.necro_skill_pathing_1, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_necro_req.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.necro_skill_pathing_2, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
...
FSM_vars.state_machine_necro_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804803", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necro_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x804801", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necro_req.AddState(
...
FSM_vars.state_machine_necro_req.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.necro_skill_pathing_3, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.necro_skill_pathing_3, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_necro_req.AddState(
...
FSM_vars.state_machine_necro_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802F03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necro_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802F01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necro_req.AddState(
...
FSM_vars.state_machine_necro_req.AddState(
    name="GOING OUT ASHFORD ABBEY",
    execute_fn=lambda: Routines.Movement.FollowPath(FSM_vars.necro_skill_pathing_4, FSM_vars.movement_handler),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.necro_skill_pathing_4, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.state_machine_necro_req.AddState(
...
FSM_vars.state_machine_necro_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802B03", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necro_req.AddState(
    name="TAKING QUEST",
    execute_fn=lambda: Player.SendDialog(int("0x802B01", 16)),
    transition_delay_ms=1500,
    run_once=True,
)
FSM_vars.state_machine_necro_req.AddState(
...
# DULL CARAPACES
FSM_vars.state_machine_dull_carapaces.AddState(
    name="ASCALON",
    execute_fn=lambda: (
        (
            PySystem.Console.Log(
                "TH3KUM1KO - DULL CARAPACES FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
            ),
            LDoA_TravelToDistrict(bot_vars.ascalon_map, 6, 0),
        )
...
FSM_vars.state_machine_dull_carapaces.AddState(
    name="WAITING YOUR SLOW PC TO LOAD",
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - DULL CARAPACES FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_dull_carapaces.AddState(
    name="HEY THERE IS A FIRE ALLY",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - DULL CARAPACES FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info),
        useitem(30847),
    ),
    run_once=False,
)
...
FSM_vars.state_machine_dull_carapaces.AddState(
    name="FARMING LODESTONES",
    execute_fn=lambda: handle_map_path_loot(FSM_vars.dull_carapaces_pathing_2),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.dull_carapaces_pathing_2, FSM_vars.movement_handler
    )
    or Death(),
    run_once=False,
)
...
FSM_vars.state_machine_dull_carapaces.AddState(
    name="COUNTER",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - DULL CARAPACES FARM", "ADD COUNTER", PySystem.Console.MessageType.Info),
...
# GARGOYLE SKULLS
FSM_vars.state_machine_gargoyle_skulls.AddState(
    name="BARRADIN",
    execute_fn=lambda: (
        (
            PySystem.Console.Log(
                "TH3KUM1KO - GARGOYLE SKULLS FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
            ),
            LDoA_TravelToDistrict(bot_vars.barradin_map, 6, 0),
        )
        if not Map.IsExplorable()
        else None
    ),
...
FSM_vars.state_machine_gargoyle_skulls.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - GARGOYLE SKULLS FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_gargoyle_skulls.AddState(
    name="HEY THERE IS A FIRE ALLY",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - GARGOYLE SKULLS FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info),
        useitem(30847),
    ),
    run_once=False,
)
...
FSM_vars.state_machine_gargoyle_skulls.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.greenhills_to_catacombs_pathing, FSM_vars.movement_handler
    )
    or Map.IsMapLoading(),
    run_once=False,
)
...
FSM_vars.state_machine_gargoyle_skulls.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.gargoyle_skulls_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_gargoyle_skulls.AddState(
    name="COUNTER",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - LDoA LVL 2-10", "ADD COUNTER", PySystem.Console.MessageType.Info),
        increment_run_counter(),
    ),
    transition_delay_ms=1000,
    run_once=True,
...
# GRAWL NECKLACES
FSM_vars.state_machine_grawl_necklaces.AddState(
    name="BARRADIN",
    execute_fn=lambda: (
        (
            PySystem.Console.Log(
                "TH3KUM1KO - GRAWL NECKLACES FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
            ),
            LDoA_TravelToDistrict(bot_vars.barradin_map, 6, 0),
        )
        if not Map.IsExplorable()
...
FSM_vars.state_machine_grawl_necklaces.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - GRAWL NECKLACES FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_grawl_necklaces.AddState(
    name="HEY THERE IS A FIRE ALLY",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - GRAWL NECKLACES FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info),
        useitem(30847),
    ),
    run_once=False,
)
...
FSM_vars.state_machine_grawl_necklaces.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.grawl_necklaces_pathing, FSM_vars.movement_handler
    )
    or Survivor(),
    run_once=False,
)
...
FSM_vars.state_machine_grawl_necklaces.AddState(
    name="COUNTER",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - GRAWL NECKLACES FARM", "ADD COUNTER", PySystem.Console.MessageType.Info),
        increment_run_counter(),
    ),
...
# ICY LODESTONES
FSM_vars.state_machine_icy_lodestones.AddState(
    name="ARE WE IN HOGWARTS?",
...
FSM_vars.state_machine_icy_lodestones.AddState(
    name="WAITING YOUR SLOW PC TO LOAD", exit_condition=lambda: Map.IsExplorable(), transition_delay_ms=3000
)

FSM_vars.state_machine_icy_lodestones.AddState(
    name="HEY THERE IS A FIRE ALLY", execute_fn=lambda: useitem(30847), run_once=False
)
...
FSM_vars.state_machine_icy_lodestones.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.icy_lodestones_pathing, FSM_vars.movement_handler
    )
    or Survivor(),
    run_once=False,
)
...
FSM_vars.state_machine_icy_lodestones.AddState(
    name="COUNTER",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - ICY LODESTONES FARM", "ADD COUNTER", PySystem.Console.MessageType.Info),
        increment_run_counter(),
    ),
    exit_condition=lambda: Map.IsExplorable(),
    transition_delay_ms=1000,
...
# ENCHANTED LODESTONES
FSM_vars.state_machine_lodestone.AddState(
    name="BARRADIN",
    execute_fn=lambda: (
        (
            PySystem.Console.Log(
                "TH3KUM1KO - ENCHANTED LODESTONE FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
            ),
            LDoA_TravelToDistrict(bot_vars.barradin_map, 6, 0),
        )
        if not Map.IsExplorable()
...
FSM_vars.state_machine_lodestone.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - ENCHANTED LODESTONE FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_lodestone.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - ENCHANTED LODESTONE FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info
        ),
        useitem(30847),
    ),
    run_once=False,
...
FSM_vars.state_machine_lodestone.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.enchanted_lodestone_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)

# RED IRIS FLOWERS
FSM_vars.state_machine_red_iris_flowers.AddState(
    name="ARE WE IN ASCALON?",
    execute_fn=lambda: (
        (
            PySystem.Console.Log(
                "TH3KUM1KO - RED IRIS FLOWERS FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
            ),
            LDoA_TravelToDistrict(bot_vars.ascalon_map, 6, 0),
        )
        if not Map.IsExplorable()
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    name="GOING OUT ASCALON",
    execute_fn=lambda: (
        Routines.Movement.FollowPath(FSM_vars.ascalon_pathing, FSM_vars.movement_handler)
        if not Map.IsExplorable()
        else None
    ),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(FSM_vars.ascalon_pathing, FSM_vars.movement_handler)
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    name="RUNNING OUT OF TOWN",
    execute_fn=lambda: (
        (
            PySystem.Console.Log(
                "TH3KUM1KO - RED IRIS FLOWERS FARM", "RUNNING OUT OF TOWN", PySystem.Console.MessageType.Info
            ),
            Keystroke.PressAndRelease(Key.R.value),
        )
        if not Map.IsExplorable()
        else None
    ),
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - RED IRIS FLOWERS FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - RED IRIS FLOWERS FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info
        ),
        useitem(30847),
    ),
    run_once=False,
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.red_iris_flowers_pathing_1, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.red_iris_flowers_pathing_2, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.red_iris_flowers_pathing_3, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.red_iris_flowers_pathing_4, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.red_iris_flowers_pathing_5, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_red_iris_flowers.AddState(
    name="COUNTER",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - RED IRIS FLOWERS FARM", "ADD COUNTER", PySystem.Console.MessageType.Info),
        increment_run_counter(),
    ),
    exit_condition=lambda: Map.IsExplorable(),
    transition_delay_ms=1000,
...
# SKELETAL LIMBS
FSM_vars.state_machine_skele_limbs.AddState(
    name="BARRADIN",
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SKELETAL LIMBS", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
        ),
        LDoA_TravelToDistrict(bot_vars.barradin_map, 6, 0),
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
...
FSM_vars.state_machine_skele_limbs.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SKELETAL LIMBS", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_skele_limbs.AddState(
    name="HEY THERE IS A FIRE ALLY",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - SKELETAL LIMBS", "USING FIRE STONE", PySystem.Console.MessageType.Info),
        useitem(30847),
    ),
    run_once=False,
)
...
FSM_vars.state_machine_skele_limbs.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.skele_limbs_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_skale_fin.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SKALE FIN FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
        ),
        LDoA_TravelToDistrict(bot_vars.ranik_map, 6, 0),
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
    run_once=True,
...
FSM_vars.state_machine_skale_fin.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SKALE FIN FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=5000,
...
FSM_vars.state_machine_skale_fin.AddState(
    name="HEY THERE IS A FIRE ALLY",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - SKALE FIN FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info),
        useitem(30847),
    ),
    run_once=False,
)
...
FSM_vars.state_machine_skale_fin.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.skale_fin_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_skale_fin.AddState(
    name="COUNTER",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO  WORN BELTS", "ADD COUNTER", PySystem.Console.MessageType.Info),
        increment_run_counter(),
    ),
    exit_condition=lambda: Map.IsExplorable(),
    transition_delay_ms=1000,
...
# SPIDER LEGS
FSM_vars.state_machine_spider_leg.AddState(
    name="FORT RANIK",
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SPIDER LEGS FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
        ),
        LDoA_TravelToDistrict(bot_vars.ranik_map, 6, 0),
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
...
FSM_vars.state_machine_spider_leg.AddState(
    name="MESSAGE",
    execute_fn=lambda: PySystem.Console.Log(
        "TH3KUM1KO - SPIDER LEGS FARM", "GOING OUT", PySystem.Console.MessageType.Info
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=100,
    run_once=True,
...
FSM_vars.state_machine_spider_leg.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SPIDER LEGS FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_spider_leg.AddState(
    name="HEY THERE IS A FIRE ALLY",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - SPIDER LEGS FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info),
        useitem(30847),
    ),
    run_once=False,
)
...
FSM_vars.state_machine_spider_leg.AddState(
    name="MESSAGE",
    execute_fn=lambda: PySystem.Console.Log(
        "TH3KUM1KO - SPIDER LEGS FARM", "HUNTING SPIDERS", PySystem.Console.MessageType.Info
    ),
    exit_condition=lambda: Map.IsExplorable(),
    transition_delay_ms=100,
    run_once=True,
...
FSM_vars.state_machine_spider_leg.AddState(
    name="FARMING SPIDER LEGS",
    execute_fn=lambda: handle_map_path_loot(FSM_vars.spider_leg_pathing_1),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.spider_leg_pathing_1, FSM_vars.movement_handler
    ),
    run_once=False,
)

FSM_vars.state_machine_spider_leg.AddState(
    name="INTERACTING WITH BASKET OF APPLES ",
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SPIDER LEGS FARM", "INTERACTING WITH BASKET OF APPLES", PySystem.Console.MessageType.Info
        ),
        Keystroke.PressAndRelease(Key.Numpad1.value),
    ),
    transition_delay_ms=1000,
    run_once=True,
)
...
FSM_vars.state_machine_spider_leg.AddState(
    name="INTERACTING WITH TOWN CRIER",
...
FSM_vars.state_machine_spider_leg.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - SPIDER LEGS FARM", "DROPPING THE BASKET OF APPLES", PySystem.Console.MessageType.Info
        ),
        Keystroke.PressAndRelease(Key.Numpad2.value),
    ),
    transition_delay_ms=1000,
    run_once=True,
)
...
FSM_vars.state_machine_spider_leg.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.spider_leg_pathing_2, FSM_vars.movement_handler
    ),
    run_once=False,
)

# UNNATURAL SEEDS
FSM_vars.state_machine_unnatural_seeds.AddState(
    name="BARRADIN",
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - UNNATURAL SEEDS FARM", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
        ),
        LDoA_TravelToDistrict(bot_vars.barradin_map, 6, 0),
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
...
FSM_vars.state_machine_unnatural_seeds.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  UNNATURAL SEEDS FARM", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_unnatural_seeds.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  UNNATURAL SEEDS FARM", "USING FIRE STONE", PySystem.Console.MessageType.Info
        ),
        useitem(30847),
    ),
    run_once=True,
...
FSM_vars.state_machine_unnatural_seeds.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.unnatural_seeds_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)

# WORN BELTS
FSM_vars.state_machine_worn_belts.AddState(
    name="BARRADIN",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO - WORN BELTS", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info),
        LDoA_TravelToDistrict(bot_vars.barradin_map, 6, 0),
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
...
FSM_vars.state_machine_worn_belts.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  TH3KUM1KO - WORN BELTS", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_worn_belts.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  TH3KUM1KO - WORN BELTS", "USING FIRE STONE", PySystem.Console.MessageType.Info
        ),
        useitem(30847),
    ),
    run_once=True,
...
FSM_vars.state_machine_worn_belts.AddState(
    name="FARMING WORN BELTS",
    execute_fn=lambda: handle_map_path_loot(FSM_vars.worn_belts_pathing),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.worn_belts_pathing, FSM_vars.movement_handler
    )
    or Survivor(),
    run_once=False,
)
...
# BAKED HUSKS
FSM_vars.state_machine_baked_husks.AddState(
    name="ASHFORD ABBEY",
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO BAKED HUSKS", "MOVING TO ASHFORD ABBEY", PySystem.Console.MessageType.Info
        ),
        PySystem.Console.Log("BAKED HUSKS", "Executing travel function", PySystem.Console.MessageType.Info),
        PySystem.Console.Log(
            "BAKED HUSKS", f"Traveling to map ID: {bot_vars.abbey_map}", PySystem.Console.MessageType.Info
        ),
        LDoA_TravelToOutpost(bot_vars.abbey_map),
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
    transition_delay_ms=1000,
    run_once=True,
...
FSM_vars.state_machine_baked_husks.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO BAKED HUSKS", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_baked_husks.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO BAKED HUSKS", "USING FIRE STONE", PySystem.Console.MessageType.Info
        ),
        useitem(30847),
    ),
    run_once=True,
...
FSM_vars.state_machine_baked_husks.AddState(
    name="FARMING BAKED HUSKS",
    execute_fn=lambda: handle_map_path_baked_husk(FSM_vars.baked_husk_pathing),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.baked_husk_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
FSM_vars.state_machine_baked_husks.AddState(
    name="COUNTER",
    execute_fn=lambda: (
        PySystem.Console.Log("TH3KUM1KO  BAKED HUSKS", "ADD COUNTER", PySystem.Console.MessageType.Info),
        increment_run_counter(),
    ),
    exit_condition=lambda: Map.IsExplorable(),
    transition_delay_ms=1000,
...
# CHARR GATE OPENER
FSM_vars.state_machine_charr_gate_opener.AddState(
    name="ASCALON",
    execute_fn=lambda: (
        (
            PySystem.Console.Log(
                "TH3KUM1KO - CHARR GATE OPENER", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
            ),
            LDoA_TravelToDistrict(bot_vars.ascalon_map, 6, 0),
        )
        if not Map.IsExplorable()
...
FSM_vars.state_machine_charr_gate_opener.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - CHARR GATE OPENER", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_charr_gate_opener.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.charr_gate_opener_pathing_2, FSM_vars.movement_handler
    ),
    run_once=False,
)

FSM_vars.state_machine_charr_gate_opener.AddState(
    name="WAITING TO STABILIZE",
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO - CHARR GATE OPENER",
            "WAITING TO STABILIZE BEFORE LEVER INTERACTION",
...
FSM_vars.state_machine_charr_gate_opener.AddState(
    name="INTERACTING WITH LEVER", execute_fn=lambda: handle_gadget_interaction(), transition_delay_ms=50, run_once=True
)

FSM_vars.state_machine_charr_gate_opener.AddState(
    name="WALKING TO PORTAL EDGE",
    execute_fn=lambda: handle_map_path_gate_opener(FSM_vars.charr_gate_opener_fast_gate_run),
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.charr_gate_opener_fast_gate_run, FSM_vars.movement_handler
    ),
    run_once=False,
)
...
# NICHOLAS SANDFORD
FSM_vars.state_machine_nicholas_sandford.AddState(
    name="BARRADIN",
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO NICHOLAS SANDFORD", "MOVING TO A SAFER DISTRICT", PySystem.Console.MessageType.Info
        ),
        LDoA_TravelToDistrict(bot_vars.ranik_map, 6, 0),
    ),
    exit_condition=lambda: LDoA_IsOutpost(),
...
FSM_vars.state_machine_nicholas_sandford.AddState(
    ...
    exit_condition=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO NICHOLAS SANDFORD", "WAITING FOR EXPLORABLE MAP", PySystem.Console.MessageType.Info
        ),
        Map.IsExplorable(),
    ),
    transition_delay_ms=3000,
...
FSM_vars.state_machine_nicholas_sandford.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  TH3KUM1KO - GOING TO NICHOLAS SANDFORD", "USING FIRE STONE", PySystem.Console.MessageType.Info
        ),
        useitem(30847),
    ),
    run_once=True,
...
FSM_vars.state_machine_nicholas_sandford.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO NICHOLAS SANDFORD",
            "INTERACTING WITH NICHOLAS SANDFORD WITH V",
            PySystem.Console.MessageType.Info,
        ),
...
FSM_vars.state_machine_nicholas_sandford.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO NICHOLAS SANDFORD",
            "INTERACTING WITH NICHOLAS SANDFORD WITH SPACE",
            PySystem.Console.MessageType.Info,
        ),
        Keystroke.PressAndRelease(Key.Space.value),
    ),
...
FSM_vars.state_machine_nicholas_sandford.AddState(
    ...
    execute_fn=lambda: (
        PySystem.Console.Log(
            "TH3KUM1KO -  GOING TO NICHOLAS SANDFORD",
            "TAKING GIFTS FROM NICHOLAS SANDFORD",
            PySystem.Console.MessageType.Info,
        ),
        Player.SendDialog(int("0x85", 16)),
    ),
    transition_delay_ms=100,
    run_once=True,
)
...
class InventoryTracker:
    ...
    def get_count_items(self):
        ...
        try:
            ...
            PySystem.Console.Log(
                "INVENTORY TRACKER", f"Error counting items: {e}", PySystem.Console.MessageType.Warning
            )
    ...
    def get_farmed_items(self):
        ...
        try:
            ...
            PySystem.Console.Log(
                "INVENTORY TRACKER", f"Error getting farmed items: {e}", PySystem.Console.MessageType.Warning
            )
    ...
    try:
        ...
        if PyImGui.begin("\uf647  TH3KUM1KO'S PRESEARING BIBLE", bot_vars.window_module.window_flags):
            ...
            if PyImGui.begin_tab_bar("MainTabBar"):
                if PyImGui.begin_tab_item("LDoA"):
                    ...
                    PyImGui.text_wrapped(
                        "WITH THESE FUNCTIONS, THE BOT WILL LEVEL UP YOUR CHARACTER BASED ON ITS PROFESSION, REACHING LEVEL 2 THROUGH THE INITIAL QUESTS. ONCE LEVEL 2 IS ACHIEVED, THE BOT WILL AUTOMATICALLY TAKE THE 'CHARR AT THE GATE' QUEST."
                    )
                    state.radio_button_selected = PyImGui.radio_button(
                        " \uf132 WARRIOR", state.radio_button_selected, 20
                    )
                    PyImGui.same_line(200, -1.0)
                    state.radio_button_selected = PyImGui.radio_button(
                        " \uf54c NECROMANCER", state.radio_button_selected, 23
                    )
                    state.radio_button_selected = PyImGui.radio_button(
                        " \uf1bb RANGER", state.radio_button_selected, 21
                    )
                    PyImGui.same_line(200, -1.0)
                    state.radio_button_selected = PyImGui.radio_button(
                        " \ue2ca MESMER", state.radio_button_selected, 24
                    )
                    state.radio_button_selected = PyImGui.radio_button(" \uf644 MONK", state.radio_button_selected, 22)
                    PyImGui.same_line(200, -1.0)
                    state.radio_button_selected = PyImGui.radio_button(
                        " \uf6e8 ELEMENTALIST", state.radio_button_selected, 25
                    )
                    PyImGui.spacing()
                    PyImGui.text_wrapped(
                        "WITH THIS FUNCTION, THE BOT WILL LEAVE ASCALON USING THE 'CHARR AT THE GATE' QUEST, FOLLOW RURIK, AND WAIT FOR THE PRINCE TO KILL THREE CHARR BEFORE RETURNING TO THE CITY AND RESTARTING THE LOOP. THIS FUNCTION ENSURES SURVIVOR TITLE PROGRESSION."
                    )
                    state.radio_button_selected = PyImGui.radio_button(
                        " \uf443 LEVEL 2-10 - CHARR AT THE GATE", state.radio_button_selected, 0
                    )
                    PyImGui.spacing()
                    PyImGui.text_wrapped(
                        "ONCE YOU REACH LEVEL 10, WAIT FOR THE ROTATION OF THE VANGUARD QUESTS UNTIL THE 'FARMER HAMNET' QUEST BECOMES AVAILABLE. THE BOT WILL KILL THE FIRST TWO MOBS OUTSIDE FOIBLE'S FAIR AND REPEAT THE LOOP UNTIL LEVEL 20. THIS FUNCTION ENSURES SURVIVOR TITLE PROGRESSION."
                    )
                    state.radio_button_selected = PyImGui.radio_button(
                        " \uf43f LEVEL 11-20 - FARMER HAMNET", state.radio_button_selected, 1
                    )
                    PyImGui.spacing()
                    if IsBotStarted():
            ...
            if PyImGui.begin_tab_item("TRAVEL"):
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf51d GO TO ASHFORD ABBEY", state.radio_button_selected, 2
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf6e8 GO TO FOIBLE'S FAIR", state.radio_button_selected, 3
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf447 GO TO FORT RANIK", state.radio_button_selected, 4
                )
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf43a GO TO THE BARRADIN ESTATE", state.radio_button_selected, 5
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf5a0 THE GRAND TOUR", state.radio_button_selected, 19
                )
            ...
            if PyImGui.begin_tab_item("NIC ITEMS"):
                ...
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf188 DULL CARAPACES", state.radio_button_selected, 8
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf54c GARGOYLE SKULLS", state.radio_button_selected, 9
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf4d6 GRAWL NECKLACES", state.radio_button_selected, 10
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf7ad ICY LODESTONES", state.radio_button_selected, 11
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf3a5 ENCHANTED LODESTONES", state.radio_button_selected, 12
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf5bb RED IRIS FLOWERS", state.radio_button_selected, 13
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf5d7 SKELETAL LIMBS", state.radio_button_selected, 14
                )
                state.radio_button_selected = PyImGui.radio_button("\ue4f2 SKALE FINS", state.radio_button_selected, 15)
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf717 SPIDER LEGS", state.radio_button_selected, 16
                )
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf4d8 UNNATURAL SEEDS", state.radio_button_selected, 17
                )
                state.radio_button_selected = PyImGui.radio_button("\ue19b WORN BELTS", state.radio_button_selected, 18)
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf06b NICHOLAS SANDFORD", state.radio_button_selected, 26
                )
                PyImGui.spacing()
            ...
            if PyImGui.begin_tab_item("MISC"):
                ...
                state.radio_button_selected = PyImGui.radio_button(
                    "\uf70c CHARR GATE OPENER", state.radio_button_selected, 42
                )
            ...
            if PyImGui.begin_tab_item("SKILLS"):
                ...
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00c PRIMARY/SECONDARY REQUIRED##WARRIOR_REQ", state.radio_button_selected, 28
                )
                PyImGui.same_line(300, -1.0)
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00c PRIMARY/SECONDARY REQUIRED##NECRO_REQ", state.radio_button_selected, 34
                )
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00d PRIMARY/SECONDARY NO REQ##WARRRIOR_NOREQ", state.radio_button_selected, 29
                )
                PyImGui.same_line(300, -1.0)
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00d PRIMARY/SECONDARY NO REQ##NECRO_NOREQ", state.radio_button_selected, 35
                )
                ...
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00c PRIMARY/SECONDARY REQUIRED##RANGER_REQ", state.radio_button_selected, 30
                )
                PyImGui.same_line(300, -1.0)
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00c PRIMARY/SECONDARY REQUIRED##MESMER_REQ", state.radio_button_selected, 36
                )
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00d PRIMARY/SECONDARY NO REQ##RANGER_NOREQ", state.radio_button_selected, 31
                )
                PyImGui.same_line(300, -1.0)
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00d PRIMARY/SECONDARY NO REQ##MESMER_NOREQ", state.radio_button_selected, 37
                )
                ...
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00c PRIMARY/SECONDARY REQUIRED##MONK_REQ", state.radio_button_selected, 32
                )
                PyImGui.same_line(300, -1.0)
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00c PRIMARY/SECONDARY REQUIRED##ELEMENTALIST_REQ", state.radio_button_selected, 38
                )
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00d PRIMARY/SECONDARY NO REQ##MONK_NOREQ", state.radio_button_selected, 33
                )
                PyImGui.same_line(300, -1.0)
                state.radio_button_selected = PyImGui.radio_button(
                    " \uf00d PRIMARY/SECONDARY NO REQ##ELEMENTALIST_NOREQ", state.radio_button_selected, 39
                )
                PyImGui.spacing()
        ...
        PySystem.Console.Log(
            ...
            f"Error in {current_function}: {str(e)}",
...
def main():
    ...
    try:
        ...
        PySystem.Console.Log(
            bot_vars.window_module.module_name, f"ImportError encountered: {str(e)}", PySystem.Console.MessageType.Error
        )
        PySystem.Console.Log(
            bot_vars.window_module.module_name,
        ...
        PySystem.Console.Log(
            bot_vars.window_module.module_name, f"ValueError encountered: {str(e)}", PySystem.Console.MessageType.Error
        )
        PySystem.Console.Log(
            bot_vars.window_module.module_name,
            f"Stack trace: {traceback.format_exc()}",
        ...
        PySystem.Console.Log(
            bot_vars.window_module.module_name, f"TypeError encountered: {str(e)}", PySystem.Console.MessageType.Error
        )
        PySystem.Console.Log(
            bot_vars.window_module.module_name,
            f"Stack trace: {traceback.format_exc()}",
