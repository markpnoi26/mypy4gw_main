    ...
    try:
        ...
        if not bot_state.movement_handler.is_following():
            ...
            bot_state.movement_handler.move_to_waypoint(
                bot_config.exit_coordinates[0], bot_config.exit_coordinates[1], tolerance=100
            )
        else:
...
def main():
    ...
    try:
        # Bot control window
        expanded, _ = ImGui.begin_with_close(
            "Simple Farming Bot", None, PyImGui.WindowFlags(PyImGui.WindowFlags.AlwaysAutoResize)
        )
        if expanded:
            ...
            # Start Button
            start_color = (
                Color(0, 150, 0, 255).to_tuple() if not bot_state.is_running else Color(100, 100, 100, 255).to_tuple()
            )
            ...
            # Stop Button
            stop_color = (
                Color(150, 0, 0, 255).to_tuple() if bot_state.is_running else Color(100, 100, 100, 255).to_tuple()
            )
            ...
            PyImGui.text("Status:")
            status_color = (
                Color(0, 255, 0, 255).to_tuple() if bot_state.is_running else Color(255, 255, 255, 255).to_tuple()
            )
