...
def EquipSkillBar():
    ...
    if profession == "Dervish":
        if level == 2:
            ...
            yield from Routines.Yield.Skills.LoadSkillbar(
                "OgCjkOrCbMiXp74dADAAAAABAA"
            )  # leave 2 holes in the skill bar to avoid the pop up for 2nd profession
        elif level == 5:
            yield from Routines.Yield.Skills.LoadSkillbar("OgCjkOrCbMiXp74dADAAAAABAA")
        else:
...
def DoCraftArmorWithDoubleMats(bot: Botting):
    ...
    for item_id, mats, qtys in armor_pieces:
        ...
        if not result:
            ConsoleLog(
                "CraftArmorWithDoubleMats", f"Failed to craft item ({item_id}).", PySystem.Console.MessageType.Error
            )
        ...
        if not result:
            ConsoleLog(
                "CraftArmorWithDoubleMats", f"Failed to equip item ({item_id}).", PySystem.Console.MessageType.Error
            )
            bot.helpers.Events.on_unmanaged_fail()
            return False
        yield
        ...
        while elapsed_ms < timeout_ms:
            try:
                if PyDialog.PyDialog.is_dialog_active():
                    buttons = [
                        button
                        for button in PyDialog.PyDialog.get_active_dialog_buttons()
...
def Secondary_Training(bot: Botting):
    ...
    if profession == "Necromancer":
        ...
        bot.Move.XYAndDialog(
            -6557.00, 1837, 0x7F
        )  # this is fine, need minimal skill bar for level 4 to avoid skill equip window
        bot.Move.XYAndDialog(-7161, 4808, 0x825907)
...
def destroy_starter_armor_and_useless_items() -> Generator[Any, Any, None]:
    ...
    if primary == "Dervish":
        starter_armor = [15712, 15710, 15711, 15713, 15709]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Paragon":
        starter_armor = [15717, 15715, 15716, 15718, 15714]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Warrior":
        starter_armor = [15702, 15700, 15701, 15703, 15699]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Ranger":
        starter_armor = [15707, 15705, 15706, 15708, 15704]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Monk":
        starter_armor = [15697, 15695, 15696, 15698, 15694]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Elementalist":
        starter_armor = [15692, 15690, 15691, 15693, 15689]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Mesmer":
        starter_armor = [15682, 15680, 15681, 15683, 15679]  # Head  # Chest  # Gloves  # Pants  # Boots
    elif primary == "Necromancer":
        starter_armor = [15687, 15685, 15686, 15688, 15684]  # Head  # Chest  # Gloves  # Pants  # Boots
...
def Farm_Until_Level_10(bot):
    ...
    for _ in range(17):
        bot.States.AddCustomState(EquipSkillBar, "Equip Skill Bar")
        bot.Move.FollowPath([(1268, -311), (-1618, -783), (-2600, -1119), (-3546, -1444)])
...
def _draw_texture():
    ...
    path = os.path.join(
        PySystem.Console.get_projects_path(), "Bots", "Leveling", "Nightfall", "Nightfall_leveler-art.png"
    )
...
def tooltip():
    ...
    PyImGui.text_wrapped(
        "This bot levels a character from 1 to 20 in the Nightfall campaign, unlocking key features and content along the way. It is designed to be efficient and user-friendly, providing a smooth leveling experience for new players or those looking to quickly level an alt."
    )
    PyImGui.spacing()
