...
from Core.enums_src.Title_enums import TitleID, TITLE_TIERS
from Core.ImGui_src.ImGuisrc import ImGui
import Py4GW
import PyImGui
import os
import random
...
bot = Botting(BOT_NAME, upkeep_honeycomb_active=True, upkeep_hero_ai_active=True, upkeep_auto_loot_active=True)
...
def bot_routine(bot: Botting) -> None:
    ...
    bot.States.AddCustomState(
        lambda: _gh_merchant_setup_if_enabled(bot, OUTPOST_TO_TRAVEL), "GH Merchant Setup If Enabled"
    )
...
def _reenable_merchant_widgets():
    ...
    yield from _wait_for_alt_dispatch_completion(
        "enable_widgets", refs, SharedCommandType.EnableWidget, timeout_ms=15000
    )
...
def _wait_for_alt_dispatch_completion(
    stage_name: str, message_refs: list[tuple[str, int]], command, timeout_ms: int = 30000
):
    if not message_refs:
        return
    ...
    if pending:
        ...
        ConsoleLog(
            BOT_NAME,
            f"[Merchant] {stage_name}: timeout waiting for alt completion. Pending: {pending_accounts}",
...
def _wait_for_alts_on_current_map(stage_name: str, expected_alts: int, target_map_id: int, timeout_ms: int = 30000):
    ...
    ConsoleLog(
        BOT_NAME,
        f"[Merchant] {stage_name}: alt arrival timeout on map {target_map_id}",
        PySystem.Console.MessageType.Warning,
    )
...
def _gh_merchant_setup_if_enabled(bot: Botting, outpost_id: int):
    ...
    yield from _wait_for_alt_dispatch_completion(
        "travel_gh", travel_refs, SharedCommandType.TravelToGuildHall, timeout_ms=10000
    )
    ...
    if not Map.IsGuildHall():
        ConsoleLog(
            BOT_NAME,
            "[Merchant] Failed to reach Guild Hall, skipping merchant setup",
            PySystem.Console.MessageType.Warning,
        )
        return

    yield from _wait_for_alts_on_current_map(
        "travel_gh_arrival", expected_gh_alts, int(Map.GetMapID()), timeout_ms=60000
    )
    ...
    rare_xy = (
        _find_npc_xy_by_name("Rare", max_dist=30000.0)
        if (_merchant_sell_jadeite_shards or _merchant_buy_ectos)
        else None
    )
    ...
    if _merchant_sell_materials and mat_xy:
        ...
        if merchant_xy:
            ...
            if leftover_ids:
                ...
                yield from Routines.Yield.wait(300)
            yield from _wait_for_alt_dispatch_completion(
                "sell_merchant_leftovers", leftover_refs, SharedCommandType.MerchantMaterials
            )
    ...
    if merchant_xy:
        ...
        yield from _wait_for_alt_dispatch_completion(
            "sell_nonsalvageable_golds", sell_gold_refs, SharedCommandType.MerchantMaterials
        )
    ...
    if _merchant_sell_jadeite_shards:
        if rare_xy:
            ...
            yield from _wait_for_alt_dispatch_completion(
                "sell_jadeite_shards", jadeite_refs, SharedCommandType.MerchantMaterials
            )
        else:
            ConsoleLog(
                BOT_NAME,
                "[Merchant] No Rare Material Trader found - skipping Jadeite Shard sale",
                PySystem.Console.MessageType.Warning,
            )
    ...
    if _merchant_buy_ectos:
        if rare_xy:
            ...
            ConsoleLog(
                BOT_NAME,
                "[Merchant] Ecto buy skipped - no Rare Material Trader found",
                PySystem.Console.MessageType.Warning,
            )
    ...
    if PyImGui.begin_combo(label, preview, PyImGui.ImGuiComboFlags.NoFlag):
        for index, hero in enumerate(_HERO_OPTIONS):
            ...
            if PyImGui.selectable(
                f"{_HERO_OPTION_LABELS[index]}##{label}_{index}", index == current_index, 0, [0.0, 0.0]
            ):
                current_index = index
        PyImGui.end_combo()
    ...
    if _party_mode != 1:
        ...
        for account in accounts:
            ...
            if account_email == own_email or character_name == own_name:
                return [
                    (
                        account_email or own_email,
                        character_name or own_name,
                        _get_account_luxon_points(account, own_email),
                    )
                ]
...
def _donate_luxon_if_threshold_met(bot: Botting):
    ...
    yield from _wait_for_alt_dispatch_completion(
        "donate_luxon", refs, SharedCommandType.DonateToGuild, timeout_ms=90000
    )
    yield from Routines.Yield.wait(1000)
...
def _load_settings(bot: Botting) -> None:
    ...
    _party_mode = 1 if cfg.get_bool(_SETTINGS_SECTION, _MULTIBOX_ALTS_KEY, _party_mode == 1) else 0
    _randomize_district = cfg.get_bool(_SETTINGS_SECTION, _RANDOMIZE_DISTRICT_KEY, _randomize_district)
    _restock_use_conset = cfg.get_bool(_SETTINGS_SECTION, _USE_CONSET_KEY, _restock_use_conset)
    _restock_use_pcons = cfg.get_bool(_SETTINGS_SECTION, _USE_PCONS_KEY, _restock_use_pcons)
    _restock_use_summoning_stones = cfg.get_bool(
        _SETTINGS_SECTION, _USE_SUMMONING_STONES_KEY, _restock_use_summoning_stones
    )
    _restock_kits_enabled = cfg.get_bool(_SETTINGS_SECTION, _USE_RESTOCK_KITS_KEY, _restock_kits_enabled)
    _id_kits_target = max(0, int(cfg.get_int(_SETTINGS_SECTION, _ID_KITS_TARGET_KEY, _id_kits_target)))
    _salvage_kits_target = max(0, int(cfg.get_int(_SETTINGS_SECTION, _SALVAGE_KITS_TARGET_KEY, _salvage_kits_target)))
    _merchant_sell_materials = cfg.get_bool(_SETTINGS_SECTION, _MERCHANT_SELL_MATERIALS_KEY, _merchant_sell_materials)
    _merchant_sell_jadeite_shards = cfg.get_bool(
        _SETTINGS_SECTION,
        _MERCHANT_SELL_JADEITE_SHARDS_KEY,
        _merchant_sell_jadeite_shards,
    )
    _merchant_buy_ectos = cfg.get_bool(_SETTINGS_SECTION, _MERCHANT_BUY_ECTOS_KEY, _merchant_buy_ectos)
    _merchant_ecto_threshold = max(
        0, int(cfg.get_int(_SETTINGS_SECTION, _MERCHANT_ECTO_THRESHOLD_KEY, _merchant_ecto_threshold))
    )
    _merchant_alt_wait_ms = max(
        0,
        min(
            _MAX_ALT_SETTLE_WAIT_MS,
            int(cfg.get_int(_SETTINGS_SECTION, _MERCHANT_ALT_WAIT_MS_KEY, _merchant_alt_wait_ms)),
        ),
    )
    _donation_min_luxon_points = max(
        _MIN_DONATION_THRESHOLD,
        min(
            _MAX_DONATION_THRESHOLD,
            int(cfg.get_int(_SETTINGS_SECTION, _DONATION_MIN_LUXON_POINTS_KEY, _donation_min_luxon_points)),
        ),
    )
    CONSET_RESTOCK_TARGET = max(
        0,
        min(
            _MAX_RESTOCK_TARGET, int(cfg.get_int(_SETTINGS_SECTION, _CONSET_RESTOCK_TARGET_KEY, CONSET_RESTOCK_TARGET))
        ),
    )
    PCON_RESTOCK_TARGET = max(
        0, min(_MAX_RESTOCK_TARGET, int(cfg.get_int(_SETTINGS_SECTION, _PCON_RESTOCK_TARGET_KEY, PCON_RESTOCK_TARGET)))
    )
    SUMMONING_STONES_RESTOCK_TARGET = max(
        0,
        min(
            _MAX_RESTOCK_TARGET,
            int(cfg.get_int(_SETTINGS_SECTION, _SUMMONING_STONES_RESTOCK_TARGET_KEY, SUMMONING_STONES_RESTOCK_TARGET)),
        ),
    )
...
def _draw_settings():
    ...
    new_donation_threshold = PyImGui.input_int(
        "Donate at Luxon faction >=##mount_qinkai_donate_threshold", _donation_min_luxon_points
    )
    ...
    PyImGui.separator()
    new_conset_target = max(
        0,
        min(
            _MAX_RESTOCK_TARGET, PyImGui.input_int("Conset restock target##mount_qinkai_conset", CONSET_RESTOCK_TARGET)
        ),
    )
    ...
    new_pcon_target = max(
        0, min(_MAX_RESTOCK_TARGET, PyImGui.input_int("Pcons restock target##mount_qinkai_pcons", PCON_RESTOCK_TARGET))
    )
    if new_pcon_target != PCON_RESTOCK_TARGET:
        PCON_RESTOCK_TARGET = new_pcon_target
        changed = True

    new_summoning_target = max(
        0,
        min(
            _MAX_RESTOCK_TARGET,
            PyImGui.input_int(
                "Summoning Stones restock target##mount_qinkai_summoning", SUMMONING_STONES_RESTOCK_TARGET
            ),
        ),
    )
    ...
    if _restock_kits_enabled:
        ...
        new_sell_materials = PyImGui.checkbox(
            "Sell common materials##mount_qinkai_sell_materials", _merchant_sell_materials
        )
        if new_sell_materials != _merchant_sell_materials:
            _merchant_sell_materials = new_sell_materials
            changed = True

        new_sell_jadeite = PyImGui.checkbox(
            "Sell Jadeite Shards to Rare Material Trader##mount_qinkai_jadeite", _merchant_sell_jadeite_shards
        )
        if new_sell_jadeite != _merchant_sell_jadeite_shards:
            _merchant_sell_jadeite_shards = new_sell_jadeite
            changed = True

        new_buy_ectos = PyImGui.checkbox(
            "Buy Glob of Ectoplasm when storage over threshold##mount_qinkai_ectos", _merchant_buy_ectos
        )
        ...
        if _merchant_buy_ectos:
            new_ecto_threshold = PyImGui.input_int(
                "Storage threshold (gold)##mount_qinkai_ecto_threshold", _merchant_ecto_threshold
            )
