...
from dev.reference.aC_Scripts.aC_api.Blessing_Core import get_blessing_npc
from dev.reference.aC_Scripts.aC_api.Titles import (
    display_title_track,
    display_faction,
    display_title_progress,
    vanguard_tiers,
    norn_tiers,
    asura_tiers,
    deldrimor_tiers,
...
class FSMVars:
    def __init__(self):
        ...
        self.path_and_aggro = FollowPathAndAggro(
            self.explorable_pathing, self.movement_handler, aggro_range=2500, log_actions=True
        )
...
class FollowPathAndAggro:
    ...
    def _throttled_scan(self):
        ...
        if dist_moved >= self._scan_move_thresh or self._enemy_scan_timer.HasElapsed(self._scan_interval_ms):
        ...
        if not self.follow_handler.is_following():
            ...
            if not next_point:
                ...
                if self.log_actions:
                    ConsoleLog(
                        "FollowPathAndAggro",
                        "PathHandler returned None â€“ halting movement.",
                ...
                if hasattr(self.path_handler, "reset"):
                    ...
                    if retry_point:
                        ...
                        ConsoleLog(
                            "FollowPathAndAggro",
                            f"Path reset after failure, moving to {retry_point}",
                            Console.MessageType.Warning,
                        )
...
def load_map_script():
    ...
    FSM_vars.path_and_aggro = FollowPathAndAggro(
        FSM_vars.explorable_pathing, FSM_vars.movement_handler, aggro_range=2500, log_actions=True
    )
...
def pause_all(debug: bool = False):
    ...
    if not FSM_vars.state_machine.is_paused():
        if debug:
            ConsoleLog("FSM", "[DEBUG] Pausing Main FSM", Console.MessageType.Warning)
...
def InitializeStateMachine():
    ...
    FSM_vars.state_machine.AddState(
        name="Wait For Map Load", exit_condition=lambda: not Map.IsMapLoading(), transition_delay_ms=1000
    )
    FSM_vars.state_machine.AddState(
    ...
    FSM_vars.state_machine.AddState(
        ...
        exit_condition=lambda: has_any_blessing(Player.GetAgentID())
        or Map.IsMapLoading()
        or (get_blessing_npc()[0] is None),
...
def DrawWindow():
    ...
    if bot_vars.selected_region:
        ...
        maps = sorted(
            [f[:-3] for f in os.listdir(os.path.join(MAPS_DIR, bot_vars.selected_region)) if f.endswith(".py")]
        )
    ...
    PyImGui.pop_style_color(
        6
    )  # unwinds window_bg_color, frame_bg_color, frame_hover_color, frame_active_color, body_text_color, separator_color (#1..#6)
