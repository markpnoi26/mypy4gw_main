from Core import Botting, Routines, GLOBAL_CACHE, ModelID, Map, Agent, ConsoleLog, Player, AgentArray, Map
from Core import *
import Py4GW
import os
import PyImGui
...
def _register_aggro_path(
    bot, path, header_name=None, detection_radius=2500.0, clear_radius=2500.0, on_enemy_detected=None
):
...
def VanquishWatchdog(bot: "Botting", completed_header_name: str):
    ...
    while True:
        if Map.IsVanquishCompleted():
            ConsoleLog(
                "VanquishWatchdog",
                f"Vanquish trigger activated. Jumping to: {completed_header_name}",
...
def bot_routine(bot: Botting) -> None:
    ...
    for vq_idx, vq in enumerate(_queued_vanquishes):
            ...
            if transit_count > 0:
                ...
                for i in range(transit_count):
                    ...
                    bot.States.AddCustomState(
                        lambda vi=vq_idx, si=section_idx, tc=t_coord: _set_section_header(
                            _section_headers[vi][si], tc[0], tc[1]
                        ),
        ...
        bot.States.AddCustomState(
            lambda vi=vq_idx, si=section_idx, vc=vp_coord: _set_section_header(_section_headers[vi][si], vc[0], vc[1]),
        ...
        bot.UI.PrintMessageToConsole(
            BotSettings.BOT_NAME, f"Starting Reverse Aggro Path 1 (range={_aggro_range_reverse1})."
        )
        reversed_path = _build_reversed_path(vq.vanquish_path)
        rp_coord = _get_first_path_coord(reversed_path)
        bot.States.AddCustomState(
            lambda vi=vq_idx, si=section_idx, rc=rp_coord: _set_section_header(_section_headers[vi][si], rc[0], rc[1]),
        ...
        bot.UI.PrintMessageToConsole(
            BotSettings.BOT_NAME, f"Starting Reverse Aggro Path 2 (range={_aggro_range_reverse2})."
        )
        reversed_path = _build_reversed_path(vq.vanquish_path)
        rp5_coord = _get_first_path_coord(reversed_path)
        bot.States.AddCustomState(
            lambda vi=vq_idx, si=section_idx, rc=rp5_coord: _set_section_header(_section_headers[vi][si], rc[0], rc[1]),
        ...
        if is_last:
            ...
            bot.UI.PrintMessageToConsole(
                BotSettings.BOT_NAME, f"Vanquish SUCCESS: {vq.display}. Moving to next Vanquish."
            )
            bot.Multibox.ResignParty()
            bot.Wait.ForTime(1000)
            bot.Wait.UntilOnOutpost()
...
def _stop_bot():
    ...
    if _reverse_detections:
        ...
        ConsoleLog(
            BotSettings.BOT_NAME,
            "REVERSE PASS ENEMY DETECTIONS (for source file updates):",
        ...
        for map_name, phases in _reverse_detections.items():
            ...
            for phase, coords in sorted(phases.items()):
                if coords:
                    ...
                    ConsoleLog(
                        BotSettings.BOT_NAME,
                        f"  Total: {len(coords)} detections, {len(unique)} unique positions",
                        PySystem.Console.MessageType.Info,
                        True,
                    )
        ConsoleLog(BotSettings.BOT_NAME, "=" * 60, PySystem.Console.MessageType.Info, True)
...
def _conset_upkeep(bot):
    ...
    while True:
        ...
        armor_params = (
            ...
            GLOBAL_CACHE.Skill.GetID("Armor_of_Salvation_item_effect"),
            0,
            0,
        )
...
def _on_party_wipe(bot: "Botting"):
    ...
    if Map.IsOutpost():
        ...
        ConsoleLog("on_party_wipe", f"Resurrected in outpost. Re-executing vanquish. Jumping to: {target}")
        if bot.config.FSM.current_state:
            bot.config.FSM.current_state.reset()
...
def _draw_settings():
    ...
    # --- Map combo ---
    maps = sorted([f[:-3] for f in os.listdir(REGION_DIR) if f.endswith(".py")])
...
def _draw_settings_consumables():
    ...
    _restock_use_summoning_stones = PyImGui.checkbox(
        "Restock & use Summoning Stones (Multibox)", _restock_use_summoning_stones
    )
    ...
    current_build_settings = (
        ...
        use_honeycomb,
        _restock_res_scroll,
        _restock_use_summoning_stones,
        _loop_queue,
        _aggro_range_forward,
        _aggro_range_reverse1,
        _aggro_range_reverse2,
    )
