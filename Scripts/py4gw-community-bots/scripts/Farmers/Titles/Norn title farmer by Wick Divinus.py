...
def bot_routine(bot: Botting) -> None:
    ...
    bot.States.AddCustomState(
        lambda: _refresh_path_to_revelations_if_completed(bot), "Refresh Path to Revelations If Completed"
    )
...
def _dispatch_to_account_emails(
    account_emails: list[str], command, params, extra_data=("", "", "", "")
) -> list[tuple[str, int]]:
...
def _min_party_morale(target_morale: int = _HONEYCOMB_TARGET_MORALE) -> int:
    try:
        morale_values = [
            int(morale) for _, morale in (GLOBAL_CACHE.Party.GetPartyMorale() or []) if int(morale or 0) > 0
        ]
        if morale_values:
