...
from Core.botting_src.property import Property
...
bot = Botting(
    "Deldrimor title farm by Wick Divinus",
...
# Hero config
_hero_cfg = JsonFactory(
    "Widgets/Bots/Deldrimor title farm by Wick Divinus/Heroes.json"
)  # account scope; hero config is per-account/character
...
_HERO_OPTIONS: List[HeroType] = [HeroType.None_] + sorted(
    [h for h in HeroType if h != HeroType.None_], key=lambda h: _humanize_hero_name(h.name)
)
...
def Snowman(bot: Botting):
    ...
    bot.States.AddCustomState(
        lambda x=-23884.0, y=13954.0, d=0x84: _do_dialog_at(bot, x, y, d, broadcast_to_alts=False), "Blessing Dialog"
    )
...
def _leave_party_before_sifhalla_travel(bot: Botting):
    if _party_mode == 1:
        ...
        yield from _wait_for_alt_dispatch_completion(
            "leave_party", leave_refs, SharedCommandType.LeaveParty, timeout_ms=5000
        )
        ...
        if acc.AccountEmail != my_email:
            for name in _PRETRAVEL_DISABLE_WIDGETS:
                GLOBAL_CACHE.ShMem.SendMessage(
                    my_email, acc.AccountEmail, SharedCommandType.DisableWidget, (0, 0, 0, 0), (name, "", "", "")
                )
    yield from Routines.Yield.wait(1500)
    ...
    for acc in GLOBAL_CACHE.ShMem.GetAllAccountData():
        if acc.AccountEmail != my_email:
            for name in _MERCHANT_MANAGED_WIDGETS:
                idx = int(
                    GLOBAL_CACHE.ShMem.SendMessage(
                        my_email, acc.AccountEmail, SharedCommandType.EnableWidget, (0, 0, 0, 0), (name, "", "", "")
                    )
                )
                if idx >= 0:
...
def _gh_merchant_setup_if_enabled(bot: Botting, outpost_id: int):
    ...
    if _merchant_sell_materials and mat_xy:
        ...
        if merchant_xy:
            ...
            leftover_refs = (
                _dispatch_to_alts(
                    SharedCommandType.MerchantMaterials, (mx, my, 0, 0), ("sell_merchant_leftovers", "", "10", "")
                )
                if _party_mode == 1
                else []
            )
    ...
    if merchant_xy:
        ...
        sell_gold_refs = (
            _dispatch_to_alts(
                SharedCommandType.MerchantMaterials, (mx, my, 0, 0), ("sell_nonsalvageable_golds", "", "", "")
            )
            if _party_mode == 1
            else []
        )
        ...
        sell_scroll_refs = (
            _dispatch_to_alts(
                SharedCommandType.MerchantMaterials, (mx, my, 0, 0), ("sell_scrolls", _SCROLL_MODEL_FILTER, "", "")
            )
            if _party_mode == 1
            else []
        )
        ...
        kit_refs = (
            _dispatch_to_alts(SharedCommandType.MerchantItems, (mx, my, _id_kits_target, _salvage_kits_target))
            if _party_mode == 1
            else []
        )
...
def _load_consumable_settings(bot: Botting) -> None:
    ...
    _summoning_stones_restock_target = max(
        0,
        min(
            _MAX_CONSUMABLE_RESTOCK_TARGET,
            int(
...
def _draw_settings(bot: Botting):
    ...
    new_summoning_target = PyImGui.input_int(
        "Summoning Stones restock target##deldrimor_summoning_stones_target", _summoning_stones_restock_target
    )
...
REFORGED_TEXTURE = os.path.join(
    PySystem.Console.get_projects_path(), "Sources", "Wick Divinus bots", "Reforged_Icon.png"
)
