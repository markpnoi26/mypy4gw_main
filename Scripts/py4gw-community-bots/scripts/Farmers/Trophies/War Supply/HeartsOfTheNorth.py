...
def CheckAndDepositGold(bot: Botting) -> None:
    ...
    def _check_and_deposit_gold(bot: Botting):
        ...
        if BotSettings.DEBUG:
            print(
                f"[DEBUG] CheckAndDepositGold: current_map={current_map}, gold={gold_on_char}, storage={gold_in_storage}"
            )
        ...
        if gold_on_char > BotSettings.GOLD_THRESHOLD_DEPOSIT:
            ...
            if gold_in_storage < 800000:
                ...
                if BotSettings.DEBUG:
                    print(
                        f"Storage ({gold_in_storage}) has reached 800k+, keeping gold on character for ecto purchases"
                    )
        else:
            if BotSettings.DEBUG:
    ...
    def _enter_quest(bot: Botting):
        ...
        while not PyDialog.PyDialog.is_dialog_active():
            if time.time() > deadline:
                ConsoleLog(
                    MODULE_NAME,
                    "[EnterQuest] Timed out waiting for Keiran's dialog",
        ...
        ConsoleLog(
            MODULE_NAME,
            f"[EnterQuest] base={hex(base_id)} offset={hex(_HOTN_DIALOG_BASE_OFFSET + mission.mission_slot)} -> sending {hex(target_id)}",
        )
...
def RunQuest(bot: Botting) -> None:
    ...
    def _fresh_build():
        bot.OverrideBuild(KeiranThackerayEOTN(fsm=bot.config.FSM, debug_fn=lambda: BotSettings.DEBUG))
        bot.Templates.AggressiveForceHeroAI(
            enable_imp=False
        )  # re-arm HeroAI template each run (Pacifist disables it at run end)
        yield
...
def _draw_stats_tab():
    ...
    _tflags = PyImGui.TableFlags.Borders | PyImGui.TableFlags.RowBg | PyImGui.TableFlags.SizingStretchSame
    ...
    _vflags = PyImGui.TableFlags.Borders | PyImGui.TableFlags.RowBg | PyImGui.TableFlags.SizingStretchProp
...
def _draw_settings(bot: Botting):
    ...
    changed = (
        use_war_supplies != BotSettings.WAR_SUPPLIES_ENABLED
        or custom_bow_id != BotSettings.CUSTOM_BOW_ID
        or gold_threshold != BotSettings.GOLD_THRESHOLD_DEPOSIT
        or debug != BotSettings.DEBUG
        or show_help != BotSettings.SHOW_HELP
    )
...
def _draw_hotn_help() -> None:
    import PyImGui
    from Core import ImGui, Color, IconsFontAwesome5
    ...
    PyImGui.text_wrapped(
        "Welcome to the complete Hearts of the North farming bot. This bot is still in development. As additional missions have been mapped they will be added. Expect issues with WIP missions, but feel free to test."
    )
    ...
    if PyImGui.collapsing_header("Currently Working Missions"):
        ...
        PyImGui.text_colored(
            IconsFontAwesome5.ICON_CHECK + "  Auspicious Beginnings", green_color.to_tuple_normalized()
        )
        PyImGui.text_colored(
            IconsFontAwesome5.ICON_ARROWS_ROTATE + "  A Vengance of Blades", wip_color.to_tuple_normalized()
        )
        PyImGui.text_colored(
            IconsFontAwesome5.ICON_ARROWS_ROTATE + "  Shadows in the Jungle", wip_color.to_tuple_normalized()
        )
        PyImGui.text_colored(IconsFontAwesome5.ICON_ARROWS_ROTATE + "  Rise", wip_color.to_tuple_normalized())
        PyImGui.text_colored(
            IconsFontAwesome5.ICON_TIMES + "  All Missions in Sequence", red_color.to_tuple_normalized()
        )
        PyImGui.unindent(10)
        PyImGui.spacing()
...
def _draw_hotn_window(icon_path: str) -> None:
    ...
    """
    from Core import ImGui, Color, Routines
    from Core.py4gwcorelib_src.Settings import Settings
    from Core.ImGui_src.IconsFontAwesome5 import IconsFontAwesome5
    from Core.Py4GWcorelib import ConsoleLog, Console
    ...
    if ImGui.Begin(
        ...
        if PyImGui.begin_tab_bar(bot.config.bot_name + "_tabs"):
            ...
            if PyImGui.begin_tab_item("Main"):
                ...
                current_header_step, header_for_current, current_step, total_steps, step_name, finished = (
                    bot.UI._find_current_header_step()
                )
                ...
                if PyImGui.begin_table(
                    ...
                    _active_label = (
                        "All Missions (Sequence)" if BotSettings.SEQUENCE_MODE else BotSettings.SELECTED_MISSION
                    )
                ...
                PyImGui.separator()
                btn_icon = (
                    IconsFontAwesome5.ICON_STOP_CIRCLE if bot.config.fsm_running else IconsFontAwesome5.ICON_PLAY_CIRCLE
                )
                ...
                PyImGui.progress_bar(
                    bot.config.state_percentage, inner_w, 0, f"{bot.config.state_percentage * 100:.2f}%"
                )
