...
def _resolve_section_from_step(step_name: str) -> str:
    if isinstance(step_name, str) and (
        "Path_to_Nicholas" in step_name or "Talus_Chute" in step_name or "Nicholas_the_Traveler" in step_name
    ):
        return "nick"
    return "farm"
...
def OnPartyWipe(bot: "Botting"):
    ...
    if isinstance(current_step, str):
        ...
        _WIPE_RESUME_SECTION = (
            _resolve_section_from_step(current_step)
            if _resolve_section_from_step(current_step) != "farm"
            else _WIPE_RESUME_SECTION
        )
    else:
        _WIPE_RESUME_STEP = ""
        _WIPE_RESUME_SECTION = _resolve_section_from_fsm(bot)
