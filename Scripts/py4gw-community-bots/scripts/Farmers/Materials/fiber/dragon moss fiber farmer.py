...
from Core.enums import ModelID, Range
from Core.enums_src.IO_enums import Key
from Core.py4gwcorelib_src.Keystroke import Keystroke
...
class DragonMossRangerAssassin(BuildMgr):
    ...
    def _maintain_defenses(self):
        ...
        if (not has_shadow_form or shadow_form_remaining <= 3500) and Routines.Checks.Skills.IsSkillSlotReady(
            SHADOW_FORM_SLOT
        ):
            if (
...
def _recover_from_failure(bot_instance: Botting, reason: str):
    ...
    ConsoleLog(
        BOT_NAME,
        f"Recovery path returned to outpost for {reason}; jumping to Ensure Anjeka.",
        PySystem.Console.MessageType.Info,
    )
...
def recover_to_anjekas(resign_if_alive: bool):
    ...
    if Map.IsExplorable():
        if player_id and Agent.IsDead(player_id):
            ConsoleLog(
                BOT_NAME,
                "Player is dead during recovery; waiting briefly before returning to outpost.",
                PySystem.Console.MessageType.Info,
            )
        ...
        transition_started = Map.IsMapLoading() or Map.IsOutpost() or Map.IsMapIDMatch(Map.GetMapID(), ANJEKAS_SHRINE)
...
def requires_town_checkpoint() -> bool:
    ensure_rotation_settings_loaded()
    return rotation_settings.enable_crafting_rotation or rotation_settings.auto_salvage_dragon_root_on_town_return
...
def draw_rotation_settings() -> None:
    ...
    new_fiber_index = PyImGui.combo(
        "Fiber threshold", fiber_index, [str(value) for value in CRAFTING_THRESHOLD_OPTIONS]
    )
    ...
    new_bones_index = PyImGui.combo(
        "Bones threshold", bones_index, [str(value) for value in CRAFTING_THRESHOLD_OPTIONS]
    )
...
def calculate_inventory_scroll_crafting_batch_count() -> int:
    ...
    current_skill_points, _ = Player.GetSkillPointData()
    if (
        Inventory.GetFreeSlotCount() <= 0
        and GLOBAL_CACHE.Inventory.GetFirstModelID(SCROLL_OF_RESURRECTION_MODEL_ID) == 0
    ):
        return 0
    return calculate_scroll_crafting_batch_count(
        current_fiber,
...
def log_character_match_debug(configured_name: str, pregame_names: list[str], available_names: list[str]) -> None:
    ...
    if not pregame_names:
        ...
        ConsoleLog(
            BOT_NAME, f"Pregame character list ({len(pregame_names)} entries):", PySystem.Console.MessageType.Info
        )
    ...
    if not available_names:
        ...
        ConsoleLog(
            BOT_NAME, f"Available character list ({len(available_names)} entries):", PySystem.Console.MessageType.Info
        )
...
def resolve_character_match(configured_name: str, character_names: list[str]) -> tuple[int, str, str] | None:
    ...
    loose_matches = [
        (index, name) for index, name in enumerate(character_names) if loose_character_name(name) == configured_loose
    ]
    ...
    if configured_loose:
        partial_matches = [
            (index, name)
            for index, name in enumerate(character_names)
            if configured_loose in loose_character_name(name) or loose_character_name(name) in configured_loose
        ]
...
def reroll_character_for_crafting(target_character_name: str, timeout_ms: int = CRAFTING_REROLL_TIMEOUT_MS) -> bool:
    ...
    if current_player_name and normalize_character_name(current_player_name) == normalize_character_name(
    ...
    if not Map.Pregame.InCharacterSelectScreen():
        ConsoleLog(
            BOT_NAME, "Logging out to character select for crafting rotation.", PySystem.Console.MessageType.Info
        )
    ...
    if not (
        ...
        ConsoleLog(
            BOT_NAME, "Character select never reported a chosen character index.", PySystem.Console.MessageType.Error
        )
        return False
    ...
    while current_index != character_index:
        if Utils.GetBaseTimestamp() - start_time > timeout_ms:
            ...
            log_character_match_debug(
                target_character_name, get_pregame_character_names(), get_available_character_names()
            )
            return False
    ...
    if not matched_loaded_character:
        ConsoleLog(
            BOT_NAME,
            (f"Character switch verification failed: expected '{resolved_name}', " f"but loaded '{active_name}'."),
            PySystem.Console.MessageType.Error,
        )
        return False
...
def deposit_scrolls_to_storage() -> None:
    ...
    if remaining_scrolls > 0:
        ...
        ConsoleLog(
            BOT_NAME,
            "All crafted Scrolls of Resurrection were deposited to storage.",
            PySystem.Console.MessageType.Success,
        )
...
def withdraw_material_to_inventory_target(model_id: int, target_inventory_count: int, material_name: str) -> bool:
    ...
    if current_inventory_count >= target_inventory_count:
        ...
        ConsoleLog(
            BOT_NAME,
            f"{material_name} in inventory after withdrawal: {current_inventory_count}.",
            PySystem.Console.MessageType.Info,
        )
        ConsoleLog(
            BOT_NAME,
...
def switch_back_to_farmer_after_crafting(context_message: str) -> bool:
    ...
    if not farmer_name:
        ConsoleLog(
            BOT_NAME,
            "No farmer character is recorded for the crafting rotation return path.",
            PySystem.Console.MessageType.Warning,
        )
        return False

    ConsoleLog(
        BOT_NAME, f"{context_message} -> switching back to farmer '{farmer_name}'.", PySystem.Console.MessageType.Info
    )
...
def reach_and_interact_kwat() -> bool:
    for attempt in range(1, KWAT_MAX_REACH_ATTEMPTS + 1):
        ...
        if attempt < KWAT_MAX_REACH_ATTEMPTS:
            ConsoleLog(
                BOT_NAME,
                "Kwat interaction did not open the crafter window yet; retrying.",
                PySystem.Console.MessageType.Warning,
            )
            yield from Routines.Yield.wait(KWAT_RETRY_DELAY_MS)
    ...
    ConsoleLog(
        BOT_NAME,
        f"Failed to interact with {KWAT_NPC_NAME} after {KWAT_MAX_REACH_ATTEMPTS} attempts.",
        PySystem.Console.MessageType.Warning,
    )
    return False
...
def run_scroll_crafting_rotation(bot_instance: Botting) -> bool:
    ...
    if not crafter_name:
        ConsoleLog(
            BOT_NAME,
            "Crafting rotation is enabled, but no crafter character is configured.",
            PySystem.Console.MessageType.Warning,
        )
        return False
    ...
    try:
        if farmer_name and crafter_name != farmer_name:
            ConsoleLog(
                BOT_NAME,
                f"Switching from farmer '{farmer_name}' to crafter '{crafter_name}'.",
                PySystem.Console.MessageType.Info,
            )
            if not (
                yield from reroll_character_for_crafting(
        ...
        planned_scroll_count, target_fiber_inventory, target_bones_inventory = (
            calculate_balanced_material_withdraw_targets(available_crafting_gold)
        )
...
def maybe_run_crafting_rotation(bot_instance: Botting):
    ...
    if not rotation_succeeded:
        farmer_restored = bool(runtime.farmer_character_name) and normalize_character_name(
            Player.GetName()
        ) == normalize_character_name(runtime.farmer_character_name)
        if farmer_restored:
            ConsoleLog(
                BOT_NAME,
...
def run_dragon_moss_farm(bot_instance: Botting):
    ...
    try:
        ...
        if not (
            ...
            ConsoleLog(
                BOT_NAME, "Failed to move into Return cast range. Aborting run.", PySystem.Console.MessageType.Warning
            )
            return
        ...
        if not (
            ...
            ConsoleLog(
                BOT_NAME,
                "Return failed to cast on the gate anchor. Aborting run.",
                PySystem.Console.MessageType.Warning,
            )
            return
        ...
        if not kill_window_completed:
            ConsoleLog(
                BOT_NAME,
                "Kill window did not finish cleanly. Marking run as failed.",
                PySystem.Console.MessageType.Warning,
            )
            return
        ...
        if not loot_completed:
            ConsoleLog(
                BOT_NAME,
                "Loot phase ended with pending drops. Marking run as failed.",
                PySystem.Console.MessageType.Warning,
            )
            return
...
def reset_run():
    ...
    if runtime.last_run_succeeded:
        ...
        runtime.failed_runs += 1
        ConsoleLog(
            BOT_NAME, f"Run failed ({runtime.failed_runs} total failures).", PySystem.Console.MessageType.Warning
        )
...
def cast_targeted_skill_when_ready(
    ...
    while Utils.GetBaseTimestamp() - start_time <= ready_timeout_ms:
        ...
        if target_agent_id == 0 or not Agent.IsValid(target_agent_id) or not Agent.IsAlive(target_agent_id):
            ConsoleLog(
                BOT_NAME,
                f"{skill_name} target {target_agent_id} is no longer valid.",
                PySystem.Console.MessageType.Warning,
            )
            return False
        ...
        if Routines.Checks.Skills.IsSkillSlotReady(slot) and Routines.Checks.Skills.CanCast():
            if (
                ...
                ConsoleLog(
                    BOT_NAME, f"Casted {skill_name} on target {target_agent_id}.", PySystem.Console.MessageType.Info
                )
                return True
        yield from Routines.Yield.wait(100)
    ...
    ConsoleLog(
        BOT_NAME,
        f"{skill_name} was not ready to cast on target {target_agent_id} in time.",
        PySystem.Console.MessageType.Warning,
    )
    return False
...
def ensure_target_in_skill_range(
    ...
    if target_agent_id == 0 or not Agent.IsValid(target_agent_id) or not Agent.IsAlive(target_agent_id):
        ConsoleLog(
            BOT_NAME,
            f"{skill_name} target {target_agent_id} is invalid before range check.",
            PySystem.Console.MessageType.Warning,
        )
        return False
...
def cast_post_return_setup():
    ConsoleLog(
        BOT_NAME, "Post-Return setup: casting Winnowing then Serpent's Quickness.", PySystem.Console.MessageType.Info
    )
    if not (
    ...
    while True:
        ...
        if nearby_moss_count == 0:
            ...
            if clear_polls >= KILL_CLEAR_POLLS_REQUIRED:
                ConsoleLog(
                    BOT_NAME,
                    "Kill window completed. No nearby Dragon Moss remain.",
                    PySystem.Console.MessageType.Success,
                )
                return True
        else:
...
def loot_run_drops():
    ...
    while True:
        ...
        if loot_agent_ids:
            ...
            clear_polls = 0
            ConsoleLog(
                BOT_NAME, f"Attempting to loot {len(loot_agent_ids)} item(s).", PySystem.Console.MessageType.Info
            )
        ...
        if seen_loot and clear_polls >= LOOT_CLEAR_POLLS_REQUIRED:
            runtime.failed_loot_agent_ids.clear()
            ConsoleLog(
                BOT_NAME, "Loot phase completed. No eligible drops remain nearby.", PySystem.Console.MessageType.Success
            )
            return True
