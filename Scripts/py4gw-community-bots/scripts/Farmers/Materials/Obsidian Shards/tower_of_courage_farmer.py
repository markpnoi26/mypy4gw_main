...
from Core.enums import name_to_map_id
import PyImGui

from dev.reference.tower_of_courage import loot_policy
...
class TowerOfCourageRangerAssassin(BuildMgr):
    ...
    def _maintain_defenses(self):
        ...
        if self.refresh_i_am_unstoppable and Routines.Checks.Skills.IsSkillSlotReady(I_AM_UNSTOPPABLE_SLOT):
            return (
        ...
        if Routines.Checks.Skills.IsSkillSlotReady(SHROUD_OF_DISTRESS_SLOT) and (
            not GLOBAL_CACHE.Effects.HasEffect(player_id, SHROUD_OF_DISTRESS_SKILL_ID) or shroud_time_remaining < 10000
        ):
            return (
                yield from Routines.Yield.Skills.CastSkillSlot(
    ...
    while Utils.GetBaseTimestamp() - start_time <= ready_timeout_ms:
        ...
        if Routines.Checks.Skills.IsSkillSlotReady(slot) and Routines.Checks.Skills.CanCast():
            if (yield from cast_skill_slot(slot, aftercast_delay=aftercast_delay)):
                if (
                    yield from wait_for_condition(
                        lambda: not Routines.Checks.Skills.IsSkillSlotReady(slot) or Agent.IsDead(Player.GetAgentID()),
                        timeout_ms=1000,
                        step_ms=50,
                    )
                ):
...
def wait_for_ranger_kill_skills() -> bool:
    ...
    while Utils.GetBaseTimestamp() - start_time <= WHIRLING_READY_TIMEOUT_MS:
        if Agent.IsDead(Player.GetAgentID()):
            return False
        if Routines.Checks.Skills.IsSkillSlotReady(MENTAL_BLOCK_SLOT) and Routines.Checks.Skills.IsSkillSlotReady(
            WHIRLING_DEFENSE_SLOT
        ):
            return True
        yield from Routines.Yield.wait(100)
