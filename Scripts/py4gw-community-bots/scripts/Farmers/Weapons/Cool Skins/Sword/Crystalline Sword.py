from Core import Botting, get_texture_for_model, ModelID, Routines, ConsoleLog, Map, Agent, Player
...
def DetectChestAndOpen(bot: Botting, max_distance=1000, max_attempts=3):
    ...
    for attempt in range(max_attempts):
        ...
        ConsoleLog(
            BOT_NAME,
            f"[DEBUG] Tentative {attempt + 1}/{max_attempts} - GetNearestGadget = {chest_id}",
            PySystem.Console.MessageType.Warning,
        )

        if chest_id == 0:
            ConsoleLog(
                BOT_NAME, f"[INFO] Aucun coffre trouve (tentative {attempt + 1})", PySystem.Console.MessageType.Info
            )
        ...
        ConsoleLog(
            BOT_NAME,
            f"[SUCCESS] Coffre trouve - ID:{chest_id}, PlayerNum:{player_num}",
            PySystem.Console.MessageType.Warning,
        )
        ...
        if chest_pos:
            ...
            ConsoleLog(
                BOT_NAME, f"[ACTION] Deplacement vers ({chest_x:.0f}, {chest_y:.0f})", PySystem.Console.MessageType.Info
            )
            ...
            if not chest_still_exists:
                ConsoleLog(
                    BOT_NAME, f"[SUCCESS] Coffre {chest_id} ouvert avec succes!", PySystem.Console.MessageType.Info
                )
                ...
                ConsoleLog(
                    BOT_NAME,
                    f"[WARNING] Coffre {chest_id} toujours present, nouvelle tentative...",
                    PySystem.Console.MessageType.Warning,
                )
                if attempt < max_attempts - 1:
                    yield from Routines.Yield.wait(500)
                    ...
                    ConsoleLog(
                        BOT_NAME,
                        f"[WARNING] Abandon apres {max_attempts} tentatives, passage au suivant",
                        PySystem.Console.MessageType.Warning,
                    )
                    opened_chests.add(chest_id)
...
def UnstuckPlayer():
    ...
    ConsoleLog(
        BOT_NAME, f"[UNSTUCK] Joueur bloque detecte, tentative de deblocage...", PySystem.Console.MessageType.Warning
    )
    ...
    for offset_x, offset_y in offsets:
        ...
        ConsoleLog(
            BOT_NAME,
            f"[UNSTUCK] Tentative deplacement vers ({new_x:.0f}, {new_y:.0f})",
...
def _on_party_wipe(bot: "Botting"):
    ...
    while Agent.IsDead(Player.GetAgentID()):
        ...
        if not Routines.Checks.Map.MapValid():
            ConsoleLog(
                BOT_NAME,
                f"[WIPE] Teleporte en ville apres mort du PNJ, relance du run...",
                PySystem.Console.MessageType.Warning,
            )
            ...
            ConsoleLog(
                BOT_NAME,
                f"[WIPE] Redemarrage du run depuis la boucle, team conservee",
                PySystem.Console.MessageType.Info,
            )
            return
...
def tooltip():
    ...
    PyImGui.bullet_text(
        "Designed for Normal Mode (NM) for faster and easy run, but can be change editing True or False in the code."
    )
