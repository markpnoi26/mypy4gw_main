...
from Core import Botting, Py4GW, Quest, Player, GLOBAL_CACHE, Routines, PyAgent, Skill
...
def broadcast_item(model_id: int, repeat=5, use_locally=True, delay=100):
    ...
    for email in get_other_accounts():
        GLOBAL_CACHE.ShMem.SendMessage(
            ...
            command=SharedCommandType.UseItem,
    ...
    if use_locally:
        for _ in range(max(1, int(repeat))):
            ...
            if not item_id:
                ConsoleLog(
                    "ItemBroadCaster",
                    f"ItemId für Model {model_id} nicht gefunden.",
...
def broadcast_skill(skill_name: str, cast_locally=True, delay=300):
    ...
    for email in get_other_accounts():
        GLOBAL_CACHE.ShMem.SendMessage(
            sender_email=sender,
            receiver_email=email,
            command=SharedCommandType.UseSkill,
...
def UseTrap():
    ...
    bot.States.AddCustomState(
        lambda: broadcast_item(SKELETON_TRAP_ID, repeat=1, use_locally=True, delay=0), "Place Skeleton Trap"
    )
