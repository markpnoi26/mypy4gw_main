...
class YAVB_FSM:
    ...
    def _initialize_fsm(self):
        self.FSM.AddState(
            name="Deactivate Inventory Handler",
            execute_fn=self.FSM_Helpers.DeactivateInventoryHandler,
        )
        self.FSM.AddState(name="Deactivate Hero AI", execute_fn=self.FSM_Helpers.DeactivateHeroAI)
        self.FSM.AddYieldRoutineStep(
            name="Travel to Longeyes Ledge", coroutine_fn=self.FSM_Helpers.TravelToLongeyesLedge
        )
        self.FSM.AddYieldRoutineStep(
            name="Load SkillBar",
            coroutine_fn=self.FSM_Helpers.LoadSkillBar,
        )
        self.FSM.AddYieldRoutineStep(
            name="Inventory Handling",
            coroutine_fn=self.FSM_Helpers.InventoryHandling,
        )
        ...
        self.FSM.AddYieldRoutineStep(
            name="Leave Outpost",
            coroutine_fn=self.FSM_Helpers.LeaveOutpost,
        )
        self.FSM.AddYieldRoutineStep(
            name="Wait for Bjora Marches Map Load", coroutine_fn=self.FSM_Helpers.WaitforBjoraMarchesMapLoad
        )
        self.FSM.AddState(
            name="Add Bjora Marches Stuck Coroutine",
        ...
        self.FSM.AddState(name="Use Cupcake", execute_fn=self.FSM_Helpers.UseCupcake, transition_delay_ms=100)
        self.FSM.AddYieldRoutineStep(name="Traverse Bjora Marches", coroutine_fn=self.FSM_Helpers.TraverseBjoraMarches)
        self.FSM.AddYieldRoutineStep(
            name="Wait for Jaga Moraine Map Load", coroutine_fn=self.FSM_Helpers.WaitforJagaMoraineMapLoad
        )
        self.FSM.AddState(
            name="Remove Bjora Marches Stuck Coroutine",
        ...
        self.FSM.AddYieldRoutineStep(
            name="Wait for right aggro ball", coroutine_fn=self.FSM_Helpers.WaitforRightAggroBall
        )
        self.FSM.AddYieldRoutineStep(
            name="Farming Route to Kill Spot", coroutine_fn=self.FSM_Helpers.FarmingRoutetoKillSpot
        )
        self.FSM.AddYieldRoutineStep(name="Kill Enemies", coroutine_fn=self.FSM_Helpers.KillEnemies)
        self.FSM.AddState(
            name="Remove Skill Casting Coroutine", execute_fn=self.FSM_Helpers.RemoveSkillCastingCoroutine
        )
        self.FSM.AddYieldRoutineStep(name="Loot Items", coroutine_fn=self.FSM_Helpers.LootItems)
        self.FSM.AddYieldRoutineStep(
            name="Identify and Salvage Items", coroutine_fn=self.FSM_Helpers.IdentifyAndSalvageItems
        )
        ...
        self.FSM.AddYieldRoutineStep(
            name="Wait for Bjora Marches return Map Load",
            coroutine_fn=self.FSM_Helpers.WaitforBjoraMarches_returnMapLoad,
        )
