from Core.Builds.CombatAutomatorExcluded.ShadowFormAssassinVaettir import ShadowFormAssassinVaettir
from Core.Builds.CombatAutomatorExcluded.ShadowFormMesmerVaettir import ShadowFormMesmerVaettir
from .LogConsole import LogConsole
from Core import GLOBAL_CACHE, Agent
from Core import Routines
from Core.enums import ModelID, TitleID, SharedCommandType, Range
...
class _FSM_Helpers:
    ...
    def _stop_execution(self):
        if self._parent.script_running:
            ...
            build = (
                self._parent.build or ShadowFormAssassinVaettir()
                if self._parent.primary_profession == Profession.Assassin.value
                else ShadowFormMesmerVaettir()
            )
    ...
    def _get_materials_to_sell(self):
        ...
        materials_to_sell = ItemArray.Filter.ByCondition(
            bag_item_array, lambda item_id: GLOBAL_CACHE.Item.Type.IsMaterial(item_id)
        )
    ...
    def _get_unidentified_items(self):
        ...
        unidentified_items = ItemArray.Filter.ByCondition(
            bag_item_array, lambda item_id: not (GLOBAL_CACHE.Item.Usage.IsIdentified(item_id))
        )
        white_items = ItemArray.Filter.ByCondition(
            bag_item_array,
            lambda item_id: GLOBAL_CACHE.Item.Rarity.IsWhite(item_id)
            and not GLOBAL_CACHE.Item.Usage.IsIdentified(item_id),
        )
        unidentified_items = [
            item for item in unidentified_items if item not in white_items
        ]  # Remove white items from unidentified items
    ...
    def _get_items_to_salvage(self):
        bags_to_check = GLOBAL_CACHE.ItemArray.CreateBagList(1, 2, 3, 4)
        bag_item_array = GLOBAL_CACHE.ItemArray.GetItemArray(bags_to_check)
        white_items_to_salvage = ItemArray.Filter.ByCondition(
            bag_item_array,
            lambda item_id: GLOBAL_CACHE.Item.Usage.IsSalvageable(item_id)
            and GLOBAL_CACHE.Item.Rarity.IsWhite(item_id),
        )
        items_to_salvage = ItemArray.Filter.ByCondition(
            bag_item_array,
            lambda item_id: GLOBAL_CACHE.Item.Usage.IsSalvageable(item_id)
        ...
        if current_map != self._parent.LONGEYES_LEDGE:
            ...
            if not (
                yield from Routines.Yield.Map.TravelToOutpost(
                    self._parent.LONGEYES_LEDGE, log=self._parent.detailed_logging
                )
            ):
    ...
    def LoadSkillBar(self):
        ...
        if not (yield from build.ValidateSkills()):
            ...
            self._parent.LogMessage(
                "Skillbar validation", "FAILED, Check your skillbar configuration.", LogConsole.LogSeverity.ERROR
            )
            yield from self._stop_execution()
        else:
            self._parent.AdvanceProgress(0.66)
    ...
    def InventoryHandling(self):
        ...
        for cycle in range(2):
            if self._inventory_handling_checks():
                ...
                self._parent.AdvanceProgress(progress)
                self._parent.LogMessage(
                    "Inventory Handling", "checks failed, starting Inventory handling", LogConsole.LogSeverity.INFO
                )
                interact_result = yield from Routines.Yield.Agents.InteractWithAgentXY(-23110, 14942)
                if not interact_result:
                    self._parent.LogMessage(
                        "Inventory Handling",
                        "Failed to interact with Merchant, stopping script.",
                        LogConsole.LogSeverity.ERROR,
                    )
                    yield from self._stop_execution()
                    return
                ...
                if len(self._get_materials_to_sell()) > 0:
                    self._parent.LogMessage(
                        "Inventory Handling", f"Selling Materials to make Space", LogConsole.LogSeverity.INFO
                    )
                    yield from Routines.Yield.Merchant.SellItems(
                        self._get_materials_to_sell(), log=self._parent.detailed_logging
                    )
                ...
                if self._get_number_of_id_kits_to_buy() > 0:
                    ...
                    yield from Routines.Yield.Merchant.BuyIDKits(
                        self._get_number_of_id_kits_to_buy(), log=self._parent.detailed_logging
                    )
                ...
                if self._get_number_of_salvage_kits_to_buy() > 0:
                    self._parent.LogMessage(
                        "Inventory Handling", "Restocking Salvage Kits", LogConsole.LogSeverity.INFO
                    )
                    yield from Routines.Yield.Merchant.BuySalvageKits(
                        self._get_number_of_salvage_kits_to_buy(), log=self._parent.detailed_logging
                    )
                ...
                if len(self._get_unidentified_items()) > 0:
                    ...
                    yield from Routines.Yield.Items.IdentifyItems(
                        self._get_unidentified_items(), log=self._parent.detailed_logging
                    )
                ...
                if len(self._get_items_to_salvage()) > 0:
                    ...
                    yield from Routines.Yield.Items.SalvageItems(
                        self._get_items_to_salvage(), log=self._parent.detailed_logging
                    )
                ...
                self._parent.LogDetailedMessage(
                    "Inventory Handling", "No Inventory Handling needed, skipping.", LogConsole.LogSeverity.SUCCESS
                )
                break
    ...
    def WitdrawBirthdayCupcake(self):
        ...
        if GLOBAL_CACHE.Inventory.GetModelCountInStorage(ModelID.Birthday_Cupcake.value) > 0:
            ...
            if not items_witdrawn:
                self._parent.LogMessage(
                    "Cupcake Usage", "Failed to withdraw Cupcake from Storage", LogConsole.LogSeverity.ERROR
                )
    ...
    def WitdrawPumpkinCookie(self):
        ...
        if total_needed_cookies > 0:
            self._parent.LogDetailedMessage(
                "Pumpkin Cookie",
                f"Withdrawing {total_needed_cookies} Cookies from Storage",
                LogConsole.LogSeverity.INFO,
            )
            items_witdrawn = GLOBAL_CACHE.Inventory.WithdrawItemFromStorageByModelID(
                ModelID.Pumpkin_Cookie.value, total_needed_cookies
            )
            self._parent.AdvanceProgress(0.5)
            if not items_witdrawn:
                self._parent.LogDetailedMessage(
                    "Pumpkin Cookie", "Failed to withdraw Pumpkin Cookies from Storage", LogConsole.LogSeverity.ERROR
                )
            yield from Routines.Yield.wait(150)
        else:
            self._parent.LogDetailedMessage(
                "Pumpkin Cookie",
                "No need to withdraw Pumpkin Cookies, already have enough.",
                LogConsole.LogSeverity.SUCCESS,
            )
    ...
    def WaitforBjoraMarchesMapLoad(self):
        ...
        if not wait_of_map_load:
            self._parent.LogMessage(
                "Map Load", "Timeout Loading Bjora Marches, stopping script.", LogConsole.LogSeverity.ERROR
            )
            yield from self._stop_execution()
    ...
    def AddBjoraMarchesStuckCoroutine(self):
        ...
        self._parent.LogDetailedMessage(
            "Stuck Coroutine", "Added to Bjora Marches Stuck Coroutines.", LogConsole.LogSeverity.INFO
        )
    ...
    def UseCupcake(self):
        ...
        if self._parent.use_cupcakes:
            self._parent.LogMessage(
                "Cupcake Usage", "Using Cupcake for Bjora Marches Traversal", LogConsole.LogSeverity.INFO
            )
            self._send_message(
    ...
    def WaitforJagaMoraineMapLoad(self):
        ...
        wait_of_map_load = yield from Routines.Yield.Map.WaitforMapLoad(
            self._parent.JAGA_MORAINE, log=self._parent.detailed_logging
        )
        if not wait_of_map_load:
            self._parent.LogMessage(
                "Map Load", "Timeout Loading Jaga Moraine, stopping script.", LogConsole.LogSeverity.ERROR
            )
            self._parent.run_to_jaga_stats.EndCurrentRun(failed=True, stuck_timeouts=1)
            self._parent.running_to_jaga = False
            yield from self._stop_execution()
    ...
    def RemoveBjoraMarchesStuckCoroutine(self):
        ...
        self._parent.LogDetailedMessage(
            "Stuck Coroutine", "Removed from Bjora Marches Stucvk Coroutines.", LogConsole.LogSeverity.INFO
        )
    ...
    def TakeBounty(self):
        ...
        self._parent.AdvanceProgress(0.33)
        timeout = yield from Routines.Yield.Movement.FollowPath(
            path_points_to_npc,
            custom_exit_condition=lambda: self._movement_eval_exit_on_map_loading_or_death(),
            timeout=20000,
        )
    ...
    def WaitforLeftAggroBall(self):
        ...
        self._parent.LogMessage(
            "Waiting for Left Aggro Ball", "Waiting for enemies to ball up.", LogConsole.LogSeverity.INFO
        )
    ...
    def WaitforRightAggroBall(self):
        ...
        self._parent.LogMessage(
            "Waiting for Right Aggro Ball", "Waiting for enemies to ball up.", LogConsole.LogSeverity.INFO
        )
    ...
    def RemoveSkillCastingCoroutine(self):
        ...
        self._parent.LogDetailedMessage(
            "Skill Casting Coroutine", "Removed from Coroutines.", LogConsole.LogSeverity.INFO
        )
    ...
    def LootItems(self):
        ...
        filtered_agent_ids = self.loot_singleton.GetfilteredLootArray(
            distance=Range.Earshot.value, multibox_loot=False, allow_unasigned_loot=True
        )
        yield from Routines.Yield.Items.LootItems(
            filtered_agent_ids, log=self._parent.detailed_logging, progress_callback=self._parent.AdvanceProgress
        )

    def IdentifyAndSalvageItems(self):
        self._parent.LogMessage(
            "Identifying and Salvaging Items",
            "Starting Identification and Salvaging Routine",
            LogConsole.LogSeverity.INFO,
        )
    ...
    def WaitforBjoraMarches_returnMapLoad(self):
        self._parent.LogMessage("Waiting for Map Loading", "Bjora Marches", LogConsole.LogSeverity.INFO)
        wait_of_map_load = yield from Routines.Yield.Map.WaitforMapLoad(
            self._parent.BJORA_MARCHES, log=self._parent.detailed_logging
        )
        if not wait_of_map_load:
            self._parent.LogMessage(
                "Map Load", "Timeout Loading Bjora Marches, stopping script.", LogConsole.LogSeverity.ERROR
