...
class LogConsole:
    ...
    class LogEntry:
        def __init__(
            self, message: str, extra_info: Optional[str], severity: Optional['LogConsole.LogSeverity'] = None
        ):
            if severity is None:
    ...
    def __init__(
        ...
        self.window_flags = PyImGui.WindowFlags(PyImGui.WindowFlags.AlwaysAutoResize)
        ...
        if self.window_module.begin():
            if PyImGui.begin_child("Log Messages", (0, 0), True, PyImGui.WindowFlags.AlwaysVerticalScrollbar):
                if PyImGui.begin_table(
                    "LogTable", 3, PyImGui.TableFlags.RowBg | PyImGui.TableFlags.ScrollY | PyImGui.TableFlags.Borders
                ):
