...
class YAVB:
    def __init__(self):
        ...
        # window vars
        self.window_flags = PyImGui.WindowFlags(PyImGui.WindowFlags.AlwaysAutoResize | PyImGui.WindowFlags.MenuBar)
        ...
        self.option_window_module = ImGui.WindowModule(
            ...
            window_flags=PyImGui.WindowFlags(PyImGui.WindowFlags.AlwaysAutoResize),
        )
    ...
    def LogMessage(
        ...
        severity: LogConsole.LogSeverity = LogConsole.LogSeverity.INFO,
    ):
    ...
    def load_config(self):
        ...
        self.option_window_snapped_border = ih.get_str(
            "OptionWindow", "snapped_border", self.option_window_snapped_border
        )
        ...
        # Merchant
        self.identification_kits_restock = ih.get_int(
            "Merchant", "identification_kits_restock", self.identification_kits_restock
        )
        self.salvage_kits_restock = ih.get_int("Merchant", "salvage_kits_restock", self.salvage_kits_restock)
        self.keep_empty_inventory_slots = ih.get_int(
            "Merchant", "keep_empty_inventory_slots", self.keep_empty_inventory_slots
        )

        self.supported_professions = {k: v for k, v in ih.items("SupportedProfessions").items() if k.isdigit()}
    ...
    def HandleStuckBjoraMarches(self):
        while True:
            ...
            if self.current_run_node and self.current_run_node.GetRunDuration() > 30000:
                self.LogMessage(
                    "Stuck Detection", "Current run node is taking too long, resetting.", LogConsole.LogSeverity.WARNING
                )
                self.stuck_counter = 0
            ...
            if Map.GetMapID() == self.BJORA_MARCHES:
                ...
                if self.movement_check_timer.IsExpired():
                    ...
                    if self.old_player_position == current_player_pos:
                        self.LogMessage(
                            "Stuck Detection", "Player is stuck, sending stuck command.", LogConsole.LogSeverity.WARNING
                        )
                        Player.SendChatCommand("stuck")
        ...
        while True:
            ...
            if Map.GetMapID() == self.JAGA_MORAINE:
                ...
                if self.stuck_counter >= 10:
                    self.LogMessage(
                        "Stuck Detection", "Unrecoverable stuck detected, resetting.", LogConsole.LogSeverity.ERROR
                    )
                    self.stuck_counter = 0
                    build.SetStuckCounter(self.stuck_counter)
                    yield from self.FSM_Helpers._reset_execution()
