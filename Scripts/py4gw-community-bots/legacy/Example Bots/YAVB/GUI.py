from Core import PyImGui, ImGui, Color
            ...
            if PyImGui.begin_child("YAVB Options Child Window", (300, 250), True, PyImGui.WindowFlags.NoFlag):
                ...
                if PyImGui.collapsing_header("ID & Salvage Options"):
                    PyImGui.text_wrapped(
                        "ID & Salvage is handled by AutoHandler Module of the Inventory+ Widget. Configure it there."
                    )
                if PyImGui.collapsing_header("Merchant Options"):
                    PyImGui.text_wrapped(
                        "After inventory AutoHandler has finished, all remaining items are sent to the Merchant for id/selling."
                    )
                    PyImGui.separator()
                    PyImGui.push_item_width(100)
                    self.parent.identification_kits_restock = PyImGui.input_int(
                        "ID Kits to Restock", self.parent.identification_kits_restock
                    )
                    ImGui.show_tooltip("ID Kits to Restock")
                    self.parent.salvage_kits_restock = PyImGui.input_int(
                        "Salvage Kits to Restock", self.parent.salvage_kits_restock
                    )
                    ImGui.show_tooltip("Salvage Kits to Restock")
                    self.parent.keep_empty_inventory_slots = PyImGui.input_int(
                        "Keep Empty Inventory Slots", self.parent.keep_empty_inventory_slots
                    )
                ...
                if PyImGui.collapsing_header("Bot Options"):
                    ...
                    self.parent.use_pumpkin_cookies = PyImGui.checkbox(
                        "Use Pumpkin Cookies", self.parent.use_pumpkin_cookies
                    )
                    ImGui.show_tooltip("Use Pumpkin Cookies for clearing Death Penalty in case of death.")
                    self.parent.pumpkin_cookies_restock = PyImGui.input_int(
                        "Pumpkin Cookies Restock", self.parent.pumpkin_cookies_restock
                    )
        ...
        if self.parent.window_module.begin():
            if PyImGui.begin_menu_bar():
                ...
                if PyImGui.begin_menu("Options"):
                    self.parent.option_window_visible = PyImGui.checkbox(
                        "Show window", self.parent.option_window_visible
                    )
                ...
                if PyImGui.begin_menu("Console"):
                    ...
                    if prev_value != self.parent.detailed_logging:
                        self.parent.LogDetailedMessage(
                            "Detailed logging",
                            f"{'ENABLED' if self.parent.detailed_logging else 'DISABLED'}.",
                            LogConsole.LogSeverity.INFO,
                        )
                    ImGui.show_tooltip(
                        "Will output Extra Info to the YAVB Console,\nWill output Full Logging to the Py4GWConsole."
                    )
                    ...
                    if self.parent.console_snapped:
                        ...
                        if prev_value != self.parent.console_snapped_border:
                            self.parent.console.SetSnapped(
                                self.parent.console_snapped, self.parent.console_snapped_border
                            )
                        ...
                        if PyImGui.begin_table("YAVB maintable", 1, PyImGui.TableFlags.NoFlag):
                            ...
                            if PyImGui.begin_tab_bar("YAVB Tabs"):
                                if PyImGui.begin_tab_item("Main"):
                                    ...
                                    if PyImGui.button(icon + "##Playbutton"):
                                        if self.parent.script_running:
                                            if self.script_paused:
                                                ...
                                                self.parent.LogDetailedMessage(
                                                    "Script resumed", "", LogConsole.LogSeverity.INFO
                                                )
                                                ...
                                                self.parent.LogDetailedMessage(
                                                    "Script paused", "", LogConsole.LogSeverity.INFO
                                                )
                                            ...
                                            self.parent.LogDetailedMessage(
                                                "Script started", "", LogConsole.LogSeverity.INFO
                                            )
                                            self.parent.state = "Running"
                                    ...
                                    if not self.parent.script_running:
                                        PyImGui.push_style_color(
                                            PyImGui.ImGuiCol.Button, Color(50, 50, 50, 255).to_tuple_normalized()
                                        )
                                        PyImGui.push_style_color(
                                            PyImGui.ImGuiCol.ButtonHovered, Color(70, 70, 70, 255).to_tuple_normalized()
                                        )
                                        PyImGui.push_style_color(
                                            PyImGui.ImGuiCol.ButtonActive, Color(90, 90, 90, 255).to_tuple_normalized()
                                        )
                                        PyImGui.push_style_color(
                                            PyImGui.ImGuiCol.Text, Color(70, 70, 70, 255).to_tuple_normalized()
                                        )
                                    ...
                                    PyImGui.progress_bar(
                                        self.parent.state_percentage,
                                        (child_width - 10),
                                        0,
                                        f"{self.parent.state_percentage * 100:.2f}%",
                                    )
                                    PyImGui.pop_item_width()
                                    PyImGui.separator()
                                    ...
                                    PyImGui.progress_bar(
                                        ...
                                        (child_width - 10),
                                        0,
                                        f"{self.parent.overall_progress * 100:.2f}%",
                                    )
                                    PyImGui.pop_item_width()
                                ...
                                if PyImGui.begin_tab_item("Statistics"):
                                    ...
                                    PyImGui.text(
                                        f"Run Effectivity: {self.parent.farming_stats.GetRuneffectivity():.2f}%"
                                    )
                                    PyImGui.text(
                                        f"Kill Effectivity: {self.parent.farming_stats.GetKillEffectivity():.2f}%"
                                    )
                                    PyImGui.text(
                                        f"Avg Kills on Success: {self.parent.farming_stats.GetAverageKillsOnSuccess():.2f}"
                                    )
                                ...
                                if PyImGui.begin_tab_item("Debug"):
                                    ...
                                    for agent_id in filtered_agent_ids:
                                        ...
                                        PyImGui.text(
                                            f"agent_id: {agent_id}, name: {name}, item_id: {item_id}, model_id: {model_id}"
                                        )
