...
def IsSkillBarLoaded():
    ...
    if primary_profession != "Assassin" and secondary_profession != "Assassin":
        ...
        PySystem.Console.Log(
            ...
            f"{current_function} - This bot requires A/Any or Any/A to work, halting.",
...
FSM_vars.loot_chest.AddState(name="Reset Follow Path To Chest", execute_fn=lambda: ResetFollowPath())
FSM_vars.loot_chest.AddState(
    name="MoveToChest",
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.chest_found_pathing, FSM_vars.movement_handler
    ),
    run_once=False,
)
FSM_vars.loot_chest.AddState(
...
FSM_vars.loot_chest.AddState(
    name="InteractAgent", execute_fn=lambda: Routines.Targeting.InteractTarget(), transition_delay_ms=1000
)
FSM_vars.loot_chest.AddState(name="Accept Dialog", execute_fn=Player.SendDialog(2), transition_delay_ms=1000)
FSM_vars.loot_chest.AddState(
    name="Select Item",
...
FSM_vars.loot_chest.AddState(
    name="PickUpItem", execute_fn=lambda: Routines.Targeting.InteractTarget(), transition_delay_ms=1000
)
...
FSM_vars.state_machine.AddSubroutine(
    name="Inventory Handling", sub_fsm=FSM_vars.sell_to_vendor, condition_fn=lambda: DoesNeedInventoryHandling()
)
FSM_vars.state_machine.AddState(
...
FSM_vars.state_machine.AddState(
    name="Waiting for Explorable Map Load",
...
FSM_vars.state_machine.AddState(
    ...
    exit_condition=lambda: Routines.Movement.IsFollowPathFinished(
        FSM_vars.explorable_pathing, FSM_vars.movement_handler
    )
    or IsChestFound(max_distance=2500),
    run_once=False,
)
FSM_vars.state_machine.AddSubroutine(
    name="Loot Chest", sub_fsm=FSM_vars.loot_chest, condition_fn=lambda: IsChestFound(max_distance=2500)
)
