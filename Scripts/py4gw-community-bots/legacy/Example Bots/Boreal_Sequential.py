...
path_points_to_look_for_chest: List[Tuple[float, float]] = [
    (2928, -24873),
    (2724, -22040),
    (-371, -20086),
    (-3294, -18164),
...
def DrawWindow():
    ...
    if bot_variables.config.window_module.first_run:
        PyImGui.set_next_window_size(
            bot_variables.config.window_module.window_size[0], bot_variables.config.window_module.window_size[1]
        )
        PyImGui.set_next_window_pos(
            bot_variables.config.window_module.window_pos[0], bot_variables.config.window_module.window_pos[1]
        )
...
def RunBotSequentialLogic():
    ...
    while True:
        ...
        Routines.Sequential.Movement.FollowPath(
            path_points_to_exit_outpost, custom_exit_condition=lambda: Map.IsMapLoading()
        )
        ...
        Routines.Sequential.Movement.FollowPath(
            path_points_to_look_for_chest, custom_exit_condition=lambda: IsChestFound(max_distance=2500)
        )
        bot_variables.config.routine_finished = True
