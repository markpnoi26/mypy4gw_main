...
def IsSkillBarLoaded():
    ...
    if (primary_profession == "Assassin" and secondary_profession != "Mesmer") or (
        ...
        ConsoleLog(
            MODULE_NAME,
            f"{current_function} - This bot requires A/Me to work, halting.",
...
def filter_salvage_array():
    ...
    salvageable_items = ItemArray.Filter.ByCondition(
        salvageable_items, lambda item_id: Item.Usage.IsIdentified(item_id) or Item.Rarity.IsWhite(item_id)
    )
...
def print_filter_salvage_array():
    ...
    print(
        f"After identified/white filter: {[ (item_id, Item.Rarity.GetRarity(item_id)[1]) for item_id in salvageable_items ]}"
    )
    ...
    print(
        f"After salvageable filter: {[ (item_id, Item.Rarity.GetRarity(item_id)[1]) for item_id in salvageable_items ]}"
    )
    ...
    if not bot_variables.salvage_config.salvage_purples:
        ...
        print(
            f"After purple filter: {[ (item_id, Item.Rarity.GetRarity(item_id)[1]) for item_id in salvageable_items ]}"
        )
...
def filter_items_to_deposit():
    ...
    items_to_deposit = ItemArray.Filter.ByCondition(
        items_to_deposit, lambda item_id: Item.GetModelID(item_id) not in banned_models
    )
...
def RunBotSequentialLogic():
    ...
    while True:
        ...
        if not reset_from_jaga_moraine:
            ...
            # exit outpost
            Routines.Sequential.Movement.FollowPath(
                path_points=path_points_to_leave_outpost, custom_exit_condition=lambda: Map.IsMapLoading()
            )
            ...
            Routines.Sequential.Movement.FollowPath(
                path_points_to_traverse_bjora_marches, custom_exit_condition=lambda: player_is_dead_or_map_loading()
            )
        ...
        Routines.Sequential.Movement.FollowPath(
            path_points_to_npc, custom_exit_condition=lambda: player_is_dead_or_map_loading()
        )
        # take bounty
        ...
        bot_variables.config.pause_stuck_routine = False
        Routines.Sequential.Movement.FollowPath(
            path_points_to_farming_route1, custom_exit_condition=lambda: player_is_dead()
        )
        if handle_death():
            reset_from_jaga_moraine = False
            continue
        ...
        Routines.Sequential.Movement.FollowPath(
            path_points_to_farming_route2, custom_exit_condition=lambda: player_is_dead()
        )
        if handle_death():
            reset_from_jaga_moraine = False
            continue
        ...
        Routines.Sequential.Movement.FollowPath(
            path_points_to_killing_spot, custom_exit_condition=lambda: player_is_dead()
        )
        if handle_death():
            reset_from_jaga_moraine = False
            continue
        ...
        Routines.Sequential.Movement.FollowPath(
            path_points_to_exit_jaga_moraine, custom_exit_condition=lambda: player_is_dead_or_map_loading()
        )
        Routines.Sequential.Map.WaitforMapLoad(bjora_marches, log_to_console)
        bot_variables.config.finished_routine = False

        Routines.Sequential.Movement.FollowPath(
            path_points_to_return_to_jaga_moraine, custom_exit_condition=lambda: player_is_dead_or_map_loading()
        )
        Routines.Sequential.Map.WaitforMapLoad(jaga_moraine, log_to_console)
...
def BjoraMarchesSkillCasting():
    ...
    shadow_form_buff_time_remaining = (
        Effects.GetEffectTimeRemaining(player_agent_id, shadow_form) if has_shadow_form else 0
    )
    ...
    if shadow_form_buff_time_remaining <= 3500:  # about to expire, recast
        # ** Cast Deadly Paradox **
        if Routines.Sequential.Skills.CastSkillID(
            deadly_paradox, extra_condition=(not has_deadly_paradox), log=log_to_console
        ):
            sleep(0.1)
        # ** Cast Shadow Form **
...
def JagaMoraineSkillCasting():
    ...
    if Routines.Checks.Agents.InDanger(Range.Spellcast):
        ...
        if shadow_form_buff_time_remaining <= 3500:  # about to expire, recast
            ...
            if Routines.Sequential.Skills.CastSkillID(
                ...
                ConsoleLog(
                    MODULE_NAME, "Casting Deadly Paradox.", PySystem.Console.MessageType.Info, log=log_to_console
                )
                sleep(0.1)
    ...
    if bot_variables.config.in_killing_routine:
        ...
        if target:
            ...
            if Routines.Sequential.Skills.CastSkillSlot(
                arcane_echo_slot, extra_condition=both_ready, log=log_to_console
            ):
