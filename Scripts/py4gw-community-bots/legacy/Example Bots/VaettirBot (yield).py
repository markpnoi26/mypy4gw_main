...
from Core import ImGui, PyImGui
from Core import GLOBAL_CACHE, Agent
from Core import ConsoleLog
from Core import ModelID, TitleID
from Core import ItemArray
from Core import AgentArray
from Core import Routines
from Core import Range
from Core import Utils, Color
from Core import LootConfig
...
path_points_to_exit_jaga_moraine: List[Tuple[float, float]] = [
    (12289, -17700),
    (13970, -18920),
    (15400, -20400),
    (15850, -20550),
...
path_points_to_return_to_jaga_moraine: List[Tuple[float, float]] = [
    (-20300, 5600)
]  ## A Dekoy Accadia: removed unnecessary coordinates to re-enter Jaga.
...
bot_variables.config.window_module = ImGui.WindowModule(
...
def IsSkillBarLoaded():
    ...
    if primary_profession == "Assassin" and secondary_profession != "Mesmer":
        ...
        ConsoleLog(
            IsSkillBarLoaded, f"This bot requires A/Me to work, halting.", PySystem.Console.MessageType.Error, log=True
        )
        return False
    ...
    ConsoleLog(
        MODULE_NAME, f"SkillBar Loaded.", PySystem.Console.MessageType.Info, log=bot_variables.config.log_to_console
    )
    return True
...
def SetHardMode():
    ...
    ConsoleLog(
        MODULE_NAME, "Hard mode set.", PySystem.Console.MessageType.Info, log=bot_variables.config.log_to_console
    )
...
def NeedsToHandleInventory():
    ...
    count_of_id_kits = GLOBAL_CACHE.Inventory.GetModelCount(
        ModelID.Superior_Identification_Kit.value
    )  # 5899 model for ID kit
    count_of_salvage_kits = GLOBAL_CACHE.Inventory.GetModelCount(
        ModelID.Salvage_Kit.value
    )  # 2992 model for salvage kit
...
def filter_identify_array():
    ...
    unidentified_items = ItemArray.Filter.ByCondition(
        unidentified_items, lambda item_id: not GLOBAL_CACHE.Item.Rarity.IsWhite(item_id)
    )
    unidentified_items = ItemArray.Filter.ByCondition(
        unidentified_items, lambda item_id: not GLOBAL_CACHE.Item.Usage.IsIdentified(item_id)
    )
...
def filter_salvage_array():
    ...
    salvageable_items = ItemArray.Filter.ByCondition(
        salvageable_items,
        lambda item_id: GLOBAL_CACHE.Item.Usage.IsIdentified(item_id) or GLOBAL_CACHE.Item.Rarity.IsWhite(item_id),
    )

    salvageable_items = ItemArray.Filter.ByCondition(
        salvageable_items, lambda item_id: GLOBAL_CACHE.Item.Usage.IsSalvageable(item_id)
    )
...
def filter_items_to_deposit():
    ...
    items_to_deposit = ItemArray.Filter.ByCondition(
        items_to_deposit, lambda item_id: GLOBAL_CACHE.Item.GetModelID(item_id) not in banned_models
    )
...
def handle_death():
    if Agent.IsDead(Player.GetAgentID()) or not bot_variables.config.is_script_running:
        ConsoleLog(
            MODULE_NAME,
            f"Player is dead while traversing {Map.GetMapName(Map.GetMapID())} . Reseting Environment.",
            PySystem.Console.MessageType.Error,
            log=bot_variables.config.log_to_console,
        )
        return True
    return False
...
def handle_return_inventory_check():
    ...
    if GLOBAL_CACHE.Inventory.GetFreeSlotCount() < bot_variables.inventory_config.leave_free_slots:
        ConsoleLog(
            MODULE_NAME,
            f"Inventory is full, going to merchant.",
    ...
    if count_of_id_kits <= 0:
        ConsoleLog(
            MODULE_NAME,
            f"Need to buy ID kits, going to merchant.",
            PySystem.Console.MessageType.Info,
            log=bot_variables.config.log_to_console,
        )
        return True
    ...
    if count_of_salvage_kits <= 0:
        ConsoleLog(
            MODULE_NAME,
            f"Need to buy Salvage kits, going to merchant.",
            PySystem.Console.MessageType.Info,
            log=bot_variables.config.log_to_console,
        )
        return True
...
def InventoryHandler(log_to_console=False):
    if NeedsToHandleInventory():
        ...
        if log_to_console:
            ConsoleLog(
                MODULE_NAME, "Buying ID and Salvage kits.", PySystem.Console.MessageType.Info, log=log_to_console
            )
        ...
        if log_to_console:
            ConsoleLog(
                MODULE_NAME,
                f"IDing {len(items_to_idenfity)} items.",
                PySystem.Console.MessageType.Info,
                log=log_to_console,
            )
        ...
        if log_to_console:
            ConsoleLog(
                MODULE_NAME,
                f"Salvaging {items_to_salvage} items.",
                PySystem.Console.MessageType.Info,
                log=log_to_console,
            )
    ...
    while True:
        ...
        if not bot_variables.config.reset_from_jaga_moraine:
            ...
            if not IsSkillBarLoaded():
                ...
                ConsoleLog(
                    MODULE_NAME,
                    "You need the following build: OwVUI2h5lPP8Id2BkAiAvpLBTAA",
                    PySystem.Console.MessageType.Error,
                    log=True,
                )
                break
            ...
            yield from Routines.Yield.Movement.FollowPath(
                path_points=path_points_to_leave_outpost,
                custom_exit_condition=lambda: Map.IsMapLoading() or not bot_variables.config.is_script_running,
                log=False,
            )
            ...
            yield from Routines.Yield.Movement.FollowPath(
                ...
                custom_exit_condition=lambda: player_is_dead_or_map_loading(jaga_moraine)
                or not bot_variables.config.is_script_running,
            )
            ConsoleLog(
                MODULE_NAME, "Finished traversing bjora marches", PySystem.Console.MessageType.Info, log=log_to_console
            )
            ...
            if Routines.Checks.Map.MapValid():
                if handle_death():
                    ConsoleLog(
                        MODULE_NAME,
                        "Player is dead while traversing bjora marches. Reseting Environment.",
                        PySystem.Console.MessageType.Error,
                        log=log_to_console,
                    )
                    bot_variables.config.reset_from_jaga_moraine = False
                    continue
            ConsoleLog(
                MODULE_NAME, "Entering Waiting for map load", PySystem.Console.MessageType.Info, log=log_to_console
            )
        ...
        yield from Routines.Yield.Movement.FollowPath(
            path_points_to_npc,
            custom_exit_condition=lambda: player_is_dead() or not bot_variables.config.is_script_running,
        )
        # take bounty
        ...
        if handle_death():
            ConsoleLog(
                MODULE_NAME,
                "Player is dead while tont farming Route 1. Reseting Environment.",
                PySystem.Console.MessageType.Error,
                log=log_to_console,
            )
            bot_variables.config.reset_from_jaga_moraine = False
        ...
        ConsoleLog(
            MODULE_NAME, "Finished waiting for left aggro ball", PySystem.Console.MessageType.Info, log=log_to_console
        )
        bot_variables.config.pause_stuck_routine = False
        ...
        if handle_death():
            ConsoleLog(
                MODULE_NAME,
                "Player is dead while to farming Route 2. Reseting Environment.",
                PySystem.Console.MessageType.Error,
                log=log_to_console,
            )
            bot_variables.config.reset_from_jaga_moraine = False
        ...
        yield from Routines.Yield.wait(15000)
        ConsoleLog(
            MODULE_NAME, "Finished waiting for right aggro ball", PySystem.Console.MessageType.Info, log=log_to_console
        )
        bot_variables.config.pause_stuck_routine = False
        ...
        if handle_death():
            ConsoleLog(
                MODULE_NAME,
                "Player is dead while to killing spot. Reseting Environment.",
                PySystem.Console.MessageType.Error,
                log=log_to_console,
            )
            bot_variables.config.reset_from_jaga_moraine = False
        ...
        while len(enemy_array) > 0:  # sometimes not all enemies are killed
            if handle_death():
                ConsoleLog(
                    MODULE_NAME,
                    "Player is dead while killing enemies. Reseting Environment.",
                    PySystem.Console.MessageType.Error,
                    log=log_to_console,
                )
                bot_variables.config.reset_from_jaga_moraine = False
        ...
        if handle_death():
            ConsoleLog(
                MODULE_NAME,
                "Player is dead before looting. Reseting Environment.",
                PySystem.Console.MessageType.Error,
                log=log_to_console,
            )
            bot_variables.config.reset_from_jaga_moraine = False
        ...
        if handle_death():
            ConsoleLog(
                MODULE_NAME,
                "Player is dead while looting. Reseting Environment.",
                PySystem.Console.MessageType.Error,
                log=log_to_console,
            )
            bot_variables.config.reset_from_jaga_moraine = False
        ...
        if handle_return_inventory_check():
            ConsoleLog(
                MODULE_NAME,
                "Inventory handling needed, Reseting Environment.",
                PySystem.Console.MessageType.Info,
                log=log_to_console,
            )
            bot_variables.config.reset_from_jaga_moraine = False
        ...
        yield from Routines.Yield.Movement.FollowPath(
            ...
            custom_exit_condition=lambda: player_is_dead_or_map_loading(bjora_marches)
            or not bot_variables.config.is_script_running,
        )
        yield from Routines.Yield.Map.WaitforMapLoad(bjora_marches, log_to_console)
...
def BjoraMarchesSkillCasting():
    ...
    shadow_form_buff_time_remaining = (
        GLOBAL_CACHE.Effects.GetEffectTimeRemaining(player_agent_id, shadow_form) if has_shadow_form else 0
    )
    ...
    if shadow_form_buff_time_remaining <= 3500:  # about to expire, recast
        ...
        yield from Routines.Yield.Skills.CastSkillID(
            deadly_paradox, extra_condition=(not has_deadly_paradox), log=log_to_console, aftercast_delay=100
        )
        # ** Cast Shadow Form **
    ...
    if nearest_enemy:
        ...
        if (
            yield from Routines.Yield.Skills.CastSkillID(
                heart_of_shadow, extra_condition=is_enemy_behind, log=log_to_console, aftercast_delay=350
            )
        ):
            return
    yield
...
def JagaMoraineSkillCasting():
    ...
    if bot_variables.config.in_killing_routine:
        ...
        both_ready = Routines.Checks.Skills.IsSkillSlotReady(
            wastrels_demise_slot
        ) and Routines.Checks.Skills.IsSkillSlotReady(arcane_echo_slot)
        target = GetNotHexedEnemy()
        if target:
            Routines.Yield.Agents.ChangeTarget(target)
            if (
                yield from Routines.Yield.Skills.CastSkillSlot(
                    arcane_echo_slot, extra_condition=both_ready, log=log_to_console, aftercast_delay=2000
                )
            ):
                ...
                if (
                    yield from Routines.Yield.Skills.CastSkillSlot(
                        arcane_echo_slot, log=log_to_console, aftercast_delay=350
                    )
                ):
                    ConsoleLog(
                        MODULE_NAME, "Casting Echoed Wastrel.", PySystem.Console.MessageType.Info, log=log_to_console
                    )
                    return
        target = GetNotHexedEnemy()
        if target:
            ...
            if (
                yield from Routines.Yield.Skills.CastSkillSlot(
                    wastrels_demise_slot, log=log_to_console, aftercast_delay=350
                )
            ):
                return
    yield
...
def DrawWindow():
    ...
    try:
        ...
        if PyImGui.begin(
            bot_variables.config.window_module.window_name, bot_variables.config.window_module.window_flags
        ):
            ...
            if PyImGui.collapsing_header("Config"):
                bot_variables.config.log_to_console = PyImGui.checkbox(
                    "Log to Console", bot_variables.config.log_to_console
                )
                ...
                if PyImGui.tree_node("Misc"):
                    for label, attr in [
                        ...
                        setattr(
                            bot_variables.inventory_config,
                            attr,
