...
class Global_Vars:
    ...
    def update(self):
        ...
        (
            self.title_frame_coords.left,
            self.title_frame_coords.top,
            self.title_frame_coords.right,
            self.title_frame_coords.bottom,
        ) = self.title_frame.coords()
...
def DrawWindow():
    ...
    caption_color = (
        Color(0, 255, 0, 255)
        if global_vars.widget_active and global_vars.pet_bahavior != PetBehavior.Heel
        else (
            Color(243, 230, 0, 255)
            if global_vars.widget_active and global_vars.pet_bahavior == PetBehavior.Heel
    ...
    if ImGui.floating_button(
        caption=caption,
        x=global_vars.title_frame_coords.left + 75,
        y=global_vars.title_frame_coords.top + 3,
        width=90,
        height=30,
        color=caption_color,
    ):
...
def draw():
    ...
    if (
        global_vars.owner_target_id != 0
        and global_vars.owner_target_id != global_vars.pet_target_id
        and (global_vars.pet_bahavior == PetBehavior.Guard or global_vars.pet_bahavior == PetBehavior.Fight)
    ):
        ...
        if (
            global_vars.non_enemy_target_grace_timer.IsRunning()
            and not global_vars.non_enemy_target_grace_timer.HasElapsed(global_vars.non_enemy_target_grace_ms)
        ):
            return
