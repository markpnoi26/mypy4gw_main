...
CONSOLIDATE_TO_BACK = (
    False  # Sort non-filter-assigned items to the back of wildcard panes so free slots appear at the front
)
...
def _get_material_slot_candidates_by_model_id() -> dict:
    ...
    for model_name, slot_indices in [
        ...
        if model_member is None:
            continue
        try:
            fallback_slot_by_model[int(model_member.value if hasattr(model_member, "value") else model_member)] = [
                int(slot_index) for slot_index in slot_indices
            ]
        except Exception:
            continue
...
def _log_material_storage_counts_to_console():
    ...
    for slot_index in range(max(0, bag_size)):
        ...
        try:
            ...
            model_name = "Unknown"
        ConsoleLog(
            MODULE_NAME, f"Slot {slot_index}: ModelID {model_id} ({model_name}) = {quantity}", Console.MessageType.Info
        )
...
def _save_allowed_model_ids_for_storage(bag_enum):
    ...
    ini_handler.set(
        INI_KEY, f"allowed_model_ids_storage_{bag_enum.value}", ",".join(str(model_id) for model_id in allowed)
    )
...
def _sort_items_within_storage_by_model_id(
    entries, bag_states, available_storage_bags, target_bags=None, max_move_actions=None, dry_run: bool = False
):
    ...
    for bag_enum in bags_to_process:
        ...
        if any(
            entry.get("bag_enum") != bag_enum
            or entry.get("slot") != desired_slot_by_item_id.get(entry.get("item_id"), -1)
...
def _consolidate_items_to_back(
    ...
    desired_target_by_item_id = {all_consolidatable_sorted[i]["item_id"]: target_positions[i] for i in range(n_items)}
...
def _plan_sort_moves(
    ...
    initial_pos = {e["item_id"]: (e["bag_enum"], e["slot"]) for e in entries if e.get("quantity", 0) > 0}
...
def _try_move_wrong_entry(
    ...
    def _classify_bag_priority(bag_enum):
        if (
            has_model_priority
            and bag_enum in target_model_bags
            and _is_item_allowed_in_storage(
                type_name,
                model_id,
                bag_enum,
                allowed_by_bag,
...
def _process_phase_materials(task, ctx):
    ...
    while steps_left > 0 and task["phase"] == "materials":
        ...
        material_storage_by_model[model_id] = min(
            MATERIAL_STACK_MAX, int(material_storage_by_model.get(model_id, 0)) + int(moved_units)
        )
        ...
        for item in items:
            ...
            try:
                model_id = (
                    int(item.model_id)
                    if hasattr(item, "model_id")
                    else int(GLOBAL_CACHE.Item.GetModelID(hovered_item_id))
                )
            except Exception:
                model_id = 0
...
def _draw_window():
    ...
    if _cache_needs_refresh or _idle_refresh_timer.IsExpired():
        ...
        _cached_correct_items, _cached_total_items, _cached_correct_ratio = _calculate_correct_item_progress(
            _cached_available_bags
        )
    ...
    PyImGui.progress_bar(
        tabs_used_ratio, -1, 0, f"{tabs_used_ratio * 100.0:.1f}% Full ({tabs_total_used}/{tabs_total_size})"
    )
    ...
    if SHOW_DEBUG and AUTO_DEPOSIT_MATERIALS:
        if _sort_task_state is not None:
            ...
            PyImGui.text(
                f"Material deposit: {material_done}/{material_total} checked | Moves: {material_actions} | Units: {material_units}"
            )
                    ...
                    if PyImGui.button(f"Remove##remove_{bag_key}") and can_remove:
                        if can_remove_type:
                            ...
                            if len(allowed_types) == 0:
                                ...
                                _selected_allowed_type_idx_by_storage[bag_key] = min(
                                    selected_idx, len(allowed_types) - 1
                                )
                            _save_allowed_types_for_storage(bag_enum)
                        else:
                            selected_model_idx = _selected_allowed_model_id_idx_by_storage.get(bag_key, 0)
                            ...
                            if len(allowed_model_ids) == 0:
                                ...
                                _selected_allowed_model_id_idx_by_storage[bag_key] = min(
                                    selected_model_idx, len(allowed_model_ids) - 1
                                )
