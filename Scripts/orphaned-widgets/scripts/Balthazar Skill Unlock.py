...
import Py4GW
from Core import Agent, Color, Console, ConsoleLog, ImGui, Map, Party, Player, PyImGui, Skill
...
class BalthazarSkillUnlockWidget:
    ...
    def _candidate_skill_json_paths(self) -> List[str]:
        ...
        candidates = []
        if project_root:
            candidates.append(os.path.join(project_root, "Core", "skill_descriptions.json"))
        candidates.append(
            os.path.abspath(
                os.path.join(
                    script_dir,
    ...
    def _load_skill_catalog(self) -> List[SkillOption]:
        ...
        self.catalog_error = "Could not locate Core/skill_descriptions.json."
        return []
    ...
    def _update_pending_unlock(self) -> None:
        ...
        if (not pending.unlocked_requested_before and unlocked_requested_now) or (
            not pending.unlocked_send_before and unlocked_send_now
        ):
            self.status_message = (
        ...
        if PyImGui.begin_child("BalthazarSkillMatches", (0, 220), True, PyImGui.WindowFlags.NoFlag):
            for index, option in enumerate(self.matches):
                ...
                if PyImGui.selectable(
                    f"{label}##balth_skill_{index}", is_selected, PyImGui.SelectableFlags.NoFlag, (0, 0)
                ):
