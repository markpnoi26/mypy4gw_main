...
class Config:
    def __init__(self):
        ...
        self.last_known_hero_behaviour = ini_handler.get_int(
            MODULE_NAME, "last_known_hero_behaviour", self.hero_behaviour
        )
        ...
        self.smart_incoming_fallback_enabled = ini_handler.get_bool(
            MODULE_NAME, "smart_incoming_fallback_enabled", False
        )
    ...
    def load_hexes(self):
        self.hexes_melee = (
            ini_handler.get_str(MODULE_NAME, "hexes_melee", "").replace(" ", "_").split(",")
            if ini_handler.get_str(MODULE_NAME, "hexes_melee", "")
            else []
        )
        self.hexes_caster = (
            ini_handler.get_str(MODULE_NAME, "hexes_caster", "").replace(" ", "_").split(",")
            if ini_handler.get_str(MODULE_NAME, "hexes_caster", "")
            else []
        )
        self.hexes_all = (
            ini_handler.get_str(MODULE_NAME, "hexes_all", "").replace(" ", "_").split(",")
            if ini_handler.get_str(MODULE_NAME, "hexes_all", "")
            else []
        )
        self.hexes_paragon = (
            ini_handler.get_str(MODULE_NAME, "hexes_paragon", "").replace(" ", "_").split(",")
            if ini_handler.get_str(MODULE_NAME, "hexes_paragon", "")
            else []
        )
        self.hexes_user = (
            ini_handler.get_str(MODULE_NAME, "hexes_user", "").replace(" ", "_").split(",")
            if ini_handler.get_str(MODULE_NAME, "hexes_user", "")
            else []
        )
...
config_module = ImGui.WindowModule(
    ...
    window_name="Hero Helper Configuration",
    window_size=(300, 175),
...
class Helper:
    ...
    def visible_checkbox(label, value):
        ...
        PyImGui.push_style_color(
            PyImGui.ImGuiCol.FrameBgHovered, Utils.ColorToTuple(Utils.RGBToColor(105, 105, 105, 255))
        )
        PyImGui.push_style_color(
            PyImGui.ImGuiCol.FrameBgActive, Utils.ColorToTuple(Utils.RGBToColor(125, 125, 125, 255))
        )
    ...
    @staticmethod
    def log_event(
        hero_id=None, skill_name=None, target_name=None, skill_id=None, target_id=None, message=None, cooldown=10
    ):
    ...
    def get_active_effects(agent_id):
        return {effect.skill_id: effect.time_remaining or 5 for effect in Effects.GetEffects(agent_id) or []}
    ...
    def update_effect_cache(agent_id, effect_id, time_left):
        Helper.effect_cache[(agent_id, effect_id)] = {"result": True, "expires": time.time() + time_left}
    ...
    def get_heroes_with_skill(skill_id):
        return [
            h
            for h in Helper.cache_hero_skills()
    ...
    def count_enemies_in_range(custom_range, position=None):
        ...
        return sum(
            1
            for eid in AgentArray.GetEnemyArray() or []
    ...
    def get_best_panic_target():
        ...
        for enemy_id in enemy_ids:
            ...
            candidates.append(
                (-clustered_enemies, 0 if Agent.IsCaster(enemy_id) else 1, distance_from_player, enemy_id)
            )
    ...
    def get_best_xinrae_target():
        ...
        for ally_id in ally_ids:
            ...
            candidates.append((-enemies_near, Agent.GetHealth(ally_id), distance_from_player, ally_id))
        ...
        def _get_shared_party_account(agent_id):
            ...
            for account in GLOBAL_CACHE.ShMem.GetAllAccountData(sort_results=False, include_isolated=True) or []:
                ...
                if int(account.AgentPartyData.PartyID) != int(own_party_id):
                    continue
                same_map = (
                    int(account.AgentData.Map.MapID) == int(own_map_id)
                    and int(account.AgentData.Map.Region) == int(own_region)
                    and int(account.AgentData.Map.District) == int(own_district)
                    and int(account.AgentData.Map.Language) == int(own_language)
                )
                if same_map:
                    return account
            return None
    ...
    def is_hero_attacking():
        return any(Agent.IsAttacking(hero.agent_id) for hero in Party.GetHeroes() or [] if hero.agent_id)

    @staticmethod
    def can_cast(agent_id=None):
        agent_id = agent_id or Player.GetAgentID()
        return agent_id and not any(
            [Agent.IsDead(agent_id), Agent.IsKnockedDown(agent_id), Agent.IsCasting(agent_id), Agent.IsMoving(agent_id)]
        )
    ...
    @staticmethod
    def smartcast_hero_skill(
        skill_id,
        min_enemies=0,
        enemy_range_check=None,
        effect_check=False,
        cast_target_id=None,
        hero_target=False,
        distance_check_range=None,
        allow_out_of_combat=False,
        min_health_perc=None,
        min_energy_perc=None,
    ...
    def cache_agent_names():
        ...
        agents = (
            set(AgentArray.GetNPCMinipetArray()) | set(AgentArray.GetAllyArray()) | set(AgentArray.GetNeutralArray())
        )
    ...
    def format_spell_title_case(spell_name):
        ...
        return "_".join(word.capitalize() if word.lower() not in skip else word.lower() for word in words)
...
def smart_interrupt():
    ...
    skill_class_pairs = [(Skill.GetID("Cry_of_Frustration"), "Mesmer"), (Skill.GetID("Power_Drain"), "Mesmer")]
...
def smart_hex_removal():
    ...
    hex_removal_skills = ["Shatter_Hex", "Remove_Hex", "Smite_Hex", "Blessed_Light"]
...
def smart_cond_removal():
    ...
    condition_found = (
        (holds_melee and Helper.check_for_effects(player_id, cond_sets["melee"]))
        or (not holds_melee and Helper.check_for_effects(player_id, cond_sets["caster"]))
        or Helper.check_for_effects(player_id, cond_sets["both"])
    )
...
def smart_sos():
    ...
    needs_sos = (
        (enemies_in_range > 6 and spirits_in_range < 3)
        or (enemies_in_range > 4 and spirits_in_range < 2)
        or (enemies_in_range > 2 and spirits_in_range < 1)
    )
...
def draw_feature_status(label: str, enabled: bool):
    ...
    status_color = (
        ...
        if enabled
        else Utils.ColorToTuple(Utils.RGBToColor(220, 90, 90, 255))
    )
        ...
        (
            ...
            [
                (
                    "Panic",
                    "smart_panic_enabled",
                    "Casts Panic on clustered enemy groups, preferring caster clumps.",
                    "Panic",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "Strength of Honor",
                    "smart_honor_enabled",
                    "[DISABLE HERO CASTING] Maintains Honor on melee player.",
                    "Strength_of_Honor",
                ),
                (
                    "Life Bond",
                    "smart_life_bond_enabled",
                    "[DISABLE HERO CASTING] Maintains Life Bond on melee player.",
                    "Life_Bond",
                ),
                (
                    "Vigorous Spirit",
                    "smart_vigorous_enabled",
                    "Casts Vigorous Spirit on melee player in combat.",
                    "Vigorous_Spirit",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "Blood is Power",
                    "smart_bip_enabled",
                    "Automatically cast Blood is Power on player when needed.",
                    "Blood_is_Power",
                ),
                (
                    "Dark Aura",
                    "smart_dark_aura_enabled",
                    "Hero maintains Masochism on itself, then Dark Aura on Necromancer player.",
                    "Dark_Aura",
                ),
            ],
        ),
        (
            ...
            [
                (
                    "Incoming!",
                    "smart_incoming_fallback_enabled",
                    "Hero uses Incoming! and Fall Back! even when Essence of Celerity is active.",
                    "Incoming",
                ),
                (
                    "Fall Back!",
        ...
        (
            ...
            [
                (
                    "Signet of Spirits",
                    "smart_sos_enabled",
                    "Automatically casts Signet of Spirits when necessary.",
                    "Signet_of_Spirits",
                ),
                (
                    "Soul Twisting",
                    "smart_st_enabled",
                    "Automatically casts Shelter and Union based on combat conditions.",
                    "Soul_Twisting",
                ),
                (
                    "Splinter Weapon",
                    "smart_splinter_enabled",
                    "Casts Splinter on melee player in combat.",
                    "Splinter_Weapon",
                ),
                (
                    "Xinrae's Weapon",
                    "smart_xinrae_enabled",
                    "Casts Xinrae's Weapon on the best nearby ally without an existing weapon spell, prioritizing pressure and low health.",
                    "Xinraes_Weapon",
                ),
            ],
        ),
...
def draw_tab_condition_cleanse(config):
    ...
    PyImGui.text_wrapped(
        "Assign Prio Cleanse Conditions to Melee, Caster, or Both\n(Leave blank to keep default priority):"
    )
    ...
    toggle_config_value(
        "Enable Condition", "smart_con_cleanse_toggled", button_width, 25, "Toggle automatic condition handling"
    )
...
def draw_tab_interrupt(config):
    ...
    button_width = int(PyImGui.get_content_region_avail()[0])
    toggle_config_value(
        "Enable Hero Interrupt", "smart_interrupt_toggled", button_width, 34, "Toggle automatic hero interrupts"
    )
...
def draw_config_tabs(widget_config):
    ...
    open_interrupt = PyImGui.begin_tab_item(
        responsive_tab_label(
            IconsFontAwesome5.ICON_BOLT, "Smart Interrupt", "Interrupt", "interrupt_tab", per_tab_width
        )
    )
    if PyImGui.is_item_hovered():
...
def configure():
    ...
    if PyImGui.begin(config_module.window_name, config_module.window_flags):
        PyImGui.text_colored(
            f"{IconsFontAwesome5.ICON_COG} Hero Helper", Utils.ColorToTuple(Utils.RGBToColor(255, 210, 120, 255))
        )
        PyImGui.same_line(0, 8)
