    ...
    try:
        ...
        if sacrifice_model_ids and sacrifice_model_ids.issubset(current_model_ids):
            PySystem.Console.Log(
                MODULE_NAME, "Armor write skipped - sacrifice armor is equipped.", PySystem.Console.MessageType.Info
            )
            return
...
def _restore_combat_widgets_after_dialog(state: dict) -> None:
    ...
    try:
        ...
        if bool(state.get("heroai_was_enabled", False)) and not bool(
            widget_handler.is_widget_enabled(_HEROAI_WIDGET_NAME)
        ):
...
def _refresh_custom_behavior_after_skillbar_change() -> None:
    ...
    try:
        ...
        behavior_name = (
            loader.custom_combat_behavior.__class__.__name__ if loader.custom_combat_behavior is not None else "None"
        )
        PySystem.Console.Log(
            MODULE_NAME,
...
def _refresh_heroai_build_after_skillbar_change() -> None:
    try:
        from Widgets.Multibox import HeroAI as HeroAI_Widget
