...
class RingSector(Shape):
    def __init__(
        self, x, y, inner_r, outer_r, start_deg, end_deg, color, hovered_color=0xFFFFFFFF, outline_col=0xFF000000
    ):
    ...
    def draw(self):
        ...
        if abs(self.end_deg - self.start_deg) < 360.0:
            for rad in [s_rad, e_rad]:
                PyImGui.path_clear()
                PyImGui.path_line_to(
                    self.center[0] + math.cos(rad) * self.inner_r, self.center[1] + math.sin(rad) * self.inner_r
                )
                PyImGui.path_line_to(
                    self.center[0] + math.cos(rad) * self.outer_r, self.center[1] + math.sin(rad) * self.outer_r
                )
                PyImGui.path_stroke(self.outline_col, False, self.border_thickness)
...
class Circle(Shape):
    ...
    def draw(self):
        ...
        PyImGui.draw_list_add_circle_filled(self.center[0], self.center[1], self.radius, draw_col, self.segments)

        # Draw outline primitive
        if self.border_thickness > 0:
            PyImGui.draw_list_add_circle(
                self.center[0], self.center[1], self.radius, self.outline_col, self.segments, self.border_thickness
            )
...
class Rectangle(Shape):
    ...
    def update_hover(self, mouse_x: float, mouse_y: float):
        ...
        self.is_hovered = (
            self.pos[0] <= mouse_x <= self.pos[0] + self.size[0]
            and self.pos[1] <= mouse_y <= self.pos[1] + self.size[1]
        )
    ...
    def draw(self):
        ...
        PyImGui.draw_list_add_rect_filled(
            self.pos[0], self.pos[1], x2, y2, draw_col, self.rounding, 15  # 15 = all corners
        )

        # Draw outline
        PyImGui.draw_list_add_rect(
            self.pos[0], self.pos[1], x2, y2, self.outline_col, self.rounding, 15, self.border_thickness
        )
...
class Triangle(Shape):
    ...
    def draw(self):
        ...
        PyImGui.draw_list_add_triangle_filled(p[0][0], p[0][1], p[1][0], p[1][1], p[2][0], p[2][1], draw_col)
        PyImGui.draw_list_add_triangle(
            p[0][0], p[0][1], p[1][0], p[1][1], p[2][0], p[2][1], self.outline_col, self.border_thickness
        )
...
def draw_radial_interaction_layer():
    ...
    if PyImGui.begin("##RadialInteractionLayer", flags):
        ...
        sector = RingSector(
            center_x, center_y, float(inner_radius), float(outer_radius), degree_start, degree_end, color, hovered_color
        )
