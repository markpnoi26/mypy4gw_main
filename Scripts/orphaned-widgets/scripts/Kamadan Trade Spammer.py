...
from Core import ActionQueueManager, ConsoleLog, Map, Party, Player, Routines, ThrottledTimer, Timer
...
def draw_ui() -> None:
    ...
    if PyImGui.begin(BOT_NAME, window_flags):
        ...
        cooldown_labels = [
            f"{minutes} minute" if minutes == 1 else f"{minutes} minutes" for minutes in COOLDOWN_OPTIONS_MINUTES
        ]
