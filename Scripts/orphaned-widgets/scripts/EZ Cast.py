...
from Core import Color, ImGui
import Core as GW
from Core.native_src.context.WorldContext import AttributeStruct
import time
from typing import List, Deque
from collections import deque
import Core.dNodes.dNodes as dNodes
from Widgets.lib.SkillInfo import SkillData as SkillDataOG
...
TRIGGER_COLOR: Core.Color = Core.Color.from_tuple((0.50, 0.2, 0.2, 1.0))  # RED
LOGIC_COLOR: Core.Color = Core.Color.from_tuple((0.2, 0.5, 0.2, 1.0))  # GREEN
GAME_COLOR: Core.Color = Core.Color.from_tuple((0.5, 0.35, 0.05, 1.0))  # YELLOW
DATA_COLOR: Core.Color = Core.Color.from_tuple((0, 0.3, 0.8, 1.0))  # LIGHT BLUE
OUTPUT_COLOR: Core.Color = Core.Color.from_tuple((0.8, 0, 0.8, 1.0))  # PINK
...
class PinFloat(dNodes.Pin):
    ...
    def draw_override(self):
        ...
        PyImGui.draw_list_add_circle(
            self.location[0] + self.radius - self.thickness + 2,
            self.location[1] + self.radius - self.thickness + 2,
            self.radius - self.thickness,
            Core.Color._pack_rgba(180, 20, 20, 255),
            3,
            self.thickness,
        )
...
class NodeEffect(dNodes.Node):
    ...
    def draw_body(self):
        self.output_pin.value = (
            GW.GLOBAL_CACHE.Effects.GetEffectTimeRemaining(GW.Player.GetAgentID(), self.skill_id) / 1000
        )
...
class NodeUseSkill(dNodes.Node):
    ...
    def draw_body(self):
        ...
        self.skill_slot = PyImGui.combo(
            f"##useskillcombo{self.id}", self.skill_slot, ["", "1", "2", "3", "4", "5", "6", "7", "8"]
        )
...
class NodeTargetData(dNodes.Node):
    ...
    def draw_body(self):
        global cache
        self.index = PyImGui.combo(f"##TargetDataSelector{self.id}", self.index, ["HP", "Distance", "Allegiance"])
        PyImGui.text(f"{self.output_pin.value: .2f}")
...
class NodeSelector(dNodes.Node):
    ...
    def draw_body(self):
        PyImGui.push_item_width(PyImGui.get_content_region_avail()[0])
        self.index = PyImGui.combo(
            f"##NodeSelector{self.id}",
            self.index,
            ["Selector", "Triggers", "Game State", "Logic/Math", "Data", "Output"],
        )
        PyImGui.pop_item_width()
        match self.index:
            case 1:
                self.sub_index = PyImGui.combo(
                    f"##TriggerSelector{self.id}", self.sub_index, ["Select Below", "On Frame"]
                )
            case 2:
                self.sub_index = PyImGui.combo(
                    f"##TriggerGameState{self.id}",
                    self.sub_index,
                    ["Select Below", "Effect", "Energy", "Player Free", "Time Delta", "Target Info"],
                )
            case 3:
                self.sub_index = PyImGui.combo(
                    f"##LogicSelector{self.id}",
                    self.sub_index,
                    ["Select Below", "And", "Or", "Not", "Greater Than", "Operation"],
                )
            case 4:
                self.sub_index = PyImGui.combo(f"##DataSelector{self.id}", self.sub_index, ["Select Below", "Number"])
            case 5:
                self.sub_index = PyImGui.combo(f"##DataSelector{self.id}", self.sub_index, ["Select Below", "UseSkill"])
            case _:
                pass
...
def DrawGenericSkills():
    ...
    if generic_skill_collapse:
        cache.reset_generic_skill_on_mapload = PyImGui.checkbox(
            "Reset on map load", cache.reset_generic_skill_on_mapload
        )
        ...
        for i in range(0, len(cache.combat_ranges)):
            ...
            if cache.combat:
                PyImGui.push_style_color(
                    PyImGui.ImGuiCol.SliderGrab, (1, 0.5, 0.5, 1)
                )  # ImGui::PushStyleColor(ImGuiCol_Text, sf::Color(255, 255, 255, 255));)
            else:
                PyImGui.push_style_color(
                    PyImGui.ImGuiCol.SliderGrab, (0, 0.5, 0.5, 1)
                )  # ImGui::PushStyleColor(ImGuiCol_Text, sf::Color(255, 255, 255, 255));)
            cache.combat_range_slider = PyImGui.slider_int(
                "##combatsliderRange", cache.combat_range_slider, 0, cache.combat_ranges[len(cache.combat_ranges) - 1]
            )
            PyImGui.pop_style_color(1)
...
def DrawRefrainMaintainer():
    ...
    if section_header:
        PyImGui.text_wrapped(
            """This function will use "Help Me!" to maintain refrains intelligently. It will alternatively use "Dont Trip!" and "I am Unstoppable!" if both are present. If only "Don't Trip!" is available, it will require a recharge reduction such as an Essence of Celerity to work."""
        )
        cache.refrain_buffer = PyImGui.slider_float("Grace buffer", cache.refrain_buffer, 0, 2)
        if PyImGui.is_item_hovered():
            PyImGui.set_tooltip(
                "The time in seconds that a refrain should have remaining when the shout ends.\nValues lower than ping will result in dropped refrains."
            )
...
def DrawAuraOfTheAssassin():
    ...
    if section_header:
        ...
        if PyImGui.is_item_hovered():
            PyImGui.set_tooltip("The health percent that Aota will scan for enemies to reach before casting AP.")
...
def UseGenericSkills(energy, player_id, now):
    ...
    for n in range(0, 3):
        for i in range(1, 9):
            if not cache.skill_array[n][i - 1]:
                continue
            if cache.combat_skill[i - 1] and not cache.combat:
                continue
            if not GW.Routines.Checks.Skills.IsSkillSlotReady(i):
                continue
            ...
            if GW.Skill.Data.GetEnergyCost(skill_id) + cache.generic_energy_buffer > energy:
                continue
            # GW.Console.Log("", f"Skill adr {skill_instance.adrenaline}, adr_a {skill_instance.adrenaline_b}")
            if skill_instance.adrenaline_a < skill_data.adrenaline != 0:
                continue
            ...
            if n == 0 and effect == 0:
                effect_valid = True
            if n == 1 and effect / 1000.0 < skill_data.activation + cache.generic_skill_use_buffer:
                effect_valid = True
            if n == 2:
                effect_valid = True
            if effect_valid:
...
def MaintainRefrains_(player_id, now):
    ...
    if command is None:
        ...
        help_me_duration = (
            help_me_skill.duration_0pts
            + ((help_me_skill.duration_15pts - help_me_skill.duration_0pts) / 15) * command.level
        )
    ...
    if dont_trip_slot != 0:
        ...
        if iau_slot != 0:
            if iau is None and GW.Routines.Checks.Skills.IsSkillSlotReady(iau_slot):
                if (
                    dont_trip is not None
                    and base_durations[0] + dont_trip.time_remaining / 1000
                    > iau_dur
                    > base_durations[0] + dont_trip.time_remaining / 1000 - cache.refrain_buffer
                ):
                    cache.busy_timer = cache.ezcast_cast_minimum_timer
...
def MaintainRefrains(player_id, now):
    ...
    if help_me_slot != 0 and GW.Routines.Checks.Skills.IsSkillSlotReady(help_me_slot):
        ...
        if command_attr:
            ...
            h_dur = (
                help_me_skill.duration_0pts
                + ((help_me_skill.duration_15pts - help_me_skill.duration_0pts) / 15) * command_attr.level
            )
        else:
...
def Update():
    ...
    if cache.generic_skill_use_checkbox:
        if UseGenericSkills(cache.energy, cache.player_id, now):
            return
    # TODO autoritlord
    if cache.qa_checkbox:
        if QuickAttack(cache.energy, cache.player_id, now, delta):
            return
    if cache.refrainer_use_checkbox:
        if MaintainRefrains(cache.player_id, now):
            return
    if cache.aota_checkbox:
        if AuraOfTheAssassin(cache.energy, cache.player_id, now):
            return
