...
from Core import PyImGui, Color
        ...
        if Map.IsOutpost():
                    ...
                    if health <= get_threshold(agent_id):
                        if global_vars.log_low_health:
                            ...
                            PySystem.Console.Log(
                                module_name,
                                f"Player: {Agent.GetNameByID(agent_id)} ({agent_id}) have low health: {round(health * max_health)}",
            ...
            if global_vars.low_life:
                if global_vars.travel_timer.HasElapsed(global_vars.travel_time):
                    ...
                    PySystem.Console.Log(
                        module_name,
                        f"Traveling to: {Map.GetMapName(global_vars.last_outpost)}({global_vars.last_outpost})",
                        PySystem.Console.MessageType.Info,
                    )
                    global_vars.low_life = False
