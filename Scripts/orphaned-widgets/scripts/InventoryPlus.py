...
from Core.enums_src.Texture_enums import get_texture_for_model
...
class ModelPopUp:
    def __init__(
        self,
        title: str,
        model_dictionary: dict[int, str],
        current_blacklist: list[int],
        export_filename: str = "",
        ini_section: str = "",
        ini_var_name: str = "",
        ini_relative_path: str = "",
    ):
        self.is_open = False
    ...
    def Show(self):
        ...
        if PyImGui.begin_table(f"ModelIDTable##{self.Title}", 2):
            ...
            if PyImGui.begin_child("ModelIDList", (295, 375), True, PyImGui.WindowFlags.NoFlag):
                sorted_models = sorted(self.model_dictionary.items(), key=lambda x: x[1].lower())  # sort by NAME
            ...
            if PyImGui.begin_child("BlacklistModelIDList", (295, 375), True, PyImGui.WindowFlags.NoFlag):
                ...
                sorted_blacklist = sorted(self.blacklist, key=lambda mid: self.model_dictionary.get(mid, "").lower())
                ...
                if PyImGui.button(f"Copy to All Accounts ({len(other_accounts)})"):
                    ...
                    for account in other_accounts:
                        ...
                        if os.path.exists(target_ini):
                            Settings(self._ini_relative_path, "global").set(
                                self._ini_section, self._ini_var_name, blacklist_str
                            )
                            copied += 1
...
def _get_item_id_at_bag_slot(bag_id: int, slot: int) -> int:
    from Core import Item, ItemArray
...
def _is_supported_salvage_kit_item(item_id: int, inventory_item_ids: set[int] | None = None) -> bool:
    ...
    return (
        ...
        and int(Item.GetModelID(item_id))
        in {
            ModelID.Salvage_Kit,
            ModelID.Expert_Salvage_Kit,
            ModelID.Superior_Salvage_Kit,
...
def _get_supported_salvage_kit_id(selected_kit: ItemSlotData | None = None) -> int:
    ...
    supported_kits = [
        item_id for item_id in inventory_item_ids if _is_supported_salvage_kit_item(item_id, inventory_item_id_set)
    ]
...
def _wait_for_salvage_session_idle(
    ...
    while waited_ms < max(0, timeout_ms):
        if not _call_inventory_bool_method(inventory_instance, "IsSalvaging") and not _call_inventory_bool_method(
            inventory_instance, "IsSalvageTransactionDone"
        ):
            return True
...
def _salvage_single_item_with_supported_kit(
    item_id: int, label: str, selected_kit: ItemSlotData | None = None, allowed_rarities: set[str] | None = None
):
    import PyItem
    from Core.Py4GWcorelib import ActionQueueManager, ConsoleLog, Console
    from Core import Item
    from Core.enums_src.Model_enums import ModelID
    ...
    salvage_poll_ms = 50
    salvage_dialog_auto_handle, salvage_dialog_strategy, salvage_dialog_auto_confirm_materials, salvage_dialog_debug = (
        _get_salvage_dialog_auto_settings()
    )
    ...
    if advanced_kit_tracking and inventory_instance is not None:
        try:
            ...
            ConsoleLog(
                "SalvageItems", f"Advanced salvage start failed (item_id={item_id}).", Console.MessageType.Warning
            )
            return "failed"
    else:
        ...
        queue_drained = yield from Routines.Yield.Items._wait_for_empty_queue(
            "SALVAGE", timeout_ms=queue_wait_timeout_ms
        )
        if not queue_drained:
            ConsoleLog(
                "SalvageItems",
                f"Timed out waiting for salvage queue after starting salvage (item_id={item_id}).",
    ...
    if require_materials_confirmation and not manual_choice_required:
        ...
        if not queue_drained:
            ConsoleLog(
                "SalvageItems",
                f"Timed out waiting for salvage queue after confirmation (item_id={item_id}).",
                Console.MessageType.Warning,
            )
            return "failed"
    ...
    while waited_ms < result_wait_timeout_ms:
        try:
            ...
            if (
                handled_salvage_dialog
                and not supports_state_tracking
                and handled_dialog_settle_ms >= handled_dialog_post_check_ms
            ):
                post_status = (
                    "salvaged"
                    if item_progressed
                    else _get_post_salvage_status(item_id, item_instance, allowed_rarities)
                )
                Inventory._salvage_choice_debug_log(
                    salvage_dialog_debug,
                    "SalvageItems",
    ...
    if manual_choice_required:
        ...
        ConsoleLog(
            "SalvageItems",
            f"Timed out waiting for manual salvage completion (item_id={item_id}).",
            Console.MessageType.Warning,
        )
        return "manual_timeout"

    ConsoleLog(
        "SalvageItems", f"Timed out waiting for salvage result (item_id={item_id}).", Console.MessageType.Warning
    )
    return "failed"
...
def _queue_salvage_routine(
    item_ids: list[int], label: str, rarities: list[str] | None = None, selected_kit: ItemSlotData | None = None
):
    from Core.GlobalCache import GLOBAL_CACHE
...
def _queue_salvage_stack(item: ItemSlotData, selected_kit: ItemSlotData | None = None):
    ...
    routine = _run_salvage_stack_routine(
        item.BagID, item.Slot, f"Salvage Stack [{item.Rarity}]", selected_kit=selected_kit
    )
    GLOBAL_CACHE.Coroutines.append(routine)
...
def _withdraw_all_matching_model_from_storage(model_id: int) -> Generator[object, None, None]:
    from Core.GlobalCache import GLOBAL_CACHE
    from Core.Py4GWcorelib import Console, ConsoleLog
    from Core.Routines import Routines
...
class InventoryPlusWidget:
    ...
    def _init_popups(self):
        ...
        self.PopUps["Identification ModelID Lookup"] = ModelPopUp(
            ...
            ini_section="AutoIdentify",
            ini_var_name="id_model_blacklist",
            ini_relative_path=_ini_rel,
        )
        ...
        self.PopUps["Salvage Item Type Lookup"] = ModelPopUp(
            ...
            ini_section="AutoSalvage",
            ini_var_name="item_type_blacklist",
            ini_relative_path=_ini_rel,
        )
        ...
        self.PopUps["Salvage ModelID Lookup"] = ModelPopUp(
            ...
            ini_section="AutoSalvage",
            ini_var_name="salvage_blacklist",
            ini_relative_path=_ini_rel,
        )
        ...
        self.PopUps["Deposit Trophy ModelID Lookup"] = ModelPopUp(
            ...
            ini_section="AutoDeposit",
            ini_var_name="deposit_trophies_blacklist",
            ini_relative_path=_ini_rel,
        )
        ...
        self.PopUps["Deposit Material ModelID Lookup"] = ModelPopUp(
            ...
            ini_section="AutoDeposit",
            ini_var_name="deposit_materials_blacklist",
            ini_relative_path=_ini_rel,
        )
        ...
        self.PopUps["Deposit Event Item ModelID Lookup"] = ModelPopUp(
            ...
            ini_section="AutoDeposit",
            ini_var_name="deposit_event_items_blacklist",
            ini_relative_path=_ini_rel,
        )
        ...
        self.PopUps["Deposit Dye ModelID Lookup"] = ModelPopUp(
            ...
            ini_section="AutoDeposit",
            ini_var_name="deposit_dyes_blacklist",
            ini_relative_path=_ini_rel,
        )
        ...
        self.PopUps["Deposit ModelID Lookup"] = ModelPopUp(
            ...
            ini_section="AutoDeposit",
            ini_var_name="deposit_model_blacklist",
            ini_relative_path=_ini_rel,
        )

    def _sync_popups_with_handler(self):
        self.PopUps["Identification ModelID Lookup"]._source_blacklist = self.auto_inventory_handler.id_model_blacklist

        self.PopUps["Salvage Item Type Lookup"]._source_blacklist = self.auto_inventory_handler.item_type_blacklist

        self.PopUps["Salvage ModelID Lookup"]._source_blacklist = self.auto_inventory_handler.salvage_blacklist
        ...
        self.PopUps["Deposit Dye ModelID Lookup"]._source_blacklist = self.auto_inventory_handler.deposit_dyes_blacklist

        self.PopUps["Deposit ModelID Lookup"]._source_blacklist = self.auto_inventory_handler.deposit_model_blacklist
    ...
    def _set_i_window_enabled(self, enabled: bool) -> None:
        from Core.UIManager import UIManager, WindowID
    ...
    def load_settings(self):
        def _parse_color(value: str, default_color: Color) -> Color:
            try:
                parts = [int(c.strip()) for c in value.strip("()").split(",")]
        ...
        cfg.identify_whites = self._ini_get_bool(
            section="Identification", var_name="identify_whites", default=cfg.identify_whites
        )
        cfg.identify_blues = self._ini_get_bool(
            section="Identification", var_name="identify_blues", default=cfg.identify_blues
        )
        cfg.identify_greens = self._ini_get_bool(
            section="Identification", var_name="identify_greens", default=cfg.identify_greens
        )
        cfg.identify_purples = self._ini_get_bool(
            section="Identification", var_name="identify_purples", default=cfg.identify_purples
        )
        cfg.identify_golds = self._ini_get_bool(
            section="Identification", var_name="identify_golds", default=cfg.identify_golds
        )
        cfg.show_identify_all = self._ini_get_bool(
            section="Identification", var_name="show_identify_all", default=cfg.show_identify_all
        )
        cfg.identify_all_whites = self._ini_get_bool(
            section="Identification", var_name="identify_all_whites", default=cfg.identify_all_whites
        )
        cfg.identify_all_blues = self._ini_get_bool(
            section="Identification", var_name="identify_all_blues", default=cfg.identify_all_blues
        )
        cfg.identify_all_greens = self._ini_get_bool(
            section="Identification", var_name="identify_all_greens", default=cfg.identify_all_greens
        )
        cfg.identify_all_purples = self._ini_get_bool(
            section="Identification", var_name="identify_all_purples", default=cfg.identify_all_purples
        )
        cfg.identify_all_golds = self._ini_get_bool(
            section="Identification", var_name="identify_all_golds", default=cfg.identify_all_golds
        )

        cfg = self.salvage_settings
        cfg.salvage_whites = self._ini_get_bool(
            section="Salvage", var_name="salvage_whites", default=cfg.salvage_whites
        )
        cfg.salvage_blues = self._ini_get_bool(section="Salvage", var_name="salvage_blues", default=cfg.salvage_blues)
        cfg.salvage_greens = self._ini_get_bool(
            section="Salvage", var_name="salvage_greens", default=cfg.salvage_greens
        )
        cfg.salvage_purples = self._ini_get_bool(
            section="Salvage", var_name="salvage_purples", default=cfg.salvage_purples
        )
        ...
        cfg.salvage_all_whites = self._ini_get_bool(
            section="Salvage", var_name="salvage_all_whites", default=cfg.salvage_all_whites
        )
        cfg.salvage_all_blues = self._ini_get_bool(
            section="Salvage", var_name="salvage_all_blues", default=cfg.salvage_all_blues
        )
        cfg.salvage_all_greens = self._ini_get_bool(
            section="Salvage", var_name="salvage_all_greens", default=cfg.salvage_all_greens
        )
        cfg.salvage_all_purples = self._ini_get_bool(
            section="Salvage", var_name="salvage_all_purples", default=cfg.salvage_all_purples
        )
        cfg.salvage_all_golds = self._ini_get_bool(
            section="Salvage", var_name="salvage_all_golds", default=cfg.salvage_all_golds
        )

        cfg = self.colorize_settings
        cfg.enable_colorize = self._ini_get_bool(
            section="Colorize", var_name="enable_colorize", default=cfg.enable_colorize
        )
        ...
        white_str = self._ini_get_str(
            section="Colorize", var_name="white_color", default=str(cfg.white_color.to_tuple())
        )
        cfg.white_color = _parse_color(white_str, cfg.white_color)

        blue_str = self._ini_get_str(section="Colorize", var_name="blue_color", default=str(cfg.blue_color.to_tuple()))
        cfg.blue_color = _parse_color(blue_str, cfg.blue_color)

        green_str = self._ini_get_str(
            section="Colorize", var_name="green_color", default=str(cfg.green_color.to_tuple())
        )
        cfg.green_color = _parse_color(green_str, cfg.green_color)

        purple_str = self._ini_get_str(
            section="Colorize", var_name="purple_color", default=str(cfg.purple_color.to_tuple())
        )
        cfg.purple_color = _parse_color(purple_str, cfg.purple_color)

        gold_str = self._ini_get_str(section="Colorize", var_name="gold_color", default=str(cfg.gold_color.to_tuple()))
        cfg.gold_color = _parse_color(gold_str, cfg.gold_color)

        cfg = self.deposit_settings
        cfg.use_ctrl_click = self._ini_get_bool(
            section="Deposit", var_name="use_ctrl_click", default=cfg.use_ctrl_click
        )

        cfg = self.inventory_window_settings
        cfg.enable_i_window = self._ini_get_bool(
            section="InventoryWindow", var_name="enable_i_window", default=cfg.enable_i_window
        )

    def load_auto_handler_settings(self):
        self.auto_inventory_handler.module_active = self._ini_get_bool(
            section="AutoManager", var_name="module_active", default=False
        )
        self.auto_inventory_handler._LOOKUP_TIME = self._ini_get_int(
            section="AutoManager", var_name="lookup_time", default=15000
        )

        self.auto_inventory_handler.id_whites = self._ini_get_bool(
            section="AutoIdentify", var_name="id_whites", default=False
        )
        self.auto_inventory_handler.id_blues = self._ini_get_bool(
            section="AutoIdentify", var_name="id_blues", default=True
        )
        self.auto_inventory_handler.id_greens = self._ini_get_bool(
            section="AutoIdentify", var_name="id_greens", default=False
        )
        self.auto_inventory_handler.id_purples = self._ini_get_bool(
            section="AutoIdentify", var_name="id_purples", default=True
        )
        self.auto_inventory_handler.id_golds = self._ini_get_bool(
            section="AutoIdentify", var_name="id_golds", default=False
        )

        self.auto_inventory_handler.salvage_whites = self._ini_get_bool(
            section="AutoSalvage", var_name="salvage_whites", default=True
        )
        self.auto_inventory_handler.salvage_blues = self._ini_get_bool(
            section="AutoSalvage", var_name="salvage_blues", default=True
        )
        self.auto_inventory_handler.salvage_purples = self._ini_get_bool(
            section="AutoSalvage", var_name="salvage_purples", default=True
        )
        self.auto_inventory_handler.salvage_golds = self._ini_get_bool(
            section="AutoSalvage", var_name="salvage_golds", default=False
        )
        self.auto_inventory_handler.salvage_dialog_auto_handle = self._ini_get_bool(
            section="AutoSalvage", var_name="salvage_dialog_auto_handle", default=False
        )
        self.auto_inventory_handler.salvage_dialog_auto_confirm_materials = self._ini_get_bool(
            section="AutoSalvage", var_name="salvage_dialog_auto_confirm_materials", default=False
        )
        self.auto_inventory_handler.salvage_dialog_debug = self._ini_get_bool(
            section="AutoSalvage", var_name="salvage_dialog_debug", default=False
        )
        salvage_dialog_strategy = self._ini_get_int(
            section="AutoSalvage", var_name="salvage_dialog_strategy", default=0
        )
        self.auto_inventory_handler.salvage_dialog_strategy = _normalize_salvage_dialog_strategy(
            salvage_dialog_strategy
        )
        self.auto_inventory_handler.salvage_dialog_fallback_index = max(
            1, self._ini_get_int(section="AutoSalvage", var_name="salvage_dialog_fallback_index", default=1)
        )

        self.auto_inventory_handler.deposit_trophies = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_trophies", default=True
        )
        self.auto_inventory_handler.deposit_materials = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_materials", default=True
        )
        self.auto_inventory_handler.deposit_event_items = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_event_items", default=True
        )
        self.auto_inventory_handler.deposit_dyes = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_dyes", default=True
        )
        self.auto_inventory_handler.deposit_blues = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_blues", default=True
        )
        self.auto_inventory_handler.deposit_purples = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_purples", default=True
        )
        self.auto_inventory_handler.deposit_golds = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_golds", default=True
        )
        self.auto_inventory_handler.deposit_greens = self._ini_get_bool(
            section="AutoDeposit", var_name="deposit_greens", default=True
        )
        self.auto_inventory_handler.keep_gold = self._ini_get_int(
            section="AutoDeposit", var_name="keep_gold", default=5000
        )
    ...
    def load_blacklists_from_ini(self):
        ...
        self.auto_inventory_handler.id_model_blacklist = [
            int(x) for x in id_model_blacklist.split(",") if x.strip().isdigit()
        ]
        item_type_blacklist = self._ini_get_str(section="AutoSalvage", var_name="item_type_blacklist", default="")
        self.auto_inventory_handler.item_type_blacklist = [
            int(x) for x in item_type_blacklist.split(",") if x.strip().isdigit()
        ]
        salvage_blacklist = self._ini_get_str(section="AutoSalvage", var_name="salvage_blacklist", default="")
        self.auto_inventory_handler.salvage_blacklist = [
            int(x) for x in salvage_blacklist.split(",") if x.strip().isdigit()
        ]
        deposit_trophies_blacklist = self._ini_get_str(
            section="AutoDeposit", var_name="deposit_trophies_blacklist", default=""
        )
        self.auto_inventory_handler.deposit_trophies_blacklist = [
            int(x) for x in deposit_trophies_blacklist.split(",") if x.strip().isdigit()
        ]
        deposit_materials_blacklist = self._ini_get_str(
            section="AutoDeposit", var_name="deposit_materials_blacklist", default=""
        )
        self.auto_inventory_handler.deposit_materials_blacklist = [
            int(x) for x in deposit_materials_blacklist.split(",") if x.strip().isdigit()
        ]
        deposit_event_items_blacklist = self._ini_get_str(
            section="AutoDeposit", var_name="deposit_event_items_blacklist", default=""
        )
        self.auto_inventory_handler.deposit_event_items_blacklist = [
            int(x) for x in deposit_event_items_blacklist.split(",") if x.strip().isdigit()
        ]
        deposit_dyes_blacklist = self._ini_get_str(section="AutoDeposit", var_name="deposit_dyes_blacklist", default="")
        self.auto_inventory_handler.deposit_dyes_blacklist = [
            int(x) for x in deposit_dyes_blacklist.split(",") if x.strip().isdigit()
        ]
        deposit_model_blacklist = self._ini_get_str(
            section="AutoDeposit", var_name="deposit_model_blacklist", default=""
        )
        self.auto_inventory_handler.deposit_model_blacklist = [
            int(x) for x in deposit_model_blacklist.split(",") if x.strip().isdigit()
        ]
    ...
    def _draw_generic_item_menu_item(self, selected_item: ItemSlotData | None = None):
        ...
        is_storage_item = selected_item is not None and Bags.Storage1 <= selected_item.BagID <= Bags.Storage14
        ...
        if (
            selected_item
            and (selected_item.IsIdentified or selected_item.Rarity != "White")
            and Routines.Checks.Items.IsSalvageable(selected_item.ItemID)
        ):
            # ---------------------------------------------------------------
    ...
    def _build_inventory_interaction_context(self) -> InventoryInteractionContext:
        ...
        context = InventoryInteractionContext(
            ...
            i_visible=self.inventory_window_settings.enable_i_window
            and UIManager.IsWindowVisible(WindowID.WindowID_Inventory),
    ...
    def _get_colorized_slot_colors(self, item_data: ItemSlotData) -> tuple[Color, Color] | None:
        if (
            (item_data.Rarity == "White" and not self.colorize_settings.color_whites)
            or (item_data.Rarity == "Blue" and not self.colorize_settings.color_blues)
            or (item_data.Rarity == "Green" and not self.colorize_settings.color_greens)
            or (item_data.Rarity == "Purple" and not self.colorize_settings.color_purples)
            or (item_data.Rarity == "Gold" and not self.colorize_settings.color_golds)
        ):
            return None
    ...
    def _resolve_i_slot_frame_id(
        ...
        for candidate_prefix in prefixes:
            ...
            slot_frame_id = GWFrame(FrameId.InventoryWindow).find_child(*candidate_prefix, bag_id - 1, slot + 2)
    ...
    def DetectInventoryAction(self):
        from Core.GlobalCache import GLOBAL_CACHE
        from Core.enums_src.IO_enums import MouseButton
        from Core.enums_src.Model_enums import ModelID
        ...
        if PyImGui.begin_popup("SlotContextMenu"):
            ...
            if self.selected_item:
                ...
                elif self.selected_item.IsSalvageKit and self.selected_item.ModelID in (
                    ModelID.Salvage_Kit,
                    ModelID.Expert_Salvage_Kit,
                    ModelID.Superior_Salvage_Kit,
                ):
    ...
    def _draw_destroy_confirmation_modal(self) -> None:
        ...
        if not PyImGui.begin_popup_modal("DestroyItemConfirm", True, PyImGui.WindowFlags.AlwaysAutoResize):
            return
                ...
                if popup.result_blacklist is not None:
                    ...
                    if popup.Title == "Identification ModelID Lookup":
                        ...
                        if new_blacklist != self.auto_inventory_handler.id_model_blacklist:
                            ...
                            self._ini_set(
                                section="AutoIdentify", var_name="id_model_blacklist", value=id_model_blacklist_str
                            )
                        ...
                        if new_blacklist != self.auto_inventory_handler.item_type_blacklist:
                            ...
                            self._ini_set(
                                section="AutoSalvage", var_name="item_type_blacklist", value=item_type_blacklist_str
                            )
                        ...
                        if new_blacklist != self.auto_inventory_handler.salvage_blacklist:
                            ...
                            self._ini_set(
                                section="AutoSalvage", var_name="salvage_blacklist", value=salvage_blacklist_str
                            )
                        ...
                        if new_blacklist != self.auto_inventory_handler.deposit_trophies_blacklist:
                            ...
                            self._ini_set(
                                section="AutoDeposit",
                                var_name="deposit_trophies_blacklist",
                                value=deposit_trophies_blacklist_str,
                            )
                        ...
                        if new_blacklist != self.auto_inventory_handler.deposit_materials_blacklist:
                            ...
                            self._ini_set(
                                section="AutoDeposit",
                                var_name="deposit_materials_blacklist",
                                value=deposit_materials_blacklist_str,
                            )
                        ...
                        if new_blacklist != self.auto_inventory_handler.deposit_event_items_blacklist:
                            ...
                            self._ini_set(
                                section="AutoDeposit",
                                var_name="deposit_event_items_blacklist",
                                value=deposit_event_items_blacklist_str,
                            )
                        ...
                        if new_blacklist != self.auto_inventory_handler.deposit_dyes_blacklist:
                            ...
                            self._ini_set(
                                section="AutoDeposit",
                                var_name="deposit_dyes_blacklist",
                                value=deposit_dyes_blacklist_str,
                            )
                        ...
                        if new_blacklist != self.auto_inventory_handler.deposit_model_blacklist:
                            ...
                            self._ini_set(
                                section="AutoDeposit",
                                var_name="deposit_model_blacklist",
                                value=deposit_model_blacklist_str,
                            )
    ...
    def _set_merchant_checkbox_state(self, tick_state: bool):
        from Core import Item, ItemArray, Bags
        from Core.GlobalCache import GLOBAL_CACHE

        merchant_item_list = GLOBAL_CACHE.Trading.Trader.GetOfferedItems()
        merchant_item_models = {int(GLOBAL_CACHE.Item.GetModelID(item_id)) for item_id in merchant_item_list}
            ...
            if PyImGui.begin_tab_bar("MaterialTraderTabs"):
                    ...
                    if not selected_item_ids:
                        ...
                        if PyImGui.begin_table("SelectedMaterialStacksTable", 4):
                            ...
                            for item_id in selected_item_ids:
                                ...
                                updated_qty = PyImGui.input_int(
                                    f"##SellQty_{item_id}",
                                    current_qty,
                                    batch_size,
                                    batch_size,
                                    PyImGui.InputTextFlags.NoFlag,
                                )
                                ...
                                if updated_qty != old_qty:
                                    ...
                                    self.merchant_sell_quantities[item_id] = int(
                                        self.merchant_sell_quantities.get(item_id, updated_qty)
                                    )
                                PyImGui.table_next_column()
                        ...
                        if PyImGui.button(
                            f"{IconsFontAwesome5.ICON_FILE_INVOICE_DOLLAR} Sell Selected Stacks", 220, 42
                        ):
                ...
                if PyImGui.begin_tab_item("Buy"):
                    ...
                    if combo_items:
                        ...
                        self.selected_combo_merchant = PyImGui.combo(
                            "Offered Item", self.selected_combo_merchant, combo_items
                        )
                        PyImGui.pop_item_width()
                    else:
                        PyImGui.text_colored(
                            "No offered items detected.", ColorPalette.GetColor("dark_red").to_tuple_normalized()
                        )

                    self.merchant_buy_quantity = PyImGui.input_int(
                        "Buy Quantity",
                    ...
                    if PyImGui.button("Buy Selected Item", 180, 42):
                        if combo_items:
                            GLOBAL_CACHE.Coroutines.append(
                                BuyMerchantItems(
                                    merchant_item_list.copy(), self.selected_combo_merchant, self.merchant_buy_quantity
                                )
                            )
    ...
    def ShowConfigWindow(self):
        ...
        expanded = ImGui.Begin(
            ...
            name="Inventory Plus Configuration",
            p_open=self.show_config_window,
            flags=PyImGui.WindowFlags.AlwaysAutoResize,
        )
        if expanded:
            auto_state_color = (
                GW_GREEN if self.auto_inventory_handler.module_active else ColorPalette.GetColor("dark_red")
            )
            ...
            if PyImGui.begin_tab_bar("InventoryPlusConfigTabs"):
                ...
                if PyImGui.begin_tab_item("Windows"):
                    ...
                    if window_cfg.enable_i_window:
                        PyImGui.text_wrapped(
                            "When enabled, InventoryPlus handles slot highlighting and context actions for the game's 'I' inventory window."
                        )
                    else:
                        color = ColorPalette.GetColor("dark_red")
                        PyImGui.text_colored(
                            "Disabled: InventoryPlus forces the game's 'I' inventory window closed.",
                            color.to_tuple_normalized(),
                        )
                        PyImGui.text_colored(
                            "I-window slot mapping, coloring, and click handling are skipped.",
                            color.to_tuple_normalized(),
                        )
                    PyImGui.end_tab_item()
                ...
                if PyImGui.begin_tab_item("Identification"):
                    if PyImGui.collapsing_header("Identification Menu Options:"):
                        ...
                        cfg.identify_whites = ini_colored_checkbox(
                            label="Show ID Whites in Menu",
                            section="Identification",
                            var_name="identify_whites",
                            cfg_obj=cfg,
                            color=GW_WHITE,
                            default=cfg.identify_whites,
                        )
                        cfg.identify_blues = ini_colored_checkbox(
                            label="Show ID Blues in Menu",
                            section="Identification",
                            var_name="identify_blues",
                            cfg_obj=cfg,
                            color=GW_BLUE,
                            default=cfg.identify_blues,
                        )
                        cfg.identify_purples = ini_colored_checkbox(
                            label="Show ID Purples in Menu",
                            section="Identification",
                            var_name="identify_purples",
                            cfg_obj=cfg,
                            color=GW_PURPLE,
                            default=cfg.identify_purples,
                        )
                        cfg.identify_golds = ini_colored_checkbox(
                            label="Show ID Golds in Menu",
                            section="Identification",
                            var_name="identify_golds",
                            cfg_obj=cfg,
                            color=GW_GOLD,
                            default=cfg.identify_golds,
                        )
                        cfg.identify_greens = ini_colored_checkbox(
                            label="Show ID Greens in Menu",
                            section="Identification",
                            var_name="identify_greens",
                            cfg_obj=cfg,
                            color=GW_GREEN,
                            default=cfg.identify_greens,
                        )
                        cfg.show_identify_all = ini_colored_checkbox(
                            label="Show Identify All in Menu",
                            section="Identification",
                            var_name="show_identify_all",
                            cfg_obj=cfg,
                            color=GW_WHITE,
                            default=cfg.show_identify_all,
                        )
                        if cfg.show_identify_all:
                            PyImGui.indent(20)
                            cfg.identify_all_whites = ini_colored_checkbox(
                                "Include Whites",
                                "Identification",
                                "identify_all_whites",
                                cfg,
                                GW_WHITE,
                                default=cfg.identify_all_whites,
                            )
                            cfg.identify_all_blues = ini_colored_checkbox(
                                "Include Blues",
                                "Identification",
                                "identify_all_blues",
                                cfg,
                                GW_BLUE,
                                default=cfg.identify_all_blues,
                            )
                            cfg.identify_all_greens = ini_colored_checkbox(
                                "Include Greens",
                                "Identification",
                                "identify_all_greens",
                                cfg,
                                GW_GREEN,
                                default=cfg.identify_all_greens,
                            )
                            cfg.identify_all_purples = ini_colored_checkbox(
                                "Include Purples",
                                "Identification",
                                "identify_all_purples",
                                cfg,
                                GW_PURPLE,
                                default=cfg.identify_all_purples,
                            )
                            cfg.identify_all_golds = ini_colored_checkbox(
                                "Include Golds",
                                "Identification",
                                "identify_all_golds",
                                cfg,
                                GW_GOLD,
                                default=cfg.identify_all_golds,
                            )
                            PyImGui.unindent(20)
                    PyImGui.separator()
                    ...
                    if PyImGui.collapsing_header("Automatic Handling Options:"):
                        color = ColorPalette.GetColor("dark_red")
                        PyImGui.text_colored(
                            "These settings periodically identify items in your inventory based on the options below.",
                            color.to_tuple_normalized(),
                        )
                        PyImGui.text_colored(
                            "Used by bots/scripts as the shared auto-identification policy.",
                            color.to_tuple_normalized(),
                        )
                        self.auto_inventory_handler.id_whites = ini_colored_checkbox(
                            "Automatically ID Whites",
                            "AutoIdentify",
                            "id_whites",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.id_whites,
                        )
                        self.auto_inventory_handler.id_blues = ini_colored_checkbox(
                            "Automatically ID Blues",
                            "AutoIdentify",
                            "id_blues",
                            self.auto_inventory_handler,
                            GW_BLUE,
                            default=self.auto_inventory_handler.id_blues,
                        )
                        self.auto_inventory_handler.id_purples = ini_colored_checkbox(
                            "Automatically ID Purples",
                            "AutoIdentify",
                            "id_purples",
                            self.auto_inventory_handler,
                            GW_PURPLE,
                            default=self.auto_inventory_handler.id_purples,
                        )
                        self.auto_inventory_handler.id_golds = ini_colored_checkbox(
                            "Automatically ID Golds",
                            "AutoIdentify",
                            "id_golds",
                            self.auto_inventory_handler,
                            GW_GOLD,
                            default=self.auto_inventory_handler.id_golds,
                        )
                        self.auto_inventory_handler.id_greens = ini_colored_checkbox(
                            "Automatically ID Greens",
                            "AutoIdentify",
                            "id_greens",
                            self.auto_inventory_handler,
                            GW_GREEN,
                            default=self.auto_inventory_handler.id_greens,
                        )
                ...
                if PyImGui.begin_tab_item("Salvage"):
                    ...
                    if PyImGui.collapsing_header("Salvage Menu Options:"):
                        color = ColorPalette.GetColor("dark_red")
                        PyImGui.text_colored(
                            "These settings periodically salvage items for materials based on the options below.",
                            color.to_tuple_normalized(),
                        )
                        PyImGui.text_colored(
                        ...
                        PyImGui.separator()
                        cfg.salvage_whites = ini_colored_checkbox(
                            label="Show Salvage Whites in Menu",
                            section="Salvage",
                            var_name="salvage_whites",
                            cfg_obj=cfg,
                            color=GW_WHITE,
                            default=cfg.salvage_whites,
                        )
                        cfg.salvage_blues = ini_colored_checkbox(
                            label="Show Salvage Blues in Menu",
                            section="Salvage",
                            var_name="salvage_blues",
                            cfg_obj=cfg,
                            color=GW_BLUE,
                            default=cfg.salvage_blues,
                        )
                        cfg.salvage_purples = ini_colored_checkbox(
                            label="Show Salvage Purples in Menu",
                            section="Salvage",
                            var_name="salvage_purples",
                            cfg_obj=cfg,
                            color=GW_PURPLE,
                            default=cfg.salvage_purples,
                        )
                        cfg.salvage_golds = ini_colored_checkbox(
                            label="Show Salvage Golds in Menu",
                            section="Salvage",
                            var_name="salvage_golds",
                            cfg_obj=cfg,
                            color=GW_GOLD,
                            default=cfg.salvage_golds,
                        )
                        cfg.show_salvage_all = ini_colored_checkbox(
                            label="Show Salvage All in Menu",
                            section="Salvage",
                            var_name="show_salvage_all",
                            cfg_obj=cfg,
                            color=GW_WHITE,
                            default=cfg.show_salvage_all,
                        )
                        if cfg.show_salvage_all:
                            PyImGui.indent(20)
                            cfg.salvage_all_whites = ini_colored_checkbox(
                                "Include Whites",
                                "Salvage",
                                "salvage_all_whites",
                                cfg,
                                GW_WHITE,
                                default=cfg.salvage_all_whites,
                            )
                            cfg.salvage_all_blues = ini_colored_checkbox(
                                "Include Blues",
                                "Salvage",
                                "salvage_all_blues",
                                cfg,
                                GW_BLUE,
                                default=cfg.salvage_all_blues,
                            )
                            cfg.salvage_all_purples = ini_colored_checkbox(
                                "Include Purples",
                                "Salvage",
                                "salvage_all_purples",
                                cfg,
                                GW_PURPLE,
                                default=cfg.salvage_all_purples,
                            )
                            cfg.salvage_all_golds = ini_colored_checkbox(
                                "Include Golds",
                                "Salvage",
                                "salvage_all_golds",
                                cfg,
                                GW_GOLD,
                                default=cfg.salvage_all_golds,
                            )
                    ...
                    if PyImGui.collapsing_header("Ignore Lists:"):
                        ...
                        if PyImGui.is_item_hovered():
                            PyImGui.begin_tooltip()
                            PyImGui.text(
                                f"{len(self.auto_inventory_handler.item_type_blacklist)} Types Ignored. (e.g., Weapons, Armor, etc.)"
                            )
                            PyImGui.end_tooltip()
                        PyImGui.same_line(0, -1)
                    ...
                    if PyImGui.collapsing_header("Automatic Handling Options:"):
                        color = ColorPalette.GetColor("dark_red")
                        PyImGui.text_colored(
                            "These settings periodically salvage items in your inventory based on the options below.",
                            color.to_tuple_normalized(),
                        )
                        PyImGui.text_colored(
                            "Used by bots/scripts as the shared auto-salvage policy.", color.to_tuple_normalized()
                        )
                        self.auto_inventory_handler.salvage_whites = ini_colored_checkbox(
                            "Automatically Salvage Whites",
                            "AutoSalvage",
                            "salvage_whites",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.salvage_whites,
                        )
                        self.auto_inventory_handler.salvage_rare_materials = ini_colored_checkbox(
                            "Automatically Salvage Rare Materials",
                            "AutoSalvage",
                            "salvage_rare_materials",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.salvage_rare_materials,
                        )
                        self.auto_inventory_handler.salvage_blues = ini_colored_checkbox(
                            "Automatically Salvage Blues",
                            "AutoSalvage",
                            "salvage_blues",
                            self.auto_inventory_handler,
                            GW_BLUE,
                            default=self.auto_inventory_handler.salvage_blues,
                        )
                        self.auto_inventory_handler.salvage_purples = ini_colored_checkbox(
                            "Automatically Salvage Purples",
                            "AutoSalvage",
                            "salvage_purples",
                            self.auto_inventory_handler,
                            GW_PURPLE,
                            default=self.auto_inventory_handler.salvage_purples,
                        )
                        self.auto_inventory_handler.salvage_golds = ini_colored_checkbox(
                            "Automatically Salvage Golds",
                            "AutoSalvage",
                            "salvage_golds",
                            self.auto_inventory_handler,
                            GW_GOLD,
                            default=self.auto_inventory_handler.salvage_golds,
                        )

                    if PyImGui.collapsing_header("Upgrade/Component Dialog:"):
                        color = ColorPalette.GetColor("dark_red")
                        PyImGui.text_colored(
                            "Handles the salvage popup that appears when upgrades or components can be salvaged.",
                            color.to_tuple_normalized(),
                        )
                        PyImGui.text_colored(
                            "If text cannot be read reliably, InventoryPlus falls back to the visible order under child[5].",
                            color.to_tuple_normalized(),
                        )
                        self.auto_inventory_handler.salvage_dialog_auto_handle = ini_colored_checkbox(
                            "Auto Handle Salvage Choice Dialog",
                            "AutoSalvage",
                            "salvage_dialog_auto_handle",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.salvage_dialog_auto_handle,
                        )
                        self.auto_inventory_handler.salvage_dialog_auto_confirm_materials = ini_colored_checkbox(
                            "Auto Confirm Crafting Materials Warning",
                            "AutoSalvage",
                            "salvage_dialog_auto_confirm_materials",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.salvage_dialog_auto_confirm_materials,
                        )
                        self.auto_inventory_handler.salvage_dialog_debug = ini_colored_checkbox(
                            "Debug Salvage Choice Dialog",
                            "AutoSalvage",
                            "salvage_dialog_debug",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.salvage_dialog_debug,
                        )
                        PyImGui.text_wrapped(
                            "The crafting materials warning destroys upgrades. Leave auto-confirm off if you want to approve that prompt manually."
                        )
                        if self.auto_inventory_handler.salvage_dialog_auto_handle:
                            ...
                            current_dialog_strategy = _normalize_salvage_dialog_strategy(
                                int(self.auto_inventory_handler.salvage_dialog_strategy)
                            )
                            if current_dialog_strategy != int(self.auto_inventory_handler.salvage_dialog_strategy):
                                ...
                                self._ini_set(
                                    section="AutoSalvage",
                                    var_name="salvage_dialog_strategy",
                                    value=current_dialog_strategy,
                                )
                            new_dialog_strategy = PyImGui.combo(
                                "Salvage Choice Strategy", current_dialog_strategy, salvage_dialog_strategy_labels
                            )
                            if new_dialog_strategy != current_dialog_strategy:
                                self.auto_inventory_handler.salvage_dialog_strategy = new_dialog_strategy
                                self._ini_set(
                                    section="AutoSalvage", var_name="salvage_dialog_strategy", value=new_dialog_strategy
                                )
                                current_dialog_strategy = new_dialog_strategy

                            if current_dialog_strategy == 0:
                                PyImGui.text_wrapped(
                                    "Crafting Materials prefers a materials text match when available, otherwise the last visible option."
                                )
                            else:
                                PyImGui.text_wrapped(
                                    "Upgrades/Components prefers a non-material text match when available, otherwise the first visible option."
                                )
                    PyImGui.end_tab_item()
                if PyImGui.begin_tab_item("Deposit"):
                    ...
                    cfg = self.deposit_settings
                    cfg.use_ctrl_click = ini_colored_checkbox(
                        label="Use Ctrl + Left Click To Deposit Items",
                        section="Deposit",
                        var_name="use_ctrl_click",
                        cfg_obj=cfg,
                        color=GW_WHITE,
                        default=cfg.use_ctrl_click,
                    )
                    ...
                    if PyImGui.collapsing_header("Automatic Handling Options:"):
                        ...
                        PyImGui.text_wrapped(
                            "This feature is used by Bots and other scripts to automatically manage your inventory."
                        )
                        PyImGui.text_wrapped(
                            "Each time you enter Outposts Items matching the criteria will be deposited into your Xunlai Vault."
                        )
                        PyImGui.separator()

                        self.auto_inventory_handler.deposit_materials = ini_colored_checkbox(
                            IconsFontAwesome5.ICON_HAMMER + " Deposit Materials",
                            "AutoDeposit",
                            "deposit_materials",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.deposit_materials,
                        )
                        self.auto_inventory_handler.deposit_trophies = ini_colored_checkbox(
                            IconsFontAwesome5.ICON_TROPHY + " Deposit Trophies",
                            "AutoDeposit",
                            "deposit_trophies",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.deposit_trophies,
                        )
                        self.auto_inventory_handler.deposit_event_items = ini_colored_checkbox(
                            IconsFontAwesome5.ICON_HAT_WIZARD + " Deposit Event Items",
                            "AutoDeposit",
                            "deposit_event_items",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.deposit_event_items,
                        )
                        self.auto_inventory_handler.deposit_dyes = ini_colored_checkbox(
                            IconsFontAwesome5.ICON_FLASK + " Deposit Dyes",
                            "AutoDeposit",
                            "deposit_dyes",
                            self.auto_inventory_handler,
                            GW_WHITE,
                            default=self.auto_inventory_handler.deposit_dyes,
                        )
                        self.auto_inventory_handler.deposit_blues = ini_colored_checkbox(
                            "Deposit Blues",
                            "AutoDeposit",
                            "deposit_blues",
                            self.auto_inventory_handler,
                            GW_BLUE,
                            default=self.auto_inventory_handler.deposit_blues,
                        )
                        self.auto_inventory_handler.deposit_purples = ini_colored_checkbox(
                            "Deposit Purples",
                            "AutoDeposit",
                            "deposit_purples",
                            self.auto_inventory_handler,
                            GW_PURPLE,
                            default=self.auto_inventory_handler.deposit_purples,
                        )
                        self.auto_inventory_handler.deposit_golds = ini_colored_checkbox(
                            "Deposit Golds",
                            "AutoDeposit",
                            "deposit_golds",
                            self.auto_inventory_handler,
                            GW_GOLD,
                            default=self.auto_inventory_handler.deposit_golds,
                        )
                        self.auto_inventory_handler.deposit_greens = ini_colored_checkbox(
                            "Deposit Greens",
                            "AutoDeposit",
                            "deposit_greens",
                            self.auto_inventory_handler,
                            GW_GREEN,
                            default=self.auto_inventory_handler.deposit_greens,
                        )
                    ...
                    if PyImGui.collapsing_header("Ignore Lists:"):
                        ...
                        if PyImGui.begin_table("DepositBlacklistTable", 2):
                            ...
                            PyImGui.table_next_column()
                            PyImGui.text(
                                f"{len(self.auto_inventory_handler.deposit_materials_blacklist)} Models Ignored"
                            )
                            ...
                            PyImGui.table_next_column()
                            PyImGui.text(
                                f"{len(self.auto_inventory_handler.deposit_trophies_blacklist)} Models Ignored"
                            )
                            ...
                            PyImGui.table_next_column()
                            PyImGui.text(
                                f"{len(self.auto_inventory_handler.deposit_event_items_blacklist)} Models Ignored"
                            )
                ...
                if PyImGui.begin_tab_item("Colorize"):
                    ...
                    PyImGui.separator()
                    cfg.color_whites = ini_colored_checkbox(
                        label="Color White Items",
                        section="Colorize",
                        var_name="color_whites",
                        cfg_obj=cfg,
                        color=GW_WHITE,
                        default=cfg.color_whites,
                    )
                    cfg.color_blues = ini_colored_checkbox(
                        label="Color Blue Items",
                        section="Colorize",
                        var_name="color_blues",
                        cfg_obj=cfg,
                        color=GW_BLUE,
                        default=cfg.color_blues,
                    )
                    cfg.color_greens = ini_colored_checkbox(
                        label="Color Green Items",
                        section="Colorize",
                        var_name="color_greens",
                        cfg_obj=cfg,
                        color=GW_GREEN,
                        default=cfg.color_greens,
                    )
                    cfg.color_purples = ini_colored_checkbox(
                        label="Color Purple Items",
                        section="Colorize",
                        var_name="color_purples",
                        cfg_obj=cfg,
                        color=GW_PURPLE,
                        default=cfg.color_purples,
                    )
                    cfg.color_golds = ini_colored_checkbox(
                        label="Color Gold Items",
                        section="Colorize",
                        var_name="color_golds",
                        cfg_obj=cfg,
                        color=GW_GOLD,
                        default=cfg.color_golds,
                    )
                    PyImGui.separator()
                    ...
                    def _parse_color(value: str, default: tuple[int, int, int, int]):
                        try:
                            return tuple(int(c.strip()) for c in value.strip("()").split(","))
                        except Exception:
                            return default

                    white_color_str = self._ini_get_str(
                        section="Colorize", var_name="white_color", default="(255, 255, 255, 255)"
                    )
                    ...
                    if color != cfg.white_color.to_tuple_normalized():
                        cfg.white_color = Color(
                            int(255 * color[0]), int(255 * color[1]), int(255 * color[2]), int(255 * color[3])
                        )
                        self._ini_set(section="Colorize", var_name="white_color", value=cfg.white_color.to_tuple())

                    blue_color_str = self._ini_get_str(
                        section="Colorize", var_name="blue_color", default="(0, 0, 255, 255)"
                    )
                    ...
                    if color != cfg.blue_color.to_tuple_normalized():
                        cfg.blue_color = Color(
                            int(255 * color[0]), int(255 * color[1]), int(255 * color[2]), int(255 * color[3])
                        )
                        self._ini_set(section="Colorize", var_name="blue_color", value=cfg.blue_color.to_tuple())

                    green_color_str = self._ini_get_str(
                        section="Colorize", var_name="green_color", default="(0, 255, 0, 255)"
                    )
                    ...
                    if color != cfg.green_color.to_tuple_normalized():
                        cfg.green_color = Color(
                            int(255 * color[0]), int(255 * color[1]), int(255 * color[2]), int(255 * color[3])
                        )
                        self._ini_set(section="Colorize", var_name="green_color", value=cfg.green_color.to_tuple())

                    purple_color_str = self._ini_get_str(
                        section="Colorize", var_name="purple_color", default="(128, 0, 128, 255)"
                    )
                    ...
                    if color != cfg.purple_color.to_tuple_normalized():
                        cfg.purple_color = Color(
                            int(255 * color[0]), int(255 * color[1]), int(255 * color[2]), int(255 * color[3])
                        )
                        self._ini_set(section="Colorize", var_name="purple_color", value=cfg.purple_color.to_tuple())

                    gold_color_str = self._ini_get_str(
                        section="Colorize", var_name="gold_color", default="(255, 215, 0, 255)"
                    )
                ...
                if PyImGui.begin_tab_item("Auto Handler"):
                    ...
                    PyImGui.text_wrapped(
                        "Enable the options in the Identification and Salvage tabs to activate automatic handling for those item rarities."
                    )
                    ...
                    old_val = self._ini_get_int(
                        section="AutoManager", var_name="lookup_time", default=self.auto_inventory_handler._LOOKUP_TIME
                    )
                    ...
                    PyImGui.text(
                        f"next check in {max(0, self.auto_inventory_handler._LOOKUP_TIME - self.auto_inventory_handler.lookup_throttle.GetTimeElapsed()):.3f} ms"
                    )
                    PyImGui.end_tab_item()
                PyImGui.end_tab_bar()
            PyImGui.separator()
