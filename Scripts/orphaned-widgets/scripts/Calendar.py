...
from Core.enums import MONTHS
from Core.enums import NICHOLAS_CYCLE
from Core.enums import PVE_WEEKLY_BONUSES
from Core.enums import PVP_WEEKLY_BONUSES
from Core.enums import ZAISHEN_REFERENCE_DATE
from Core.enums import ZAISHEN_MISSION
from Core.enums import ZAISHEN_BOUNTY
from Core.enums import ZAISHEN_COMBAT
from Core.enums import ZAISHEN_VANQUISH
...
def _month_grid(year: int, month: int) -> tuple[tuple[date | None, ...], ...]:
    ...
    return tuple(
        tuple(day if day.month == month else None for day in week) for week in cal.monthdatescalendar(year, month)
    )
...
class Calendar:
    ...
    def get_events_for_day(self, events_dict: dict) -> Optional[dict]:
        ...
        return events_dict.get(self.current.year, {}).get(self.current.month, {}).get(self.current.day)
...
def toggle_button(
    ...
    width=0,
    height=0,
    button_color: Color = Color(0, 0, 0, 255),
    hover_color: Color = Color(0, 0, 0, 255),
    active_color: Color = Color(0, 0, 0, 255),
) -> bool:
    """
...
class ButtonLayout:
    def __init__(self):
        ...
        self.button_day = ToggleButton(
            f"{IconsFontAwesome5.ICON_CALENDAR_DAY}", toggled=True, width=self.button_width, height=self.button_height
        )
        self.button_trimester = ToggleButton(
            f"{IconsFontAwesome5.ICON_CALENDAR_WEEK}", toggled=False, width=self.button_width, height=self.button_height
        )
        self.button_year = ToggleButton(
            f"{IconsFontAwesome5.ICON_CALENDAR_DAYS}", toggled=False, width=self.button_width, height=self.button_height
        )

        self.button_prev = Button(
            f"{IconsFontAwesome5.ICON_CHEVRON_LEFT}", width=self.button_width, height=self.button_height
        )
        self.button_next = Button(
            f"{IconsFontAwesome5.ICON_CHEVRON_RIGHT}", width=self.button_width, height=self.button_height
        )
                    ...
                    if day is None or day.month != cal.current.month:
                        ...
                        if PyImGui.button(str(day.day), 30, 30):
                            ...
                            PySystem.Console.Log(
                                "Calendar", f"Clicked raw day: {day}", PySystem.Console.MessageType.Info
                            )
                            PySystem.Console.Log(
                                "Calendar",
                                f"Global calendar set to: {calendar.current}",
...
def _save_widget_enabled_state(widget: WidgetLike) -> bool:
    ...
    Settings(manager_ini_key, "account").set(
        f"Widget:{widget.name}", _get_widget_enabled_var_name(widget), widget.enabled
    )
    return True
