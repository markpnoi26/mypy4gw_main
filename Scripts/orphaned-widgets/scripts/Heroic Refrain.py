from Core import PyImGui, GLOBAL_CACHE, Timer, ThrottledTimer
...
from Core import Player, Party, PyImGui, Timer
...
def update_buff_cache():
    ...
    for index, player_data in player_data_cache.items():
        try:
            ...
            player_data['has_heroic_refrain'] = (
                any(buff.SkillId == 3431 and buff.Remaining > 0 for buff in account.AgentData.Buffs.Buffs)
                if account
                else False
            )
        except:
...
def render_heroic_refrain_ui():
    ...
    accounts = [account for account in GLOBAL_CACHE.ShMem.GetAllAccountData() if account is not None]
