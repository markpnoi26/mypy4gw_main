...
from Core import GLOBAL_CACHE, ImGui, Color, Range, Player
...
def _reload_heroai_runtime_skills() -> None:
    ...
    try:
        ...
        try:
            import Core.SkillManager as skill_manager_module
...
def _locate_condition_line(
    lines: list[str], condition_name: str
...
def _persist_skill_nature(skill_id: int, nature_enum: SkillNature) -> tuple[bool, str]:
    ...
    if match:
        ...
        insert_pos = insert_match.end()
        indent_match = re.search(r'\n([\t ]+)skill\.Nature', block_text) or re.search(
...
def _collect_accounts() -> List[AccountEntry]:
    ...
    for account in shmem_accounts:
        ...
        entries.append(
            AccountEntry(label=email, email=email, character=char_name or email or "Unknown", account=account)
        )
    ...
    if PyImGui.begin_combo("##target_combo", current_label, 0):
        for option in _TARGET_OPTIONS:
            ...
            if PyImGui.selectable(
                f"{label}##target_option_{option.value}", is_active, PyImGui.SelectableFlags(0), (0.0, 0.0)
            ):
    ...
    if PyImGui.begin_combo("##nature_combo", current_label, 0):
        for option in _NATURE_OPTIONS:
            ...
            if selected_value is None and label.lower() == (fallback_label or "").lower():
                is_active = True
            if PyImGui.selectable(
                f"{label}##nature_option_{option.value}", is_active, PyImGui.SelectableFlags(0), (0.0, 0.0)
            ):
    ...
    if PyImGui.begin_combo("##skilltype_combo", current_label, 0):
        for option in _SKILLTYPE_OPTIONS:
            ...
            if selected_value is None and label.lower() == (fallback_label or "").lower():
                is_active = True
            if PyImGui.selectable(
                f"{label}##skilltype_option_{option.value}", is_active, PyImGui.SelectableFlags(0), (0.0, 0.0)
            ):
...
def _render_condition_list(skill_id: int, active_conditions: Set[str], condition_values: dict[str, Any]) -> None:
    ...
    if not active_conditions:
        ...
        for condition_name in sorted(active_conditions, key=str.lower):
            ...
            assignment = (
                f"skill.Conditions.{condition_name} = {_format_condition_literal(condition_name, effective_value)}"
            )
    ...
    if PyImGui.begin_combo("##condition_add_combo", selected_condition, 0):
        for option in _CONDITION_OPTIONS:
            ...
            if PyImGui.selectable(
                f"{option}##condition_add_option_{option}", is_active, PyImGui.SelectableFlags(0), (0.0, 0.0)
            ):
    ...
    if isinstance(default_value, bool):
        ...
        if is_range_counter:
            ...
            value_text = (
                _format_numeric_literal(float(current_add_value), isinstance(default_value, float))
                if current_add_value not in (None, "")
                else _format_numeric_literal(default_value, isinstance(default_value, float))
            )
...
def _draw_edit_window() -> None:
    ...
    if PyImGui.begin_table("##edit_target_options", 2, flags, 0, 0):
        PyImGui.table_setup_column(
            "Target / Nature / Type", PyImGui.TableColumnFlags.WidthFixed, EDIT_POPUP_TARGET_COLUMN_WIDTH
        )
        ...
        _render_condition_list(
            _edit_snapshot.skill_id, _edit_snapshot.active_conditions, _edit_snapshot.condition_values
        )
        PyImGui.end_table()
    else:
...
def _draw_condition_cell(row: SkillRow) -> None:
    ...
    if _edit_snapshot and _edit_snapshot.skill_id == row.skill_id:
        conditions_to_show = _summarize_snapshot_conditions(
            _edit_snapshot.active_conditions, _edit_snapshot.condition_values
        )
...
def _draw_window() -> None:
    ...
    PyImGui.text_colored(
        "Use with Caution: Modifying skill conditions can lead to unintended consequences. Always review changes before applying.",
        Color(255, 0, 0, 255).to_tuple_normalized(),
    )
