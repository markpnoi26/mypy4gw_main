...
from Core import Py4GW
from Core import Agent, Color, GLOBAL_CACHE, ConsoleLog, Player, SharedCommandType, Utils
from Core.Map import Map
from Core.ImGui import ImGui
from Core.ImGui_src.IconsFontAwesome5 import IconsFontAwesome5
from Core.ImGui_src.Textures import TextureState, ThemeTexture, ThemeTextures
from Core.ImGui_src.types import ImGuiStyleVar, StyleTheme
from Core.enums import ImguiFonts
...
from Core.py4gwcorelib_src.Timer import Timer
...
class HeroAI_PlayerPanelRenderer:
    ...
    def get_window_title(self, account) -> str:
        ...
        return (
            f"{primary_prof}{('/' if secondary_prof else '')}{secondary_prof}{account.AgentData.Level} {display_name}"
        )
    ...
    def draw_simple_panel(self, identifier: str, game_option) -> None:
        ...
        if PyImGui.begin_table(f"FollowerGameOptionTable##{identifier}", 3, 0, 0, btn_size + 2):
            ...
            following = ImGui.toggle_button(
                IconsFontAwesome5.ICON_RUNNING + f"##Following{identifier}",
                game_option.Following,
                btn_size * 1.45,
                btn_size,
            )
            ...
            looting = ImGui.toggle_button(
                IconsFontAwesome5.ICON_COINS + f"##Looting{identifier}", game_option.Looting, btn_size * 1.45, btn_size
            )
            ...
            combat = ImGui.toggle_button(
                IconsFontAwesome5.ICON_SKULL_CROSSBONES + f"##Combat{identifier}",
                game_option.Combat,
                btn_size * 1.45,
                btn_size,
            )
        ...
        if self.appearance_window.get_show_player_skill_toggles():
            style.ButtonPadding.push_style_var(
                5 if style.Theme not in ImGui.Textured_Themes else 0,
                3 if style.Theme not in ImGui.Textured_Themes else 2,
            )
            if PyImGui.begin_table(f"FollowerSkillsTable##{identifier}", NUMBER_OF_SKILLS, 0, 0, (btn_size / 3)):
                ...
                for i in range(NUMBER_OF_SKILLS):
                    ...
                    skill_active = ImGui.toggle_button(
                        f"{i + 1}##Skill{i}{identifier}", game_option.Skills[i], skill_size, skill_size
                    )
...
class HeroAI_RichPlayerPanelRenderer:
    ...
    def _draw_health_bar_local(
                ...
                if indicator:
                    ...
                    if indicator_texture:
                        indicator_texture.value.get_texture().draw_in_drawlist(
                            (item_rect[0] + item_rect[2] - x_offset, item_rect[1]), (height, height)
                        )
                    x_offset += height + 2
        ...
        text_rect = (
            ...
            item_rect[1] + ((height - textsize[1]) / 2) + self._scale(3),
            textsize[0],
            textsize[1],
        )
    ...
    def _draw_energy_bar_local(
        ...
        text_rect = (
            item_rect[0] + ((width - text_size[0]) / 2),
            item_rect[1] + ((height - text_size[1]) / 2) + self._scale(3),
            text_size[0],
            text_size[1],
        )
        PyImGui.draw_list_add_text(
    ...
    def _get_skill_target(self, account, cached_skill: CachedSkillInfo) -> int | None:
        ...
        return (
            ...
            else target_id if not is_item and not is_gadget and target_id else account.AgentData.AgentID
        )
        ...
        for slot, skill_info in enumerate(account.AgentData.Skillbar.Skills):
            ...
            if skill.texture_path:
                ...
                PyImGui.draw_list_add_rect(
                    item_rect_min[0],
                    item_rect_min[1],
                    item_rect_min[0] + skill_size,
                    item_rect_min[1] + skill_size,
            ...
            if not enough_adrenaline:
                ...
                fill_rect = (
                    ...
                    item_rect_min[1],
                    skill_size,
                    skill_size - (skill_size * adrenaline_fraction),
                )
            ...
            if queued_skill_messages:
                queued_skill_usage = {
                    index: msg
                    for index, msg in GLOBAL_CACHE.ShMem.GetAllMessages()
                ...
                if queued_skill_usage and draw_textures:
                    ...
                    indices_to_delete = [
                        index for index, msg in queued_skill_messages.items() if msg[1] == skill.skill_id
                    ]
            ...
            if PyImGui.is_item_clicked(0) and enough_adrenaline:
                if PyImGui.get_io().key_shift:
                    ...
                    if target_id is not None and skill_id != 0:
                        ...
                        self.message_cache[account.AccountEmail][SharedCommandType.UseSkill][message_index] = (
                            target_id,
                            skill.skill_id,
                        )
            ...
            if draw_textures:
                ...
                ThemeTextures.Skill_Frame.value.get_texture(texture_theme).draw_in_drawlist(
                    item_rect_min[:2], (skill_size, skill_size), state=texture_state
                )
        ...
        if PyImGui.begin_popup("Enter Skill Template Code"):
            ...
            if ImGui.button("Load"):
                GLOBAL_CACHE.ShMem.SendMessage(
                    ...
                    ExtraData=(self.template_code, 0, 0, 0),
                )
    ...
    def _draw_buffs_and_upkeeps(self, account, skill_size: float = 28):
        ...
        def draw_hardmode():
            if any(effect.SkillId == hard_mode_effect_id for effect in effects):
                ...
                texture = (
                    ThemeTextures.HardMode.value.get_texture(StyleTheme.Guild_Wars)
                    if to_kill > 0
                    else ThemeTextures.HardModeCompleted.value.get_texture(StyleTheme.Guild_Wars)
                )
        ...
        if ImGui.begin_table(
            "##effects_table" + account.AccountEmail,
    ...
    def _draw_buffs_bar(self, account):
        ...
        PyImGui.set_next_window_pos(
            win_pos[0],
            win_pos[1] + win_size[1] + (13 if style.Theme == StyleTheme.Guild_Wars else 4),
    ...
    def draw_panel(self, account, cached_data: CacheData, messages: list) -> None:
        ...
        if height > 0:
            if ImGui.begin_child("##bars" + account.AccountEmail, (panel_width, height)):
                ...
                health_state, deep_wounded, enchanted, conditioned, hexed, has_weaponspell = self._get_conditioned(
                    account
                )
        ...
        for name, value in opt_dict.items():
            ...
            active = ImGui.toggle_button(
                name + f"##{account.AccountEmail}",
                value,
                control_row_width / len(opt_dict) - self._scale(3),
                control_height,
            )
            ImGui.pop_font()
            if active != value:
...
class HeroAI_AppearanceWindow:
    ...
    def draw_window(self) -> None:
        ...
        expanded, open_ = ImGui.begin_with_close(
            "HeroAI Appearance Settings", self.is_open(), PyImGui.WindowFlags.AlwaysAutoResize
        )
        self.set_open(open_)
        ...
        if expanded:
            ...
            new_scale_percent = PyImGui.slider_int(
                "##panel_scale", scale_percent, int(PANEL_MIN_SCALE * 100), int(PANEL_MAX_SCALE * 100)
            )
            ...
            if show_players:
                ...
                new_show_players_in_separate_window = PyImGui.checkbox(
                    "Show Players In Separate Window", show_players_in_separate_window
                )
                ...
                if show_players_in_separate_window:
                    ...
                    new_show_players_in_individual_windows = PyImGui.checkbox(
                        "Pop Up Each Player In A Separate Window", show_players_in_individual_windows
                    )
                ...
                if not new_use_rich_player_panels:
                    ...
                    new_show_player_skill_toggles = PyImGui.checkbox(
                        "Show Player Skill Toggles", show_player_skill_toggles
                    )
...
class HeroAI_MainWindow:
    ...
    def draw_window(self) -> None:
        expanded, open_ = ImGui.begin_with_close(
            HeroAIFloatingIcon.MODULE_NAME, self.floating_button.visible, PyImGui.WindowFlags.AlwaysAutoResize
        )
    ...
    def DrawPanelButtons(self):
        ...
        if PyImGui.begin_table(f"GameOptionTable##{"Global"}", 5, 0, 0, btn_size + 2):
            ...
            PyImGui.table_next_column()
            following = ImGui.toggle_button(
                IconsFontAwesome5.ICON_RUNNING + "##Following" + "Global",
            ...
            PyImGui.table_next_column()
            looting = ImGui.toggle_button(
                IconsFontAwesome5.ICON_COINS + "##Looting" + "Global",
            ...
            combat = ImGui.toggle_button(
                IconsFontAwesome5.ICON_SKULL_CROSSBONES + "##Combat" + "Global",
            ...
            new_show_appearance = ImGui.toggle_button(
                IconsFontAwesome5.ICON_COG + "##Appearance" + "Global", show_appearance, btn_size, btn_size
            )
        ...
        if self.appearance_window.get_show_main_skill_toggles():
            ...
            if PyImGui.begin_table(f"SkillsTable##{"Global"}", NUMBER_OF_SKILLS, 0, 0, (btn_size / 3)):
                ...
                for i in range(NUMBER_OF_SKILLS):
                    PyImGui.table_next_column()
                    skill_active = ImGui.toggle_button(
                        f"{i + 1}##Skill{i}" + "Global", cached_data.global_options.Skills[i], skill_size, skill_size
                    )
        ...
        if (
            self.appearance_window.get_show_players()
            and not self.appearance_window.get_show_players_in_separate_window()
        ):
            PyImGui.dummy((0, 10))
            PyImGui.separator()
...
class HeroAI_PlayersWindow:
    ...
    def draw_window(self) -> None:
        ...
        expanded, open_ = ImGui.begin_with_close(
            "HeroAI Players",
            self.appearance_window.get_show_players_in_separate_window(),
            PyImGui.WindowFlags.AlwaysAutoResize,
        )
